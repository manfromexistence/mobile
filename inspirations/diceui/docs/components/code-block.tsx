...
const INSTALL_COMMAND_REGEX = /^(npm|pnpm|yarn|bun|npx)\s+(install|add|i|create)\s+/;
...
export function CodeBlock({ lang, ...props }: React.ComponentProps<typeof FumadocsCodeBlock>) {
  ...
  React.useEffect(() => {
    ...
    const onClick = (event: MouseEvent) => {
      const button = (event.target as HTMLElement).closest('button[aria-label="Copy Text"]');
      if (!button || !containerRef.current) return;

      const code = containerRef.current.querySelector("pre")?.textContent ?? "";
      const eventName = getIsInstallCommand(code) ? "copy_install_command" : "copy_code";
