...
function TimePicker(props: TimePickerProps) {
  ...
  const onPopoverOpenChange = React.useCallback(
    (newOpen: boolean, eventDetails: PopoverPrimitive.Root.ChangeEventDetails) => {
      if (!newOpen && eventDetails.reason === "outside-press") {
        if (eventDetails.isCanceled) return;

        if (openOnFocus && inputGroupRef.current) {
          const target = eventDetails.event.target;
          if (target instanceof Node && inputGroupRef.current.contains(target)) {
            eventDetails.cancel();
            return;
          }
        }
...
function TimePickerClear(props: ButtonProps) {
  const { render, className, children, disabled: disabledProp, ...clearProps } = props;
