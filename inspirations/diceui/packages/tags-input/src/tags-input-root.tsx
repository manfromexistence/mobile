...
interface TagsInputRootProps<T = InputValue>
   ...
   * @default undefined
   */
  children?: React.ReactNode | ((context: { value: InputValue[] }) => React.ReactNode);
...
const TagsInputRoot = React.forwardRef<CollectionElement, TagsInputRootProps<InputValue>>(
  (props, ref) => {
    const {
      value: valueProp,
      defaultValue,
    ...
    const { isFormControl, onTriggerChange } = useFormControl<CollectionElement>();
    const composedRef = useComposedRefs(ref, collectionRef, (node) => onTriggerChange(node));
    ...
    const onItemAdd = React.useCallback(
      ...
      [value, max, addOnPaste, delimiter, setValue, onInvalid, onValidate, disabled, readOnly],
    );
