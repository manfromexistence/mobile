...
const ComboboxAnchor = React.forwardRef<AnchorElement, ComboboxAnchorProps>(
  (props, forwardedRef) => {
    ...
    const composedRef = useComposedRefs(forwardedRef, context.anchorRef, (node) =>
      context.onHasAnchorChange(!!node),
    );
    ...
    return (
      <Primitive.div
        ...
        {...anchorProps}
        ref={composedRef}
        onBlur={composeEventHandlers(anchorProps.onBlur, () => setIsFocused(false))}
        onClick={composeEventHandlers(anchorProps.onClick, (event) => {
          if (preventInputFocus) return;
          event.currentTarget.focus();
          context.inputRef.current?.focus();
        })}
        onFocus={composeEventHandlers(anchorProps.onFocus, () => setIsFocused(true))}
        onPointerDown={composeEventHandlers(anchorProps.onPointerDown, (event) => {
