...
const CheckboxGroupItem = React.forwardRef<HTMLButtonElement, CheckboxGroupItemProps>(
  (props, ref) => {
    ...
    const { isFormControl, trigger, onTriggerChange } = useFormControl<HTMLButtonElement>();
    const composedRef = useComposedRefs(ref, (node) => onTriggerChange(node));
    const lastClickTimeRef = React.useRef(0);
    const hasConsumerStoppedPropagationRef = React.useRef(false);

    return (
      <CheckboxGroupItemProvider value={value} checked={isChecked} disabled={isDisabled}>
        <Primitive.button
          type="button"
          role="checkbox"
