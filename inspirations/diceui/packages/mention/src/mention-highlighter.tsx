  ...
  React.forwardRef<HighlighterElement, MentionHighlighterProps>((props, forwardedRef) => {
    ...
    const onSegmentsRender = React.useCallback(() => {
      ...
      for (const { start, end } of context.mentions) {
        ...
        if (start > lastIndex) {
          segments.push(<span key={`text-${lastIndex}`}>{value.slice(lastIndex, start)}</span>);
        }
      ...
      if (lastIndex < value.length) {
        segments.push(<span key={`text-end-${value.length}`}>{value.slice(lastIndex)}</span>);
      }
    ...
    return (
      <div {...highlighterProps} ref={composedRef} dir={context.dir} style={highlighterStyle}>
        {onSegmentsRender()}
      </div>
    );
  }),
