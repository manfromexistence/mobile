...
const MentionInput = React.forwardRef<InputElement, MentionInputProps>((props, forwardedRef) => {
  ...
  const onMentionUpdate = React.useCallback(
    (element: InputElement, selectionStart: number | null = null) => {
      ...
      const lastTriggerIndex = value.lastIndexOf(context.trigger, currentPosition);
      ...
      const textAfterTrigger = value.slice(lastTriggerIndex + 1, currentPosition);
        ...
        if (mentionBeforeCursor) {
          ...
          if (isCursorInsideMention || isCtrlOrCmd) {
            ...
            const newValue =
              ...
              input.value.slice(mentionBeforeCursor.end + (hasTrailingSpace ? 1 : 0));

            input.value = newValue;
            context.onInputValueChange?.(newValue);

            const remainingValues = context.value.filter((v) => v !== mentionBeforeCursor.value);
            context.onValueChange?.(remainingValues);
          ...
          if (hasTrailingSpace && cursorPosition === mentionBeforeCursor.end + 1 && !isCtrlOrCmd) {
            ...
            input.value = newValue;
            context.onInputValueChange?.(newValue);
            input.setSelectionRange(mentionBeforeCursor.end, mentionBeforeCursor.end);
            return;
          }
      ...
      if (event.inputType === "deleteContentBackward") {
        ...
        if (mentionAtCursor) {
          ...
          const newValue =
            ...
            input.value.slice(mentionAtCursor.end + (hasTrailingSpace ? 1 : 0));

          input.value = newValue;
          context.onInputValueChange?.(newValue);

          const remainingValues = context.value.filter((v) => v !== mentionAtCursor.value);
          context.onValueChange?.(remainingValues);
  ...
  const onPointerDown = React.useCallback(
    (event: React.PointerEvent<InputElement>) => {
      ...
      const textWidth = getTextWidth(input.value.slice(0, input.value.length), input);
      ...
      const clickedMention = context.mentions.find(
        (mention) =>
          approximateClickPosition >= mention.start && approximateClickPosition < mention.end,
      );
      ...
      requestAnimationFrame(async () => {
        ...
        for (let i = 1; i < parts.length; i++) {
          ...
          for (let wordCount = words.length; wordCount > 0; wordCount--) {
            ...
            const candidateWords = words.slice(0, wordCount).filter((_, index) => index % 2 === 0); // Take only non-space segments
  ...
  return (
    <div style={{ position: "relative" }}>
      ...
      <Primitive.input
        ...
        {...props}
        ref={composedRef}
        onBeforeInput={composeEventHandlers(props.onBeforeInput, onBeforeInput)}
