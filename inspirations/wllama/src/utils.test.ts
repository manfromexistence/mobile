...
describe("shard processing", () => {
  test("parseShardNumber extracts correct info", () => {
    ...
    expect(parseShardNumber("abcdef-123456-00001-of-00005.gguf?param=value")).toEqual({
      baseURL: "abcdef-123456",
      current: 1,
      total: 5,
    });

    expect(parseShardNumber("abcdef-123456-00001-of-00005.gguf?no-inline")).toEqual({
      baseURL: "abcdef-123456",
      current: 1,
      total: 5,
    });
    ...
    expect(
      parseShardNumber("abcdef-123456-00001-of-00005.gguf?param1=value1&param2=value2"),
    ).toEqual({
      baseURL: "abcdef-123456",
      current: 1,
      total: 5,
  ...
  test("parseModelUrl generates correct shard URLs", () => {
    ...
    const complexPathWithQuery = "https://example.com/models/llama-00001-of-00002.gguf?no-inline";
  ...
  test("sortFileByShard sorts files by shard number", () => {
    const files = [
      new File([], "e2fc714c4727ee9395f324cd2e7f331f-model-00003-of-00005.gguf"),
      new File([], "187ef4436122d1cc2f40dc2b92f0eba0-model-00001-of-00005.gguf"),
      new File([], "c4357687ea2b461cb07cf0a0a3de939f-model-00002-of-00005.gguf"),
      new File([], "6a4d40512eabd63221cbdf3df4636cd7-model-00005-of-00005.gguf"),
      new File([], "0952e4c6ba320f5278605eb5333eec0f-model-00004-of-00005.gguf"),
    ];

    sortFileByShard(files);

    expect(files.map((f) => parseShardNumber(f.name).current)).toEqual([1, 2, 3, 4, 5]);
...
describe("GGUF file validation", () => {
  test("isValidGgufFile should correctly validate GGUF files", () => {
    ...
    expect(isValidGgufFile("model.gguf?param1=value1&param2=value2")).toBe(true);
    expect(isValidGgufFile("model.gguf?param=value&special=!@#$%^&*()")).toBe(true);
    expect(isValidGgufFile("model.gguf?")).toBe(true);
