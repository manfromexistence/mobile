...
var WebGPU = {
  ...
  iterateExtensions: (root, handlers) => {
    for (
      var ptr = Number((growMemViews(), HEAPU64)[root / 8]);
      ptr;
  ...
  makeColor: (ptr) => ({
    ...
    g: (growMemViews(), HEAPF64)[(ptr + 8) / 8],
    b: (growMemViews(), HEAPF64)[(ptr + 16) / 8],
    a: (growMemViews(), HEAPF64)[(ptr + 24) / 8],
  }),
  makeExtent3D: (ptr) => ({
    width: (growMemViews(), HEAPU32)[ptr / 4],
    height: (growMemViews(), HEAPU32)[(ptr + 4) / 4],
    depthOrArrayLayers: (growMemViews(), HEAPU32)[(ptr + 8) / 4],
  }),
  makeOrigin3D: (ptr) => ({
    x: (growMemViews(), HEAPU32)[ptr / 4],
    y: (growMemViews(), HEAPU32)[(ptr + 4) / 4],
    z: (growMemViews(), HEAPU32)[(ptr + 8) / 4],
  }),
  makeTexelCopyTextureInfo: (ptr) => ({
    texture: WebGPU.getJsObject(Number((growMemViews(), HEAPU64)[ptr / 8])),
    mipLevel: (growMemViews(), HEAPU32)[(ptr + 8) / 4],
  ...
  makeRenderPipelineDesc: (descriptor) => {
    ...
    function makeDepthStencilState(dssPtr) {
      ...
      return {
        ...
        stencilWriteMask: (growMemViews(), HEAPU32)[(dssPtr + 56) / 4],
...
function _emwgpuDeviceCreateShaderModule(devicePtr, descriptor, shaderModulePtr) {
  ...
  var sType = (growMemViews(), HEAP32)[(nextInChainPtr + 8) / 4];
  var desc = { label: WebGPU.makeStringFromOptionalStringView(descriptor + 8), code: "" };
  switch (sType) {
    case 2: {
...
var _wgpuDeviceCreateBindGroup = function (devicePtr, descriptor) {
  ...
  var ret = (() => {
    function makeEntry(entryPtr) {
      ...
      return { binding: (growMemViews(), HEAPU32)[(entryPtr + 8) / 4], resource };
    }
