...
async function readRowLaneState(
  ...
  return page.evaluate((targetRowPath) => {
    ...
    const decorationSection = row.querySelector('[data-item-section="decoration"]');
    ...
    return {
      containsGitChange: row.getAttribute("data-item-contains-git-change") === "true",
      decorationIcon:
        decorationSection?.querySelector("[data-icon-name]")?.getAttribute("data-icon-name") ??
        null,
      ...
      hasGitDot: gitSection?.querySelector('[data-icon-name="file-tree-icon-dot"]') != null,
...
test.describe("trees-dev item customization route", () => {
  test("preserves selected-file decoration while switching composition controls", async ({
    ...
    await expect(page.getByRole("heading", { level: 1, name: "Item Customization" })).toBeVisible();

    const tree = page.locator("file-tree-container").first();
    const appRow = tree.locator('button[data-type="item"][data-item-path="alpha/src/app.ts"]');
    ...
    await page.locator('[data-test-item-customization-trigger-mode="true"]').selectOption("both");
    await expect(
      page.locator('[data-test-item-customization-button-visibility="true"]'),
    ).toBeEnabled();
    await expect(
    ...
    const triggerAffordanceGeometry = await readTriggerAffordanceGeometry(page, "alpha/src/app.ts");
    expect(triggerAffordanceGeometry).not.toBeNull();
    expect(
      Math.abs(
        triggerAffordanceGeometry!.triggerCenterX - triggerAffordanceGeometry!.decorativeCenterX,
      ),
    ...
    await expect
      .poll(async () => (await readRowLaneState(page, "alpha/src/app.ts"))?.decorationIcon)
      .toBe("trees-dev-item-app");
    await expect
      .poll(async () => (await readRowLaneState(page, "alpha/src/app.ts"))?.decorationText)
      .toBeNull();
    ...
    await expect
      .poll(async () => (await readRowLaneState(page, "alpha/src/app.ts"))?.hasActionLane)
      .toBe(false);

    await appRow.click({ button: "right" });
    await expect(page.locator('[data-test-item-customization-menu="true"]')).toBeVisible();
  ...
  test("selection clicks preserve collapsed expansion state and visible row order", async ({
    ...
    const tree = page.locator("file-tree-container").first();
    const srcFolder = tree.locator('button[data-type="item"][data-item-path="alpha/src/"]');
    const keepRow = tree.locator('button[data-type="item"][data-item-path="beta/keep.txt"]');
    ...
    await expect.poll(() => readVisibleTreeState(page, "alpha/src/")).toEqual(collapsedTreeState);
  });
  ...
  test("shared git-status presets drive both the customization route and git-status page", async ({
    ...
    await expect
      ...
      .toBe("ignored");
    await expect.poll(async () => (await readRowLaneState(page, "beta/"))?.gitLabel).toBeNull();
    await expect
      .poll(async () => (await readRowLaneState(page, "beta/archive/notes.txt"))?.gitLabel)
      .toBe("M");
    await expect
      .poll(async () => (await readRowLaneState(page, "gamma/logs/"))?.hasGitDot)
      .toBe(true);
    await expect
      .poll(async () => (await readRowLaneState(page, "gamma/logs/"))?.containsGitChange)
      .toBe(true);

    await page.goto("/trees-dev/git-status");

    await expect(page.getByRole("heading", { level: 1, name: "Git Status" })).toBeVisible();
    await expect(
      page
    ...
    await page
      ...
      .selectOption("ignored-and-overrides");
    await expect(page.locator('[data-test-git-status-active-description="true"]')).toContainText(
      "ignored inheritance on beta/",
    );
    await expect
      .poll(async () => (await readRowLaneState(page, "beta/archive/notes.txt"))?.gitLabel)
      .toBe("M");
