...
test.describe("trees-dev drag-and-drop route", () => {
  test("host dropzone reads the file name without moving the tree row", async ({ page }) => {
    await page.goto("/trees-dev/drag-and-drop");
    await expect(page.getByRole("heading", { level: 1, name: "Drag and Drop" })).toBeVisible();
    ...
    await expect(page.locator('[data-test-host-dropzone-last-file="true"]')).toContainText(
      "logo.png",
    );
    await expect(page.getByText(`external-drop:${sourcePath}`, { exact: true })).toBeVisible();
