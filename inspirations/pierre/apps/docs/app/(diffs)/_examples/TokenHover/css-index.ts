...
export interface CSSHoverInfo {
  ...
  description: string;
  syntax?: string;
  category: "property" | "custom-property" | "value" | "at-rule" | "selector" | "function";
  mdnURL?: string;
  origin?: string;
...
function mdnUrl(entry: VSCodeEntry): string | undefined {
  return entry.references?.find((r) => r.url.includes("developer.mozilla.org"))?.url;
}

function baselineInfo(entry: VSCodeEntry): CSSHoverInfo["baseline"] | undefined {
...
function statusBadge(entry: VSCodeEntry): CSSHoverInfo["statusBadge"] | undefined {
  if (
...
const CSS_VALUES: Record<string, CSSHoverInfo> = {
  ...
  auto: {
    name: "auto",
    description: "Lets the browser calculate and select a value automatically based on context.",
    category: "value",
  },
  inherit: {
    name: "inherit",
    description: "Causes the property to take the computed value of its parent element.",
    category: "value",
  },
  "inline-size": {
    name: "inline-size",
  ...
  column: {
    name: "column",
    description: "A flex-direction value that arranges flex items vertically from top to bottom.",
    category: "value",
  },
  wrap: {
    name: "wrap",
    description: "A flex-wrap value that allows flex items to flow onto multiple lines.",
    category: "value",
  },
  nowrap: {
    name: "nowrap",
...
const CSS_CUSTOM_PROPERTIES: Record<string, CSSHoverInfo> = {
  "--color-surface": {
    ...
    mdnURL: "https://developer.mozilla.org/en-US/docs/Web/CSS/Using_CSS_custom_properties",
  },
  "--color-border": {
    name: "--color-border",
    description: "The standard border color for interactive and structural elements.",
  ...
  "--color-text-muted": {
    ...
    description: "A subdued text color for secondary content, captions, and placeholders.",
...
const SELECTOR_PATTERNS: Array<{
...
}> = [
  {
    ...
    getInfo: () => ({
      ...
      syntax: ".parent { & .child { ... } }",
      category: "selector",
      mdnURL: "https://developer.mozilla.org/en-US/docs/Web/CSS/Nesting_selector",
    }),
  },
  {
  ...
  },
  {
    ...
    getInfo: (t) => ({
      ...
      specificity: "(0, 1, 0)",
      category: "selector",
      mdnURL: "https://developer.mozilla.org/en-US/docs/Web/CSS/Class_selectors",
    }),
  },
  {
    test: (t) => t.startsWith("#") && t.length > 1,
...
export function lookupCSSToken(tokenText: string): CSSHoverInfo | null {
  ...
  const pseudo =
    ...
    (trimmed.endsWith("(") ? pseudoByName.get(trimmed + ")") : pseudoByName.get(trimmed + "()"));
 ...
 */
const VALUE_DESCRIPTIONS: Record<string, (value: string) => CSSHoverInfo | null> = {
  container: (value) => {
  ...
  "text-box-trim": (value) => {
    const keywords: Record<string, string> = {
      ...
      "trim-both": "Trims half-leading space from both the first and last formatted lines.",
 ...
 */
export function lookupCSSPropertyValue(property: string, rawValue: string): CSSHoverInfo | null {
