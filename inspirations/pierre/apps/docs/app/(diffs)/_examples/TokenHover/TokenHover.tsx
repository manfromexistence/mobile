...
import { type CSSHoverInfo, lookupCSSPropertyValue, lookupCSSToken } from "./css-index";
 ...
 */
function resolveCompoundToken(tokenText: string, tokenElement: HTMLElement): ResolvedToken {
  ...
  if (prevText === "." || prevText === "#") return { text: `${prevText}${tokenText}` };
  ...
  if (tokenText === ":" && nextText.length > 0 && !/\s/.test(nextText[0])) {
    ...
    if (prevText === "&" || prevText.endsWith("&")) return { text: `&${pseudo}` };
    return { text: pseudo };
  }
  ...
  if (prevText === ":" && tokenText.length > 0 && !/\s/.test(tokenText[0])) {
    ...
    if (grandPrev === "&" || grandPrev.endsWith("&")) return { text: `&${pseudo}` };
    return { text: pseudo };
  }
...
export function TokenHover({ prerenderedDiff }: TokenHoverProps) {
  ...
  const onTokenEnter = useCallback(
    ({ tokenText, tokenElement }: DiffTokenEventBaseProps) => {
      ...
      if (activeTokenRef.current != null && activeTokenRef.current !== tokenElement) {
        activeTokenRef.current.style.textDecoration = "";
        activeTokenRef.current.style.backgroundColor = "";
        activeTokenRef.current.style.borderRadius = "";
      }
      ...
      tokenElement.style.backgroundColor = "color-mix(in srgb, currentColor 12%, transparent)";
  ...
  return (
    <div className="space-y-5">
      <FeatureHeader
        ...
        description={
          <>
            Attach hover callbacks to individual syntax tokens with <code>onTokenEnter</code> and{" "}
            <code>onTokenLeave</code>. Here we’ve added a static CSS knowledge index to show
            information-rich tooltips on hover. Try hovering over CSS properties, values, and
            at-rules below. See the{" "}
...
function BaselineBadge({
  ...
  return (
    <span className={`mt-1 inline-flex items-center gap-1 ${colorClass}`}>
      <Icon size={12} />
      {label}
      {date != null && <span className="opacity-50">since {formatBaselineDate(date)}</span>}
    </span>
  );
}
...
const HoverTooltip = forwardRef<HTMLDivElement, HoverTooltipProps>(function HoverTooltip(
  { info, x, y, onMouseEnter, onMouseLeave },
  ref,
) {
  ...
  const setRefs = useCallback(
    (el: HTMLDivElement | null) => {
      ...
      else if (ref != null) (ref as { current: HTMLDivElement | null }).current = el;
    },
    [ref],
  );
  ...
  useEffect(() => {
    ...
    left = Math.max(VIEWPORT_PAD, Math.min(left, window.innerWidth - tooltipWidth - VIEWPORT_PAD));
    ...
    >
      <div className="flex max-w-[400px] flex-col gap-1.5 rounded-lg border border-neutral-700 bg-neutral-900 p-3 text-sm shadow-xl shadow-black/30">
        <div className="flex items-center gap-2">
          <span className="font-mono font-semibold text-blue-300">{info.name}</span>
        ...
        {info.specificity != null && (
          <span className="text-xs text-neutral-400">
            Specificity: <span className="font-mono text-purple-300/90">{info.specificity}</span>
          </span>
        )}
        {info.origin != null && (
          <span className="text-xs text-neutral-500">
            Defined in <span className="font-mono text-yellow-400/80">{info.origin}</span>
          </span>
        )}
        {(info.baseline != null || info.statusBadge != null) && (
          <div className="flex items-baseline gap-2.5 text-xs">
            {info.baseline != null && <BaselineBadge baseline={info.baseline} />}
            {info.statusBadge != null && <StatusBadge status={info.statusBadge} />}
          </div>
        )}
        {info.mdnURL != null && (
          <a
