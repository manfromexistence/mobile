  ...
  return (
    <div className="space-y-5">
      <FeatureHeader
        ...
        description={
          <>
            Turn on line selection with <code>enableLineSelection: true</code>. When enabled,
            clicking a line number will select that line. Click and drag to select multiple lines,
            or hold Shift and click to extend your selection. You can also control the selection
            programmatically. Also selections will elegantly manage the differences between{" "}
      ...
      <div className="bg-muted flex flex-col gap-2 rounded-lg p-3 sm:flex-row sm:items-center sm:gap-0">
        ...
        <div className="flex min-w-0 flex-wrap gap-2 sm:ml-auto">
          ...
          <div className="flex min-w-0 flex-wrap gap-1">
            <Button
              variant="outline"
              onClick={() => setDiffStyle((current) => (current === "split" ? "unified" : "split"))}
              title={diffStyle === "split" ? "Switch to unified" : "Switch to split"}
              aria-label="Toggle diff view style"
              className="aspect-square px-0"
            >
              {diffStyle === "split" ? <IconDiffSplit size={16} /> : <IconDiffUnified size={16} />}
            </Button>
            <Button
              variant="outline"
              ...
              className="aspect-square px-0"
            >
              {disableBackground ? <IconCodeStyleBars size={16} /> : <IconCodeStyleBg size={16} />}
            </Button>
            <Button
              variant="outline"
              onClick={() => setThemeType((current) => (current === "dark" ? "light" : "dark"))}
              title={themeType === "dark" ? "Switch to light" : "Switch to dark"}
              aria-label="Toggle color theme"
              className="aspect-square px-0"
            >
              {themeType === "dark" ? <IconMoon size={16} /> : <IconSun size={16} />}
            </Button>
          </div>
        </div>
      </div>
