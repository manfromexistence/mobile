import { preloadFileDiff, preloadMultiFileDiff, preloadUnresolvedFile } from "@pierre/diffs/ssr";
...
async function SplitUnifiedSection() {
  return <SplitUnified prerenderedDiff={await preloadMultiFileDiff(SPLIT_UNIFIED)} />;
}

async function ShikiThemesSection() {
  return <ShikiThemes prerenderedDiff={await preloadMultiFileDiff(SHIKI_THEMES)} />;
}

async function DiffStylesSection() {
  return <DiffStyles prerenderedDiff={await preloadMultiFileDiff(DIFF_STYLES)} />;
}

async function FontStylesSection() {
  return <FontStyles prerenderedDiff={await preloadMultiFileDiff(FONT_STYLES)} />;
}

async function CustomHeaderSection() {
  return <CustomHeader prerenderedDiff={await preloadMultiFileDiff(CUSTOM_HEADER_EXAMPLE)} />;
}
...
async function MergeConflictSection() {
  return <MergeConflict prerenderedFile={await preloadUnresolvedFile(MERGE_CONFLICT_EXAMPLE)} />;
}

async function AnnotationsSection() {
  return <Annotations prerenderedDiff={await preloadMultiFileDiff(ANNOTATION_EXAMPLE)} />;
}

async function LineSelectionSection() {
  return <LineSelection prerenderedDiff={await preloadMultiFileDiff(LINE_SELECTION_EXAMPLE)} />;
}

async function TokenHoverSection() {
  return <TokenHover prerenderedDiff={await preloadMultiFileDiff(TOKEN_HOVER_EXAMPLE)} />;
}

async function ArbitraryFilesSection() {
  return <ArbitraryFiles prerenderedDiff={await preloadMultiFileDiff(ARBITRARY_DIFF_EXAMPLE)} />;
}

async function AcceptRejectSection() {
  return <AcceptRejectExample prerenderedDiff={await preloadFileDiff(ACCEPT_REJECT_EXAMPLE)} />;
}
