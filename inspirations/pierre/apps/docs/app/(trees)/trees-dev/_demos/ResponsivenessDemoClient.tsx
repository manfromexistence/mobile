...
const RESPONSIVENESS_DEMO_PATHS = [
  ...
  ...Array.from(
    { length: 18 },
    (_, index) => `packages/ui/src/generated/icons/icon-${String(index + 1).padStart(2, "0")}.tsx`,
  ),
  ...Array.from(
    { length: 12 },
    (_, index) =>
...
function getClampedPanelSize(container: HTMLDivElement | null, panelSize: PanelSize): PanelSize {
...
export function ResponsivenessDemoClient() {
  ...
  const [activeResizeAxis, setActiveResizeAxis] = useState<ResizeAxis | null>(null);
  ...
  const handleResizeStart = useCallback(
    (axis: ResizeAxis, event: ReactPointerEvent<HTMLDivElement>) => {
      ...
      const handlePointerMove = (moveEvent: PointerEvent) => {
        if (axis === "width") {
          setClampedAxisSize("width", startWidth + (moveEvent.clientX - startX));
          return;
        }

        setClampedAxisSize("height", startHeight + (moveEvent.clientY - startY));
      };
  ...
  const handleResizeKeyDown = useCallback(
    (axis: ResizeAxis, event: ReactKeyboardEvent<HTMLDivElement>) => {
      switch (event.key) {
        ...
        case "Home":
          event.preventDefault();
          setClampedAxisSize(axis, axis === "width" ? PANEL_MIN_WIDTH : PANEL_MIN_HEIGHT);
          return;
        case "End": {
          event.preventDefault();
  ...
  return (
    <div className="space-y-4">
      ...
      <p className="max-w-3xl text-sm text-neutral-600">
        This proof gives the tree host <code>height: 100%</code> and does not rely on an initial
        row-budget hint. Drag the right edge or the bottom edge to see whether the file tree tracks
        its container like a full explorer pane.
      </p>
      ...
      <div className="flex flex-wrap items-center gap-3 text-xs text-neutral-600">
        ...
        <span>Flatten empty dirs: {flattenEmptyDirectories ? "on" : "off"}</span>
      </div>
      ...
      >
        <div className="flex h-full min-h-0">
          ...
          <section className="hidden min-w-0 flex-1 flex-col gap-3 p-4 text-sm text-zinc-400 sm:flex">
            ...
            <p>
              In real apps the tree usually sits beside editor or detail content. This filler pane
              keeps the demo honest without needing a full-page layout.
            </p>
            <div className="grid gap-3 lg:grid-cols-2">
              <div className="rounded-md border border-white/10 bg-white/5 p-3">
                <p className="font-medium text-zinc-100">What to watch</p>
                <p className="mt-1 text-xs leading-5 text-zinc-400">
                  The tree viewport should grow and shrink with this container instead of staying
                  stuck at its initial height.
                </p>
              </div>
              <div className="rounded-md border border-white/10 bg-white/5 p-3">
                <p className="font-medium text-zinc-100">Current setup</p>
                <p className="mt-1 text-xs leading-5 text-zinc-400">
                  Width changes come from the right edge. Height changes come from the bottom edge.
                  The explorer keeps the same model while the host box resizes around it.
                </p>
              </div>
            </div>
          </section>
