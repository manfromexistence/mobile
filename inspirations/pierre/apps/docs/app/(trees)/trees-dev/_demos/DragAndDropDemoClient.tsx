...
import { FileTree, type FileTreeDropResult, type FileTreeMutationEvent } from "@pierre/trees";
...
function getDroppedPath(dataTransfer: DataTransfer): string | null {
  ...
  const pathWithoutTrailingSlash = rawPath.endsWith("/") ? rawPath.slice(0, -1) : rawPath;
...
interface DragAndDropDemoClientProps {
  ...
  sharedOptions: Omit<FileTreePathOptions, "dragAndDrop" | "id" | "preparedInput">;
}
...
export function DragAndDropDemoClient({
  ...
  const [lastHostDroppedFileName, setLastHostDroppedFileName] = useState<string | null>(null);

  const handleHostDropzoneDragOver = (event: ReactDragEvent<HTMLDivElement>): void => {
    event.preventDefault();
    event.dataTransfer.dropEffect = "move";
  };

  const handleHostDropzoneDrop = (event: ReactDragEvent<HTMLDivElement>): void => {
    event.preventDefault();
    ...
    <div className="space-y-6">
      <header className="space-y-2">
        ...
        <p className="text-muted-foreground max-w-3xl text-sm leading-6">
          Drag tree rows inside the tree or out to a host-page dropzone. Internal drops commit
          through the built-in move APIs. External drops expose the dragged path through the
          browser&apos;s text/plain DataTransfer payload.
        </p>
      </header>
        ...
        controls={
          <div className="flex flex-col gap-2 text-xs leading-5">
            ...
            <p className="text-muted-foreground">
              The log below shows both mutation events and drop observer hooks so the demo stays
              path-first and mutation-first.
            </p>
          </div>
        }
        footer={
        ...
        <div className="space-y-4">
          ...
          >
            ...
            <p className="text-xs">
              ...
              <span data-test-host-dropzone-last-file="true" className="font-mono">
