...
export function TreesDevSidebar({ onNavigate }: { onNavigate?: () => void }) {
  ...
  const { flattenEmptyDirectories, setFlattenEmptyDirectories, handleResetControls } =
    useTreesDevSettings();
  ...
  return (
    <nav
      ...
      onClick={onNavigate}
    >
      <p className="text-muted-foreground px-3 pb-1 text-xs font-medium">Demos</p>
      {DEMO_PAGES.map(({ slug, label }) => {
        const href = slug === "" ? "/trees-dev" : `/trees-dev/${slug}`;
        const isActive =
          slug === "" ? pathname === "/trees-dev" : pathname.startsWith(`/trees-dev/${slug}`);
        return (
      ...
      <p className="text-muted-foreground px-3 pb-1 text-xs font-medium">Settings</p>
      ...
      <div className="flex flex-col gap-3 px-3 py-1">
        <div className="flex items-center gap-2">
          ...
          />
          <Label htmlFor="flatten-empty-directories" className="cursor-pointer text-xs">
            Flatten Empty Dirs
          </Label>
        </div>
