...
import type { ContextMenuTriggerMode, FileTreeCompositionOptions } from "@pierre/trees";
...
export function DemoContextMenuClient({ preloadedDataById }: DemoContextMenuClientProps) {
  ...
  const bothComposition = useMemo(() => buildContextMenuComposition("both"), []);
  const rightClickComposition = useMemo(() => buildContextMenuComposition("right-click"), []);
  const buttonComposition = useMemo(() => buildContextMenuComposition("button"), []);
    ...
    <TreeExampleSection>
      <FeatureHeader
        ...
        description={
          <>
            ...
            and the React <code>renderContextMenu</code> prop. This demo exposes trigger modes for
            right-click, trigger button, or both, and menu actions for new files, new folders,
            rename, and delete. This demo uses Shadcn UI components for the context menu as an
      ...
      <div className="space-y-4">
        <div className="flex flex-wrap items-center gap-3">
          <ButtonGroup
            value={activeMode}
            onValueChange={(value) => setActiveMode(value as ContextMenuTriggerMode)}
          >
