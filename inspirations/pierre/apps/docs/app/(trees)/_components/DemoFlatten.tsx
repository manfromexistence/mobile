...
const FLATTENED_MATCHED_EXPANDED_PATHS = ["build", "build/assets/images/social"] as const;
const flattenStyle = {
  colorScheme: "dark",
  ...
  return (
    <TreeExampleSection>
      <FeatureHeader
        ...
        description={
          <>
            Enable the <code>flattenEmptyDirectories</code> boolean option in tree options to
            collapse single-child folder chains into one row for a more compact tree.{" "}
            <Link
      ...
      <div className="grid grid-cols-1 gap-6 md:grid-cols-2">
        <div className={mobileView === "hierarchical" ? "space-y-2" : "hidden space-y-2 md:block"}>
          <TreeExampleHeading icon={<IconFileTreeFill />}>Default expanded</TreeExampleHeading>
          <FlattenDemoTree
        ...
        <div className={mobileView === "flattened" ? "space-y-2" : "hidden space-y-2 md:block"}>
          <TreeExampleHeading icon={<IconFolders />}>Flattened directories</TreeExampleHeading>
          <FlattenDemoTree
            flattenEmptyDirectories
