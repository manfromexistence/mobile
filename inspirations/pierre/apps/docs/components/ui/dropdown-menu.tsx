  ...
  (
    ...
    const setRefs = React.useCallback(
      (node: React.ElementRef<typeof DropdownMenuPrimitive.Content> | null) => {
        const scrollIntoView = node != null && scrollSelectedIntoView && localRef.current !== node;
...
>(({ className, inset = false, selected = false, variant = "default", ...props }, ref) => (
...
>(({ className, children, checked, ...props }, ref) => (
  <DropdownMenuPrimitive.CheckboxItem
    ref={ref}
    className={cn(
      "focus:bg-accent focus:text-accent-foreground relative flex cursor-default items-center rounded-sm py-1.5 pr-9 pl-3 text-sm transition-colors outline-none select-none data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
      className,
    )}
    checked={checked}
    {...props}
