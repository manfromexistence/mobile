...
describe("resolveDiffshubViewerRoute", () => {
  ...
  describe("GitHub (default host) canonical paths", () => {
    test("PR path renders without rewrite", () => {
      expect(resolveDiffshubViewerRoute(["owner", "repo", "pull", "123"], undefined)).toEqual({
        domain: undefined,
        kind: "render",
    ...
    test("commit path renders without rewrite", () => {
      expect(resolveDiffshubViewerRoute(["owner", "repo", "commit", "abc1234"], undefined)).toEqual(
        {
          domain: undefined,
          kind: "render",
    ...
    test("empty-string domain is treated as default GitHub", () => {
      expect(resolveDiffshubViewerRoute(["owner", "repo", "pull", "123"], "")).toEqual({
        domain: undefined,
        kind: "render",
        upstreamPath: "/owner/repo/pull/123",
        url: "https://github.com/owner/repo/pull/123",
  ...
  describe("GitHub (default host) redirects", () => {
    test("PR changes tab redirects to canonical PR path", () => {
      expect(
        resolveDiffshubViewerRoute(["owner", "repo", "pull", "123", "changes"], undefined),
      ).toEqual({
        kind: "redirect",
    ...
    test("PR files tab redirects to canonical PR path", () => {
      expect(
        resolveDiffshubViewerRoute(["owner", "repo", "pull", "123", "files"], undefined),
      ).toEqual({
        kind: "redirect",
        target: "/owner/repo/pull/123",
      });
    ...
    test("PR changes tab scoped to a SHA redirects to commit path", () => {
      expect(
        ...
        kind: "redirect",
        target: "/pierrecomputer/pierre/commit/83fea5e63ef8751ddbcfabe33154bc2e096c3d85",
      });
    });

    test("PR files tab scoped to a SHA redirects to commit path", () => {
      expect(
        resolveDiffshubViewerRoute(["owner", "repo", "pull", "123", "files", "abc1234"], undefined),
      ).toEqual({
        kind: "redirect",
  ...
  describe("alternate domain", () => {
    test("renders against the requested host without rewriting", () => {
      expect(
        resolveDiffshubViewerRoute(["owner", "repo", "pull", "123", "changes"], "gitlab.com"),
      ).toEqual({
        domain: "gitlab.com",
        kind: "render",
