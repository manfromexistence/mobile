...
describe("getPatchViewerHref", () => {
  describe("full GitHub URLs", () => {
    ...
    test("PR changes tab URL", () => {
      expect(getPatchViewerHref("https://github.com/owner/repo/pull/123/changes")).toBe(
        "/owner/repo/pull/123",
      );
    });

    test("PR files tab URL", () => {
      expect(getPatchViewerHref("https://github.com/owner/repo/pull/123/files")).toBe(
        "/owner/repo/pull/123",
      );
    });

    test("compare URL", () => {
      expect(getPatchViewerHref("https://github.com/torvalds/linux/compare/v6.0...v7.0")).toBe(
        "/torvalds/linux/compare/v6.0...v7.0",
      );
    });

    test("commit URL", () => {
      expect(getPatchViewerHref("https://github.com/owner/repo/commit/abc123def")).toBe(
        "/owner/repo/commit/abc123def",
      );
    });
    ...
    test("PR changes tab scoped to a commit SHA", () => {
      ...
      ).toBe("/pierrecomputer/pierre/commit/83fea5e63ef8751ddbcfabe33154bc2e096c3d85");
    });

    test("PR files tab scoped to a commit SHA", () => {
      expect(getPatchViewerHref("https://github.com/owner/repo/pull/123/files/abc1234")).toBe(
  ...
  describe("domain-relative URLs (no protocol)", () => {
    test("github.com PR path", () => {
      expect(getPatchViewerHref("github.com/owner/repo/pull/123")).toBe("/owner/repo/pull/123");
    });

    test("github.com compare path", () => {
      expect(getPatchViewerHref("github.com/torvalds/linux/compare/v6.0...v7.0")).toBe(
        "/torvalds/linux/compare/v6.0...v7.0",
      );
    });
  });
  ...
  describe("GitHub shorthand (owner/repo#number)", () => {
    ...
    test("nodejs/node#59805", () => {
      expect(getPatchViewerHref("nodejs/node#59805")).toBe("/nodejs/node/pull/59805");
    });
  });
  ...
  describe("raw GitHub diff URLs", () => {
    test("raw diff URL", () => {
      expect(
        getPatchViewerHref("https://patch-diff.githubusercontent.com/raw/owner/repo/pull/123.diff"),
