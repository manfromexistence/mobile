...
import { extractImportSpecifiers, findCoreViolations } from "../../scripts/assert-core-clean";
...
describe("core dist guard", () => {
  ...
  test("flags a file that imports a forbidden module", () => {
    ...
    try {
      ...
      writeFileSync(join(tmpDir, "index.js"), `import "shiki";\nexport const x = 1;\n`);
  ...
  test("flags a file that contains tokenColors", () => {
    ...
    try {
      mkdirSync(tmpDir, { recursive: true });
      writeFileSync(join(tmpDir, "index.js"), `export const theme = { tokenColors: [] };\n`);
      const violations = findCoreViolations(tmpDir);
      expect(violations.length).toBeGreaterThan(0);
  ...
  test("flags a file that references the window global", () => {
    ...
    try {
      mkdirSync(tmpDir, { recursive: true });
      writeFileSync(join(tmpDir, "index.js"), `export const w = window.location;\n`);
      const violations = findCoreViolations(tmpDir);
      expect(violations.length).toBeGreaterThan(0);
  ...
  test("source-specific imports stay out of core/react", () => {
    ...
    for (const file of jsFiles) {
      ...
      const importsShikiCore = specs.some(
        (s) => s === "shiki/core" || s === "@shikijs/core" || s === "@shikijs/primitive",
      );
