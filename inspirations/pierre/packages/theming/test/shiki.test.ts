...
async function loadBundledThemeEntries(): Promise<BundledThemeEntry[]> {
  return Promise.all(
    themeNames.map(async (name) => {
      ...
      if (themeModule.default.type !== "dark" && themeModule.default.type !== "light") {
...
describe("shikiThemes", () => {
  test("matches @shikijs/themes light and dark theme lists alphabetically", async () => {
    ...
    const expectedLightNames = sortNames(
      bundled.filter((theme) => theme.type === "light").map((theme) => theme.name),
    );
    const expectedDarkNames = sortNames(
      bundled.filter((theme) => theme.type === "dark").map((theme) => theme.name),
    );

    expect(shikiThemes.getThemeNames({ colorScheme: "light" })).toEqual(expectedLightNames);
    expect(shikiThemes.getThemeNames({ colorScheme: "dark" })).toEqual(expectedDarkNames);
    expect(sortNames(shikiThemes.getThemeNames())).toEqual(sortNames([...themeNames]));
  });
  ...
  test("exports every bundled Shiki theme with the matching color scheme", async () => {
    ...
    const descriptors = [...shikiThemes.getThemes()].sort((a, b) => a.name.localeCompare(b.name));
  ...
  test("returns descriptors with names and known color schemes", () => {
    ...
    expect(collection.getThemeNames({ colorScheme: "light" })).toContain("catppuccin-latte");
    expect(collection.getThemeNames({ colorScheme: "dark" })).toContain("nord");
    expect(collection.getThemeNames()).not.toContain("pierre-dark");
    expect(collection.getThemes().find((theme) => theme.name === "nord")).toMatchObject({
      name: "nord",
      colorScheme: "dark",
      collection: "shiki",
    });
  ...
  test("picks only the requested packaged descriptors", () => {
    ...
    expect(picked.getThemeNames({ colorScheme: "light" })).toEqual(["github-light"]);
    expect(picked.getThemeNames({ colorScheme: "dark" })).toEqual(["solarized-dark"]);
  });
...
describe("themes", () => {
  ...
  test("supports app-level ordering from the combined collection", () => {
    ...
    expect(reverse.getThemeNames()).toEqual(["solarized-dark", "pierre-light", "github-light"]);
  });
});
