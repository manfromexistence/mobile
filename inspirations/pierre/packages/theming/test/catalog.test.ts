...
describe("createThemeCatalog", () => {
  test("keeps defaults and filters names by color scheme", () => {
    ...
    expect(catalog.getThemeNames()).toEqual(["acme-light", "acme-dark", "acme-neutral"]);
    expect(catalog.getThemeNames({ colorScheme: "light" })).toEqual(["acme-light"]);
    expect(catalog.getThemeNames({ colorScheme: "dark" })).toEqual(["acme-dark"]);
    expect(catalog.getThemes({ colorScheme: "light" }).map((t) => t.name)).toEqual(["acme-light"]);
    expect(catalog.getThemeNames({ collection: "acme" })).toEqual(["acme-light", "acme-dark"]);
    expect(
      catalog.getThemes({ collection: "acme", colorScheme: "dark" }).map((t) => t.name),
    ).toEqual(["acme-dark"]);
  });
  ...
  test("composes larger collections from smaller collections", () => {
    ...
    expect(combined.getThemeNames({ collection: "base" })).toEqual(["base-light"]);
  ...
  test("uses registerThemeIfAbsent so existing resolver entries win", async () => {
    ...
    const resolver = createThemeResolver();
    resolver.registerTheme("already-present", () => Promise.resolve(existingTheme));
...
describe("createThemeCollection", () => {
  ...
  test("pick preserves caller order and orderBy returns a reordered collection", () => {
    ...
    expect(collection.pick(["middle", "zeta"]).getThemeNames()).toEqual(["middle", "zeta"]);
    expect(collection.orderBy((a, b) => a.name.localeCompare(b.name)).getThemeNames()).toEqual([
      "alpha",
      "middle",
      "zeta",
    ]);
