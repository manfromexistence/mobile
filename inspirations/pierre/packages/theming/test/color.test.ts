...
import { auroraX, ayuDark, catppuccin, slackOchin, vesper } from "./__fixtures__/themes";
...
describe("color entry surface", () => {
  test("exposes normalizeThemeColors and the colorUtils transform bag", () => {
    ...
    for (const name of [
      ...
      expect(typeof colorUtils[name as keyof typeof colorUtils]).toBe("function");
    }
  });
});
...
describe("normalizeThemeColors: surface fallback fills", () => {
  ...
  test("sideBar.background falls all the way through to theme.bg", () => {
    expect(colorsOf({ bg: "#202020", colors: {} })["sideBar.background"]).toBe("#202020");
  });

  test("sideBar.foreground falls through editor.foreground then theme.fg", () => {
    expect(
      colorsOf({ fg: "#aaa", colors: { "editor.foreground": "#bbb" } })["sideBar.foreground"],
    ).toBe("#bbb");
    expect(colorsOf({ fg: "#aaa", colors: {} })["sideBar.foreground"]).toBe("#aaa");
  });

  test("sideBarSectionHeader.foreground falls back to the resolved sidebar fg", () => {
    expect(colorsOf(catppuccin)["sideBarSectionHeader.foreground"]).toBe("#CDD6F4");
    const c = colorsOf({
      colors: {
  ...
  test("list.activeSelectionForeground falls back to the resolved sidebar fg", () => {
    expect(colorsOf(catppuccin)["list.activeSelectionForeground"]).toBe("#CDD6F4");
    expect(colorsOf(slackOchin)["list.activeSelectionForeground"]).toBe("#FFFFFF");
  });
...
describe("colorUtils transforms", () => {
  test("pickReadableForeground promotes to the brighter readable candidate", () => {
    ...
    const fg = colorUtils.pickReadableForeground("#0F111A", ["#4B526D", "#A6ACCD", "#fff"]);
  ...
  test("pickReadableForeground returns the highest-contrast candidate when none clears 3:1", () => {
    ...
    const fg = colorUtils.pickReadableForeground("#606060", ["#707070", "#a0a0a0"]);
