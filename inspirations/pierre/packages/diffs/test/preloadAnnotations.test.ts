...
describe("preloaded annotations", () => {
  ...
  test("diffs include file-level annotation slots before leading separators", async () => {
    ...
    const annotationIndex = prerenderedHTML.indexOf('slot name="annotation-additions-0"');
    const separatorIndex = prerenderedHTML.indexOf("data-separator=", annotationIndex);
