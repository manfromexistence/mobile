...
import type { DiffLineAnnotation, FileDiffMetadata, LineTypes } from "../src/types";
...
function createDiffWithLeadingSeparator(): FileDiffMetadata {
  ...
  const newLines = oldLines.map((line, index) => (index === 24 ? "changed-25" : line));
...
describe("Annotation Rendering", () => {
  ...
  describe("file-level annotations", () => {
    test("render before a leading hunk separator in unified style", async () => {
      ...
      const { unifiedContentAST } = await renderer.asyncRender(createDiffWithLeadingSeparator());
      assertDefined(unifiedContentAST, "unifiedContentAST should be defined");
      const firstAnnotationIndex = unifiedContentAST.findIndex(isHastAnnotationElement);
      const firstSeparatorIndex = unifiedContentAST.findIndex(
        (node) => isHastElement(node) && node.properties?.["data-separator"] != null,
      );
    ...
    test("render paired top rows in split style", async () => {
      ...
      const { additionsContentAST, deletionsContentAST } = await renderer.asyncRender(diff);
      assertDefined(additionsContentAST, "additionsContentAST should be defined");
      assertDefined(deletionsContentAST, "deletionsContentAST should be defined");
    ...
    test("do not collide with first-row annotation keys in split style", async () => {
      ...
      const { additionsContentAST, deletionsContentAST } = await renderer.asyncRender(
        parseDiffFromFile(
      ...
      const annotationIndexes = getAnnotationIndexes(deletionsContentAST).concat(
        getAnnotationIndexes(additionsContentAST),
      );

      expect(annotationIndexes.filter((index) => index === "-1,-1")).toHaveLength(2);
      expect(annotationIndexes.filter((index) => index === "0,0")).toHaveLength(2);
    });
    ...
    test("render code columns for no-hunk diffs with only file-level annotations", async () => {
      ...
      const { additionsContentAST, deletionsContentAST, rowCount } = await renderer.asyncRender(
        createNoHunkDiff(),
      );
      assertDefined(additionsContentAST, "additionsContentAST should be defined");
      assertDefined(deletionsContentAST, "deletionsContentAST should be defined");
      ...
      expect(getSlotNames(additionAnnotation)).toEqual(["annotation-additions-0"]);
      expect(getSlotNames(deletionAnnotation)).toEqual(["annotation-deletions-0"]);
    });

    test("do not render in non-top diff render chunks", async () => {
      const renderer = new DiffHunksRenderer<string>({ diffStyle: "unified" });
      renderer.setLineAnnotations([{ side: "additions", lineNumber: 0, metadata: "new-file" }]);
      ...
      expect(unifiedContentAST.some((node) => getHastAnnotationIndex(node) === "-1,-1")).toBe(
        false,
      );
    });
  });
  ...
  describe("line index matching", () => {
    test("annotation lineIndex matches preceding line in unified style", async () => {
      ...
      expect(annotationProjection(unifiedAST)).toMatchSnapshot("unified annotation placement");
    });
    ...
    test("annotation lineIndex matches preceding line in split style", async () => {
      ...
      for (const ast of [additionsAST, deletionsAST]) {
        ...
        const indicesSet = isAdditions ? additionsAnnotationIndices : deletionsAnnotationIndices;
      ...
      expect(additionsAnnotationIndices.size).toBe(deletionsAnnotationIndices.size);
  ...
  describe("annotations in different line types", () => {
    test("annotations on all line types (context, addition, deletion, expanded)", async () => {
      ...
      for (let i = 1; i < unifiedAST.length; i++) {
        if (!isHastAnnotationElement(unifiedAST[i]) || !isHastElement(unifiedAST[i])) continue;
    ...
    test("annotations on all line types in split style", async () => {
      ...
      renderer.setLineAnnotations(annotations);
      const { deletionsContentAST, additionsContentAST } = await renderer.asyncRender(diff);
      assertDefined(additionsContentAST, "additionsContentAST should be defined");
      assertDefined(deletionsContentAST, "deletionsContentAST should be defined");
      const additionsAST = additionsContentAST;
      const deletionsAST = deletionsContentAST;
      ...
      expect(additionsAnnotationCount).toBe(Object.keys(additionsExpectedTypes).length);
      ...
      expect(deletionsAnnotationCount).toBe(Object.keys(deletionsExpectedTypes).length);
    });
  });
  ...
  describe("annotation collapsing in unified style", () => {
    ...
    test("in split style, annotations on both sides remain separate", async () => {
      ...
      const additionsElements = collectAllElements(additionsAST);
      const deletionsElements = collectAllElements(deletionsAST);
      const additionAnnotation = additionsElements.find(isHastAnnotationElement);
      const deletionAnnotation = deletionsElements.find(isHastAnnotationElement);
      assertDefined(additionAnnotation, "additionAnnotation should be defined");
      assertDefined(deletionAnnotation, "deletionAnnotation should be defined");
