...
describe("CodeView element pooling", () => {
  test("reuses sanitized item shells without duplicating shared assets", async () => {
    ...
    try {
      ...
      expect(getShadowText(secondElement)).not.toContain("first pooled content");
    } finally {
      viewer.cleanUp();
      await wait(0);
      cleanup();
  ...
  test("clears pooled shells when shared css options change", async () => {
    ...
    try {
      ...
      const pooledCandidates = viewer.getRenderedItems().map((item) => item.element);
      ...
      await renderItems(viewer, [makeFileItem("file:third", "third content", 5)]);
