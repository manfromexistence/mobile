...
import { CodeView, type CodeViewLineSelection } from "../src/components/CodeView";
import { DEFAULT_CODE_VIEW_LAYOUT } from "../src/constants";
import type { CodeViewScrollTarget } from "../src/types";
import { createRoot, installDom, makeFileItem, renderItems, wait } from "./domHarness";
...
describe("CodeView item id updates", () => {
  ...
  test("does not mutate a pending scroll target passed by the caller", async () => {
    ...
    try {
      ...
      expect(viewer.getRenderedItems().map((item) => item.id)).toEqual(["file:new"]);
    } finally {
      viewer.cleanUp();
      await wait(0);
      cleanup();
