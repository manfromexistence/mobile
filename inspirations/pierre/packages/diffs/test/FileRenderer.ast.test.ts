...
describe("FileRenderer AST Structure", () => {
  test("should generate correct AST structure for JavaScript file", async () => {
    ...
    assertDefined(contentColumn.properties, "contentColumn.properties should be defined");
    expect(contentColumn.properties["data-content"]).toBe("");
    assertDefined(contentColumn.children, "contentColumn.children should be defined");
    ...
    for (let i = 0; i < contentColumn.children.length; i++) {
      ...
      assertDefined(lineElement.properties, "lineElement.properties should be defined");
    ...
    for (let i = 0; i < gutter.children.length; i++) {
      ...
      assertDefined(gutterItem.properties, "gutterItem.properties should be defined");
  ...
  test("should apply syntax highlighting with CSS variables", async () => {
    ...
    const tokensWithCSSVars = styledTokens.filter(
      (node) =>
        node.style?.match(/--diffs-token-dark:#[A-F0-9]{6};--diffs-token-light:#[A-F0-9]{6}/) !==
        null,
    );
  ...
  test("renders file-level annotations before the first file line", async () => {
    ...
    expect(findHastSlotElements(firstContent).map((slot) => slot.properties?.name)).toEqual([
      "annotation-0",
    ]);
  ...
  test("does not render file-level annotations in non-top render chunks", async () => {
    ...
    expect(
      contentColumn.children.some(
        (child) =>
          child.type === "element" && child.properties?.["data-line-annotation"] === "-1,-1",
      ),
    ).toBe(false);
  });
