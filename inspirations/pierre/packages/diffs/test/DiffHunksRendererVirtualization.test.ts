...
describe("DiffHunksRenderer - Virtualization", () => {
  ...
  function countNoNewlineElements(ast: ElementContent[]): number {
    return collectAllElements(ast).filter((node) => "data-no-newline" in node.properties).length;
  }
  ...
  describe("full render baselines", () => {
    test("1.1: Zero buffers - unified mode", async () => {
      ...
      assertDefined(result.unifiedContentAST, "unifiedContentAST should be defined");
    ...
    test("1.2: Zero buffers - split mode", async () => {
      ...
      assertDefined(result.additionsContentAST, "additionsContentAST should be defined");
      assertDefined(result.deletionsContentAST, "deletionsContentAST should be defined");
  ...
  describe("no-newline metadata", () => {
    test("renders deletion-side metadata when deletions are shorter in split mode", async () => {
      ...
      expect(getTopLevelNodeKinds(result.deletionsContentAST).slice(-2)).toEqual([
        "buffer",
        "no-newline",
      ]);
      expect(getTopLevelNodeKinds(result.additionsContentAST).slice(-1)).toEqual(["buffer"]);
    });
    ...
    test("renders addition-side metadata when additions are shorter in split mode", async () => {
      ...
      expect(getTopLevelNodeKinds(result.deletionsContentAST).slice(-1)).toEqual(["buffer"]);
      expect(getTopLevelNodeKinds(result.additionsContentAST).slice(-2)).toEqual([
        "buffer",
        "no-newline",
      ]);
    });
  ...
  describe("line count math", () => {
    ...
    test("2.4: Split vs Unified line counting", async () => {
      ...
      const unifiedResult = await unifiedRenderer.asyncRender(fileDiff, renderRange);
      const splitResult = await splitRenderer.asyncRender(fileDiff, renderRange);

      assertDefined(unifiedResult.unifiedContentAST, "unifiedContentAST should be defined");
      assertDefined(splitResult.additionsContentAST, "additionsContentAST should be defined");
      assertDefined(splitResult.deletionsContentAST, "deletionsContentAST should be defined");

      const unifiedLines = countRenderedLines(unifiedResult.unifiedContentAST);
      const splitAdditionLines = countRenderedLines(splitResult.additionsContentAST);
      const splitDeletionLines = countRenderedLines(splitResult.deletionsContentAST);
  ...
  describe("expanded collapsed regions", () => {
    ...
    test("3.2: Partially expanded - fromStart only", async () => {
      ...
      const unexpandedLineCount = countRenderedLines(unexpandedResult.unifiedContentAST);
      ...
      const expandedLines = unifiedIndices.filter((idx) => idx >= 57 && idx <= 76);
    ...
    test("3.3: Partially expanded - fromEnd only", async () => {
      ...
      const expandedLines = unifiedIndices.filter((idx) => idx >= 92 && idx <= 106);
    ...
    test("3.4: Partially expanded - both fromStart and fromEnd", async () => {
      ...
      const expandedFromStart = unifiedIndices.filter((idx) => idx >= 57 && idx <= 66);
      const expandedFromEnd = unifiedIndices.filter((idx) => idx >= 97 && idx <= 106);
    ...
    test("3.5: Windowing with expanded regions (tests a9ff17b7 fix)", async () => {
      ...
      const windowedIndices = extractLineNumbers(result.unifiedContentAST).unifiedIndices;
      ...
      assertDefined(fullResult.unifiedContentAST, "fullResult.unifiedContentAST should be defined");
      const fullIndices = extractLineNumbers(fullResult.unifiedContentAST).unifiedIndices;
      ...
      expect(windowedIndices.filter((idx) => idx >= 57 && idx <= 76)).toHaveLength(20);
    });
    ...
    test("3.6: Fully expanded single hunk range", async () => {
      ...
      const fullyExpandedRange = unifiedIndices.filter((idx) => idx >= 57 && idx <= 106);
  ...
  describe("correct lines rendered", () => {
    test("6.1: Rendered content matches source - unified", async () => {
      ...
      const rows = projectColumn(result.unifiedContentAST).filter((row) => row.kind === "line");
      ...
      expect(collectRowSourceMismatches(rows, "unified", oldFileLines, newFileLines)).toEqual([]);
    });
    ...
    test("6.2: Rendered content matches source - split", async () => {
      ...
      const fullAdditionRows = projectColumn(fullResult.additionsContentAST).filter(
        (row) => row.kind === "line",
      );
      const fullDeletionRows = projectColumn(fullResult.deletionsContentAST).filter(
        (row) => row.kind === "line",
      );
      const splitPositions = [
        ...new Set([...fullAdditionRows, ...fullDeletionRows].map((row) => row.splitIndex)),
      ].sort((a, b) => (a ?? 0) - (b ?? 0));
      ...
      expect(
        collectRowSourceMismatches(additionRows, "additions", oldFileLines, newFileLines),
      ).toEqual([]);
      expect(
        collectRowSourceMismatches(deletionRows, "deletions", oldFileLines, newFileLines),
      ).toEqual([]);
    });
  });
