...
import { disposeHighlighter, getSharedHighlighter } from "../src/highlighter/shared_highlighter";
...
describe("getHighlighterThemeStyles --diffs-* parity", () => {
  test("single theme emits color/bg/global fg/bg and 2-link git colors", () => {
    expect(getHighlighterThemeStyles({ theme: "pierre-dark", highlighter })).toBe(SINGLE_DARK);
  });
