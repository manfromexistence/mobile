...
import { DEFAULT_CODE_VIEW_FILE_METRICS, DEFAULT_VIRTUAL_FILE_METRICS } from "../src/constants";
...
describe("virtual file padding metrics", () => {
  describe("VirtualizedFile", () => {
    test("uses the header height as the default header-enabled top region", () => {
      ...
      expect(instance.getLinePosition(1)?.top).toBe(baseMetrics.diffHeaderHeight);
    });

    test("applies paddingTop after the header", () => {
      const instance = createVirtualizedFile({ paddingTop: 6 });

      expect(instance.getLinePosition(1)?.top).toBe(baseMetrics.diffHeaderHeight + 6);
    });
    ...
    test("allows paddingTop to remove the header-disabled top region", () => {
      const instance = createVirtualizedFile({ paddingTop: 0 }, { disableFileHeader: true });
  ...
  describe("VirtualizedFileDiff", () => {
    test("uses the header height as the default header-enabled top region", () => {
      ...
      expect(instance.getLinePosition(1, "additions")?.top).toBe(baseMetrics.diffHeaderHeight);
    });

    test("applies paddingTop after the header", () => {
      const instance = createVirtualizedFileDiff({ paddingTop: 6 });

      expect(instance.getLinePosition(1, "additions")?.top).toBe(baseMetrics.diffHeaderHeight + 6);
    });

    test("uses spacing as the default header-disabled top region", () => {
      const instance = createVirtualizedFileDiff({}, { disableFileHeader: true });

      expect(instance.getLinePosition(1, "additions")?.top).toBe(baseMetrics.spacing);
    });

    test("allows paddingTop to remove the header-disabled top region", () => {
      const instance = createVirtualizedFileDiff({ paddingTop: 0 }, { disableFileHeader: true });
    ...
    test("does not add paddingBottom when a diff has no hunks", () => {
      ...
      expect(instance.getVirtualizedHeight()).toBe(baseMetrics.diffHeaderHeight + 6);
    });
    ...
    test("uses only the top region when collapsed", () => {
      ...
      const instance = new VirtualizedFileDiff({ collapsed: true }, virtualizer, {
        ...baseMetrics,
        paddingTop: 6,
        paddingBottom: 13,
      });
      ...
      expect(instance.getVirtualizedHeight()).toBe(baseMetrics.diffHeaderHeight + 6);
      expect(instance.getLinePosition(firstHunk.additionStart, "additions")).toEqual({
    ...
    test("keeps hunk separator gaps based on spacing", () => {
      const fileDiff = parseDiffFromFile(
        {
          name: "file.ts",
          contents: Array.from({ length: 20 }, (_, index) => `${index + 1}`).join("\n"),
        },
        {
          name: "file.ts",
      ...
      expect(instance.getLinePosition(secondHunk.additionStart, "additions")?.top).toBe(
    ...
    test("keeps current line-info separator estimates for first, middle, and trailing collapsed context", () => {
      ...
      const firstSeparatorHeight = separatorHeight + codeViewLikeMetrics.spacing;
      const middleSeparatorHeight =
        codeViewLikeMetrics.spacing + separatorHeight + codeViewLikeMetrics.spacing;
      const trailingSeparatorHeight = codeViewLikeMetrics.spacing + separatorHeight;
      const hunkLineHeight =
        (firstHunk.splitLineCount + secondHunk.splitLineCount) * codeViewLikeMetrics.lineHeight;

      instance.prepareCodeViewItem(fileDiff, 0);

      expect(firstHunk.collapsedBefore).toBeGreaterThan(0);
      expect(secondHunk.collapsedBefore).toBeGreaterThan(0);
      expect(instance.getLinePosition(firstHunk.additionStart, "additions")?.top).toBe(
        codeViewLikeMetrics.diffHeaderHeight + firstSeparatorHeight,
      );
      expect(instance.getLinePosition(secondHunk.additionStart, "additions")?.top).toBe(
    ...
    test("keeps current line-info-basic separator estimates without spacing gaps", () => {
      ...
      expect(instance.getLinePosition(firstHunk.additionStart, "additions")?.top).toBe(
        codeViewLikeMetrics.diffHeaderHeight + separatorHeight,
      );
      expect(instance.getLinePosition(secondHunk.additionStart, "additions")?.top).toBe(
        codeViewLikeMetrics.diffHeaderHeight +
          separatorHeight +
    ...
    test("does not reserve metadata separator height for final collapsed context", () => {
      ...
      expect(secondHunk.collapsedBefore).toBeGreaterThan(0);
      expect(instance.getLinePosition(firstHunk.additionStart, "additions")?.top).toBe(
        codeViewLikeMetrics.diffHeaderHeight + 32,
      );
      expect(instance.getVirtualizedHeight()).toBe(
        codeViewLikeMetrics.diffHeaderHeight +
          32 * 2 +
