...
function getTopLevelRowIndex(rows: ElementContent[], key: string): number {
  return rows.findIndex((row) => {
    return isHastElement(row) && row.properties?.["data-test-inline-row"] === key;
  });
}

function getTopLevelLineIndex(rows: ElementContent[], lineNumber: number): number {
  return rows.findIndex((row) => {
...
function getTopLevelBufferIndex(rows: ElementContent[]): number {
  return rows.findIndex((row) => {
    return isHastElement(row) && row.properties?.["data-content-buffer"] != null;
  });
}
...
describe("injected row hooks", () => {
  test("unified hook inserts before rows before the triggering line", async () => {
    ...
    const inlineRowIndex = getTopLevelRowIndex(result.unifiedContentAST, "unified-before-line-1");
  ...
  test("split hook preserves one-sided buffering before later paired rows", async () => {
    ...
    expect(getTopLevelRowNames(result.additionsContentAST)).toEqual(["split-addition-paired"]);

    const additionBufferIndex = getTopLevelBufferIndex(result.additionsContentAST);
  ...
  test("unresolved renderer emits merge conflict action rows inline", async () => {
    ...
    const actionRowIndex = result.unifiedContentAST.findIndex((row) => {
      return isHastElement(row) && row.properties?.["data-merge-conflict-actions"] != null;
    });
    ...
    const markerRows = result.unifiedContentAST.filter((row) => {
      return isHastElement(row) && row.properties?.["data-merge-conflict-marker-row"] != null;
    });
    ...
    expect(
      actionButtons.map((button) => ({
        ...
        conflictIndex: button.properties?.["data-merge-conflict-conflict-index"],
      })),
    ).toEqual([
