...
describe("resolveTheme contract", () => {
  ...
  test("dedupes concurrent loads of the same theme", async () => {
    const [a, b] = await Promise.all([resolveTheme("pierre-light"), resolveTheme("pierre-light")]);
    expect(a).toBe(b);
  });
  ...
  test("rejects a name with no registered or bundled loader", async () => {
    ...
    expect((caughtErr as Error).message).toContain("No valid theme loader registered");
  });
