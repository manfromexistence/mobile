...
import { patchScrollbarGutterSize, wrapThemeCSS } from "../src/utils/cssWrappers";
...
describe("theme CSS scrollbar gutter helpers", () => {
  ...
  test("patchScrollbarGutterSize updates the existing measured gutter declaration", () => {
    const patched = patchScrollbarGutterSize(wrapThemeCSS("--diffs-token: red;", "dark"), 6);
    const updated = patchScrollbarGutterSize(patched, 8);

    expect(updated).toContain(`${DIFFS_SCROLLBAR_GUTTER_MEASURED_PROPERTY}: 8px;`);
    expect(updated).not.toContain(`${DIFFS_SCROLLBAR_GUTTER_MEASURED_PROPERTY}: 6px;`);
    expect(updated.match(new RegExp(DIFFS_SCROLLBAR_GUTTER_MEASURED_PROPERTY, "g"))?.length).toBe(
      1,
    );
  });
});
