...
async function waitForAnnotationCount(container: HTMLElement, expected: number): Promise<void> {
  for (let attempt = 0; attempt < 50; attempt++) {
    const count = container.shadowRoot?.querySelectorAll('[data-line-annotation="-1,-1"]').length;
    if (count === expected) {
      return;
    }
    await wait(10);
...
describe("File partial render", () => {
  test("preserves file-level annotations when a top render range grows", async () => {
    ...
    try {
      ...
      const lineAnnotations: LineAnnotation<string>[] = [{ lineNumber: 0, metadata: "file" }];
      ...
      expect(content?.children[0]?.getAttribute("data-line-annotation")).toBe("-1,-1");
