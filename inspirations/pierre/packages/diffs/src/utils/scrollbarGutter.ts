...
export function getMeasuredScrollbarGutter(shadowRoot: ShadowRoot): number | undefined {
  if (measuredScrollbarGutter != null) {
    return measuredScrollbarGutter;
  }

  const host = shadowRoot.host;
  if (typeof HTMLElement !== "undefined" && host instanceof HTMLElement && !host.isConnected) {
    return undefined;
  }
...
export function createMeasuredScrollbarGutterDeclaration(
  ...
  const value =
    scrollbarGutter == null ? "var(--diffs-scrollbar-gutter-fallback)" : `${scrollbarGutter}px`;
