...
export function includesFileAnnotations(
  ...
  return (
    annotations?.some((annotation) => annotation.lineNumber === FILE_ANNOTATION_LINE_NUMBER) ??
    false
  );
}
...
export function getFileAnnotations<TAnnotation>(
  ...
  return fileAnnotations != null && fileAnnotations.length > 0 ? fileAnnotations : undefined;
}

export function shouldRenderFileAnnotations(renderRange: RenderRange): boolean {
  return renderRange.startingLine === FILE_ANNOTATION_LINE_NUMBER && renderRange.totalLines > 0;
}
