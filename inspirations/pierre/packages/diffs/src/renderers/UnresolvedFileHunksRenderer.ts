...
import { createGutterGap, createHastElement, createTextNodeElement } from "../utils/hast_utils";
import {
...
> extends DiffHunksRenderer<LAnnotation> {
  ...
  private syncInjectedRows(
    ...
    for (const action of conflictActions) {
      const anchor = action != null ? getMergeConflictActionAnchor(action, diff) : undefined;
  ...
  protected override createPreElement(split: boolean, totalLines: number): HASTElement {
  ...
  protected override getUnifiedLineDecoration({
    ...
    const mergeConflictType =
      type === "change" ? (lineType === "change-deletion" ? "current" : "incoming") : undefined;
    return {
  ...
  protected override getSplitLineDecoration({
    ...
    const mergeConflictType =
      type === "change" ? (side === "deletions" ? "current" : "incoming") : undefined;
    return {
...
function getMergeConflictGutterProperties(
  ...
  return mergeConflictType != null ? { "data-merge-conflict": mergeConflictType } : undefined;
}
...
function createMergeConflictGutterGap(
  ...
  gap.properties["data-gutter-buffer"] =
    type === "action" ? "merge-conflict-action" : `merge-conflict-${markerType ?? "marker"}`;
  return gap;
}
...
function createMergeConflictMarkerRowElement(row: MergeConflictMarkerInjectedRow): HASTElement {
  return createHastElement({
    tagName: "div",
    properties: {
...
function createMergeConflictActionsContent(conflictIndex: number): HASTElement[] {
  return [
