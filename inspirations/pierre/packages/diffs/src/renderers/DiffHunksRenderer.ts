...
import { getHighlighterIfLoaded, getSharedHighlighter } from "../highlighter/shared_highlighter";
...
export interface DiffHunksRendererOptionsWithDefaults
  extends Omit<BaseDiffOptionsWithDefaults, "themeType"> {
  headerRenderMode: FileHeaderRenderMode;
}
...
export class DiffHunksRenderer<LAnnotation = undefined> {
  ...
  public expandHunk(
    ...
    expansionLineCount: number = this.getOptionsWithDefaults().expansionLineCount,
  ): void {
    const region = {
  ...
  protected getSplitLineDecoration({ side, type }: SplitLineDecorationProps): LineDecoration {
    ...
    return {
      gutterLineType: side === "deletions" ? "change-deletion" : "change-addition",
    };
  }
  ...
  protected declare getUnifiedInjectedRowsForLine?: (
  ...
  protected declare getSplitInjectedRowsForLine?: (
    ctx: RenderedLineContext,
  ...
  public renderDiff(
    ...
    const forcePlainText =
      !hasContent || isDiffPlainText(diff) || isDiffMassive(diff, this.getTokenizeMaxLength());
    ...
    if (this.workerManager?.isWorkingPool() === true) {
      if (
        ...
        if (this.renderCache.result == null || newContent || newRenderRange || forceHighlight) {
          this.renderCache.result = this.workerManager.getPlainDiffAST(
            ...
            // everything
            isDefaultRenderRange(renderRange) ? true : expandUnchanged ? true : this.expandedHunks,
            collapsedContextThreshold,
          );
        }
      ...
      if (!forcePlainText && hasContent && (!this.renderCache.highlighted || forceHighlight)) {
    ...
    return this.renderCache.result != null
      ? this.processDiffResult(this.renderCache.diff, renderRange, this.renderCache.result)
      : undefined;
  }
  ...
  private async asyncHighlight(diff: FileDiffMetadata): Promise<RenderDiffResult> {
    ...
    const hasThemes =
      this.highlighter != null && areThemesAttached(this.options.theme ?? DEFAULT_THEMES);
    const hasLangs =
      forcePlainText || (this.highlighter != null && areLanguagesAttached(this.computedLang));
  ...
  private hasHighlightedRenderCache(diff: FileDiffMetadata, options: RenderDiffOptions): boolean {
    const { renderCache } = this;
    return (
  ...
  private processDiffResult(
    ...
    const pendingSplitContext: PendingSplitContext = {
      ...
      flush() {
        ...
        if (this.side === "additions") {
          context.pushToGutter("additions", createGutterGap(undefined, "buffer", this.size));
          additionsContentAST?.push(createEmptyRowBuffer(this.size));
        } else {
          context.pushToGutter("deletions", createGutterGap(undefined, "buffer", this.size));
    ...
    iterateOverDiff({
      ...
      callback: ({
        ...
        const splitLineIndex =
          deletionLine != null ? deletionLine.splitLineIndex : additionLine.splitLineIndex;
        const unifiedLineIndex =
          additionLine != null ? additionLine.unifiedLineIndex : deletionLine.unifiedLineIndex;
        ...
        if (diffStyle === "unified") {
          ...
          pushGutterLineNumber(
            ...
            additionLine != null ? additionLine.lineNumber : deletionLine.lineNumber,
          ...
          if (injectedRows?.before != null) {
            pushSplitInjectedRows(injectedRows.before, context, pendingSplitContext);
          }
          ...
          if (missingSide != null) {
            if (pendingSplitContext.side != null && pendingSplitContext.side !== missingSide) {
          ...
          if (injectedRows?.after != null) {
            pushSplitInjectedRows(injectedRows.after, context, pendingSplitContext);
          }
        }
        ...
        if (noEOFCRAddition || noEOFCRDeletion) {
          ...
          if (noEOFCRDeletion) {
            const noEOFType =
              type === "context" || type === "context-expanded" ? type : "change-deletion";
            if (diffStyle === "unified") {
              ...
              if (!noEOFCRAddition) {
                context.pushToGutter("additions", createGutterGap(undefined, "buffer", 1));
          ...
          if (noEOFCRAddition) {
            const noEOFType =
              type === "context" || type === "context-expanded" ? type : "change-addition";
            if (diffStyle === "unified") {
              context.unifiedContentAST.push(createNoNewlineElement(noEOFType));
              context.pushToGutter("unified", createGutterGap(noEOFType, "metadata", 1));
            } else {
              ...
              if (!noEOFCRDeletion) {
                context.pushToGutter("deletions", createGutterGap(undefined, "buffer", 1));
    ...
    additionsContentAST = shouldIncludeAdditions && hasContent ? additionsContentAST : undefined;
    deletionsContentAST = shouldIncludeDeletions && hasContent ? deletionsContentAST : undefined;
    ...
    return {
      unifiedGutterAST: unified && hasContent ? context.unifiedGutterAST.children : undefined,
      unifiedContentAST,
      deletionsGutterAST:
        shouldIncludeDeletions && hasContent ? context.deletionsGutterAST.children : undefined,
      deletionsContentAST,
      additionsGutterAST:
        shouldIncludeAdditions && hasContent ? context.additionsGutterAST.children : undefined,
      additionsContentAST,
      hunkData,
      preNode,
      themeStyles,
      baseThemeType,
      headerElement: !disableFileHeader ? this.renderHeader(this.diff) : undefined,
      totalLines,
  ...
  public renderFullAST(result: HunksRenderResult, children: ElementContent[] = []): HASTElement {
  ...
  public renderFullHTML(result: HunksRenderResult, tempChildren: ElementContent[] = []): string {
    ...
    return toHtml(
      createHastElement({
        ...
        properties: {
          "data-code": "",
          "data-container-size":
            this.getOptionsWithDefaults().hunkSeparators === "line-info" ? "" : undefined,
          [`data-${columnType}`]: "",
        },
      }),
    );
  ...
  private pushFileLevelAnnotations(
    ...
    const additionAnnotationNames =
      fileDiff.type !== "deleted"
        ...
        : [];
    if (deletionAnnotationNames.length === 0 && additionAnnotationNames.length === 0) {
      return;
    }
...
function pushUnifiedInjectedRows(rows: InjectedRow[], context: ProcessContext): void {
...
function pushLineWithAnnotation({
  ...
  if (diffStyle === "unified") {
    ...
    if (unifiedSpan != null) {
      const lineType =
        type === "change" ? (deletionLine != null ? "change-deletion" : "change-addition") : type;
    ...
    if (deletionSpan != null) {
      const lineType =
        type === "change" ? (deletionLine != null ? "change-deletion" : "context") : type;
    ...
    if (additionSpan != null) {
      const lineType =
        type === "change" ? (additionLine != null ? "change-addition" : "context") : type;
...
function pushSeparator(
  ...
  if (context.hunkSeparators === "simple") {
    if (hunkIndex > 0) {
      ...
      linesAST.push(createSeparator({ type: "simple", isFirstHunk, isLastHunk: false }));
      if (type !== "additions") {
        context.incrementRowCount(1);
      }
    }
...
function withContentProperties(
  ...
  if (lineNode == null || lineNode.type !== "element" || contentProperties == null) {
    return lineNode;
  }
  return {
    ...lineNode,
...
function isDiffMassive(diff: FileDiffMetadata, tokenizeMaxLength: number): boolean {
