...
export function FileDiffSSR<LAnnotation>({
  ...
  if (htmlObjectRef.current === null) {
    ...
    const classAttr = className !== undefined ? ` class="${escapeHtml(className)}"` : "";
    const styleAttr = style !== undefined ? ` style="${serializeStyle(style)}"` : "";

    // Render annotations as static HTML slots for SSR
    const annotationSlots =
      annotations !== undefined && annotations.length > 0 && renderAnnotation != null
        ? annotations
            .map((annotation) => {
  ...
  useEffect(() => {
    ...
    if (wrapperRef.current !== null && annotations !== undefined && !hydratedRef.current) {
