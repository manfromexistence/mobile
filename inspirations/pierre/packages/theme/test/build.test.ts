...
describe("generated theme files", () => {
  for (const file of GENERATED_FILES) {
    describe(file.path, () => {
      ...
      test("is non-empty, valid JSON with the expected metadata", () => {
        ...
        assert.equal(typeof theme.displayName, "string", `${file.path}: missing displayName`);
  ...
  test("base themes are large enough to contain full color and token definitions", () => {
    ...
    assert.ok(lightSize >= 10_000, `pierre-light.json seems too small (${lightSize} bytes)`);
    assert.ok(darkSize >= 10_000, `pierre-dark.json seems too small (${darkSize} bytes)`);
  });
});
...
describe("generated ESM wrapper modules", () => {
  ...
  test("index module exists", () => {
    assert.ok(existsSync("dist/index.mjs"), "file does not exist: dist/index.mjs");
  });
});

describe("generated Zed theme files", () => {
  test("theme family exists", () => {
    assert.ok(existsSync("zed/themes/pierre.json"), "file does not exist: zed/themes/pierre.json");
  });
});
