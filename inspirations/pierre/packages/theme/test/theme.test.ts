...
function isValidColor(color: string): boolean {
  ...
  return (
    ...
    /^color\(display-p3\s+[\d.]+\s+[\d.]+\s+[\d.]+(\s+\/\s+[\d.]+)?\)$/.test(color)
  );
}
...
// ── Palette roles ─────────────────────────────────────────────────────────────
const REQUIRED_ROLE_CATEGORIES = ["bg", "fg", "border", "accent", "states", "syntax", "ansi"];
        ...
        const collectRoleHexIssues = (obj: unknown, path: string) => {
          for (const [key, value] of Object.entries(obj as Record<string, unknown>)) {
            ...
            if (typeof value === "string") {
              if (!isValidHexColor(value)) issues.push(`${fullPath}: invalid color "${value}"`);
  ...
  for (const variant of THEME_VARIANTS) {
    describe(variant.displayName, () => {
      ...
      test("has the required top-level properties", () => {
        ...
        assert.ok(Object.keys(theme.colors).length > 0, "missing colors object");
      ...
      test("defines the critical editor colors", () => {
        for (const key of CRITICAL_COLORS) {
          assert.notEqual(theme.colors[key], undefined, `missing critical color: ${key}`);
        }
      });
      ...
      test("has no undefined or null colors", () => {
        for (const [key, value] of Object.entries(theme.colors)) {
          assert.ok(value !== undefined && value !== null, `color "${key}" is ${value}`);
        }
      });

      test("tokenColors is a non-empty array", () => {
        assert.ok(Array.isArray(theme.tokenColors), "tokenColors is not an array");
      ...
      test("every tokenColors entry is well-formed", () => {
        theme.tokenColors.forEach((token, idx) => {
          assert.notEqual(token.scope, undefined, `tokenColors[${idx}] missing scope`);
          assert.notEqual(token.settings, undefined, `tokenColors[${idx}] missing settings`);
