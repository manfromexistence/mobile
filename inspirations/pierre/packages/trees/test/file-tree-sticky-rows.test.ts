...
describe("file-tree sticky rows", () => {
  test("sticky folders are opt-in and mirror visible ancestor rows when enabled", async () => {
    ...
    try {
      ...
      expect(defaultShadowRoot?.querySelector('[data-file-tree-sticky-overlay="true"]')).toBeNull();
  ...
  test("sticky folders cascade through every partially occluded leading directory", async () => {
    ...
    try {
      ...
      expect(getMountedItemPaths(shadowRoot, dom)).not.toContain("arch/alpha/boot/");
      expect(getMountedItemPaths(shadowRoot, dom)).not.toContain("arch/alpha/boot/tools/");
  ...
  test("sticky folders drop at the exact subtree boundary and keep the renderer aligned to the layout snapshot", async () => {
    ...
    try {
      ...
      const expectedBefore = computeExpectedRenderedWindow(expectedController, 59, 180);
      expect(getStickyRowPaths(shadowRoot, dom)).toEqual(
        expectedBefore.layout.sticky.rows.map((entry) => getVisibleRowPath(entry.row)),
      );
      expect(
        getPixelStyleValue(getStickyRowButton(shadowRoot, dom, "arch/alpha/boot/tools/"), "top"),
      ).toBe(90);
      expect(getMountedItemPaths(shadowRoot, dom)).toEqual(expectedBefore.mountedPaths);

      scrollElement.scrollTop = 60;
      scrollElement.dispatchEvent(new dom.window.Event("scroll"));
      await flushDom();

      const expectedAtBoundary = computeExpectedRenderedWindow(expectedController, 60, 180);
      expect(getStickyRowPaths(shadowRoot, dom)).toEqual(
        expectedAtBoundary.layout.sticky.rows.map((entry) => getVisibleRowPath(entry.row)),
      );
      expect(
        getPixelStyleValue(getStickyRowButton(shadowRoot, dom, "arch/alpha/boot/tools/"), "top"),
      ).toBe(90);
      expect(getMountedItemPaths(shadowRoot, dom)).toEqual(expectedAtBoundary.mountedPaths);

      scrollElement.scrollTop = 61;
      scrollElement.dispatchEvent(new dom.window.Event("scroll"));
      await flushDom();

      const expectedAfter = computeExpectedRenderedWindow(expectedController, 61, 180);
      expect(getStickyRowPaths(shadowRoot, dom)).toEqual(
        expectedAfter.layout.sticky.rows.map((entry) => getVisibleRowPath(entry.row)),
      );
      expect(getStickyRowZIndex(shadowRoot, dom, "arch/alpha/boot/")).toBeGreaterThan(
        getStickyRowZIndex(shadowRoot, dom, "arch/alpha/boot/tools/"),
      );
      expect(
        getPixelStyleValue(getStickyRowButton(shadowRoot, dom, "arch/alpha/boot/tools/"), "top"),
      ).toBe(89);
      expect(getMountedItemPaths(shadowRoot, dom)).toEqual(expectedAfter.mountedPaths);
  ...
  test("sticky overlay reserves virtual layout space without changing total height or scrollTop", async () => {
    ...
    try {
      ...
      expect(stickyPaths).toEqual(
        expectedAtSixty.layout.sticky.rows.map((entry) => getVisibleRowPath(entry.row)),
      );
      expect(expectedAtSixty.layout.visible.startIndex).toBe(
        getVisibleIndexForPath(expectedController, "src/index.ts"),
      );
      expect(getPixelStyleValue(getVirtualList(shadowRoot, dom), "height")).toBe(
        expectedAtSixty.layout.physical.totalHeight,
      );
      expect(getPixelStyleValue(getVirtualStickyOffset(shadowRoot, dom), "height")).toBe(
        expectedAtSixty.layout.window.offsetTop,
      );
      expect(getTranslateYStyleValue(getVirtualStickyWindow(shadowRoot, dom))).toBe(0);
      expect(getMountedItemPaths(shadowRoot, dom)).toEqual(expectedAtSixty.mountedPaths);
      ...
      expect(getStickyRowPaths(shadowRoot, dom)).toEqual(
        expectedAtThirty.layout.sticky.rows.map((entry) => getVisibleRowPath(entry.row)),
      );
      expect(getPixelStyleValue(getVirtualStickyOffset(shadowRoot, dom), "height")).toBe(
        expectedAtThirty.layout.window.offsetTop,
      );
      expect(getTranslateYStyleValue(getVirtualStickyWindow(shadowRoot, dom))).toBe(0);
      expect(getMountedItemPaths(shadowRoot, dom)).toEqual(expectedAtThirty.mountedPaths);
      expect(getPixelStyleValue(getVirtualList(shadowRoot, dom), "height")).toBe(
        expectedAtThirty.layout.physical.totalHeight,
      );
  ...
  test("sticky virtual window keeps bottom coverage during fast downward scrolls", async () => {
    ...
    try {
      ...
      const paths = [
        ...
        ...Array.from({ length: 12 }, (_, index) => `z${String(index).padStart(2, "0")}.ts`),
      ];
      const containerWrapper = dom.window.document.createElement("div");
      dom.window.document.body.appendChild(containerWrapper);
      ...
      const stickyOverlayHeight =
        getStickyRowPaths(shadowRoot, dom).length * FILE_TREE_DEFAULT_ITEM_HEIGHT;
  ...
  test("keyboard focus scrolling respects sticky overlay height", async () => {
    ...
    try {
      ...
      expect(getStickyRowPaths(shadowRoot, dom)).toEqual([]);
      expect(getFocusedTreeElement(shadowRoot, dom)?.dataset.itemPath).toBe("src/lib/");
  ...
  test("modifier-click on a sticky row adds selection without collapsing it", async () => {
    ...
    try {
      ...
      expect([...fileTree.getSelectedPaths()].sort()).toEqual(["src/lib/", "src/lib/util.ts"]);
  ...
  test("collapsing a sticky row keeps it as the first in-flow row below its sticky parents", async () => {
    ...
    try {
      ...
      expect(getMountedItemPaths(shadowRoot, dom)[0]).toBe("arch/alpha/include/asm/bitops.h");
      ...
      expect(getMountedItemPaths(shadowRoot, dom)[0]).toBe("arch/alpha/include/asm/");
      ...
      expect(getFocusedItemPath(shadowRoot, dom)).toBe("arch/alpha/include/asm/");
  ...
  test("collapsing a sticky row keeps it below its only sticky parent", async () => {
    ...
    try {
      ...
      const fileTree = new FileTree({
        ...
        paths: ["docs/readme.md", "src/lib/util.ts", "src/lib/zeta.ts", "zzz.ts", "zzzz.ts"],
        stickyFolders: true,
      ...
      expect(getItemButton(shadowRoot, dom, "src/lib/util.ts").dataset.itemPath).toBe(
        "src/lib/util.ts",
      );
      ...
      expect(getItemButton(shadowRoot, dom, "src/lib/").dataset.itemPath).toBe("src/lib/");
  ...
  test("sticky overlay mirrors flattened row content", async () => {
    ...
    try {
      ...
      const expectedContentText = getNormalizedText(
        getItemButton(shadowRoot, dom, "src/lib/").querySelector('[data-item-section="content"]'),
      );
  ...
  test("sticky overlay rows mirror the canonical row lane structure, including action-lane reservation", async () => {
    ...
    try {
      ...
      expect(stickyButton.querySelector('[data-item-section="action"]')).not.toBeNull();
  ...
  test("sticky overlay can fill the viewport for a deep leading folder chain", async () => {
    ...
    try {
      ...
      const expectedStickyPaths = ["a/", "a/b/", "a/b/c/", "a/b/c/d/", "a/b/c/d/e/"];
  ...
  test("computes a stable window range and sticky layout", () => {
    ...
    expect(layout.offsetHeight).toBe(scrolledRange.start * FILE_TREE_DEFAULT_ITEM_HEIGHT);
  });
});
