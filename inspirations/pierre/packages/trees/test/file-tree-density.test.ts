...
import { FILE_TREE_DENSITY_PRESETS, resolveFileTreeDensity } from "../src/model/density";
...
describe("resolveFileTreeDensity", () => {
  ...
  test("FILE_TREE_DEFAULT_ITEM_HEIGHT is sourced from the default preset", () => {
    expect(FILE_TREE_DEFAULT_ITEM_HEIGHT).toBe(FILE_TREE_DENSITY_PRESETS.default.itemHeight);
  });
});
...
describe("FileTree vanilla render density host style", () => {
  test("render() paints the resolved density vars on the host and exposes them via the model accessors", async () => {
    ...
    try {
      ...
      expect(host?.style.getPropertyValue("--trees-density-override")).toBe(String(compact.factor));
  ...
  test("caller-set host inline values win, mirroring the React wrapper", async () => {
    ...
    try {
      ...
      const customHost = dom.window.document.createElement("file-tree-container");
      ...
      expect(customHost.style.getPropertyValue("--trees-item-height")).toBe("40px");
      expect(customHost.style.getPropertyValue("--trees-density-override")).toBe("1.5");
  ...
  test("reusing a host with a new density refreshes the model-owned vars", async () => {
    ...
    try {
      ...
      expect(host.style.getPropertyValue("--trees-density-override")).toBe(String(relaxed.factor));
