...
async function openFixture(page: Page) {
  ...
  await page.waitForFunction(() => window.__fileTreeGitStatusFixtureReady === true);
}
...
async function readGitStatusSnapshot(page: Page) {
  return page.evaluate(() => {
    ...
    const getItemButton = (path: string) =>
      shadowRoot.querySelector(`button[data-item-path="${path}"]`) as HTMLButtonElement | undefined;
    const getStatusPayload = (path: string) => {
      ...
      const gitLabel =
        button.querySelector('[data-item-section="git"] > span')?.textContent?.trim() ?? null;
      return {
        decoration:
          button.querySelector('[data-item-section="decoration"] > span')?.textContent?.trim() ??
          null,
        gitLabel,
    ...
    return {
      ...
      state: document.querySelector("[data-file-tree-git-status-state]")?.textContent ?? null,
      wrapperCount: shadowRoot.querySelectorAll('[data-file-tree-virtualized-wrapper="true"]')
        .length,
      ...
      srcFolder:
        srcFolder == null
          ...
          : {
              ...
              dotHeight: srcFolder
                .querySelector('[data-item-section="git"] [data-icon-name="file-tree-icon-dot"]')
                ?.getAttribute("height"),
              dotWidth: srcFolder
                .querySelector('[data-item-section="git"] [data-icon-name="file-tree-icon-dot"]')
                ?.getAttribute("width"),
      ...
      anyStatusCount: shadowRoot.querySelectorAll("[data-item-git-status]").length,
      anyFolderChangeCount: shadowRoot.querySelectorAll('[data-item-contains-git-change="true"]')
        .length,
    };
  });
}
...
test.describe("file-tree git-status proof", () => {
  test("renders git-status attrs and updates them in place across set A, set B, and disabled states", async ({
    ...
    await page.locator('[data-file-tree-git-status-action="toggle-enabled"]').click();
