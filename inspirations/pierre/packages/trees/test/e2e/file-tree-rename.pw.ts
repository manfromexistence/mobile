...
test.describe("file-tree rename proof", () => {
  ...
  const scrollUntilStickyRow = (tree: Locator, path: string): Promise<Locator> =>
  ...
  test("F2 starts inline rename and Enter commits the renamed path", async ({ page }) => {
    await page.goto("/test/e2e/fixtures/file-tree-rename.html");
    await page.waitForFunction(() => window.__fileTreeRenameFixtureReady === true);

    const mainTree = page.locator("file-tree-container").nth(0);
    const readmeRow = mainTree.locator('button[data-type="item"][data-item-path="README.md"]');
    ...
    const renamedRow = mainTree.locator('button[data-type="item"][data-item-path="RENAMED.md"]');
  ...
  test("context-menu rename closes the menu and Escape cancels the inline session", async ({
    ...
    const mainTree = page.locator("file-tree-container").nth(0);
    const indexRow = mainTree.locator('button[data-type="item"][data-item-path="src/index.ts"]');
  ...
  test("sticky-row context-menu rename hands off focus to the canonical row input", async ({
    ...
    expect(await renameRow.getAttribute("data-file-tree-sticky-row")).toBeNull();
    await expect(renameInput).toBeFocused();
    await expect(renameInput).toHaveValue("lib");
    await expect(
      mainTree.locator('button[data-file-tree-sticky-row="true"] input[data-item-rename-input]'),
    ).toHaveCount(0);
  });

  test("portaled context-menu rename keeps the inline input focused", async ({ page }) => {
    await page.goto("/test/e2e/fixtures/file-tree-portaled-context-menu-rename.html");
    await page.waitForFunction(() => window.__fileTreeRenameFixtureReady === true);

    const tree = page.locator("file-tree-container").nth(0);
    const indexRow = tree.locator('button[data-type="item"][data-item-path="src/index.ts"]');
    await indexRow.click();
    await indexRow.click({ button: "right" });
  ...
  test("trigger-opened menu still restores focus after a prior rename flow disabled restoreFocus", async ({
    ...
    await expect.poll(() => getFocusedPath(page)).not.toBe(focusedBeforeTrigger);
  });
  ...
  test("search-open rename clears the filter and keeps the target visible on the unfiltered tree", async ({
    ...
    const workerRow = mainTree.locator('button[data-item-path="src/utils/worker.ts"]');
    await expect(workerRow).toBeVisible();
    await expect(mainTree.locator('button[data-item-path="README.md"]')).toHaveCount(0);
    ...
    await expect(mainTree.locator('button[data-item-path="README.md"]')).toBeVisible();
  });
  ...
  test("flattened leaf rename keeps the input on the terminal segment and commits the folder move", async ({
    ...
    const flatRow = flatTree.locator('button[data-type="item"][data-item-type="folder"]');
    await flatRow.click();
    await flatRow.press("F2");

    const renameInput = flatTree.locator("input[data-item-flattened-rename-input]");
    await expect(renameInput).toBeFocused();
    await expect(renameInput).toHaveValue("deep");

    const segments = flatTree.locator("[data-item-flattened-subitem]");
    await expect(segments).toHaveCount(3);
    await expect(segments.nth(0).locator("[data-item-rename-input]")).toHaveCount(0);
    await expect(segments.nth(2).locator("[data-item-rename-input]")).toHaveCount(1);

    await renameInput.fill("renamed");
    await renameInput.press("Enter");

    await expect(flatTree.locator('button[data-item-path="src/utils/renamed/"]')).toBeVisible();
    await expect(page.locator("[data-file-tree-rename-log]")).toContainText(
