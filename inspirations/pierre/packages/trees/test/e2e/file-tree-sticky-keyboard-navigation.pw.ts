...
const fixturePath = "/test/e2e/fixtures/file-tree-sticky-keyboard-navigation.html";
const linuxArcScrollTop = 9_000;
const linuxArcStickyPaths = ["arch/", "arch/arc/", "arch/arc/boot/", "arch/arc/boot/dts/"] as const;
...
const syntheticBranchStickyPaths = ["a1/", "a1/b2/", "a1/b2/c1/", "a1/b2/c1/d2/"] as const;
...
const getMountedFlowRow = (
  ...
  snapshot.rows.find((row) => row.path === path && !row.isSticky && !row.isParked) ?? null;
...
test.describe("sticky keyboard navigation fixture", () => {
  ...
  test("ArrowDown from a focused synthetic sticky DOM mirror keeps b2 in the second sticky slot", async ({
    ...
    expectSyntheticBranchStickyStackPreserved(await sample(page), baseline, "a1/b2/c1/");
  });
  ...
  test("Shift+ArrowUp from a synthetic sticky folder preserves the sticky viewport while extending selection", async ({
    ...
    expectSyntheticBranchStickyStackPreserved(await sample(page), baseline, "a1/b2/");
  });
  ...
  test("clicking a non-root sticky folder collapses it at the same visual top with a fractional scrollport", async ({
    ...
    expect(after.stickyPaths).not.toContain("a1/b2/");
    expect(targetRow).not.toBeNull();
    expect(targetRow?.topWithinScroll).toBeCloseTo(sourceStickyRow.topWithinScroll, 2);
  });
