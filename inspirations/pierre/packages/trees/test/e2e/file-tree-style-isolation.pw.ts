...
async function openFixture(page: Page) {
  ...
  await page.waitForFunction(() => window.__fileTreeStyleIsolationReady === true);
}

test.describe("file-tree shadow style isolation", () => {
  test("external id selector does not style shadow-root node with same id", async ({ page }) => {
    await openFixture(page);

    const colors = await page.evaluate(() => {
      const outside = document.querySelector("[data-test-outside-duplicate-id]");
      const host = document.getElementById("test-tree-host");
      const inside = host?.shadowRoot?.querySelector("[data-test-shadow-dup-id]");
      return {
  ...
  test("file-tree internal selected-item styles do not leak to light DOM", async ({ page }) => {
    await openFixture(page);
  ...
  test("css custom properties flow into the shadow root for theming", async ({ page }) => {
    await openFixture(page);

    const selectedBackground = await page.evaluate(() => {
      const host = document.getElementById("test-tree-host");
      const selected = host?.shadowRoot?.querySelector("[data-test-selected-item]");
      return selected != null ? getComputedStyle(selected).backgroundColor : null;
    });
