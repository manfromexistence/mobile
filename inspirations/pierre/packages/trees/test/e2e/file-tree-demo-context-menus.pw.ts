...
async function readLaneSnapshot(page: Page, card: string, rowPath: string) {
  return page
    ...
    .evaluate((host, targetPath) => {
      ...
      const row = shadowRoot.querySelector(`button[data-item-path="${targetPath}"]`);
      if (!(row instanceof HTMLButtonElement)) {
        return null;
      }

      return {
        actionLaneButtonCount: row.querySelectorAll('[data-item-section="action"] button').length,
        actionLaneCount: row.querySelectorAll('[data-item-section="action"]').length,
        ...
        realTriggerCount: shadowRoot.querySelectorAll('[data-type="context-menu-trigger"]').length,
        rootButtonVisibility:
          treeRoot?.getAttribute("data-file-tree-context-menu-button-visibility") ?? null,
        rootHasActionLane:
          treeRoot?.getAttribute("data-file-tree-has-context-menu-action-lane") ?? null,
        rootTriggerMode: treeRoot?.getAttribute("data-file-tree-context-menu-trigger-mode") ?? null,
        rowHasActionLane: row.getAttribute("data-item-has-context-menu-action-lane") ?? null,
      };
    }, rowPath);
}
...
test.describe("demo context-menu regressions", () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.waitForFunction(() => window.__fileTreeDemoContextMenuFixtureReady === true);
  });

  test("docs-like portaled menu opens from the trigger button", async ({ page }) => {
    const tree = page.locator('[data-demo-context-menu-card="radix-portaled"] file-tree-container');

    await openMenuFromTrigger(tree, "src/index.ts");

    const menu = page.locator('[data-test-context-menu-variant="radix-portaled"]');
  ...
  test("always-visible affordances stay decorative while one real trigger handles menus", async ({
    ...
    const snapshot = await readLaneSnapshot(page, "radix-portaled", "src/index.ts");
  ...
  test("right-click-only tree omits the action lane and still opens from right-click", async ({
    ...
    const snapshot = await readLaneSnapshot(page, "right-click-only", "src/index.ts");
    expect(snapshot).toMatchObject({
      actionLaneButtonCount: 0,
      actionLaneCount: 0,
      realTriggerCount: 1,
    ...
    const menu = page.locator('[data-test-context-menu-variant="right-click-only"]');
    await expect(menu).toBeVisible();
    await expect(
  ...
  test("react client-rendered menu preserves button mode and opens from the trigger button", async ({
    ...
    const snapshot = await readLaneSnapshot(page, "react-client", "src/index.ts");
    expect(snapshot).toMatchObject({
      actionLaneCount: 1,
      rootButtonVisibility: "always",
      rootHasActionLane: "true",
    ...
    const tree = page.locator('[data-demo-context-menu-card="react-client"] file-tree-container');

    await openMenuFromTrigger(tree, "src/index.ts");

    const menu = page.locator('[data-test-context-menu-variant="react-client"]');
