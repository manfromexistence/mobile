...
function getTreeRoot(shadowRoot: ShadowRoot | null | undefined, dom: JSDOM): HTMLElement {
...
describe("file-tree composition surfaces", () => {
  test("preloadFileTree includes header slot and closed context-menu shell scaffolding", async () => {
    ...
    expect(payload.shadowHtml).toContain('data-file-tree-context-menu-trigger-mode="right-click"');
  ...
  test("preloadFileTree omits context-menu affordance when the feature is disabled", async () => {
    ...
    expect(payload.shadowHtml).not.toContain('data-type="context-menu-trigger"');
  ...
  test("attaches and removes header slot content on the host", async () => {
    ...
    try {
      ...
      expect(host?.querySelector('[data-test-header="true"]')?.textContent).toBe("Header action");
  ...
  test("opens and closes a host-slotted context menu without rename-specific context fields", async () => {
    ...
    try {
      ...
      const anchorElement = shadowRoot?.querySelector('[data-type="context-menu-anchor"]');
      ...
      const context = capturedContext as unknown as CapturedContextMenuOpenContext;
      ...
      expect("canRename" in (context as unknown as Record<string, unknown>)).toBe(false);
      expect("startRenaming" in (context as unknown as Record<string, unknown>)).toBe(false);
  ...
  test("Shift+F10 opens the context menu for the focused row", async () => {
    ...
    try {
      ...
      const openContext = capturedContext as unknown as CapturedContextMenuOpenContext;
      expect(openContext.anchorElement.dataset.type).toBe("context-menu-trigger");
      expect(
        shadowRoot?.querySelector('[data-type="context-menu-anchor"] slot[name="context-menu"]'),
      ).not.toBeNull();
  ...
  test("keeps the opened row visually focused and restores DOM focus on close", async () => {
    ...
    try {
      ...
      const closeContext = capturedContext as unknown as CapturedContextMenuOpenContext;
      closeContext.close();
      await flushDom();
  ...
  test("document Escape closes the menu and prevents default when focus is in slotted content", async () => {
    ...
    try {
      ...
      const slottedMenuButton = host?.querySelector('[slot="context-menu"] button');
  ...
  test("keyboard navigation retargets the trigger away from a stale hovered row", async () => {
    ...
    try {
      ...
      const trigger = shadowRoot?.querySelector('[data-type="context-menu-trigger"]');
  ...
  test("falls back to the focused row after pointer hover clears during scroll", async () => {
    ...
    try {
      ...
      const shadowRoot = fileTree.getFileTreeContainer()?.shadowRoot;
      const root = shadowRoot?.querySelector('[role="tree"]');
      const viewport = shadowRoot?.querySelector('[data-file-tree-virtualized-scroll="true"]');
      const firstRow = getItemButton(shadowRoot, dom, "README.md");
      const secondRow = getItemButton(shadowRoot, dom, "src/index.ts");
      const thirdRow = getItemButton(shadowRoot, dom, "src/lib/utils.ts");
      const trigger = shadowRoot?.querySelector('[data-type="context-menu-trigger"]');
  ...
  test("hides the focused trigger when the retained focused row is parked offscreen", async () => {
    ...
    try {
      ...
      const scrollElement = shadowRoot?.querySelector('[data-file-tree-virtualized-scroll="true"]');
      const firstRow = shadowRoot?.querySelector('button[data-type="item"][data-item-type="file"]');
      const trigger = shadowRoot?.querySelector('[data-type="context-menu-trigger"]');
  ...
  test("clears visual focus after a null-target blur from a row restored after parking", async () => {
    ...
    try {
      ...
      scrollElement.dispatchEvent(new dom.window.Event("scroll"));
      await flushDom();
      expect(getItemButton(shadowRoot, dom, focusedPath).dataset.itemParked).toBe("true");
      ...
      expect(shadowRoot?.querySelector('button[data-item-focused="true"]')).toBeNull();
      expect(trigger.dataset.visible).toBe("false");
  ...
  test("button-capable modes reserve the action lane and keep lane attrs stable", async () => {
    ...
    try {
      ...
      const treeRoot = getTreeRoot(shadowRoot, dom);
      const itemButton = getItemButton(shadowRoot, dom, "README.md");
      const decorationLane = itemButton.querySelector('[data-item-section="decoration"]');
      const actionLane = itemButton.querySelector('[data-item-section="action"]');
      const decorativeAffordance = actionLane?.querySelector(
        '[data-item-action-affordance="decorative"]',
      );

      expect(treeRoot.getAttribute("data-file-tree-context-menu-trigger-mode")).toBe("button");
      expect(treeRoot.getAttribute("data-file-tree-context-menu-button-visibility")).toBe("always");
      expect(treeRoot.getAttribute("data-file-tree-has-context-menu-action-lane")).toBe("true");
      expect(itemButton.getAttribute("data-item-context-menu-trigger-mode")).toBe("button");
      expect(itemButton.getAttribute("data-item-context-menu-button-visibility")).toBe("always");
      expect(itemButton.getAttribute("data-item-has-context-menu-action-lane")).toBe("true");
      ...
      expect(shadowRoot?.querySelectorAll('[data-type="context-menu-trigger"]')).toHaveLength(1);
  ...
  test("button mode defaults to when-needed visibility", async () => {
    ...
    try {
      ...
      expect(treeRoot.getAttribute("data-file-tree-context-menu-trigger-mode")).toBe("button");
      expect(treeRoot.getAttribute("data-file-tree-context-menu-button-visibility")).toBe(
        "when-needed",
      );
      expect(itemButton.getAttribute("data-item-context-menu-button-visibility")).toBe(
        "when-needed",
      );
      expect(itemButton.getAttribute("data-item-has-context-menu-action-lane")).toBe("true");
      expect(actionLane).not.toBeNull();
  ...
  test("right-click-only mode omits the action lane but still opens menus", async () => {
    ...
    try {
      ...
      const treeRoot = getTreeRoot(shadowRoot, dom);
      const itemButton = getItemButton(shadowRoot, dom, "README.md");
      expect(treeRoot.getAttribute("data-file-tree-context-menu-trigger-mode")).toBe("right-click");
      expect(treeRoot.getAttribute("data-file-tree-has-context-menu-action-lane")).toBeNull();
      expect(itemButton.getAttribute("data-item-has-context-menu-action-lane")).toBeNull();
      expect(itemButton.querySelector('[data-item-section="action"]')).toBeNull();
  ...
  test("adds aria-haspopup=menu only when context menu is enabled", async () => {
    ...
    try {
      ...
      expect(disabledShadowRoot?.querySelector('[data-type="context-menu-trigger"]')).toBeNull();
      ...
      expect(enabledShadowRoot?.querySelector('[data-type="context-menu-trigger"]')).not.toBeNull();
      expect(enabledTreeRoot.getAttribute("data-file-tree-context-menu-trigger-mode")).toBe(
        "right-click",
      );
      expect(
        enabledTreeRoot.getAttribute("data-file-tree-has-context-menu-action-lane"),
      ).toBeNull();
      expect(enabledItem.getAttribute("data-item-has-context-menu-action-lane")).toBeNull();
      disabled.cleanUp();
      enabled.cleanUp();
    } finally {
      cleanup();
  ...
  test("controller restores nearest parent before considering sibling fallbacks", async () => {
    const { FileTreeController } = await import("../src/model/FileTreeController");
  ...
  test("supports icon remaps and render-only row decorators", async () => {
    ...
    try {
      ...
      expect(readmeButton.querySelector('[data-item-section="decoration"]')?.textContent).toBe(
        "DOC",
      );
