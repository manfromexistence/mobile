...
test("profile:file-tree CLI help advertises the expected workload/render workflow", () => {
  ...
  expect(stdout).toContain("starts `scripts/chrome-remote-debug.sh` automatically");
...
test("profile:file-tree CLI rejects unknown workloads before browser setup", () => {
  const result = Bun.spawnSync({
    cmd: ["bun", "run", "./scripts/profileFileTree.ts", "--workload", "not-a-real-workload"],
    cwd: packageRoot,
    env: createCommandEnv(),
    stdout: "pipe",
    stderr: "pipe",
...
test("profile:file-tree CLI rejects unknown action modes before browser setup", () => {
  const result = Bun.spawnSync({
    cmd: ["bun", "run", "./scripts/profileFileTree.ts", "--actions", "not-a-real-mode"],
    cwd: packageRoot,
    env: createCommandEnv(),
    stdout: "pipe",
    stderr: "pipe",
