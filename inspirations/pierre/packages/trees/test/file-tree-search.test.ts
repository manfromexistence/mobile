...
function getVisiblePaths(
  ...
  return controller.getVisibleRows(0, controller.getVisibleCount()).map((row) => row.path);
}
...
describe("file-tree search", () => {
  ...
  test("focusNearestPath returns the correct visible path during filtered search", async () => {
    ...
    expect(controller.resolveNearestVisiblePath("src/utils/worker.ts")).toBe("src/utils/worker.ts");
    expect(controller.focusNearestPath("src/utils/worker.ts")).toBe("src/utils/worker.ts");
  ...
  test("filtered visible rows fetch only the contiguous runs they need", async () => {
    ...
    try {
      ...
      const visiblePaths = controller.getVisibleRows(0, 5).map((row) => row.path);
      ...
      expect(requestedRanges.every(([start, end]) => end - start <= 1)).toBe(true);
      expect(requestedRanges[0]?.[0]).toBeLessThan(requestedRanges[1]?.[0] ?? 0);
      expect(requestedRanges[1]?.[0]).toBeLessThan(requestedRanges[2]?.[0] ?? 0);
  ...
  test("closed-search visible count stays tied to the full store count before full projection expansion", async () => {
    ...
    expect(controller.getVisibleRows(520, 520)[0]?.path).toBe(LARGE_VISIBLE_FILES[520]);
  ...
  test("search false hides the built-in input while programmatic search still works", async () => {
    ...
    try {
      ...
      expect(shadowRoot?.querySelector('[data-item-path="src/utils/worker.ts"]')).not.toBeNull();
  ...
  test("search false keeps the printable-key open hotkey disabled", async () => {
    ...
    try {
      ...
      const shadowRoot = fileTree.getFileTreeContainer()?.shadowRoot;
      const firstButton = shadowRoot?.querySelector<HTMLButtonElement>('button[data-type="item"]');
      ...
      expect(shadowRoot?.querySelector("[data-file-tree-search-input]")).toBeNull();
      expect(shadowRoot?.querySelector('[data-item-path="README.md"]')).not.toBeNull();
  ...
  test("search keeps input focus while ArrowDown updates the focused match", async () => {
    ...
    try {
      ...
      const initialActiveDescendant = searchInput?.getAttribute("aria-activedescendant") ?? null;
      ...
      const nextActiveDescendant = searchInput?.getAttribute("aria-activedescendant") ?? null;
      ...
      const previousActiveDescendant = searchInput?.getAttribute("aria-activedescendant") ?? null;
      expect(shadowRoot?.activeElement).toBe(searchInput);
  ...
  test("Enter selects the focused search match and closes search", async () => {
    ...
    try {
      ...
      const focusedPathBeforeSubmit = focusedMatch?.getAttribute("data-item-path") ?? null;
      ...
      expect(selectedRow?.getAttribute("data-item-path")).toBe(focusedPathBeforeSubmit);
  ...
  test("Enter immediately after ArrowDown still returns focus to the selected row", async () => {
    ...
    try {
      ...
      expect(selectedRow?.getAttribute("data-item-path")).toBe("src/utils/worker/match-1.ts");
      expect(shadowRoot?.activeElement).toBe(selectedRow);
  ...
  test("searchBlurBehavior retain keeps the initial query when the input blurs pre-interaction", async () => {
    ...
    try {
      ...
      searchInput?.dispatchEvent(new dom.window.FocusEvent("blur", { bubbles: true }));
      await flushDom();
      ...
      searchInput?.dispatchEvent(new dom.window.FocusEvent("focus", { bubbles: true }));
      await flushDom();
      searchInput?.dispatchEvent(new dom.window.FocusEvent("blur", { bubbles: true }));
      await flushDom();
  ...
  test("searchFakeFocus renders a synthetic focus attribute that dismisses on interaction", async () => {
    ...
    try {
      ...
      expect(searchInput).not.toBeNull();
      expect(searchInput?.getAttribute("data-file-tree-search-input-fake-focus")).toBe("true");
      ...
      searchInput?.dispatchEvent(new dom.window.Event("pointerdown", { bubbles: true }));
      await flushDom();

      expect(searchInput?.getAttribute("data-file-tree-search-input-fake-focus")).toBeNull();
