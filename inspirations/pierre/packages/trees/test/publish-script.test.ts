...
describe("trees publish script OTP handling", () => {
  ...
  test("parseArgs rejects missing OTP values", () => {
    expect(() => parseArgs(["--otp"])).toThrow("--otp requires a one-time password");
    expect(() => parseArgs(["--otp="])).toThrow("--otp requires a one-time password");
    expect(() => parseArgs(["--otp", "--tag=beta"])).toThrow("--otp requires a one-time password");
  });
  ...
  test("pnpm publish commands forward OTP without changing release args", () => {
    ...
    expect(dryRunPublishArgs("/tmp/pierre-trees.tgz", "beta", "123456")).toEqual([
      "publish",
      "/tmp/pierre-trees.tgz",
      "--dry-run",
      "--tag",
  ...
  test("OTP values are redacted before commands are logged", () => {
    expect(redactOtp(publishArgs("/tmp/pierre-trees.tgz", "beta", "123456"))).toEqual([
      "publish",
      "/tmp/pierre-trees.tgz",
      "--tag",
      "beta",
