...
describe("file-tree virtualization windows", () => {
  test("reads viewport height from ResizeObserver border box entries", () => {
    ...
    expect(getResizeObserverViewportHeight(createResizeObserverEntry(undefined, 0))).toBeNull();
  });
  ...
  test("mounts with the initially selected item in the rendered window", async () => {
    ...
    try {
      ...
      expect(scrollElement.scrollTop).toBe((targetIndex + 1) * itemHeight - viewportHeight);
  ...
  test("scrollToPath supports top, nearest, and clamped center offsets through the public API", async () => {
    ...
    try {
      ...
      await flushDom(2);
      expect(scrollElement.scrollTop).toBe(paths.length * itemHeight - viewportHeight);
  ...
  test("scrollToPath ignores paths that are not in the current visible projection", async () => {
    ...
    try {
      ...
      expect(fileTree.getFocusedPath()).toBe(initialFocusedPath);
      expect(scrollElement.scrollTop).toBe(0);
      expect(getMountedItemPaths(shadowRoot, dom)).not.toContain("src/deep/file.ts");
  ...
  test("scrollToPath top offset accounts for sticky ancestor rows", async () => {
    ...
    try {
      ...
      const targetIndex = getVisibleIndexForPath(expectedController, targetPath);
      const targetRow = expectedController.getVisibleRows(targetIndex, targetIndex)[0] ?? null;
  ...
  test("marks the virtualized list as scrolling to suppress hover styles", async () => {
    ...
    try {
      ...
      const shadowRoot = fileTree.getFileTreeContainer()?.shadowRoot;
      const scrollElement = shadowRoot?.querySelector('[data-file-tree-virtualized-scroll="true"]');
      const listElement = shadowRoot?.querySelector('[data-file-tree-virtualized-list="true"]');
  ...
  test("collapse preserves a coherent virtualized window when affected rows move above and below the fold", async () => {
    ...
    try {
      ...
      expect(getMountedItemPaths(shadowRoot, dom)).toEqual(expectedBeforeCollapse.mountedPaths);
      ...
      expect(mountedPathsAfterCollapse).toEqual(expectedAfterCollapse.mountedPaths);
