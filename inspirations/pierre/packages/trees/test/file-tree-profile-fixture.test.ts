...
test("file-tree profile fixture options mirror the docs tree profile defaults", () => {
  ...
  expect(options.initialVisibleRowCount).toBe(FILE_TREE_PROFILE_VIEWPORT_HEIGHT / 30);
  expect(options.stickyFolders).toBe(true);
  expect(options.preparedInput).toEqual(preparePresortedFileTreeInput(workload.files));
});
...
test("file-tree profile fixture HTML stays minimal and idle-on-load", () => {
  const html = readFileSync(`${packageRoot}/test/e2e/fixtures/file-tree-profile.html`, "utf8");
