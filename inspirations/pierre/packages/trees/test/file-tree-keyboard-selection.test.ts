...
describe("file-tree keyboard and selection", () => {
  test("keyboard selection hotkeys preserve focus continuity", async () => {
    ...
    try {
      ...
      await flushDom();
      expect(getSelectedItemPaths(shadowRoot, dom)).toEqual(["a.ts", "b.ts", "c.ts"]);
  ...
  test("focused rows keep the matching separator line prominent", async () => {
    ...
    try {
      ...
      const guideStyle = shadowRoot?.querySelector('[data-file-tree-guide-style="true"]');
      const spacingItems = fileButton.querySelectorAll('[data-item-section="spacing-item"]');

      expect(spacingItems[0]?.getAttribute("data-ancestor-path")).toBe("src/");
      expect(spacingItems[1]?.getAttribute("data-ancestor-path")).toBe("src/lib/");
  ...
  test("keyboard navigation matches the baseline tree behavior", async () => {
    ...
    try {
      ...
      pressKey(getItemButton(shadowRoot, dom, "src/lib/util.ts"), dom, "ArrowLeft");
      await flushDom();
      expect(fileTree.getItem("src/lib/")?.isFocused()).toBe(true);
  ...
  test("keyboard navigation survives virtualization when the focused row unmounts", async () => {
    ...
    try {
      ...
      expect(getItemButton(shadowRoot, dom, "item050.ts").dataset.itemParked).toBe("true");
      expect(getFocusedTreeElement(shadowRoot, dom)?.dataset.itemPath).toBe("item050.ts");
      ...
      expect(getItemButton(shadowRoot, dom, "item051.ts").dataset.itemFocused).toBe("true");
      expect(getFocusedTreeElement(shadowRoot, dom)?.dataset.itemPath).toBe("item051.ts");
  ...
  test("flattened row markup does not wrap separators in extra spans", async () => {
    ...
    try {
      ...
      fileTree.render({ containerWrapper });
      const shadowRoot = fileTree.getFileTreeContainer()?.shadowRoot;
      const flattenedContainer = shadowRoot?.querySelector("[data-item-flattened-subitems]");

      expect(flattenedContainer?.innerHTML).toContain(" / ");
      expect(
        flattenedContainer?.querySelectorAll(":scope > [data-item-flattened-subitem]").length,
      ).toBe(2);
      expect(
        flattenedContainer?.querySelector(":scope > span:not([data-item-flattened-subitem])"),
      ).toBeNull();
  ...
  test("flattened row selection targets the terminal directory path", async () => {
    ...
    try {
      ...
      expect(getItemButton(shadowRoot, dom, "src/lib/").dataset.itemSelected).toBe("true");
  ...
  test("expansion between selected items does not corrupt index-based selection", async () => {
    ...
    try {
      ...
      expect(fileTree.getSelectedPaths()).toEqual(["a.ts", "z.ts"]);
      expect(getItemButton(shadowRoot, dom, "src/index.ts").dataset.itemSelected).toBeUndefined();
  ...
  test("Ctrl-click deselects the anchor then Shift-click still ranges from it", async () => {
    ...
    try {
      ...
      await flushDom();
      expect(getSelectedItemPaths(shadowRoot, dom)).toEqual(["b.ts", "c.ts", "d.ts"]);
  ...
  test("uses compatible row markup for the implemented focus/navigation and selection pieces", async () => {
    ...
    try {
      ...
      fileTree.render({ containerWrapper });
      const shadowRoot = fileTree.getFileTreeContainer()?.shadowRoot;
      const focusedRow = shadowRoot?.querySelector('[data-item-focused="true"]');
      const treeRoot = shadowRoot?.querySelector('[data-file-tree-virtualized-root="true"]');

      const flattenedFolderButton = getItemButton(shadowRoot, dom, "src/lib/");
      const nestedFileButton = getItemButton(shadowRoot, dom, "src/lib/index.ts");
      const rootFileButton = getItemButton(shadowRoot, dom, "README.md");

      expect(getRowSectionOrder(flattenedFolderButton)).toEqual(["icon", "content"]);
      expect(getRowSectionOrder(nestedFileButton)).toEqual(["spacing", "icon", "content"]);
      expect(getRowSectionOrder(rootFileButton)).toEqual(["icon", "content"]);

      expect(
        flattenedFolderButton.querySelector('[data-item-section="icon"] [data-icon-name]'),
      ).not.toBeNull();
      expect(
      ...
      expect(
        nestedFileButton.querySelector('[data-item-section="icon"] [data-icon-name]'),
      ).not.toBeNull();
      expect(
        nestedFileButton.querySelector('[data-item-section="content"] [data-icon-name]'),
      ).toBeNull();
      expect(
        nestedFileButton.querySelector(
      ...
      expect(
        nestedFileButton.querySelector('[data-item-section="content"] [data-truncate-container]'),
      ).not.toBeNull();
      expect(focusedRow).toBeNull();
      expect(shadowRoot?.querySelector('[data-item-selected="true"]')).toBeNull();
      expect(treeRoot?.getAttribute("role")).toBe("tree");
      expect(treeRoot?.getAttribute("aria-activedescendant")).toBeNull();
      ...
      expect(shadowRoot?.querySelector('[data-item-selected="true"]')).not.toBeNull();
