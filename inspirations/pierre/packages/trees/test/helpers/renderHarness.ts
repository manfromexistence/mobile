...
import { computeFileTreeLayout, type FileTreeLayoutSnapshot } from "../../src/model/layout";
import {
...
export function getTreeRoot(shadowRoot: ShadowRoot | null | undefined, dom: JSDOM): HTMLDivElement {
...
export function getVirtualList(
  ...
  const list = shadowRoot?.querySelector('[data-file-tree-virtualized-list="true"]');
...
export function getVirtualStickyOffset(
  ...
  const offset = shadowRoot?.querySelector('[data-file-tree-virtualized-sticky-offset="true"]');
...
export function getVirtualStickyWindow(
  ...
  const windowElement = shadowRoot?.querySelector('[data-file-tree-virtualized-sticky="true"]');
...
export function clickItem(
  ...
  buttonElement.dispatchEvent(new dom.window.MouseEvent("click", { bubbles: true, ...init }));
}
...
export function getSelectedItemPaths(
  ...
  return Array.from(
    ...
    .filter(
      (element): element is HTMLButtonElement => element instanceof dom.window.HTMLButtonElement,
    )
...
export function getFocusedItemPath(
  ...
  return button instanceof dom.window.HTMLButtonElement ? (button.dataset.itemPath ?? null) : null;
}
...
export function getStickyRowPaths(shadowRoot: ShadowRoot | null | undefined, dom: JSDOM): string[] {
  ...
  const root = shadowRoot?.querySelector<HTMLElement>('[data-file-tree-virtualized-root="true"]');
  const previewHidden =
  ...
  return Array.from(shadowRoot?.querySelectorAll("[data-file-tree-sticky-path]") ?? [])
    .filter((element): element is HTMLElement => element instanceof dom.window.HTMLElement)
...
export function getMountedItemPaths(
  ...
  return Array.from(
    shadowRoot?.querySelectorAll('[data-type="item"]:not([data-file-tree-sticky-row="true"])') ??
      [],
  )
    .filter(
      (element): element is HTMLButtonElement => element instanceof dom.window.HTMLButtonElement,
...
export function computeExpectedRenderedWindow(
  ...
  const visibleCount = controller.getVisibleCount();
  const rows = visibleCount <= 0 ? [] : controller.getVisibleRows(0, visibleCount - 1);
  const layout: FileTreeLayoutSnapshot<FileTreeVisibleRow> = computeFileTreeLayout(rows, {
    itemHeight: FILE_TREE_DEFAULT_ITEM_HEIGHT,
    overscan: FILE_TREE_DEFAULT_OVERSCAN,
    scrollTop: scrollTop,
    viewportHeight: viewportHeight,
  });
  const stickyPathSet = new Set(layout.sticky.rows.map((entry) => getVisibleRowPath(entry.row)));
  const mountedPaths: string[] =
    layout.window.startIndex < 0 || layout.window.endIndex < layout.window.startIndex
      ? []
      : rows
...
export function clickStickyRow(
  ...
  button.dispatchEvent(new dom.window.MouseEvent("click", { bubbles: true, ...init }));
}
