...
describe("file-tree row interactions", () => {
  ...
  test("stale directory row clicks after path reset are ignored", async () => {
    ...
    try {
      ...
      staleSrcButton.dispatchEvent(new dom.window.MouseEvent("click", { bubbles: true }));
      await flushDom();
  ...
  test("flattened directory row clicks select and toggle the terminal path", async () => {
    ...
    try {
      ...
      const fileTree = new FileTree({
        ...
        paths: ["root/branch/leaf/file.ts", "root/branch/leaf/other.ts", "root/sibling.ts"],
  ...
  test("modified clicks recreate the baseline selection semantics in spirit", async () => {
    ...
    try {
      ...
      await flushDom();
      expect(getSelectedItemPaths(shadowRoot, dom)).toEqual(["a.ts", "b.ts", "c.ts", "d.ts"]);

      clickItem(shadowRoot, dom, "c.ts", { ctrlKey: true });
      await flushDom();
      expect(getSelectedItemPaths(shadowRoot, dom)).toEqual(["a.ts", "b.ts", "d.ts"]);
  ...
  test("sticky row clicks match canonical folder row selection, focus, and toggle semantics", async () => {
    ...
    try {
      ...
      await flushDom();
      expect(getSelectedItemPaths(shadowRoot, dom)).toEqual(["src/lib/util.ts"]);
      ...
      expect(getItemButton(shadowRoot, dom, "src/lib/").dataset.itemFocused).toBe("true");
  ...
  test("flattened sticky row clicks target the flattened row only", async () => {
    ...
    try {
      ...
      const expectedScrollTop = Math.max(
        0,
        getVisibleIndexForPath(expectedController, "src/lib/") * FILE_TREE_DEFAULT_ITEM_HEIGHT -
