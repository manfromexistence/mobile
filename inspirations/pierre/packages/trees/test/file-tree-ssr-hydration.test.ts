...
describe("file-tree SSR and hydration", () => {
  test("preloadFileTree returns SSR-safe initial html", async () => {
    ...
    expect(payload.shadowHtml).not.toContain('data-file-tree-sticky-overlay="true"');
  ...
  test("initialSelectedPaths preserve selected and focused state through preload and hydrate", async () => {
    ...
    try {
      ...
      expect(payload.shadowHtml).toMatch(/data-item-path="c\.ts"[^>]*tabindex="0"/);
      ...
      expect(getSelectedItemPaths(host.shadowRoot, dom)).toEqual(["a.ts", "c.ts"]);
  ...
  test("render injects wrapped unsafeCSS into the shadow root", async () => {
    ...
    try {
      ...
      const unsafeStyle = getUnsafeCssStyle(fileTree.getFileTreeContainer()?.shadowRoot, dom);
  ...
  test("preloadFileTree and hydrate keep one wrapped unsafeCSS style element", async () => {
    ...
    try {
      ...
      expect(payload.shadowHtml.match(/data-file-tree-unsafe-css/g)?.length ?? 0).toBe(1);

      const fileTree = new FileTree(options);
      fileTree.hydrate({ fileTreeContainer: host });
      await flushDom();

      expect(host.shadowRoot?.querySelectorAll("style[data-file-tree-unsafe-css]").length).toBe(1);
  ...
  test("preloadFileTree sorts unsorted top-level entries before files and keeps root chains flattened", async () => {
    ...
    ).toEqual(["assets/images/social/", "docs/guides/", "src/", "package.json", "README.md"]);
  });
  ...
  test("hydration keeps row content aligned with row paths for unsorted raw input", async () => {
    ...
    try {
      ...
      const options = {
        ...
        initialExpandedPaths: ["assets/images/social/", "docs/guides/", "src/", "src/lib/"],
        paths: unsortedPaths,
        initialVisibleRowCount: 460 / 30,
      } satisfies ConstructorParameters<typeof FileTree>[0];
      const payload = preloadFileTree(options);
      ...
      const hydratedPaths = Array.from(
        ...
        .filter(
          (button): button is HTMLButtonElement => button instanceof dom.window.HTMLButtonElement,
        )
      ...
      expect(flattenedAssetsContent?.querySelector("[data-icon-name]")).toBeNull();
      expect(
