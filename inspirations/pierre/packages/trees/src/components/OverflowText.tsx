...
export type MiddleTruncateProps = Omit<OverflowTextProps, "mode" | "children"> &
  AllowableContentGroups & {
    ...
    split?: "center" | "extension" | "leaf-path" | number | SplitOffset | CustomSplitFn;
  };

export type MiddleTruncateFilteredProps = Pick<MiddleTruncateProps, "priority" | "variant"> & {
  splitIndex?: number;
  splitOffset?: number;
};
...
const avoidWhitespaceBoundary = (contents: string, centerIndex: number): number => {
...
export const splitExtension: CustomSplitFn = (contents) => {
  ...
  const splitIndex =
    extensionIndex >= 1 && !isTooLong ? extensionIndex : centerSplitIndex(contents);
...
export const splitLeafPath: CustomSplitFn = (contents) => {
  ...
  const splitIndex =
    leafPathIndex >= 1 && !isTooLong ? leafPathIndex : Math.ceil(contents.length / 2);
  return [contents.slice(0, splitIndex), contents.slice(splitIndex)];
};
...
export const splitLast: CustomSplitFn = (contents: string, { splitOffset } = {}) => {
...
export const splitFirst: CustomSplitFn = (contents: string, { splitOffset } = {}) => {
  // fall back to center split if the offset is not valid
  if (typeof splitOffset !== "number" || splitOffset <= 0 || splitOffset >= contents.length) {
    const centerIndex = Math.ceil(contents.length / 2);
    return [contents.slice(0, centerIndex), contents.slice(centerIndex)];
...
function OverflowMarker({ children, marker, variant = "default" }: OverflowTextProps) {
  "use no memo";
...
export function OverflowText({
  ...
  const markerNode = <OverflowMarker key="marker" marker={marker} mode={mode} variant={variant} />;
