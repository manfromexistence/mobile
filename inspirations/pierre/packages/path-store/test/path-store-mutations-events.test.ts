...
describe("path-store mutations and events", () => {
  test("keeps closed-default semantics for directories added after all startup directories were expanded", () => {
    ...
    expect(getVisiblePaths(store, 0, 10)).toEqual(["new-dir/", "src/", "src/a.ts"]);
  ...
  test("adds files and explicit directories into canonical order", () => {
    ...
    expect(store.list()).toEqual(["src/components/Button.tsx", "src/index.ts", "README.md"]);
  ...
  test("rejects duplicate additions", () => {
    ...
    expect(() => store.add("src/index.ts")).toThrow('Path already exists: "src/index.ts"');
  });
  ...
  test("throws on collisions by default and can batch operations into one event", () => {
    ...
    expect(() => store.move("README.md", "src/index.ts")).toThrow("Destination already exists");
    ...
    expect(store.list()).toEqual(["src/components/Button.tsx", "src/index.ts", "tmp/README.md"]);
  });
  ...
  test("supports watcher-style array batches and emits one batch event", () => {
    ...
    expect(getVisiblePaths(store)).toEqual(["src/", "src/new.ts", "tmp/", "tmp/old.ts"]);
    expect(store.list()).toEqual(["src/new.ts", "tmp/old.ts"]);
  });
