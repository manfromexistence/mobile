...
describe.skip("path-store benchmark CLI", () => {
  test("emits JSON for a filtered smoke run", async () => {
    ...
    expect(
      payload.results.benchmarks.some((bench) => bench.alias === "visible-first/linux-5x/30"),
    ).toBe(true);
    expect(payload.results.benchmarks[0]?.runs[0]?.stats.ticks).toBeGreaterThanOrEqual(50);
  }, 20_000);
  ...
  test("documents changed-window mutation reads in the scenario manifest", async () => {
    ...
    expect(payload.scenarios).toHaveLength(1);
    expect(payload.scenarios[0]?.name).toBe("mutate/rename-leaf/first/linux-5x/200");
    expect(payload.scenarios[0]?.postMutationReadIntent).toBe("render-changed-window");
    expect(payload.scenarios[0]?.renderTargetPath).toBe(payload.scenarios[0]?.destinationPath);
  ...
  test("supports subtree moves even when the first viewport only covers one root subtree", async () => {
    ...
    expect(payload.scenarios).toHaveLength(1);
    expect(payload.scenarios[0]?.name).toBe("mutate/move-subtree/first/linux-5x/200");
  ...
  test("compares two benchmark runs with bootstrap confidence output", async () => {
    ...
    try {
      ...
      expect(payload.comparedScenarios[0]?.p50ImprovementPct).toBeGreaterThan(15);
      expect(payload.comparedScenarios[0]?.p95ImprovementPct).toBeGreaterThan(15);
