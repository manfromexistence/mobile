...
describe("demo helpers", () => {
  test("skips move-to-parent targets whose destination already exists", () => {
    ...
    expect(getMoveVisibleFolderToParentPlan(store, "linux-1/tools/perf/tools/")).toBeNull();
    expect(getMoveVisibleFolderToParentPlan(store, "linux-1/tools/perf/util/")).toEqual({
    ...
    const visibleRows = store
      ...
      .filter(
        (row) => row.path.startsWith("linux-1/tools/perf/") && row.path !== "linux-1/tools/perf/",
      );
    const candidate = findMoveVisibleFolderToParentCandidate(store, visibleRows);
