...
export const Index: Record<string, any> = {`;
  for (const item of registry.items) {
    const resolveFiles = item.files?.map((file) => `registry/elevenlabs-ui/${file.path}`);
    if (!resolveFiles) {
      continue;
    }
...
async function buildRegistryJsonFile() {
  ...
  await fs.cp(
    path.join(process.cwd(), "registry.json"),
    path.join(process.cwd(), "../www/public/r/styles/elevenlabs-ui/registry.json"),
    { recursive: true },
  );
}
...
function rewriteImports(content: string, pathMappings: Map<string, string>): string {
  ...
  content = content.replace(/@\/registry\/elevenlabs-ui\/ui\//g, "@/components/ui/");

  // Rewrite imports from @/registry/elevenlabs-ui/examples/... to @/components/examples/...
  content = content.replace(/@\/registry\/elevenlabs-ui\/examples\//g, "@/components/examples/");
