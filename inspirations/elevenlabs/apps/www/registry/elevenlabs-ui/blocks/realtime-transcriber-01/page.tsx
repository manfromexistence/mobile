...
const TranscriptCharacter = React.memo(({ char, delay }: { char: string; delay: number }) => {
  return (
    <motion.span
...
export default function RealtimeTranscriber01() {
  ...
  const [connectionState, setConnectionStateState] = useState<ConnectionState>("idle");
  ...
  }, [clearSessionRefs, connectionState, scribe, selectedLanguage, updateConnectionState]);
      ...
      <div className="relative flex h-full w-full flex-col items-center justify-center gap-8 overflow-hidden px-8 py-12">
        ...
        <div className="relative flex min-h-[350px] w-full flex-1 items-center justify-center overflow-hidden">
          ...
          >
            <div
              className={cn(
                ...
                connectionState === "connecting" ? "opacity-100" : "pointer-events-none opacity-0",
              )}
            >
              <ShimmeringText
                text="Connecting..."
          ...
          <div
            className={cn(
              "absolute inset-0 flex items-center justify-center transition-opacity duration-250",
              connectionState === "idle" ? "opacity-100" : "pointer-events-none opacity-0",
            )}
          >
            <div className="flex w-full max-w-sm flex-col gap-4 px-8">
              <div className="flex flex-col items-center gap-6">
                <div className="flex flex-col items-center gap-2 text-center">
                  <h1 className="text-2xl font-semibold tracking-tight">Realtime Speech to Text</h1>
                ...
                <div className="w-full space-y-2">
                  <label className="text-foreground/70 text-sm font-medium">Language</label>
                  <LanguageSelector
...
const TranscriberTranscript = React.memo(
  ({
    ...
    const previousNumChars = useDebounce(usePrevious(characters.length) || 0, 100);
    ...
    return (
      <div className="absolute inset-0 flex flex-col">
        <div ref={scrollRef} className="flex-1 overflow-auto">
          <div className={cn("min-h-[50%] w-full px-12 py-8", isConnected && "absolute bottom-16")}>
            <div
              className={cn(
            ...
            >
              {characters.map((char, index) => {
                const delay =
                  index >= previousNumChars ? (index - previousNumChars + 1) * 0.012 : 0;
                return <TranscriptCharacter key={index} char={char} delay={delay} />;
              })}
            </div>
          </div>
        </div>
