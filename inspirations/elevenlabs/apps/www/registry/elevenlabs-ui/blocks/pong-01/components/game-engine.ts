...
export class GameEngine {
  ...
  private updatePaddles(deltaTime: number, playerInput: number) {
    if (playerInput !== 0) {
      ...
      this.data.playerPaddle.y = Math.max(
        0,
        Math.min(FIELD_HEIGHT - PADDLE_HEIGHT, this.data.playerPaddle.y + movement),
      );
    ...
    const predictedY =
      this.data.ball.y +
      (this.data.ball.velY / Math.abs(this.data.ball.velX)) * (FIELD_WIDTH - 2 - this.data.ball.x) +
    ...
    this.data.aiPaddle.y +=
      Math.sign(aiDiff) * Math.min(Math.abs(aiDiff), AI_REACTION_SPEED * deltaTime * 60);
  }
  ...
  private checkCollisions() {
    ...
    if (
      ...
      const hitPos =
        (this.data.ball.y - this.data.playerPaddle.y - PADDLE_HEIGHT / 2) / (PADDLE_HEIGHT / 2);
      this.data.ball.velY += hitPos * 0.15;

      const speed = Math.sqrt(this.data.ball.velX ** 2 + this.data.ball.velY ** 2);
      if (speed > MAX_SPEED) {
    ...
    if (
      ...
      const hitPos =
        (this.data.ball.y - this.data.aiPaddle.y - PADDLE_HEIGHT / 2) / (PADDLE_HEIGHT / 2);
      this.data.ball.velY += hitPos * 0.15;
