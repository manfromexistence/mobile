...
function buildPathMappings(files: Array<{ path?: string; target?: string }>): Map<string, string> {
...
export function fixImport(content: string, pathMappings: Map<string, string>) {
  ...
  sortedMappings.forEach(([sourcePath, targetPath]) => {
    if (sourcePath.startsWith("blocks/")) {
      ...
      const importRegex = new RegExp(`@\\/registry\\/elevenlabs-ui\\/${escapedPath}`, "g");
  ...
  content = content.replaceAll("@/registry/elevenlabs-ui/ui/", "@/components/ui/");

  // Fix example imports
  content = content.replaceAll("@/registry/elevenlabs-ui/examples/", "@/components/examples/");
