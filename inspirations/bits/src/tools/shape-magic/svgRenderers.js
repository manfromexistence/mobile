...
export const fxFilterString = (spec) => {
  ...
  if (spec.hasShadow) {
    parts.push(
      `<feGaussianBlur in="SourceAlpha" stdDeviation="${spec.shadowBlur}" result="smBlur" />`,
    );
    parts.push(
      `<feOffset in="smBlur" dx="${spec.shadowOffsetX}" dy="${spec.shadowOffsetY}" result="smOff" />`,
    );
    parts.push(
  ...
  if (spec.hasStroke) {
    parts.push(
      `<feMorphology in="SourceAlpha" operator="dilate" radius="${spec.strokeWidth}" result="smDilated" />`,
    );
...
export const generateMergedSVG = (
  shapes,
  bridges,
  cornerRadii,
  style,
...
export const generateMergedClipPathSVG = (
  shapes,
  bridges,
  cornerRadii,
  style,
...
export const generateCSSClipPath = (
  shapes,
  bridges,
  cornerRadii,
  globalRadius,
...
export const generateReactComponent = (
  ...
  const negativeSpaces = computeNegativeSpaces(
    shapes,
    bridges,
    globalRadius,
    minX,
