...
const maxClose =
  values["max-close"] === undefined
    ? undefined
    : requirePositiveInteger("max-close", values["max-close"]);
...
async function main() {
  console.log(
    `${values.execute ? "EXECUTE" : "DRY RUN"}: PR cleanup for ${repo.owner}/${repo.name}`,
  );
  ...
  if (!values.execute) {
    ...
    console.log(
      `Showing ${Math.min(printLimit, selected.length)} of ${selected.length} matching PRs:\n`,
    );
    for (const pr of selected.slice(0, printLimit)) {
      console.log(`#${pr.number} ${pr.createdAt} positive=${pr.positiveReactions} ${pr.url}`);
    }
    if (selected.length > printLimit)
      console.log(`... ${selected.length - printLimit} more not shown`);
    return;
  }
...
async function githubRequest(path: string, init: RequestInit, attempt = 0): Promise<Response> {
  ...
  if (
    ...
    console.warn(
      `GitHub request failed; sleeping ${Math.ceil(retryMs / 1000)}s before retry ${attempt + 1}`,
    );
    await sleep(retryMs);
...
function subtractMonths(date: Date, months: number) {
  ...
  result.setUTCDate(
    Math.min(
      day,
      new Date(Date.UTC(result.getUTCFullYear(), result.getUTCMonth() + 1, 0)).getUTCDate(),
    ),
  );
  return result;
}
