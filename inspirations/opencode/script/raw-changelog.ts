...
async function commits(from: string, to: string) {
  ...
  for (const hash of log.split("\n").filter(Boolean)) {
    ...
    for (const file of diff.split("\n").filter(Boolean)) {
      ...
      else if (file.startsWith("packages/desktop/") || file.startsWith("packages/app/"))
        areas.add("app");
      else if (file.startsWith("packages/sdk/") || file.startsWith("packages/plugin/"))
        areas.add("sdk");
      else if (file.startsWith("sdks/vscode/") || file.startsWith("github/"))
...
async function published(to: string) {
  if (to === "HEAD") return;
  const body = await $`gh release view ${ref(to)} --repo ${repo} --json body --jq .body`
    .text()
    .catch(() => "");
  if (!body) return;
...
async function thanks(from: string, to: string, reuse: boolean) {
  ...
  const lines = [
    `**Thank you to ${users.size} community contributor${users.size > 1 ? "s" : ""}:**`,
  ];
...
function format(from: string, to: string, list: Commit[], thanks: string[]) {
  ...
  for (const commit of list) {
    ...
    grouped
      .get(section(commit.areas))!
      .get(type(commit.message))!
      .push(`- \`${commit.hash}\` ${commit.message}${attr}`);
  }
