...
const it = testEffect(
  LayerNode.compile(LayerNode.group([Storage.node, FSUtil.node, CrossSpawnSpawner.node])),
);
...
function remap(root: string, file: string) {
  ...
  if (file.startsWith(Global.Path.data + path.sep))
    return path.join(root, path.relative(Global.Path.data, file));
  return file;
}
...
describe("Storage", () => {
  ...
  it.live("migration 1 tolerates malformed legacy records", () =>
    Effect.gen(function* () {
      ...
      yield* fs.writeWithDirs(
        path.join(legacy, "storage", "session", "message", "probe", "0.json"),
        "[]",
      );
      yield* fs.writeWithDirs(
      ...
      yield* Effect.gen(function* () {
        ...
        expect(
          yield* svc.read<{ id: string; title: string }>(["session", project, "ses_legacy"]),
        ).toEqual({
          id: "ses_legacy",
          title: "legacy",
        });
        expect(
          yield* svc.read<{ role: string; text: string }>(["message", "ses_legacy", "msg_legacy"]),
        ).toEqual({
          role: "user",
          text: "hello",
        });
