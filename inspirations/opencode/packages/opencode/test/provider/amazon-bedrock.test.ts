...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      yield* withAuthJson(
        JSON.stringify({ "amazon-bedrock": { type: "api", key: "test-bearer-token" } }),
      );
  ...
  () =>
    Effect.gen(function* () {
      ...
      const model = yield* Provider.use.getModel(
        ...
        ModelV2.ID.make("openai.gpt-5.5"),
      );
...
it.instance(
    ...
    }),
  {
    config: {
      provider: {
        "amazon-bedrock": { options: { profile: "my-custom-profile", region: "us-east-1" } },
      },
    },
  },
);
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(providers[ProviderV2.ID.amazonBedrock]).toBeDefined();
      expect(
        providers[ProviderV2.ID.amazonBedrock].models["us.anthropic.claude-opus-4-5-20251101-v1:0"],
      ).toBeDefined();
    }),
  {
    config: {
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(providers[ProviderV2.ID.amazonBedrock]).toBeDefined();
      expect(
        providers[ProviderV2.ID.amazonBedrock].models[
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(providers[ProviderV2.ID.amazonBedrock]).toBeDefined();
      expect(
        providers[ProviderV2.ID.amazonBedrock].models["eu.anthropic.claude-opus-4-5-20251101-v1:0"],
      ).toBeDefined();
    }),
  {
    config: {
      provider: {
        "amazon-bedrock": {
          ...
          models: {
            "eu.anthropic.claude-opus-4-5-20251101-v1:0": { name: "Claude Opus 4.5 (EU)" },
          },
        },
      },
    },
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(providers[ProviderV2.ID.amazonBedrock]).toBeDefined();
      expect(
        providers[ProviderV2.ID.amazonBedrock].models["anthropic.claude-opus-4-5-20251101-v1:0"],
      ).toBeDefined();
    }),
  {
    config: {
...
describe("Bedrock cross-region prefix detection", () => {
  ...
  test("should detect global. prefix", () => {
    expect(
      crossRegionPrefixes.some((p) =>
        "global.anthropic.claude-opus-4-5-20251101-v1:0".startsWith(p),
      ),
    ).toBe(true);
  });
  ...
  test("should detect us. prefix", () => {
    expect(
      crossRegionPrefixes.some((p) => "us.anthropic.claude-opus-4-5-20251101-v1:0".startsWith(p)),
    ).toBe(true);
  });

  test("should detect eu. prefix", () => {
    expect(
      crossRegionPrefixes.some((p) => "eu.anthropic.claude-opus-4-5-20251101-v1:0".startsWith(p)),
    ).toBe(true);
  });

  test("should detect jp. prefix", () => {
    expect(
      crossRegionPrefixes.some((p) => "jp.anthropic.claude-sonnet-4-20250514-v1:0".startsWith(p)),
    ).toBe(true);
  });

  test("should detect apac. prefix", () => {
    expect(
      crossRegionPrefixes.some((p) => "apac.anthropic.claude-sonnet-4-20250514-v1:0".startsWith(p)),
    ).toBe(true);
  });

  test("should detect au. prefix", () => {
    expect(
      crossRegionPrefixes.some((p) => "au.anthropic.claude-sonnet-4-5-20250929-v1:0".startsWith(p)),
    ).toBe(true);
  });

  test("should NOT detect prefix for non-prefixed model", () => {
    expect(
      crossRegionPrefixes.some((p) => "anthropic.claude-opus-4-5-20251101-v1:0".startsWith(p)),
    ).toBe(false);
  });
