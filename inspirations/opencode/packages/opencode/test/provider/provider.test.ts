...
const languageBaseURL = (language: unknown) =>
  (language as { config: { baseURL: string } }).config.baseURL;
...
it.instance(
  ...
  }),
  {
    config: {
      provider: {
        anthropic: {
          models: { "my-alias": { id: "claude-sonnet-4-20250514", name: "My Custom Alias" } },
        },
      },
    },
  },
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    expect(provider.models["deepseek-r1"].capabilities.interleaved).toEqual({
      field: "reasoning_content",
    });
    expect(provider.models["deepseek-details"].capabilities.interleaved).toEqual({
      field: "reasoning_details",
    });
    expect(provider.models["custom-model"].capabilities.interleaved).toBe(false);
    expect(
      providers[ProviderV2.ID.make("custom-anthropic-provider")].models["deepseek-r1"].capabilities
        .interleaved,
    ).toBe(false);
  }),
  {
...
it.instance(
  ...
  }),
  {
    config: {
      provider: {
        anthropic: { options: { timeout: 60000, headerTimeout: 10000, chunkTimeout: 15000 } },
      },
    },
  },
);
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    expect(providers[ProviderV2.ID.make("custom-openai")].options.baseURL).toBe(
    ...
    config: {
      provider: {
        "custom-openai": {
          ...
          env: [],
          models: {
            "gpt-4": { name: "GPT-4", tool_call: true, limit: { context: 128000, output: 4096 } },
          },
...
it.instance(
  ...
  }),
  {
    config: {
      provider: {
        anthropic: { options: { apiKey: "test-api-key" }, whitelist: ["nonexistent-model"] },
      },
    },
  },
);
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    expect(providers[ProviderV2.ID.make("custom-api")].models["model-1"].api.url).toBe(
    ...
    config: {
      provider: {
        "custom-api": {
          ...
          env: [],
          models: {
            "model-1": { name: "Model 1", tool_call: true, limit: { context: 8000, output: 2000 } },
          },
          options: { apiKey: "test-key" },
        },
      },
...
it.instance(
  ...
  Effect.gen(function* () {
    const providers = yield* list;
    expect(providers[ProviderV2.ID.make("custom-api")].options.baseURL).toBe(
      "https://custom.override.com/v1",
    );
  }),
  {
    config: {
...
it.instance(
  ...
  }),
  {
    config: {
      provider: {
        anthropic: { models: { "claude-sonnet-4-20250514": { name: "Custom Name for Sonnet" } } },
      },
    },
  },
);
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    expect(providers[ProviderV2.ID.make("local-llm")].models["llama-3"].api.npm).toBe(
      "@ai-sdk/openai-compatible",
    );
    expect(providers[ProviderV2.ID.make("local-llm")].options.baseURL).toBe(
    ...
    config: {
      provider: {
        "local-llm": {
          ...
          env: [],
          models: {
            "llama-3": { name: "Llama 3", tool_call: true, limit: { context: 8192, output: 2048 } },
          },
...
it.instance(
  ...
  Effect.gen(function* () {
    const providers = yield* list;
    expect(
      providers[ProviderV2.ID.make("no-tools")].models["basic-model"].capabilities.toolcall,
    ).toBe(false);
  }),
  {
    config: {
...
it.instance(
  ...
  Effect.gen(function* () {
    const providers = yield* list;
    expect(
      providers[ProviderV2.ID.make("default-tools")].models["model"].capabilities.toolcall,
    ).toBe(true);
  }),
  {
    config: {
    ...
    config: {
      provider: {
        "fallback-env": {
          ...
          models: {
            model: { name: "Model", tool_call: true, limit: { context: 4000, output: 1000 } },
          },
...
it.instance("getProvider returns undefined for nonexistent provider", () =>
  Effect.gen(function* () {
    const provider = yield* Provider.Service.use((svc) =>
      svc.getProvider(ProviderV2.ID.make("nonexistent")),
    );
...
it.instance(
  ...
  }),
  {
    config: {
      provider: {
        anthropic: { options: { headers: { "X-Custom": "custom-value" }, timeout: 30000 } },
      },
    },
  },
);
...
it.instance(
  ...
  }),
  {
    config: {
      provider: {
        nvidia: { options: { apiKey: "test-api-key", baseURL: "http://localhost:8000/v1" } },
      },
    },
  },
);
...
it.instance(
  ...
  Effect.gen(function* () {
    const providers = yield* list;
    expect(providers[ProviderV2.ID.make("nvidia")].options.headers["X-BILLING-INVOKE-ORIGIN"]).toBe(
      "CustomOrigin",
    );
  }),
  {
...
it.instance(
  ...
  Effect.gen(function* () {
    ...
    expect(providers[ProviderV2.ID.make("vertex-proxy")].options.baseURL).toBe(
...
it.effect("opencode loader keeps paid models when auth exists", () =>
  Effect.gen(function* () {
    ...
    const original = yield* Effect.promise(() =>
      Filesystem.readText(authPath).catch(() => undefined),
    );
