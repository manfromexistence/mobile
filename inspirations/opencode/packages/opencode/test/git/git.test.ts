  ...
  it.live("branch() returns undefined for detached HEAD", () =>
    Effect.gen(function* () {
      ...
      const hash = (yield* Effect.promise(() =>
        $`git rev-parse HEAD`.cwd(tmp.path).quiet().text(),
      )).trim();
  ...
  it.live("show() returns empty text for binary blobs", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        fs.writeFile(path.join(tmp.path, "bin.dat"), new Uint8Array([0, 1, 2, 3])),
      );
      yield* Effect.promise(() => $`git add .`.cwd(tmp.path).quiet());
      yield* Effect.promise(() =>
        $`git commit --no-gpg-sign -m "add binary"`.cwd(tmp.path).quiet(),
      );
