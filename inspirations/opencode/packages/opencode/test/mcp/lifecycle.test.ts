...
interface MockClientState {
  ...
  promptPages: Record<
    string,
    { prompts: Array<{ name: string; description?: string }>; nextCursor?: string }
  >;
  ...
  Client: class MockClient {
    ...
    async request(
      ...
      if (request.method === "tools/list") {
        return schema.parse(
          this._state?.toolPages[
            request.params === undefined ? "initial" : (request.params.cursor ?? "")
          ] ?? {
    ...
    async listPrompts(params?: { cursor?: string }) {
      ...
      const page =
        this._state?.promptPages[params === undefined ? "initial" : (params.cursor ?? "")];
      if (page) return page;
    ...
    async listResources(params?: { cursor?: string }) {
      ...
      const page =
        this._state?.resourcePages[params === undefined ? "initial" : (params.cursor ?? "")];
      if (page) return page;
    ...
    async listResourceTemplates(params?: { cursor?: string }) {
      ...
      const page =
        this._state?.resourceTemplatePages[
          params === undefined ? "initial" : (params.cursor ?? "")
        ];
      if (page) return page;
    ...
    MCP.Service.use((mcp: MCPNS.Interface) =>
      Effect.gen(function* () {
        ...
        const result = yield* Effect.promise(
          () => handler?.() ?? Promise.reject(new Error("roots handler missing")),
        );
  ...
  () =>
    MCP.Service.use((mcp: MCPNS.Interface) =>
      Effect.gen(function* () {
        ...
        expect(
          (addResult.status as any)["my-server"]?.status ?? (addResult.status as any).status,
        ).toBe("connected");
    ...
    MCP.Service.use((mcp: MCPNS.Interface) =>
      Effect.gen(function* () {
        ...
        serverState.toolPages = {
          ...
          "tools-2": {
            tools: [{ name: "tool-two", inputSchema: { type: "object", properties: {} } }],
          },
        };
        ...
        serverState.resourcePages = {
          initial: {
            resources: [{ name: "resource-one", uri: "test://one" }],
        ...
        serverState.resourceTemplatePages = {
          ...
          "resource-templates-2": {
            resourceTemplates: [{ name: "template-two", uriTemplate: "test://two/{id}" }],
          },
        };
    ...
    MCP.Service.use((mcp: MCPNS.Interface) =>
      Effect.gen(function* () {
        ...
        serverState.resources = [
          ...
          {
            name: "my-resource",
            uri: "ui://component-state",
            description: "A second resource with same name",
          },
        ];
