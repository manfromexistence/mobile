...
const withCallbackStop = Effect.addFinalizer(() =>
...
mcpTest.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* authenticateScoped("test-oauth-server-3", (url) =>
        Deferred.doneUnsafe(authorization, Effect.succeed(url)),
      );

      const url = yield* awaitWithTimeout(
        Deferred.await(opened),
        "Timed out waiting for open()",
        "5 seconds",
      );
      ...
      expect(transportCalls.at(-1)?.options.requestInit?.headers).toEqual({
