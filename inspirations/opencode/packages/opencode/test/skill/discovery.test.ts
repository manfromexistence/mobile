    ...
    async fetch(req) {
      ...
      if (url.pathname === "/mutable/index.json") {
        return Response.json({
          skills: [{ name: "mutable", version: mutableVersion, files: mutableFiles }],
        });
      }
...
describe("Discovery.pull", () => {
  ...
  it.live("refreshes a remote skill when its version changes", () =>
    Effect.gen(function* () {
      ...
      expect(yield* Effect.promise(() => Bun.file(path.join(first[0], "SKILL.md")).text())).toBe(
        "# Old",
      );
      ...
      expect(yield* Effect.promise(() => Bun.file(path.join(second[0], "SKILL.md")).text())).toBe(
        "# Old",
      );
      expect(yield* Effect.promise(() => Bun.file(path.join(second[0], "old.md")).text())).toBe(
        "old reference",
      );
      ...
      expect(yield* Effect.promise(() => Bun.file(path.join(second[0], "old.md")).exists())).toBe(
        false,
      );
