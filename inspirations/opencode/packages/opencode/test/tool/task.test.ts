...
describe("tool.task", () => {
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const registry = yield* ToolRegistry.Service;
        const description =
          (yield* registry.tools({ ...ref, agent: build })).find((tool) => tool.id === TaskTool.id)
            ?.description ?? "";
  ...
  it.instance("promotes a running foreground task without restarting it", () =>
    Effect.gen(function* () {
      ...
      expect((yield* jobs.wait({ id: result.metadata.sessionId })).info?.output).toBe(
        "background done",
      );
  ...
  background.instance("background task completion waits for running updates", () =>
    Effect.gen(function* () {
      ...
      if (notification.parts[0]?.type === "text")
        expect(notification.parts[0].text).toContain("second done");
    }),
  );
