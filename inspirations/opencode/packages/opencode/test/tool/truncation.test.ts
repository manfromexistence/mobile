  ...
  describe("output", () => {
    ...
    test("loads truncate effect in a fresh process", async () => {
      const out = await Process.run(
        [process.execPath, "run", path.join(ROOT, "src", "tool", "truncate.ts")],
        {
          cwd: ROOT,
        },
      );
  ...
  describe("cleanup", () => {
    ...
    it.live("deletes files older than 7 days and preserves recent files", () =>
      Effect.gen(function* () {
        ...
        const old = path.join(
          Truncate.DIR,
          Identifier.create("tool", "ascending", Date.now() - 10 * DAY_MS),
        );
        const recent = path.join(
          Truncate.DIR,
          Identifier.create("tool", "ascending", Date.now() - 3 * DAY_MS),
        );
