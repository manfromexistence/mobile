...
const put = Effect.fn("ReadToolTest.put")(function* (
  p: string,
...
describe("tool.read external_directory permission", () => {
  ...
  it.live(
    "uses worktree-relative path for read permission so user rules match like edit/write",
    () =>
      Effect.gen(function* () {
        const dir = yield* tmpdirScoped({ git: true });
  ...
  it.live(
    "asks for directory-scoped external_directory permission when reading external directory",
    () =>
      Effect.gen(function* () {
        const outer = yield* tmpdirScoped();
        const dir = yield* tmpdirScoped({ git: true });
