  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(permissionCall.metadata.files.map((f) => f.type).sort()).toEqual([
          "add",
          "delete",
          "update",
        ]);
...
EOF`;
      ...
      yield* execute({ patchText }, ctx);
      expect(yield* readText(path.join(test.directory, "heredoc_test.txt"))).toBe(
        "heredoc content\n",
      );
    }),
  );
...
EOF`;
      ...
      yield* execute({ patchText }, ctx);
      expect(yield* readText(path.join(test.directory, "heredoc_no_cat.txt"))).toBe(
        "no cat prefix\n",
      );
    }),
  );
