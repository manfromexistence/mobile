...
it.instance("migrates tui-specific keys from opencode.json when tui.json does not exist", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      expect(
        JSON.parse(yield* fs.readFileString(path.join(test.directory, "tui.json"))),
      ).toMatchObject({
        theme: "migrated-theme",
        scroll_speed: 5,
      });
      ...
      expect(
        yield* fs.existsSafe(path.join(test.directory, "opencode.json.tui-migration.bak")),
      ).toBe(true);
  ...
  withCleanState(
    Effect.gen(function* () {
      ...
      const server = JSON.parse(
        yield* fs.readFileString(path.join(test.directory, "opencode.json")),
      );
      expect(server.theme).toBeUndefined();
      expect(server.tui).toBeUndefined();
    }),
...
it.instance("skips migration when opencode.jsonc is syntactically invalid", () =>
  ...
  "username": "still-broken"
      ...
      expect(
        yield* fs.existsSafe(path.join(test.directory, "opencode.jsonc.tui-migration.bak")),
      ).toBe(false);
...
it.instance("migration backup preserves JSONC comments", () =>
  ...
  "tui": {
    ...
    "scroll_speed": 1.5
      ...
      const backup = yield* fs.readFileString(
        path.join(test.directory, "opencode.jsonc.tui-migration.bak"),
      );
...
it.instance(
  "project config takes precedence over OPENCODE_TUI_CONFIG (matches OPENCODE_CONFIG)",
  () =>
    withCleanState(
      Effect.gen(function* () {
        const fs = yield* FSUtil.Service;
...
it.instance(
  "ignores unknown keybind names without dropping valid overrides from the same file",
  () =>
    withCleanState(
      Effect.gen(function* () {
        const fs = yield* FSUtil.Service;
...
it.instance("resolves keybind lookup from canonical keybinds", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      expect(config.keybinds.get("which-key.group.next")?.[0]?.key).toBe(
        "ctrl+alt+right,ctrl+alt+]",
      );
      expect(
        (config.keybinds.get("which-key.toggle")?.[0] as { desc?: unknown } | undefined)?.desc,
      ).toBe("Toggle which-key panel");
      ...
      expect(
        config.keybinds
          .gather("plugins.dialog", ["dialog.plugins.install"])
...
winIt("ignores terminal suspend bindings on Windows", () =>
  withCleanState(
    Effect.gen(function* () {
      ...
      const test = yield* TestInstance;
      yield* fs.writeJson(path.join(test.directory, "tui.json"), {
        keybinds: { terminal_suspend: "alt+z" },
      });
...
it.instance(
  "silently skips non-ENOENT read failures (e.g. tui.json is a directory) - fallback layer still loads",
  () =>
    withCleanState(
      Effect.gen(function* () {
        const fs = yield* FSUtil.Service;
