...
function remoteConfigClient(input: {
  ...
  return HttpClient.make((request) => {
    ...
    if (
      request.url.includes("config.example.com") &&
      (input.remote !== undefined || input.remoteHtml !== undefined)
    ) {
...
const clearEffect = (wait = false) =>
  Config.use
    ...
    .pipe(
      ...
      Effect.provide(layer),
      Effect.andThen(
        wait ? Effect.promise(() => InstanceRuntime.disposeAllInstances()) : Effect.void,
      ),
    );
...
const writeManagedSettingsEffect = (settings: object, filename?: string) =>
  FSUtil.use.writeWithDirs(
    path.join(managedConfigDir, filename ?? "opencode.json"),
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.all(
      [
        ...
        input.local
          ? writeConfigEffect(path.join(directory, ".opencode"), schemaConfig(input.local))
          : undefined,
      ].filter(
        (effect): effect is Effect.Effect<void, FSUtil.Error, FSUtil.Service> =>
          effect !== undefined,
      ),
...
it.instance("updates config and preserves empty shell sentinel", () =>
  Effect.gen(function* () {
    ...
    yield* Config.Service.use((svc) =>
      svc.update(ConfigParse.schema(ConfigV1.Info, { shell: "" }, "test:config")),
    );
...
it.effect("updates global config and omits empty shell key in jsonc", () =>
  withGlobalConfig(
    { config: { shell: "bash", model: "test/model" }, name: "opencode.jsonc" },
    ({ dir }) =>
      Effect.gen(function* () {
        yield* Config.use.updateGlobal({ shell: "" });
        ...
        const parsed = ConfigParse.schema(
          ConfigV1.Info,
          ConfigParse.jsonc(writtenConfig, file),
          file,
        );
...
it.instance("handles file inclusion with replacement tokens", () =>
  Effect.gen(function* () {
    const test = yield* TestInstance;
    yield* FSUtil.use.writeWithDirs(
      path.join(test.directory, "included.md"),
      "const out = await Bun.$`echo hi`",
    );
    yield* writeConfigEffect(test.directory, {
      $schema: "https://opencode.ai/config.json",
...
it.effect("installs dependencies in writable OPENCODE_CONFIG_DIR", () =>
  Effect.gen(function* () {
    ...
    expect(yield* FSUtil.use.readFileString(path.join(configDir, ".gitignore"))).toContain(
      "package-lock.json",
    );
  }).pipe(
    Effect.provide(testInstanceStoreLayer),
  ...
  withConfigTree(
    ...
    withProcessEnv(
      ...
      Effect.gen(function* () {
        ...
        expect(config.plugin_origins?.find((item) => item.spec === "global-plugin")?.scope).toBe(
          "global",
        );
      }),
    ),
  ...
  withConfigTree(
    ...
    Effect.gen(function* () {
      ...
      expect(
        plugins.filter(
          (p) =>
            p.includes("global-plugin") ||
            p.includes("local-plugin") ||
            p.includes("duplicate-plugin"),
        ).length,
      ).toBe(3);
    }),
  ),
...
it.effect("keeps plugin origins aligned with merged plugin list", () =>
  withConfigTree(
    ...
    Effect.gen(function* () {
      ...
      expect(
        origins.find((item) => ConfigPlugin.pluginSpecifier(item.spec) === "shared-plugin@2.0.0")
          ?.scope,
      ).toBe("local");
    }),
  ),
...
test("config parser preserves permission order while rejecting unknown top-level keys", () => {
  ...
  try {
    ...
    const error = err as {
      data?: { issues?: Array<{ code?: string; keys?: string[]; path?: string[] }> };
    };
...
const templatedHeaderWellKnown = wellKnown({
  ...
  remote: {
    mcp: {
      confluence: { type: "remote", url: "https://confluence.example.com/mcp", enabled: true },
    },
  },
});

templatedHeaderWellKnown.it.instance(
  "wellknown remote_config supports templated env vars in headers",
  () =>
    Effect.gen(function* () {
      const config = yield* Config.use.get();
      expect(templatedHeaderWellKnown.seen.wellKnown).toBe(
...
const remotePrecedenceWellKnown = wellKnown({
  config: {
    mcp: {
      confluence: { type: "remote", url: "https://confluence.example.com/mcp", enabled: false },
    },
  },
...
remotePrecedenceWellKnown.it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const config = yield* Config.use.get();
      expect(remotePrecedenceWellKnown.seen.remote).toBe(
...
invalidRemoteWellKnown.it.instance(
  "wellknown remote_config rejects non-object config responses",
  () =>
    Effect.gen(function* () {
      const exit = yield* Config.use.get().pipe(Effect.exit);
...
const loginPageWellKnown = wellKnown({
  remoteConfig: { url: "https://config.example.com/opencode.json" },
  remoteHtml:
    "<!DOCTYPE html><html><head><title>Sign in</title></head><body>Login required</body></html>",
});
...
describe("resolvePluginSpec", () => {
  test("keeps package specs unchanged", async () => {
    ...
    expect(await ConfigPlugin.resolvePluginSpec("oh-my-opencode@2.4.3", file)).toBe(
  ...
  test("resolves windows-style relative plugin directory specs", async () => {
    ...
    expect(ConfigPlugin.pluginSpecifier(hit)).toBe(
      pathToFileURL(path.join(tmp.path, "plugin", "index.ts")).href,
    );
  });
  ...
  test("resolves plugin directory paths to directory urls", async () => {
    ...
    const hit = await ConfigPlugin.resolvePluginSpec("./plugin", file);
    expect(ConfigPlugin.pluginSpecifier(hit)).toBe(
      pathToFileURL(path.join(tmp.path, "plugin")).href,
    );
  });
...
describe("deduplicatePluginOrigins", () => {
  ...
  test("deduplicates direct path plugins by exact spec", () => {
    const plugins = [
      "file:///project/.opencode/plugin/demo.ts",
      "file:///project/.opencode/plugin/demo.ts",
    ];
  ...
  it.effect("loads auto-discovered local plugins as file urls", () =>
    withConfigTree(
      ...
      Effect.gen(function* () {
        ...
        const plugins = (yield* Config.use.get()).plugin ?? [];
        expect(plugins.some((p) => ConfigPlugin.pluginSpecifier(p) === "my-plugin@1.0.0")).toBe(
          true,
        );
        expect(plugins.some((p) => ConfigPlugin.pluginSpecifier(p).startsWith("file://"))).toBe(
          true,
        );
      }),
    ),
    ...
    () =>
      withProcessEnvs(
        ...
        Effect.gen(function* () {
          const test = yield* TestInstance;
          yield* FSUtil.use.writeWithDirs(
            path.join(test.directory, "CUSTOM.md"),
            "# Custom Instructions",
          );
...
describe("OPENCODE_CONFIG_CONTENT token substitution", () => {
  ...
  it.instance("substitutes {file:} tokens in OPENCODE_CONFIG_CONTENT", () =>
    Effect.gen(function* () {
      const test = yield* TestInstance;
      yield* FSUtil.use.writeWithDirs(
        path.join(test.directory, "api_key.txt"),
