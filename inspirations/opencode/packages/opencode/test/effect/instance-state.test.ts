...
const it = testEffect(
  Layer.mergeAll(LayerNode.compile(CrossSpawnSpawner.node), testInstanceStoreLayer),
);
...
const tmpdirGitScoped = Effect.gen(function* () {
  ...
  yield* Effect.promise(() =>
    $`git commit --allow-empty --amend -m ${`root commit ${dir}`}`.cwd(dir).quiet(),
  );
  return dir;
});
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      const [a, b, c] = yield* Effect.all(
        [one, two, three].map((dir) =>
          Test.use((svc) => svc.get()).pipe(provideInstanceEffect(dir)),
        ),
...
it.live("InstanceState dedupes concurrent lookups", () =>
  Effect.gen(function* () {
    ...
    const [a, b] = yield* Effect.all([access(state, dir), access(state, dir)], {
      concurrency: "unbounded",
    });
    expect(a).toBe(b);
    expect(n).toBe(1);
...
it.live("InstanceState survives deferred resume from the same instance context", () =>
  Effect.gen(function* () {
    ...
    yield* Effect.gen(function* () {
      ...
      const fiber = yield* Test.use((svc) => svc.get(gate)).pipe(
