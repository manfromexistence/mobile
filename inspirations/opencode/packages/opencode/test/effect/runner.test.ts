...
describe("Runner", () => {
  ...
  it.live(
    ...
    Effect.gen(function* () {
      ...
      const [a, b] = yield* Effect.all(
        [
          runner.ensureRunning(work).pipe(Effect.exit),
          runner.ensureRunning(work).pipe(Effect.exit),
        ],
        { concurrency: "unbounded" },
      );
  ...
  it.live(
    ...
    Effect.gen(function* () {
      ...
      const fiber = yield* runner
        .ensureRunning(Effect.never.pipe(Effect.as("never")))
        .pipe(Effect.forkChild);
  ...
  it.live(
    ...
    Effect.gen(function* () {
      ...
      const a = yield* runner
        .ensureRunning(Effect.never.pipe(Effect.as("x")))
        .pipe(Effect.forkChild);
      yield* waitForState(runner, "Running");
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.gen(function* () {
        ...
        const b = yield* runner
          .ensureRunning(Deferred.await(done).pipe(Effect.as("second")))
          .pipe(Effect.forkChild);
        yield* Effect.yieldNow;
        expect(runner.busy).toBe(true);
        ...
        Effect.ensuring(
          Effect.all([Deferred.succeed(hold, undefined), Deferred.succeed(done, undefined)], {
            discard: true,
          }).pipe(Effect.ignore),
        ),
      );
  ...
  it.live(
    ...
    Effect.gen(function* () {
      ...
      const sh = yield* runner
        .startShell(Effect.never.pipe(Effect.as("aborted")))
        .pipe(Effect.forkChild);
      yield* waitForState(runner, "Shell");
