...
describe("RuntimeFlags", () => {
  ...
  it.effect("enables native LLM via dedicated flag only", () =>
    Effect.gen(function* () {
      const explicit = yield* readFlags.pipe(
        Effect.provide(fromConfig({ OPENCODE_EXPERIMENTAL_NATIVE_LLM: "true" })),
      );
      const umbrella = yield* readFlags.pipe(
  ...
  it.effect("enables WebSockets via dedicated flag only", () =>
    Effect.gen(function* () {
      const explicit = yield* readFlags.pipe(
        Effect.provide(fromConfig({ OPENCODE_EXPERIMENTAL_WEBSOCKETS: "true" })),
      );
      const umbrella = yield* readFlags.pipe(
        Effect.provide(fromConfig({ OPENCODE_EXPERIMENTAL: "true" })),
      );
  ...
  it.effect("layer accepts partial test overrides and fills defaults from Config definitions", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(
        Effect.provide(
          RuntimeFlags.layer({ disableDefaultPlugins: true, bashDefaultTimeoutMs: 1_000 }),
        ),
      );
  ...
  it.effect("disableExternalSkills reads OPENCODE_DISABLE_EXTERNAL_SKILLS", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(
        Effect.provide(fromConfig({ OPENCODE_DISABLE_EXTERNAL_SKILLS: "true" })),
      );
  ...
  it.effect("disableLspDownload reads OPENCODE_DISABLE_LSP_DOWNLOAD", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(
        Effect.provide(fromConfig({ OPENCODE_DISABLE_LSP_DOWNLOAD: "true" })),
      );
  ...
  it.effect("disableClaudeCodePrompt reads OPENCODE_DISABLE_CLAUDE_CODE_PROMPT", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(
        Effect.provide(fromConfig({ OPENCODE_DISABLE_CLAUDE_CODE_PROMPT: "true" })),
      );
  ...
  it.effect("disableClaudeCodePrompt inherits OPENCODE_DISABLE_CLAUDE_CODE", () =>
    Effect.gen(function* () {
      const flags = yield* readFlags.pipe(
        Effect.provide(fromConfig({ OPENCODE_DISABLE_CLAUDE_CODE: "true" })),
      );
  ...
  for (const input of [
    ...
    {
      name: "zero",
      config: { OPENCODE_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS: "0" },
      expected: undefined,
    },
    {
      name: "negative",
      config: { OPENCODE_EXPERIMENTAL_BASH_DEFAULT_TIMEOUT_MS: "-1" },
      expected: undefined,
    },
    {
      name: "non-integer",
  ...
  for (const input of [
    ...
    {
      name: "negative",
      config: { OPENCODE_EXPERIMENTAL_OUTPUT_TOKEN_MAX: "-1" },
      expected: undefined,
    },
    {
      name: "non-integer",
