...
export async function provideTestInstance<R>(input: {
  ...
  try {
    if (input.init)
      await Effect.runPromise(input.init.pipe(Effect.provideService(InstanceRef, ctx)));
...
export async function withTestInstance<R>(input: {
  directory: string;
  fn: (ctx: InstanceContext) => R;
}) {
...
export class TestInstance extends Context.Service<TestInstance, { readonly directory: string }>()(
  "@test/Instance",
) {}
