...
describe("pty HttpApi bridge", () => {
  test("serves available shell list through experimental Effect routes", async () => {
    ...
    const response = await app().request(PtyPaths.shells, {
  ...
  testPty("serves PTY JSON routes through experimental Effect routes", async () => {
    ...
    try {
      ...
      await app().request(PtyPaths.remove.replace(":ptyID", info.id), {
        method: "DELETE",
        headers,
      });
    }
  ...
  test("returns typed not found errors for missing PTY HTTP resources", async () => {
    ...
    const removed = await app().request(PtyPaths.remove.replace(":ptyID", missingID), {
      method: "DELETE",
      headers,
    });
  ...
  (process.platform === "win32" ? effectIt.live.skip : effectIt.live)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const removed = yield* HttpClientRequest.delete(
          PtyPaths.remove.replace(":ptyID", info.id),
