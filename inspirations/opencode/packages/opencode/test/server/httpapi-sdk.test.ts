      ...
      return Object.assign(
        async (request: RequestInfo | URL, init?: RequestInit) => {
          ...
          return globalThis.fetch(
            new Request(new URL(`${url.pathname}${url.search}`, baseUrl), source),
          );
        },
...
function httpapiInstance<A, E>(
  ...
  it.instance(
    ...
    Effect.gen(function* () {
      ...
      return yield* run({
        sdk: yield* client(options.serverPath, instance.directory),
  ...
  httpapi(
    ...
    Effect.gen(function* () {
      ...
      const log = yield* call(() =>
        sdk.app.log({ service: "httpapi-sdk-test", level: "info", message: "hello" }),
      );
  ...
  serverPathParity("matches generated SDK global and control behavior", (serverPath) =>
    Effect.gen(function* () {
      ...
      const log = yield* capture(() =>
        sdk.app.log({ service: "sdk-parity", level: "info", message: "hello" }),
      );
    ...
    withStandardProject(serverPath, ({ sdk }) =>
      Effect.gen(function* () {
        ...
        const update = yield* capture(() =>
          sdk.session.update({ sessionID: parentID, title: "renamed" }),
        );
    ...
    withStandardProject(serverPath, ({ sdk, directory }) =>
      Effect.gen(function* () {
        const session = yield* capture(() =>
          sdk.session.create({ title: "sync-backed part event" }),
        );
        const sessionID = String(record(session.data).id);
        const seeded = yield* seedMessage(directory, sessionID);
        ...
        const events = yield* call(() =>
          sdk.event.subscribe(undefined, { signal: controller.signal }),
        );
        ...
        yield* awaitWithTimeout(
          ...
          "timed out waiting for /event server.connected",
          "2 seconds",
        );
  ...
  serverPathParity("matches generated SDK TUI validation and command routes", (serverPath) =>
    withStandardProject(serverPath, ({ sdk }) =>
      Effect.gen(function* () {
        ...
        const executeCommand = yield* capture(() =>
        ...
        const missingSession = yield* capture(() =>
          sdk.tui.selectSession({ sessionID: "ses_missing" }),
        );
        const invalidSession = yield* capture(() =>
          sdk.tui.selectSession({ sessionID: "invalid_session_id" }),
        );
