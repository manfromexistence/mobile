...
import {
  QueryBoolean,
  QueryBooleanOpenApi,
} from "../../src/server/routes/instance/httpapi/groups/query";
...
function assertAdvertisedQueryParamsAreRuntimeFields(input: {
  ...
  const advertisedOnly = queryParameters(input.operation).filter(
    (name) => !runtimeFields.has(name),
  );
  ...
  it.effect(
    ...
    Effect.sync(() => {
      ...
      for (const expected of numericSdkQueryParams) {
        expect(
          queryParameter(spec.paths[openApiPath(expected.path)]?.[expected.method], expected.name)
            ?.schema,
      ...
      for (const expected of pathParamPatterns) {
        expect(
          pathParameter(spec.paths[openApiPath(expected.path)]?.[expected.method], expected.name)
            ?.schema,
          `${expected.method.toUpperCase()} ${expected.path} ${expected.name}`,
        ).toEqual({ type: "string", pattern: expected.pattern });
      }
