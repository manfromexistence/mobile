    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() =>
          mkdir(path.join(test.directory, "packages", "opencode"), { recursive: true }),
        );
        yield* Effect.promise(() =>
          mkdir(path.join(test.directory, "packages", "app"), { recursive: true }),
        );
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const nativeIDs = (yield* SessionNs.Service.use((session) =>
          session.list({ directory: dir }),
        )).map((session) => session.id);
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const test = yield* TestInstance;
        yield* Effect.promise(() =>
          mkdir(path.join(test.directory, "packages", "opencode", "src", "deep"), {
            recursive: true,
          }),
        );
        yield* Effect.promise(() =>
    ...
    () =>
      Effect.gen(function* () {
        ...
        const sessions = yield* SessionNs.Service.use((session) =>
          session.list({ start: Date.now() + 86400000 }),
        );
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const listed = (yield* SessionNs.Service.use((session) =>
          session.list({ search: "meta-session" }),
