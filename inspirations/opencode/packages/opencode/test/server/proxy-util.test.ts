...
describe("ProxyUtil", () => {
  describe("websocketTargetURL", () => {
    ...
    test("converts https to wss", () => {
      expect(ProxyUtil.websocketTargetURL("https://example.com/path")).toBe(
        "wss://example.com/path",
      );
    });

    test("preserves query params", () => {
      expect(ProxyUtil.websocketTargetURL("http://example.com/path?foo=bar")).toBe(
        "ws://example.com/path?foo=bar",
      );
    });

    test("accepts URL objects", () => {
      expect(ProxyUtil.websocketTargetURL(new URL("http://localhost:3000/ws"))).toBe(
