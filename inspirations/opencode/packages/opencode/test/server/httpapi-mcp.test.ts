  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const addedDisconnected = yield* request(handler, "/mcp/added/disconnect", tmp.directory, {
          method: "POST",
        });
        expect(addedDisconnected.status).toBe(200);
        expect(yield* json(addedDisconnected)).toBe(true);

        const connected = yield* request(handler, "/mcp/demo/connect", tmp.directory, {
          method: "POST",
        });
        expect(connected.status).toBe(200);
        expect(yield* json(connected)).toBe(true);

        const disconnected = yield* request(handler, "/mcp/demo/disconnect", tmp.directory, {
          method: "POST",
        });
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const authenticate = yield* request(handler, "/mcp/demo/auth/authenticate", tmp.directory, {
          method: "POST",
        });
        expect(authenticate.status).toBe(400);

        const removed = yield* request(handler, "/mcp/demo/auth", tmp.directory, {
          method: "DELETE",
        });
