  ...
  return (scenario: Scenario) => {
    ...
    return runActive(options, scenario).pipe(
      Effect.timeoutOrElse({
        ...
        orElse: () =>
          Effect.die(
            new Error(`scenario timed out after ${Duration.format(options.scenarioTimeout)}`),
          ),
      }),
      Effect.as({ status: "pass", scenario } as Result),
      Effect.catchCause((cause) =>
        Effect.succeed({ status: "fail" as const, scenario, message: Cause.pretty(cause) }),
      ),
      Effect.scoped,
    );
  };
    ...
    Effect.gen(function* () {
      ...
      const dir = project
        ? yield* Effect.promise(async () =>
            (await runtime()).tmpdir(projectOptions(project, llm?.url)),
          )
        : undefined;
    ...
    Effect.flatMap((context) =>
      Effect.gen(function* () {
                ...
                modules.InstanceStore.Service.use((store) => store.load({ directory: path })).pipe(
                  ...
                  Effect.catchCause((cause) =>
                    Effect.sleep("100 millis").pipe(
                      Effect.andThen(
                        modules.InstanceStore.Service.use((store) =>
        ...
        const base: ScenarioContext = {
          ...
          session: (input) =>
            run(
              modules.Session.Service.use((svc) =>
                svc.create({ title: input?.title, parentID: input?.parentID }),
              ),
            ),
          ...
          messages: (sessionID) =>
            run(
              modules.Session.Service.use((svc) => svc.messages({ sessionID }).pipe(Effect.orDie)),
            ),
          todos: (sessionID, todos) =>
            run(modules.Todo.Service.use((svc) => svc.update({ sessionID, todos }))),
          worktree: (input) =>
            run(modules.Worktree.Service.use((svc) => svc.create(input).pipe(Effect.orDie))),
