    ...
    (info) =>
      Effect.gen(function* () {
        ...
        if (removed.status !== 200)
          return yield* Effect.fail(new Error(`failed to remove worktree: ${removed.status}`));
        const ok = yield* json<boolean>(removed);
        if (!ok)
          return yield* Effect.fail(new Error(`failed to remove worktree ${info.directory}`));
      }),
  );
}
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const [consoleState, consoleOrgs, toolList, toolIDs, worktrees, resources] =
          yield* Effect.all(
            [
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect((yield* json<Session.GlobalInfo[]>(next)).map((session) => session.id)).toContain(
          first.id,
        );
      }),
