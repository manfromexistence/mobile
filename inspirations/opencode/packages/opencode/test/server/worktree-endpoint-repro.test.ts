    ...
    return (directory: string) =>
      Effect.gen(function* () {
        ...
        Effect.timeoutOrElse({
          duration: "10 seconds",
          orElse: () =>
            Effect.fail(new Error(`timed out waiting for worktree.ready: ${directory}`)),
        }),
      );
  });
}
  ...
  return Effect.acquireRelease(
    Effect.gen(function* () {
      ...
      return {
        ...
        body,
        ready: waitReady(body.directory),
      } satisfies ScopedWorktree;
    }),
    (created) =>
...
function setProjectStartCommand(input: { server: TestServer; directory: string; command: string }) {
  return Effect.gen(function* () {
    const current = yield* request(
      input.server,
      `/project/current?directory=${encodeURIComponent(input.directory)}`,
    );
