...
const insertRemoteWorkspaceWithoutSync = (input: {
  ...
  Effect.gen(function* () {
    ...
    registerAdapter(
      input.projectID,
      input.type,
      remoteAdapter(path.join(input.dir, `.${input.type}`), input.url),
    );
...
const listenRemoteWebSocket = () =>
  listenAdditionalServer((request) => {
    const sync = syncResponse(request);
    if (sync) return sync;
    if (requestURL(request).pathname !== "/base/probe")
...
const ProbeApi = HttpApi.make("workspace-routing-probe").add(
  HttpApiGroup.make("probe")
    .add(
      ...
      HttpApiEndpoint.patch("patch", "/probe", {
...
describe("HttpApi workspace routing middleware", () => {
  it.live("proxies remote workspace HTTP requests through the selected workspace target", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClientRequest.patch(
        `/probe?workspace=${workspace.id}&keep=yes`,
      ).pipe(
        ...
        HttpClientRequest.bodyStream(
          Stream.make(
            new TextEncoder().encode('{"title":"Remote '),
            new TextEncoder().encode('workspace request"}'),
          ),
  ...
  it.live("waits for sync fence headers from remote workspace HTTP responses", () =>
    Effect.gen(function* () {
      ...
      const waited = yield* Ref.make<
        { workspaceID: WorkspaceV2.ID; state: Record<string, number> } | undefined
      >(undefined);
      ...
      const response = yield* HttpClientRequest.patch(`/probe?workspace=${workspaceID}`).pipe(
        HttpClient.execute,
      );
  ...
  it.live("proxies remote workspace WebSocket requests through the selected workspace target", () =>
    Effect.gen(function* () {
      ...
      yield* socket
        .runRaw((message) => Queue.offer(messages, String(message)))
  ...
  it.live("uses directory query/header fallback when no workspace is selected", () =>
    Effect.gen(function* () {
      ...
      const queryResponse = yield* HttpClient.get(
        `/probe?directory=${encodeURIComponent(queryDir)}`,
      );
