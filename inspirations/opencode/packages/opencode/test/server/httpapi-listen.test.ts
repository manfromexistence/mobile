...
async function openSocket(url: URL) {
  ...
  await withTimeout(
    new Promise<void>((resolve, reject) => {
      ...
      ws.addEventListener("error", () => reject(new Error("websocket failed before open")), {
        once: true,
      });
    }),
    5_000,
...
describe("HttpApi Server.listen", () => {
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      ...
      try {
        ...
        const nextWs = await openSocket(
          socketURL(restarted, nextInfo.id, tmp.path, nextTicket.ticket),
        );
  ...
  testPty("stop(true) is safe when called concurrently and repeatedly", async () => {
    ...
    try {
      ...
      await withTimeout(
        socket.closed,
        5_000,
        "timed out waiting for websocket close after concurrent stop",
      );
      await withTimeout(
        listener.stop(true),
        5_000,
        "timed out waiting for repeated listener.stop(true)",
      );
      stopped = true;
    } finally {
      if (!stopped)
        await stop(listener, "timed out cleaning up concurrent stop listener").catch(
          () => undefined,
        );
    }
  });
  ...
  testPty("stop(true) can force a graceful stop already in progress", async () => {
    ...
    try {
      ...
      await withTimeout(
        socket.closed,
        5_000,
        "timed out waiting for websocket close after forced stop",
      );
      stopped = true;
    } finally {
      if (!stopped)
        await stop(listener, "timed out cleaning up forced stop listener").catch(() => undefined);
    }
  });
  ...
  testPty("graceful stop waits for an overlapping forced stop", async () => {
    ...
    try {
      ...
      await withTimeout(
        listener.stop(),
        10_000,
        "timed out waiting for graceful stop after forced stop",
      );
      stopped = true;
      ...
      await withTimeout(
        socket.closed,
        5_000,
        "timed out waiting for websocket close before graceful stop resolved",
      );
    } finally {
      if (!stopped)
        await stop(listener, "timed out cleaning up overlapping stop listener").catch(
          () => undefined,
        );
    }
  });
  ...
  test("stop() gracefully closes an idle listener and is repeat-safe", async () => {
    ...
    await withTimeout(
      listener.stop(),
      5_000,
      "timed out waiting for repeated graceful listener.stop()",
    );
    await expect(
      fetch(new URL(PtyPaths.shells, listener.url), {
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    try {
      ...
      if (listener)
        await stop(listener, "timed out cleaning up plugin client listener").catch(() => undefined);
  ...
  testPty("rejects unsafe PTY ticket mint and connect requests", async () => {
    ...
    try {
      ...
      expect(
        (await requestTicket(listener, info.id, tmp.path, { ticketHeader: false })).status,
      ).toBe(403);
      expect(
        (await requestTicket(listener, info.id, tmp.path, { origin: "https://evil.example" }))
          .status,
      ).toBe(403);
      ...
      const ambiguous = await fetch(
        new URL(PtyPaths.connectToken.replace(":ptyID", info.id), listener.url),
        {
          method: "POST",
