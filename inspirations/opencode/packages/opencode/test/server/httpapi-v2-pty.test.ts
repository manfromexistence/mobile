...
const servedRoutes: Layer.Layer<never, EffectConfig.ConfigError, HttpServer.HttpServer> =
...
describe("v2 pty HttpApi", () => {
  testPty("serves location-wrapped PTY routes and retains exited sessions", async () => {
    ...
    expect(
      Schema.decodeUnknownSync(Location.response(Schema.Array(Pty.Info)))(await empty.json()).data,
    ).toEqual([]);
  ...
  testPty(
    ...
    async () => {
      ...
      try {
        const forbidden = await request(`/api/pty/${info.id}/connect-token`, tmp.path, {
          method: "POST",
        });
        ...
        const ticket = Schema.decodeUnknownSync(Location.response(PtyTicket.ConnectToken))(
          await token.json(),
        ).data.ticket;
  ...
  (process.platform === "win32" ? effectIt.live.skip : effectIt.live)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(created.status).toBe(200);
        const body = yield* Schema.decodeUnknownEffect(Location.response(Pty.Info))(
          yield* created.json,
        );
  ...
  (process.platform === "win32" ? effectIt.live.skip : effectIt.live)(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(created.status).toBe(200);
        const info = (yield* Schema.decodeUnknownEffect(Location.response(Pty.Info))(
          yield* created.json,
        )).data;
        ...
        yield* write(new Socket.CloseEvent(1000, "done")).pipe(Effect.catch(() => Effect.void));
        yield* HttpClientRequest.delete(`/api/pty/${info.id}`).pipe(
          directoryHeader(dir),
          HttpClient.execute,
        );
      }),
