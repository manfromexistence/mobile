...
const servedRoutes: Layer.Layer<never, Config.ConfigError, HttpServer.HttpServer> =
  HttpRouter.serve(HttpApiApp.routes, {
...
const createLocalWorkspace = (input: {
...
function json<T>(response: HttpClientResponse.HttpClientResponse) {
  if (response.status !== 200)
    return response.text.pipe(Effect.flatMap((text) => Effect.die(new Error(text))));
...
describe("session HttpApi", () => {
  it.effect("maps busy sessions to public session busy errors", () =>
    Effect.gen(function* () {
      ...
      const exit = yield* HttpSessionError.mapBusy(
        Effect.fail(new Session.BusyError({ sessionID })),
      ).pipe(Effect.exit);
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const get = yield* request(pathFor(SessionPaths.get, { sessionID: missingSession }), {
          headers,
        });
        expect(get.status).toBe(404);
        expect(yield* responseJson(get)).toEqual(missingSessionBody);

        const children = yield* request(
          pathFor(SessionPaths.children, { sessionID: missingSession }),
          { headers },
        );
        expect(children.status).toBe(404);
        expect(yield* responseJson(children)).toEqual(missingSessionBody);

        const todo = yield* request(pathFor(SessionPaths.todo, { sessionID: missingSession }), {
          headers,
        });
        ...
        const messages = yield* request(
          pathFor(SessionPaths.messages, { sessionID: missingSession }),
          { headers },
        );
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const listed = yield* requestJson<Session.Info[]>(`${SessionPaths.list}?roots=true`, {
          headers,
        });
        ...
        expect(
          yield* requestJson<Record<string, unknown>>(SessionPaths.status, { headers }),
        ).toEqual({});

        expect(
          yield* requestJson<Session.Info>(pathFor(SessionPaths.get, { sessionID: parent.id }), {
            headers,
          }),
        ...
        expect(
          yield* requestJson<unknown[]>(pathFor(SessionPaths.todo, { sessionID: parent.id }), {
            headers,
          }),
        ).toEqual([]);

        expect(
          yield* requestJson<unknown[]>(pathFor(SessionPaths.diff, { sessionID: parent.id }), {
            headers,
          }),
        ).toEqual([]);

        const messages = yield* request(
          `${pathFor(SessionPaths.messages, { sessionID: parent.id })}?limit=1`,
          {
            headers,
          },
        );
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const sessionCursor = (yield* json<{ data: Session.Info[]; cursor: { next?: string } }>(
          sessionPage,
        )).cursor.next;
        ...
        const messagePage = yield* request(`/api/session/${session.id}/message?limit=1`, {
          headers,
        });
        ...
        const nextMessagePage = yield* request(
          `/api/session/${session.id}/message?cursor=${messageCursor}`,
          {
            headers,
          },
        );
        expect(
          (yield* json<{ data: SessionMessage.Message[] }>(nextMessagePage)).data.map(
            (message) => message.id,
          ),
        ...
        const legacyMessagePage = yield* request(
          `/api/session/${session.id}/message?cursor=${legacyMessageCursor}`,
          {
            headers,
          },
        );
        expect(
          (yield* json<{ data: SessionMessage.Message[] }>(legacyMessagePage)).data.map(
            (message) => message.id,
          ),
        ).toEqual([firstMessage.id]);
        ...
        const invalidMessageCursor = yield* request(
          `/api/session/${session.id}/message?cursor=invalid`,
          { headers },
        );
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const messages = yield* requestJson<{ data: PromptBody[] }>(
        ...
        const message = yield* pollWithTimeout(
          requestJson<{ data: SessionMessage.Message[] }>(`/api/session/${session.id}/message`, {
            headers,
          }).pipe(Effect.map(({ data }) => data.find((message) => message.id === wakeID))),
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(response.status).toBe(200);
        expect((yield* json<Session.Info>(response)).summary?.diffs).toEqual([
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const updated = yield* requestJson<Session.Info>(
        ...
        const forked = yield* requestJson<Session.Info>(
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const created = yield* requestJson<Session.Info>(
          `${SessionPaths.create}?workspace=${workspace.id}`,
          {
            method: "POST",
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const listed = yield* requestJson<Session.Info[]>(`${SessionPaths.list}?${query}`, {
          headers,
        });
        expect(listed.map((item) => item.id)).toContain(created.id);

        const globalQuery = new URLSearchParams({ directory: hint });
        const global = yield* requestJson<Session.Info[]>(
          `${ExperimentalPaths.session}?${globalQuery}`,
          { headers },
        );
    ...
    () =>
      Effect.gen(function* () {
        ...
        for (const spelling of [forwardSlashes, lowercaseDrive, trailingSeparator]) {
          ...
          expect({ spelling, ids: listed.map((item) => item.id) }).toEqual({
            spelling,
            ids: [created.id],
          });
        }
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const headers = {
          "x-opencode-directory": globalWorktreeSentinel,
          "content-type": "application/json",
        };
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(
          yield* requestJson<Session.Info>(
            pathFor(SessionPaths.unrevert, { sessionID: session.id }),
            {
              method: "POST",
              headers,
            },
