...
describe("HttpApi workspace proxy", () => {
  ...
  it.live("proxies bodyless Web mutation requests as an empty body", () =>
    Effect.gen(function* () {
      ...
      const request = HttpServerRequest.fromWeb(
        new Request("http://localhost/session/abc/abort", { method: "POST" }),
      );
      const httpClient = yield* HttpClient.HttpClient;
      const response = yield* HttpApiProxy.http(
        httpClient,
        `${url}/session/abc/abort`,
        undefined,
        request,
      );

      expect(response.status).toBe(200);
      expect(yield* HttpServerResponse.toClientResponse(response).json).toEqual({
        method: "POST",
        body: "",
      });
    }),
  ...
  it.live("proxies websocket messages and protocols", () =>
    Effect.gen(function* () {
      ...
      const proxyUrl = yield* listenServer((request) =>
        HttpApiProxy.websocket(request, `${upstreamUrl}/echo`),
      );
