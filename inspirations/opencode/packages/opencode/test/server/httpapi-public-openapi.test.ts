...
function componentNames(response: OpenApiResponse | undefined) {
  ...
  return [
    ...new Set(
      [schema, ...(schema.anyOf ?? [])].flatMap((item) =>
        item.$ref ? [componentName(item.$ref)] : [],
      ),
    ),
  ];
}
...
describe("PublicApi OpenAPI v2 errors", () => {
  ...
  test("names the v2 event union without the SSE string wrapper collision", () => {
    ...
    expect(
      spec.paths["/api/event"]?.get?.responses?.["200"]?.content?.["text/event-stream"]?.schema,
    ).toEqual({
  ...
  test("documents references separately from filesystem routes", () => {
    ...
    for (const path of ["/api/fs/read/*", "/api/fs/list"]) {
      expect(spec.paths[path]?.get?.parameters, path).not.toContainEqual(
    ...
    const refs = v2Operations(spec)
      .flatMap((route) =>
        Object.entries(route.operation.responses ?? {}).flatMap(([status, response]) => {
          ...
          return ref
            ? [`${route.method.toUpperCase()} ${route.path} ${status} ${componentName(ref)}`]
            : [];
        }),
      )
    ...
    const builtInEndpointErrors = v2Operations(spec)
      .flatMap((route) =>
        Object.entries(route.operation.responses ?? {}).flatMap(([status, response]) => {
          ...
          return isBuiltInEndpointError(name)
            ? [`${route.method.toUpperCase()} ${route.path} ${status} ${name}`]
            : [];
        }),
      )
      .sort();
  ...
  test("documents v2 provider and model catalog errors", () => {
    ...
    expect(
      componentName(responseRef(spec.paths["/api/provider"]?.get?.responses?.["503"]) ?? ""),
    ).toBe("ServiceUnavailableError");
    expect(
      componentName(responseRef(spec.paths["/api/model"]?.get?.responses?.["503"]) ?? ""),
    ).toBe("ServiceUnavailableError");
    expect(
      componentName(
        responseRef(spec.paths["/api/provider/{providerID}"]?.get?.responses?.["404"]) ?? "",
      ),
    ).toBe("ProviderNotFoundError");
    expect(
      componentName(
        responseRef(spec.paths["/api/provider/{providerID}"]?.get?.responses?.["503"]) ?? "",
      ),
    ).toBe("ServiceUnavailableError");
  });
  ...
  test("documents v2 session not-found errors", () => {
    ...
    for (const route of [
      ...
      expect(componentNames(spec.paths[route[1]]?.[route[0]]?.responses?.["404"])).toContain(
  ...
  test("documents v2 unfinished session mutation errors", () => {
    ...
    for (const route of [
      ...
      expect(
        componentName(responseRef(spec.paths[route[1]]?.[route[0]]?.responses?.["503"]) ?? ""),
      ).toBe("ServiceUnavailableError");
    }
  });
  ...
  test("documents v2 session read data errors", () => {
    ...
    for (const route of [
      ...
      expect(
        componentName(responseRef(spec.paths[route[1]]?.[route[0]]?.responses?.["500"]) ?? ""),
      ).toMatch(/^UnknownError\d*$/);
    }
  });
  ...
  test("documents session busy errors", () => {
    ...
    for (const route of [
      ...
      expect(
        componentName(responseRef(spec.paths[route[1]]?.[route[0]]?.responses?.["409"]) ?? ""),
      ).toBe("SessionBusyError");
    }
  });
  ...
  test("documents permission and question not-found errors", () => {
    ...
    expect(
      componentName(
        responseRef(spec.paths["/permission/{requestID}/reply"]?.post?.responses?.["404"]) ?? "",
      ),
    ...
    for (const route of [
      ...
      expect(
        componentName(responseRef(spec.paths[route[1]]?.[route[0]]?.responses?.["404"]) ?? ""),
      ).toBe("QuestionNotFoundError");
    }
    for (const route of [
  ...
  test("documents PTY resource and ticket errors", () => {
    ...
    for (const route of [
      ...
      expect(
        componentName(responseRef(spec.paths[route[1]]?.[route[0]]?.responses?.["404"]) ?? ""),
      ).toBe("PtyNotFoundError");
    }
    expect(
      componentName(
        responseRef(spec.paths["/pty/{ptyID}/connect-token"]?.post?.responses?.["403"]) ?? "",
      ),
    ).toBe("PtyForbiddenError");
    expect(
  ...
  test("documents project not-found errors", () => {
    ...
    expect(
      componentName(
        responseRef(spec.paths["/project/{projectID}"]?.patch?.responses?.["404"]) ?? "",
      ),
    ).toBe("ProjectNotFoundError");
  });
});
