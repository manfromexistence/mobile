...
} from "../../src/server/routes/instance/httpapi/lifecycle";
import {
...
const it = testEffect(
  Layer.mergeAll(testStateLayer, NodeHttpServer.layerTest, NodeServices.layer, workspaceLayer),
);
...
describe("HttpApi instance context middleware", () => {
  ...
  it.live(
    "falls through to local instead of MissingWorkspace when configured workspace id is set",
    () =>
      Effect.gen(function* () {
        const fixedWorkspaceID = WorkspaceV2.ID.ascending();
        yield* withFixedWorkspaceID(fixedWorkspaceID);
        ...
        const response = yield* HttpClientRequest.get(
          `/probe?workspace=${unknownWorkspaceID}`,
        ).pipe(HttpClientRequest.setHeader("x-opencode-directory", dir), HttpClient.execute);
  ...
  it.live("preserves selected workspace id on instance disposal events", () =>
    Effect.gen(function* () {
      ...
      const response = yield* HttpClientRequest.post(
        `/dispose-probe?workspace=${workspace.id}`,
