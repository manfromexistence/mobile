...
function requestServer(path: string, directory: string, init: RequestInit = {}) {
  ...
  return Effect.promise(() =>
    Promise.resolve(Server.Default().app.request(path, { ...init, headers })),
  );
}
...
describe("workspace HttpApi", () => {
  ...
  it.live("serves mutation endpoints", () =>
    Effect.gen(function* () {
      ...
      const removed = yield* request(WorkspacePaths.remove.replace(":id", workspace.id), dir, {
        method: "DELETE",
      });
    ...
    Effect.gen(function* () {
      ...
      try {
        ...
        const event = Array.from(
          yield* eventResponse.stream.pipe(Stream.take(1), Stream.runCollect),
        )[0];
        ...
        yield* requestDefault(WorkspacePaths.remove.replace(":id", workspace.id), dir, {
          method: "DELETE",
        });
      }
    }),
  ...
  it.live("proxies remote workspace requests selected from session ownership", () =>
    Effect.gen(function* () {
      ...
      try {
        const response = yield* requestDefault(
          `http://localhost/session/${session.id}/message`,
          dir,
          {
            method: "POST",
            headers: { "content-type": "application/json" },
        ...
        expect(JSON.parse(responseBody)).toEqual({
          proxied: true,
          path: `/base/session/${session.id}/message`,
        });
        expect(
          proxied.filter(
            (item) => new URL(item.url).pathname === `/base/session/${session.id}/message`,
          ),
        ).toEqual([
        ...
        const aborted = yield* request(`http://localhost/session/${session.id}/abort`, dir, {
          method: "POST",
        });
        expect(aborted.status).toBe(200);
        expect(
          proxied.filter(
            (item) => new URL(item.url).pathname === `/base/session/${session.id}/abort`,
          ),
        ).toEqual([
          expect.objectContaining({
