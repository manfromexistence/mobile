...
const testInstanceStore = AppNodeBuilder.build(InstanceStore.node, [
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const createdParent = path.join(
          test.directory,
          "..",
          path.basename(test.directory) + "-http-copy",
        );
        const createdDirectory = path.join(createdParent, "copy");
        yield* Effect.addFinalizer(() =>
          Effect.promise(() => fs.rm(createdParent, { recursive: true, force: true })).pipe(
            Effect.ignore,
          ),
        );
        ...
        const externalDirectory = path.join(
          test.directory,
          "..",
          path.basename(test.directory) + "-http-refresh",
        );
        yield* Effect.addFinalizer(() =>
          Effect.promise(() => fs.rm(externalDirectory, { recursive: true, force: true })).pipe(
            Effect.ignore,
          ),
        );
        yield* Effect.promise(() =>
          $`git worktree add --detach ${externalDirectory} HEAD`.cwd(test.directory).quiet(),
        );
