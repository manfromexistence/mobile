  ...
  test("maps AI SDK stream chunks without losing session-visible fields", async () => {
    ...
    const events = await adapt([
      ...
      { type: "reasoning-start", id: "reasoning-1", providerMetadata: metadata },
      {
        ...
        id: "reasoning-1",
        text: "Think",
        providerMetadata: { openai: { delta: 3 } },
      },
    ...
    expect(events).toMatchObject([
      ...
      { type: "tool-input-delta", id: "call-1", name: "lookup", text: '"weather"}' },
      {
        ...
        id: "call-1",
        name: "lookup",
        providerMetadata: { openai: { inputDone: true } },
      },
      {
        type: "tool-call",
        id: "call-1",
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const maxTokens =
          (body.max_tokens as number | undefined) ?? (body.max_output_tokens as number | undefined);
    ...
    () =>
      Effect.gen(function* () {
        ...
        const user = {
          ...
          agent: agent.name,
          model: {
            providerID: ProviderV2.ID.make(cerebrasFixture.providerID),
            modelID: resolved.id,
          },
        } satisfies SessionV1.User;
    ...
    () =>
      Effect.gen(function* () {
        ...
        const user = {
          ...
          agent: agent.name,
          model: {
            providerID: ProviderV2.ID.make(alibabaQwenFixture.providerID),
            modelID: resolved.id,
          },
        } satisfies SessionV1.User;
        ...
        yield* Effect.promise(() =>
          Promise.race([pending.requestAborted, timeout(500)]).catch(() => undefined),
        );
      }),
    {
      config: () => ({
      ...
      Effect.gen(function* () {
        ...
        const failingNativeClient = Layer.succeed(
          ...
          LLMClient.Service.of({
            prepare: () =>
              Effect.die(new Error("native LLM client should not be used when the flag is off")),
            stream: () =>
              Stream.die(new Error("native LLM client should not be used when the flag is off")),
            generate: () =>
              Effect.die(new Error("native LLM client should not be used when the flag is off")),
          }),
        );
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* drainWith(
          llmLayerWithExecutor({ executor, flags: { experimentalNativeLlm: true } }),
          {
            user: {
    ...
    {
      config: () =>
        openAIConfig(loadFixture("openai", "gpt-5.2").model, "https://injected-openai.test/v1"),
    },
  );
      ...
      Effect.gen(function* () {
        ...
        const user = {
          ...
          model: {
            ...
            modelID: ModelV2.ID.make("MiniMax-M2.5"),
          },
        } satisfies SessionV1.User;
    ...
    () =>
      Effect.gen(function* () {
        ...
        const modelMessages = yield* Effect.promise(() =>
          MessageV2.toModelMessages(input as any, resolved),
        );
        yield* drain({
          user,
          sessionID,
        ...
        expect(
          messages.some((message) => message.content.some((part) => "cache_control" in part)),
        ).toBe(true);
        ...
        expect(
          messages[toolUseIndex].content.filter((part) => part.type === "tool_use"),
        ).toMatchObject([
          {
            type: "tool_use",
