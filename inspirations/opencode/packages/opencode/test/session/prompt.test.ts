...
const writeConfig = Effect.fn("test.writeConfig")(function* (
  dir: string,
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* awaitWithTimeout(
        llm.wait(1),
        "timed out waiting for MCP instruction request",
        "10 seconds",
      );
...
noLLMServer.instance.skip(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const messages = yield* SessionV2.Service.use((session) =>
        session.messages({ sessionID: chat.id }),
      ).pipe(
        Effect.provide(
      ...
      expect(messages).toEqual(
        expect.arrayContaining([
          expect.objectContaining({
            type: "synthetic",
            text: expect.stringContaining("Called the Read tool"),
          }),
...
it.instance("static loop consumes queued replies across turns", () =>
  Effect.gen(function* () {
    ...
    expect(first.parts.some((part) => part.type === "text" && part.text === "world one")).toBe(
      true,
    );
    ...
    expect(second.parts.some((part) => part.type === "text" && part.text === "world two")).toBe(
      true,
    );
...
it.instance("loop continues when finish is tool-calls", () =>
  Effect.gen(function* () {
    ...
    expect(result.info.role).toBe("assistant");
    if (result.info.role === "assistant") {
      expect(result.parts.some((part) => part.type === "text" && part.text === "second")).toBe(
        true,
      );
...
it.instance("subtask child inherits parent session external_directory allow", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("external_directory", "/tmp/allowed/file", rules).action).toBe(
      "allow",
    );
      ...
      const tool = yield* pollWithTimeout(
        Effect.gen(function* () {
          ...
          const assistant = msgs.findLast(
            (item) => item.info.role === "assistant" && item.info.agent === "build",
          );
...
noLLMServer.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      expect(Exit.isSuccess(exit)).toBe(true);
      yield* awaitWithTimeout(
        Deferred.await(aborted),
        "timed out waiting for task tool abort",
        "10 seconds",
      );
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const sessionID =
        tool?.state.status === "running" ? tool.state.metadata?.sessionId : undefined;
      ...
      Effect.gen(function* () {
        ...
        yield* pollWithTimeout(
          Effect.gen(function* () {
            ...
            const tool = taskMsg ? toolPart(taskMsg.parts) : undefined;
            if (tool?.state.status === "running" && tool.state.metadata?.output.includes("first"))
              return true;
          }),
  ...
  () =>
    Effect.gen(function* () {
      ...
      if (Exit.isSuccess(exit)) {
        expect(exit.value.info.role).toBe("assistant");
        expect(
          exit.value.parts.some((part) => part.type === "text" && part.text === "after-shell"),
        ).toBe(true);
      }
    ...
    withSh(() =>
      Effect.gen(function* () {
        ...
        yield* pollWithTimeout(
          afs
            .existsSafe(ready)
            .pipe(Effect.map((exists) => (exists ? (true as const) : undefined))),
      ...
      yield* pollWithTimeout(
        Effect.gen(function* () {
          ...
          if (
            ...
            tool.state.metadata?.output.includes("truncation-ready")
          )
            return true;
        }),
...
noLLMServer.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      yield* awaitWithTimeout(
        Deferred.await(ready),
        "timed out waiting for read tool to start",
        "10 seconds",
      );
      yield* prompt.cancel(chat.id);
      yield* Fiber.interrupt(fiber);
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    expect(result.info.role).toBe("assistant");
    expect(
      result.parts.some((part) => part.type === "text" && part.text.includes("processor.ts")),
    ).toBe(true);
