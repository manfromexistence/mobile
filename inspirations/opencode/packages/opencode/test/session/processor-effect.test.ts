      ...
      Effect.gen(function* () {
        ...
        const off = yield* events.listen((evt) => {
          ...
          if (data.sessionID === chat.id && data.status.type === "retry")
            states.push(data.status.attempt);
          return Effect.void;
        });
        const handle = yield* processors.create({
          assistantMessage: msg,
    ...
    ({ dir, llm }) =>
      Effect.gen(function* () {
        ...
        yield* waitFor(
          MessageV2.parts(msg.id).pipe(
            Effect.map((parts) =>
              parts.find((part): part is SessionV1.ToolPart => part.type === "tool"),
            ),
            Effect.provideService(Database.Service, database),
          ),
...
itFragmentFailure.live(
  "session.processor effect tests retain partial legacy parts without v2 events",
  () =>
    provideTmpdirInstance(
      (dir) =>
        Effect.gen(function* () {
