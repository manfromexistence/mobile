...
const zenURL = (connection: string) =>
  `https://console.opencode.ai/proxy/connections/${connection}/v1`;
...
const RECORDED_SCENARIOS = [
  {
    ...
    config: (model) =>
      providerConfig({
        ...
        options: {
          apiKey:
            envValue("OPENCODE_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
...
const redactRecordedBody = (body: string) =>
  body
    ...
    .replace(
      /"(access|access_token|refresh|refresh_token|accountId|account_id)"\s*:\s*"[^"]+"/g,
      '"$1":"redacted"',
    );
...
function authLayer(scenario: RecordedScenario) {
  ...
  return Layer.mock(Auth.Service)({
    get: (providerID) =>
      Effect.succeed(providerID === scenario.providerID ? replayAuth : undefined),
...
function recordedNativeLLMLayer(scenario: RecordedScenario) {
  ...
  const redact = {
    url: (url: string) =>
      url.replace(/\/proxy\/connections\/[^/]+\/v1/, "/proxy/connections/{connection}/v1"),
...
describe("session.llm native recorded", () => {
  for (const scenario of RECORDED_SCENARIOS.filter(isSelected)) {
    ...
    it.instance(`${scenario.name}: drives a tool loop to a final text answer`, () =>
      driveToolLoop(scenario),
    );
  }
});
