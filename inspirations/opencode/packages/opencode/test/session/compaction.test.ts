...
function createSummaryAssistantMessage(
  sessionID: SessionID,
  parentID: MessageID,
  root: string,
  text: string,
...
function cfg(compaction?: ConfigV1.Info["compaction"]) {
  ...
  return Layer.succeed(
    Config.Service,
    TestConfig.make({ get: () => Effect.succeed({ ...base, compaction }) }),
  );
}
...
describe("session.compaction.process", () => {
  ...
  it.instance(
    ...
    Effect.gen(function* () {
      ...
      const unsub = yield* events.listen((evt) => {
        ...
        if (
          (evt.data as typeof SessionCompaction.Event.Compacted.data.Type).sessionID !== session.id
        )
          return Effect.void;
  ...
  itCompaction.instance(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        expect(filtered.map((msg) => msg.info.id).slice(0, 3)).toEqual([
          parent!,
          expect.any(String),
          keep.id,
        ]);
        expect(filtered[1]?.info.role).toBe("assistant");
        expect(filtered[1]?.info.role === "assistant" ? filtered[1].info.summary : false).toBe(
          true,
        );
          ...
          (msg) =>
            ...
            msg.parts.some(
              (part) =>
                part.type === "text" &&
                part.synthetic &&
                part.text.includes("Continue if you have next steps"),
            ),
        ),
      ).toBe(false);
  ...
  it.instance(
    ...
    Effect.gen(function* () {
      ...
      expect(
        last?.parts.some(
          (part) => part.type === "text" && part.text.includes("Attached image/png: cat.png"),
        ),
      ).toBe(true);
    }),
  );
  ...
  itCompaction.instance(
    ...
    () => {
      ...
      return Effect.gen(function* () {
        ...
        expect(summary?.parts.some((part) => part.type === "text" && part.text === "summary")).toBe(
          true,
        );
      }).pipe(withCompaction({ llm: stub.llmLayer }));
    },
