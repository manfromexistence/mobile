...
describe("session.created event", () => {
      ...
      const unsub = yield* events.listen((event) => {
        if (event.type === SessionNs.Event.Created.type)
          Deferred.doneUnsafe(
            received,
            Effect.succeed(
              (event.data as typeof SessionNs.Event.Created.data.Type).info as SessionNs.Info,
            ),
          );
        return Effect.void;
      });
  ...
  it.instance("session.created event should be emitted before session.updated", () =>
    Effect.gen(function* () {
      ...
      const receivedEvents = yield* awaitDeferred(
        received,
        "timed out waiting for session created/updated events",
      );
  ...
  it.instance("emits legacy global sync payload", () =>
    Effect.gen(function* () {
      ...
      const listener = (event: {
        payload: { type?: string; syncEvent?: EventV2.SerializedEvent };
      }) => {
      ...
      const info = yield* session.create({});
      const event = yield* awaitDeferred(
        received,
        "timed out waiting for legacy global sync event",
      );
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const unsub = yield* events.listen((event) => {
          if (event.type === MessageV2.Event.PartUpdated.type)
            Deferred.doneUnsafe(
              received,
              Effect.succeed(
                (event.data as typeof MessageV2.Event.PartUpdated.data.Type).part as SessionV1.Part,
              ),
            );
          return Effect.void;
        });
        ...
        const receivedPart = yield* awaitDeferred(
          received,
          "timed out waiting for message.part.updated",
        );
...
describe("Session", () => {
  it.live("remove works without an instance", () =>
    Effect.gen(function* () {
      ...
      const info = yield* provideInstance(dir)(
        session.create({ title: "remove-without-instance" }),
      );
  ...
  it.instance("persists metadata and copies it on fork by default", () =>
    Effect.gen(function* () {
      ...
      const created = yield* Effect.acquireRelease(
        session.create({ title: "with-meta", metadata: meta }),
  ...
  it.instance("omits metadata when not provided", () =>
    Effect.gen(function* () {
      const session = yield* SessionNs.Service;
      const created = yield* Effect.acquireRelease(
        session.create({ title: "empty-meta" }),
        (info) => session.remove(info.id).pipe(Effect.ignore),
      );
      const saved = yield* session.get(created.id);
