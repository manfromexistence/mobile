...
describe("plugin.openai.ws", () => {
  test("derives websocket URLs and sends auth plus protocol headers", async () => {
    ...
    expect(OpenAIWebSocket.toWebSocketUrl("http://example.com/v1/responses")).toBe(
      "ws://example.com/v1/responses",
    );
    expect(OpenAIWebSocket.toWebSocketUrl("https://example.com/v1/responses")).toBe(
  ...
  test("enforces websocket send idle timeout", async () => {
    ...
    expect((await readTextError(response.text())).message).toContain(
      "idle timeout sending websocket request",
    );
...
describe("plugin.openai.ws-pool", () => {
  test("reuses one healthy websocket for sequential requests", async () => {
    ...
    await using server = await createWebSocketServer((socket) => {
      ...
      socket.on("message", () => {
        messages += 1;
        socket.send(
          JSON.stringify({ type: "response.completed", response: { id: `resp_${messages}` } }),
        );
      });
    });
  ...
  test("rotates a socket that exceeds max connection age", async () => {
    ...
    await using server = await createWebSocketServer((socket) => {
      ...
      socket.on("message", () => {
        socket.send(
          JSON.stringify({ type: "response.completed", response: { id: `resp_${connections}` } }),
        );
      });
    });
    const fetch = OpenAIWebSocketPool.createWebSocketFetch({
  ...
  test("terminates active websocket connections when their session is deleted", async () => {
    ...
    await using server = await createWebSocketServer((socket) => {
      ...
      socket.once("message", () => {
        ...
        socket.send(
          JSON.stringify({ type: "response.completed", response: { id: "resp_after_remove" } }),
        );
      });
    });
    const fetch = OpenAIWebSocketPool.createWebSocketFetch({
    ...
    fetch.remove("session-1");
    expect((await readTextError(firstText)).message).toContain(
  ...
  test("invalidates but does not reuse a socket after terminal failure frames", async () => {
    ...
    await using server = await createWebSocketServer((socket) => {
      ...
      socket.once("message", () => {
        socket.send(
          JSON.stringify({ type: connections === 1 ? "response.failed" : "response.completed" }),
        );
      });
    });
    const fetch = OpenAIWebSocketPool.createWebSocketFetch({
  ...
  test("resets websocket stream failures after a completed response", async () => {
    ...
    await using server = await createWebSocketServer((socket) => {
      ...
      socket.on("message", () => {
        ...
        socket.send(
          JSON.stringify({ type: "response.completed", response: { id: `resp_${requests}` } }),
        );
      });
    });
    const fetch = OpenAIWebSocketPool.createWebSocketFetch({
    ...
    expect(await second.text()).toContain("data: [DONE]");
    const third = await fetch(server.url, streamRequest());
    expect((await readTextError(third.text())).message).toContain(
      "WebSocket closed before response.completed",
    );
    const fourth = await fetch(server.url, streamRequest());
  ...
  test("releases the websocket lane when the response body is cancelled", async () => {
    ...
    await using server = await createWebSocketServer((socket) => {
      ...
      socket.once("message", () => {
        ...
        socket.send(
          JSON.stringify({ type: "response.completed", response: { id: "resp_after_cancel" } }),
        );
      });
    });
    const fetch = OpenAIWebSocketPool.createWebSocketFetch({
...
function websocketServerHandle(
  ...
  http: Awaited<ReturnType<typeof createHttpServer>>,
) {
  return {
    url: http.url,
