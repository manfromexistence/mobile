...
describe("plugin.meta", () => {
  test("tracks file plugin loads and changes", async () => {
    ...
    expect(Object.values(all).some((item) => item.spec === spec && item.source === "file")).toBe(
      true,
    );
  ...
  test("tracks npm plugin versions", async () => {
    ...
    await Bun.write(
      tmp.extra.pkg,
      JSON.stringify({ name: "acme-plugin", version: "1.1.0" }, null, 2),
    );
    ...
    const all = await PluginMeta.list();
    expect(
      Object.values(all).some((item) => item.id === "acme-plugin" && item.version === "1.1.0"),
    ).toBe(true);
    const saved = await map<{ id: string; version?: string }>(file);
    expect(
      Object.values(saved).some((item) => item.id === "acme-plugin" && item.version === "1.1.0"),
    ).toBe(true);
  });
