...
function load(dir: string, flags?: Parameters<typeof RuntimeFlags.layer>[0]) {
  ...
  return Effect.gen(function* () {
    const config = yield* Effect.promise(
      () =>
        Bun.file(source).json() as Promise<{
  ...
  it.live("rejects npm server export that resolves outside plugin directory", () =>
    withTmp(
      async (dir) => {
        ...
        await fs.symlink(
          outside,
          path.join(mod, "escape"),
          process.platform === "win32" ? "junction" : "dir",
        );

        await Bun.write(
          path.join(dir, "opencode.json"),
          JSON.stringify({ plugin: ["acme-plugin"] }, null, 2),
        );
  ...
  it.live("continues loading plugins when plugin init throws", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(
          path.join(dir, "opencode.json"),
          JSON.stringify({ plugin: [file, ok] }, null, 2),
        );
  ...
  it.live("continues loading plugins when plugin import fails", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(
          path.join(dir, "opencode.json"),
          JSON.stringify({ plugin: [missing, ok] }, null, 2),
        );
    ...
    withTmp(
      async (dir) => {
        ...
        await Bun.write(
          path.join(dir, "opencode.json"),
          JSON.stringify(
            { plugin: [[pathToFileURL(file).href, { source: "tuple", enabled: true }]] },
            null,
            2,
          ),
        );
...
`,
      ...
      (tmp) =>
        Effect.gen(function* () {
          yield* load(tmp.path);
          const lines = (yield* Effect.promise(() => fs.readFile(tmp.extra.marker, "utf8")))
            .trim()
            .split("\n");
    ...
    withTmp(
      async (dir) => {
        ...
        await Bun.write(
          ...
          'export default { id: "demo", tui: async () => {} }\n',
        );
        await Bun.write(path.join(mod, "themes", "night.json"), "{}\n");
        return { mod };
      },
  ...
  it.live("rejects oc-themes path traversal", () =>
    withTmp(
      async (dir) => {
        ...
        await Bun.write(
          file,
          JSON.stringify({ name: "acme", "oc-themes": ["../escape.json"] }, null, 2),
        );
        return { mod, file };
      },
      (tmp) =>
