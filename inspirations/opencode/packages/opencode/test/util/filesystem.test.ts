...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      const result = await Filesystem.findUp(
  ...
  describe("mimeType()", () => {
    ...
    test("returns MIME type for TypeScript (or video/mp2t due to extension conflict)", async () => {
      ...
      expect(
        mime === "video/mp2t" || mime === "application/typescript" || mime === "text/typescript",
      ).toBe(true);
    });
  ...
  describe("resolve()", () => {
    ...
    test("resolves Cygwin paths on Windows", async () => {
      ...
      const drive = tmp.path[0].toLowerCase();
      const rest = tmp.path.slice(2).replaceAll("\\", "/");
      expect(Filesystem.resolve(`/cygdrive/${drive}${rest}`)).toBe(
        Filesystem.normalizePath(tmp.path),
      );
    });
    ...
    test("resolves Cygwin drive roots on Windows", async () => {
      ...
      await using tmp = await tmpdir();
      const drive = tmp.path[0].toLowerCase();
      expect(Filesystem.resolve(`/cygdrive/${drive}`)).toBe(
        Filesystem.resolve(`${drive.toUpperCase()}:/`),
      );
    });
    ...
    test("resolves WSL mount roots on Windows", async () => {
      ...
      await using tmp = await tmpdir();
      const drive = tmp.path[0].toLowerCase();
      expect(Filesystem.resolve(`/mnt/${drive}`)).toBe(
        Filesystem.resolve(`${drive.toUpperCase()}:/`),
      );
    });
