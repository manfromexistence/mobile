...
const startWorkspaceSyncingWithFlag = (projectID: ProjectV2.ID, experimentalWorkspaces: boolean) =>
  Effect.runPromise(
    Workspace.use
      ...
      .pipe(Effect.provide(workspaceLayer(experimentalWorkspaces))),
  );
...
function localAdapter(
  dir: string,
  input?: { createDir?: boolean; remove?: (info: WorkspaceInfo) => Promise<void> },
) {
...
describe("workspace CRUD", () => {
    ...
    () =>
      Effect.gen(function* () {
        ...
        const other = workspaceInfo(otherProjectID, "manual", {
          id: WorkspaceV2.ID.ascending("wrk_c_list"),
        });
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expect(recorded.calls.configure[0]).toMatchObject({
          id: workspaceID,
          type,
          directory: null,
        });
        ...
        expect(
          (yield* workspace.status()).find((item) => item.workspaceID === workspaceID)?.status,
        ).toBe("connected");
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        expectExitContains(
          yield* Effect.exit(
            workspace.create({ type, branch: null, projectID: instance.project.id, extra: null }),
          ),
          "configure exploded",
        );
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        ...
        const synced = (yield* workspace.list(instance.project)).filter(
          (item) => item.name === discovered.name,
        );
        ...
        expect(yield* workspace.list(instance.project)).toEqual(
          expect.arrayContaining([existing, synced[0]]),
        );
      ...
      yield* provideTmpdirInstance(
        (dir) =>
          Effect.gen(function* () {
            ...
            expect(
              calls.map(
                (call) => `${call.method} ${call.url.pathname}${call.url.search}${call.url.hash}`,
              ),
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const workspace = yield* Workspace.Service;
        expect(
          yield* workspace.remove(WorkspaceV2.ID.ascending("wrk_missing_remove")),
        ).toBeUndefined();
      }),
    { git: true },
  );
    ...
    () =>
      Effect.gen(function* () {
        ...
        const info = workspaceInfo(instance.project.id, type, {
          id: WorkspaceV2.ID.ascending("wrk_remove_throws"),
        });
        registerAdapter(
          instance.project.id,
          type,
    ...
    () => {
      return Effect.gen(function* () {
        ...
        registerAdapter(
          instance.project.id,
          previousType,
          localAdapter(path.join(dir, "warp-prev-local")).adapter,
        );
        registerAdapter(
          instance.project.id,
          targetType,
          localAdapter(path.join(dir, "warp-target-local")).adapter,
        );
    ...
    () => {
      return Effect.gen(function* () {
        ...
        yield* Effect.promise(() =>
          fs.writeFile(path.join(previousDir, "tracked.txt"), "changed\n"),
        );
        ...
        registerAdapter(
          instance.project.id,
          previousType,
          localAdapter(previousDir, { createDir: false }).adapter,
        );
        registerAdapter(
          instance.project.id,
          targetType,
          localAdapter(targetDir, { createDir: false }).adapter,
        );
        const session = yield* sessionSvc.create({});
        yield* attachSessionToWorkspace(session.id, previous.id);

        yield* workspace.sessionWarp({
        ...
        expect(
          yield* Effect.promise(() => fs.readFile(path.join(targetDir, "tracked.txt"), "utf8")),
        ).toBe("changed\n");
        expect(
          yield* Effect.promise(() => fs.readFile(path.join(targetDir, "new.txt"), "utf8")),
        ).toBe("new\n");
      });
    },
    { git: true },
    ...
    () => {
      return Effect.gen(function* () {
        ...
        registerAdapter(
          instance.project.id,
          previousType,
          localAdapter(path.join(dir, "warp-detach-local")).adapter,
        );
        const session = yield* sessionSvc.create({});
        yield* attachSessionToWorkspace(session.id, previous.id);
  ...
  it.live(
    "sessionWarp syncs previous remote history, replays it, steals, and claims the sequence",
    () => {
      const calls: FetchCall[] = [];
      ...
      return Effect.gen(function* () {
        yield* HttpServer.serveEffect()(
          Effect.gen(function* () {
            ...
            if (call.url.pathname === "/warp-source/vcs/diff/raw")
              return HttpServerResponse.text("remote patch");
            ...
            if (call.url.pathname === "/warp-target/sync/steal")
              return yield* HttpServerResponse.json({ sessionID: "ok" });
            if (call.url.pathname === "/warp-target/vcs/apply")
          ...
          () =>
            Effect.gen(function* () {
              ...
              registerAdapter(
                instance.project.id,
                previousType,
                remoteAdapter(`${url}/warp-source`).adapter,
              );
              registerAdapter(
                instance.project.id,
                targetType,
                remoteAdapter(`${url}/warp-target`).adapter,
              );
              const session = yield* sessionSvc.create({});
              yield* attachSessionToWorkspace(session.id, previous.id);
...
describe("workspace sync state", () => {
    ...
    () =>
      Effect.gen(function* () {
        ...
        registerAdapter(
          instance.project.id,
          type,
          localAdapter(path.join(dir, "flag-disabled")).adapter,
        );
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.addFinalizer(() =>
          Effect.all([workspace.remove(first.id), workspace.remove(second.id)], {
            discard: true,
          }).pipe(Effect.ignore),
        );
      ...
      Effect.gen(function* () {
        ...
        expect(
          captured.events.filter(
            (event) =>
              event.workspace === info.id && event.payload.type === Workspace.Event.Status.type,
          ),
        ).toHaveLength(1);
        yield* workspace.remove(info.id);
      }),
  ...
  it.live(
    "remote start emits disconnected, connecting, and connected then refuses duplicate listeners",
    () => {
      const calls: FetchCall[] = [];
      return Effect.gen(function* () {
        yield* HttpServer.serveEffect()(
            ...
            Effect.gen(function* () {
              ...
              try {
                ...
                yield* eventuallyEffect(
                  Effect.gen(function* () {
                    expect(
                      (yield* workspace.status()).find((item) => item.workspaceID === info.id)
                        ?.status,
                    ).toBe("connected");
                  }),
                );
                ...
                expect(
                  calls.filter((call) => call.url.pathname === "/sync/global/event"),
                ).toHaveLength(1);
                expect(
                  calls.filter((call) => call.url.pathname === "/sync/sync/history"),
                ).toHaveLength(1);
                expect(yield* workspace.isSyncing(info.id)).toBe(true);
        ...
        () =>
          Effect.gen(function* () {
            ...
            registerAdapter(
              instance.project.id,
              type,
              remoteAdapter(`${url}/history-failed`).adapter,
            );
            yield* attachSessionToWorkspace((yield* sessionSvc.create({})).id, info.id);
  ...
  it.live(
    "sync history sends the local sequence fence and replays returned events in workspace context",
    () => {
          ...
          () =>
            Effect.gen(function* () {
              ...
              try {
                ...
                yield* eventuallyEffect(
                  Effect.gen(function* () {
                    expect((yield* sessionSvc.get(session.id).pipe(Effect.orDie)).title).toBe(
                      "from history",
                    );
                  }),
                );
          ...
          Effect.gen(function* () {
            ...
            try {
              ...
              registerAdapter(
                instance.project.id,
                type,
                remoteAdapter(`${url}/sse-forward`).adapter,
              );
              yield* attachSessionToWorkspace((yield* sessionSvc.create({})).id, info.id);
              ...
              yield* eventuallyEffect(
                Effect.sync(() =>
                  expect(
                    captured.events.some(
                      (event) =>
                        event.workspace === info.id && event.payload.type === "custom.remote",
                    ),
                  ).toBe(true),
                ),
              );
              expect(
                captured.events.some(
                  (event) =>
                    event.workspace === info.id && event.payload.type === "server.heartbeat",
                ),
              ).toBe(false);
              expect(
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const workspace = yield* Workspace.Service;
        expect(
          yield* workspace.waitForSync(WorkspaceV2.ID.ascending("wrk_wait_empty"), {}),
        ).toBeUndefined();
      }),
    { git: true },
  );
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* db
          ...
          .values({ aggregate_id: sessionID, seq: 4 })
          .run()
          .pipe(Effect.orDie);

        expect(
          yield* workspace.waitForSync(WorkspaceV2.ID.ascending("wrk_wait_done"), {
            [sessionID]: 4,
          }),
        ).toBeUndefined();
        expect(
          yield* workspace.waitForSync(WorkspaceV2.ID.ascending("wrk_wait_done_2"), {
            [sessionID]: 3,
          }),
        ).toBeUndefined();
      }),
    ...
    () =>
      Effect.gen(function* () {
        ...
        expectExitContains(
          yield* Effect.exit(
            workspace.waitForSync(
              WorkspaceV2.ID.ascending("wrk_wait_timeout"),
              { [sessionID]: 1 },
              undefined,
              25,
            ),
