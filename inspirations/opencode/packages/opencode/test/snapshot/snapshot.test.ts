...
const withTrackedSnapshot = <A, E, R>(
  fn: (input: { tmp: Bootstrapped; snapshot: Snapshot.Interface; before: string }) => Effect.Effect<
    A,
    E,
    R
  >,
...
const bootstrapScoped = Effect.fn("SnapshotTest.bootstrapScoped")(function* () {
  const dir = yield* tmpdirScoped({ git: true }).pipe(
...
const cleanupWorktree = (repo: string, worktree: string, files: string[] = []) =>
  Effect.promise(async () => {
    ...
    await Promise.all(
      files.map((file) => fs.rm(file, { recursive: true, force: true }).catch(() => undefined)),
    );
  });
  ...
  withTrackedSnapshot(({ tmp, snapshot, before }) =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        fs.symlink(`${tmp.path}/sub/dir/target.txt`, `${tmp.path}/sub/dir/link.txt`, "file"),
      );
...
it.instance(
  ...
  withTrackedSnapshot(({ tmp, snapshot, before }) =>
    Effect.gen(function* () {
      ...
      yield* write(
        file,
        `${(yield* Effect.promise(() => Bun.file(file).text())).trimEnd()}\nignored.txt\n`,
      );
...
it.live(
  ...
  Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      cleanupWorktree(tmp.path, worktreePath, [
        `${tmp.path}/shared.txt`,
        `${tmp.path}/primary-only.txt`,
      ]),
    );
    yield* Effect.gen(function* () {
      const snapshot = yield* Snapshot.Service;
...
it.instance(
  ...
  Effect.gen(function* () {
    const tmp = yield* bootstrap();
    const snapshot = yield* Snapshot.Service;
    const ids = Array.from({ length: MIXED_BATCH_GROUP_COUNT }, (_, i) =>
...
it.instance(
  ...
  Effect.gen(function* () {
    const tmp = yield* bootstrap();
    const snapshot = yield* Snapshot.Service;
    const base = Array.from({ length: OVER_BATCH_COUNT }, (_, i) =>
      fwd(tmp.path, "batch", `${i}.txt`),
    );
    ...
    yield* Effect.all(
      [
        ...base.map((file, i) => write(file, `next-${i}`)),
        ...fresh.map((file, i) => write(file, `fresh-${i}`)),
      ],
      { concurrency: "unbounded" },
    );
