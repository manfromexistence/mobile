...
const it = testEffect(
  LayerNode.compile(Worktree.node, [[InstanceStore.bootstrapNode, InstanceBootstrap.node]]),
);
    ...
    () =>
      Effect.gen(function* () {
        ...
        yield* Effect.promise(() =>
          $`git worktree add --no-checkout -b ${branch} ${dir}`.cwd(root).quiet(),
        );
    ...
    () =>
      Effect.gen(function* () {
        ...
        const before = yield* Effect.promise(() =>
          $`git fsmonitor--daemon status`.cwd(dir).quiet().nothrow(),
        );
