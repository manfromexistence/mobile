        ...
        Effect.fnUntraced(function* (command) {
          ...
          if (std?.command === "git" && std.args.some((a) => a === failArg)) {
            return ChildProcessSpawner.makeHandle({
              ...
              all: Stream.empty,
              getInputFd: () =>
                ({ [Symbol.for("effect/Sink/TypeId")]: Symbol.for("effect/Sink/TypeId") }) as any,
...
const failureIt = (failArg: string) =>
  testEffect(
    AppNodeBuilder.build(projectTestNode, [[Project.node, projectLayerWithFailure(failArg)]]),
  );
...
describe("Project.fromDirectory", () => {
  ...
  it.live("prefers normalized origin remote over root commit", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        $`git remote add origin git@github.com:Test-Org/Test-Repo.git`.cwd(tmp).quiet(),
      );
  ...
  it.live("normalizes equivalent origin URL forms to the same project ID", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        $`git remote add origin git@github.com:owner/repo.git`.cwd(ssh).quiet(),
      );
      yield* Effect.promise(() =>
        $`git remote add origin https://github.com/owner/repo.git`.cwd(https).quiet(),
      );
  ...
  it.live("migrates cached root project data when origin becomes available", () =>
    Effect.gen(function* () {
      ...
      yield* db
        ...
        .pipe(Effect.orDie);
      yield* Effect.promise(() =>
        $`git remote add origin git@github.com:acme/app.git`.cwd(tmp).quiet(),
      );
...
describe("Project.fromDirectory with worktrees", () => {
  ...
  it.live("tracks a linked worktree as the opened project directory", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        $`git worktree add ${worktreePath} -b test-branch-${Date.now()}`.cwd(tmp).quiet(),
      );
  ...
  it.live("worktree should share project ID with main repo", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        $`git worktree add ${worktreePath} -b shared-${Date.now()}`.cwd(tmp).quiet(),
      );
  ...
  it.live("should accumulate multiple worktrees in sandboxes", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        $`git worktree add ${worktree1} -b branch-${Date.now()}`.cwd(tmp).quiet(),
      );
      yield* Effect.promise(() =>
        $`git worktree add ${worktree2} -b branch-${Date.now() + 1}`.cwd(tmp).quiet(),
      );
    ...
    Effect.gen(function* () {
      ...
      const updated = yield* project.update({
        ...
        icon: {
          ...
          override: "data:image/png;base64,abc123",
          color: "#00ff00",
        },
...
describe("Project.fromDirectory with bare repos", () => {
  it.live("worktree from bare repo should cache in bare repo, not parent", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.addFinalizer(() =>
        Effect.promise(() => $`rm -rf ${barePath} ${worktreePath}`.quiet().nothrow()).pipe(
          Effect.ignore,
        ),
      );
  ...
  it.live("different bare repos under same parent should not share project ID", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.addFinalizer(() =>
        Effect.promise(() =>
          $`rm -rf ${bareA} ${bareB} ${worktreeA} ${worktreeB}`.quiet().nothrow(),
        ).pipe(Effect.ignore),
      );
