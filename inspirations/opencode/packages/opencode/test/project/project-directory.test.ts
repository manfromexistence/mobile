...
const it = testEffect(
  LayerNode.compile(LayerNode.group([Project.node, Database.node, CrossSpawnSpawner.node])),
);
...
describe("Project directory persistence", () => {
  ...
  it.live("stores an opened linked worktree directory", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.addFinalizer(() =>
        Effect.promise(() => $`git worktree remove ${worktree}`.cwd(tmp).quiet().nothrow()).pipe(
          Effect.ignore,
        ),
      );
      yield* Effect.promise(() =>
        $`git worktree add ${worktree} -b project-directory-${Date.now()}`.cwd(tmp).quiet(),
      );
  ...
  it.live("stores only the linked copy when first opened from an external linked worktree", () =>
    Effect.gen(function* () {
      ...
      const worktree = path.join(
        tmp,
        "..",
        path.basename(tmp) + "-project-directory-first-worktree",
      );
      yield* Effect.addFinalizer(() =>
        Effect.promise(() => $`git worktree remove ${worktree}`.cwd(tmp).quiet().nothrow()).pipe(
          Effect.ignore,
  ...
  it.live("records the active directory under its newly resolved project id", () =>
    Effect.gen(function* () {
      ...
      const remoteID = ProjectV2.ID.make(
        Hash.fast("git-remote:github.com/project-directory-test/collision"),
      );
      ...
      yield* db
        ...
        .pipe(Effect.orDie);
      yield* Effect.promise(() =>
        $`git remote add origin git@github.com:project-directory-test/collision.git`
          .cwd(tmp)
          .quiet(),
      );
  ...
  it.live("clears stale directories when the project id changes", () =>
    Effect.gen(function* () {
      ...
      yield* db
        ...
        .pipe(Effect.orDie);
      const remoteID = ProjectV2.ID.make(
        Hash.fast("git-remote:github.com/project-directory-test/migration"),
      );
      yield* Effect.promise(() =>
        $`git remote add origin git@github.com:project-directory-test/migration.git`
          .cwd(tmp)
          .quiet(),
      );
