...
describe("opencode acp initialize/auth subprocess", () => {
  ...
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        ...
        expect(
          yield* acp.request<AuthenticateResponse>("authenticate", { methodId: "opencode-login" }),
        ).toMatchObject({
          result: {},
        });

        const rejected = yield* acp.request<AuthenticateResponse>("authenticate", {
          methodId: "missing-auth-method",
        });
        expectErrorCode(rejected.error, -32602);
        expect(JSON.stringify(rejected.error)).not.toContain(
          process.env.OPENCODE_AUTH_CONTENT ?? "not-present",
        );
      }),
    60_000,
  );
  ...
  cliIt.live(
    ...
    ({ opencode }) =>
      Effect.gen(function* () {
        const acp = yield* createAcpClient({ opencode });
        const initialized = yield* acp.request<InitializeResponse>("initialize", {
          protocolVersion: 1,
        });
