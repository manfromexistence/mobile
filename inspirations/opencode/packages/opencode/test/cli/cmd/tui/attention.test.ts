...
describe("createTuiAttention", () => {
  ...
  test("skips empty messages and disabled attention", async () => {
    ...
    expect(
      await createTuiAttention({ renderer: empty, config: config() }).notify({ message: " \n " }),
    ).toEqual({
      ok: false,
      notification: false,
      sound: false,
    ...
    expect(
      await createTuiAttention({ renderer: disabled, config: config({ enabled: false }) }).notify({
        message: "hello",
      }),
    ).toEqual({
      ok: false,
  ...
  test("handles unavailable playback and delegates sound loading", async () => {
    ...
    const unavailable = createTuiAttention({
    ...
    const repeated = createTuiAttention({
