...
async function writeZedFixture(dir: string, options: ZedFixtureOptions = {}) {
  ...
  db.run("insert into workspaces values (1, ?, ?)", [
    options.workspacePaths ?? JSON.stringify([dir]),
    "2026-04-27",
  ]);
...
test("resolveZedSelection returns unavailable when the database cannot be queried", async () => {
  ...
  expect(await resolveZedSelection(path.join(tmp.path, "missing.sqlite"), tmp.path)).toEqual({
    type: "unavailable",
  });
});
