...
async function load(): Promise<Data> {
  ...
  await using tmp = await tmpdir({
    init: async (dir) => {
      ...
      await Bun.write(
        preloadedThemePath,
        JSON.stringify({ theme: { primary: "#f0f0f0" } }, null, 2),
      );
...
`,
        ...
        JSON.stringify(
          {
            plugin: [
              [
                ...
                {
                  marker: globalMarker,
                  theme_path: `./${globalThemeFile}`,
                  theme_name: globalThemeName,
                },
              ],
            ],
          },
  ...
  try {
    ...
    const config = createTuiResolvedConfig({
      ...
      plugin_origins: [
        {
          spec: [tmp.extra.localSpec, localOpts],
          scope: "local",
          source: path.join(tmp.path, "tui.json"),
        },
        {
          spec: [tmp.extra.invalidSpec, invalidOpts],
          scope: "local",
          source: path.join(tmp.path, "tui.json"),
        },
        {
          spec: [tmp.extra.preloadedSpec, preloadedOpts],
          scope: "local",
          source: path.join(tmp.path, "tui.json"),
        },
        {
  ...
  try {
    await TuiPluginRuntime.init({
      ...
      config: createTuiResolvedConfig({
        plugin: [
          tmp.extra.binarySpec,
          tmp.extra.invalidShapeSpec,
          tmp.extra.missingIDSpec,
          tmp.extra.goodSpec,
        ],
        plugin_origins: [
          { spec: tmp.extra.binarySpec, scope: "local", source: path.join(tmp.path, "tui.json") },
          {
            spec: tmp.extra.invalidShapeSpec,
            scope: "local",
            source: path.join(tmp.path, "tui.json"),
          },
          {
            spec: tmp.extra.missingIDSpec,
            scope: "local",
            source: path.join(tmp.path, "tui.json"),
          },
    ...
    expect(TuiPluginRuntime.list().find((item) => item.id === "demo.good.after-bad")?.active).toBe(
      true,
    );
    expect(TuiPluginRuntime.list().some((item) => item.spec === tmp.extra.binarySpec)).toBe(false);
    expect(TuiPluginRuntime.list().some((item) => item.spec === tmp.extra.invalidShapeSpec)).toBe(
      false,
    );
    expect(TuiPluginRuntime.list().some((item) => item.spec === tmp.extra.missingIDSpec)).toBe(
      false,
    );
  } finally {
    await TuiPluginRuntime.dispose();
...
`,
      ...
      await Bun.write(
        ...
        JSON.stringify({ plugin: [aSpec, bSpec] }, null, 2),
      );
  ...
  try {
    await TuiPluginRuntime.init({
      ...
      config: createTuiResolvedConfig({
        ...
        plugin_origins: [
          { spec: tmp.extra.spec, scope: "local", source: path.join(tmp.path, "tui.json") },
        ],
      }),
    });
...
`,
  ...
  try {
    await TuiPluginRuntime.init({
      api: createTuiPluginApi({
        keymap: harness.keymap as unknown as NonNullable<
          Parameters<typeof createTuiPluginApi>[0]
        >["keymap"],
      }),
      config: createTuiResolvedConfig({
        plugin: [tmp.extra.spec],
...
`,
  ...
  const keymap = {
    registerLayer(layer: { commands?: Array<{ name: string }> }) {
      const tracked =
        layer.commands?.some((item) => item.name === "demo.keymap.cleanup.manual") ?? false;
      return () => {
        if (!tracked) return;
        command_drop += 1;
      };
...
`,
  ...
  try {
    ...
    await Bun.write(
      tmp.extra.themePath,
      JSON.stringify({ theme: { primary: "#222222" } }, null, 2),
    );
    await Bun.write(
...
`,
    ...
    const list = await Filesystem.readJson<
      Record<string, { themes?: Record<string, { dest: string }> }>
