...
function footer() {
  ...
  return {
    ...
    submit(text: string, mode?: RunPrompt["mode"]) {
      const next = mode
        ? { text, parts: [] as RunPrompt["parts"], mode }
        : { text, parts: [] as RunPrompt["parts"] };
...
describe("run runtime queue", () => {
  ...
  test("exposes ordinary in-flight prompts for removal before sending", async () => {
    ...
    expect(
      first?.type === "queued.prompts" && event?.type === "queued.prompts"
        ? first.prompts === event.prompts
        : true,
    ).toBe(false);
    expect(ui.events.findLast((item) => item.type === "queue")).toEqual({
      type: "queue",
      queue: 1,
    });
    expect(
      event?.type === "queued.prompts" ? event.prompts.map((item) => item.prompt.text) : [],
    ).toEqual(["two"]);
