...
function terminalColors(input: Partial<TerminalColors> = {}): TerminalColors {
  return {
    palette: Array.from(
      { length: 256 },
      (_, index) => input.palette?.[index] ?? palette[index % palette.length]!,
    ),
...
test("returns syntax styles and indexed splash colors", async () => {
  ...
  try {
    ...
    expect(expectRgba(theme.footer.statusAccent).toInts()).not.toEqual(
      expectRgba(theme.footer.status).toInts(),
    );
  } finally {
...
test("keeps footer surfaces exact while scrollback stays palette matched", async () => {
  ...
  try {
    expect(expectRgba(theme.footer.selected).toInts()).toEqual(
      expectRgba(exact.backgroundElement).toInts(),
    );
    expect(expectRgba(theme.footer.border).toInts()).toEqual(expectRgba(exact.border).toInts());
    expect(expectRgba(theme.footer.pane).toInts()).toEqual(
      expectRgba(exact.backgroundMenu).toInts(),
    );
...
test("uses refreshed background brightness when cached renderer mode is stale", async () => {
  ...
  try {
    expect(expectRgba(stale.footer.surface).toInts()).toEqual(
      expectRgba(light.footer.surface).toInts(),
    );
  } finally {
...
test("keeps renderer mode when refreshed default background is unavailable", async () => {
  ...
  try {
    expect(expectRgba(light.footer.surface).toInts()).not.toEqual(
      expectRgba(dark.footer.surface).toInts(),
    );
  } finally {
    light.block.syntax?.destroy();
    light.block.subtleSyntax?.destroy();
