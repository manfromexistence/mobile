...
it.instance("plan agent denies edits except .opencode/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".opencode/plans/foo.md", plan!.permission).action).toBe(
      "allow",
    );
  }),
);
...
it.instance(
  "explore agent asks for external directories and allows whitelisted external paths",
  () =>
    Effect.gen(function* () {
      const explore = yield* load((svc) => svc.get("explore"));
      expect(explore).toBeDefined();
      expect(
        Permission.evaluate("external_directory", "/some/other/path", explore!.permission).action,
      ).toBe("ask");
      expect(
        Permission.evaluate("external_directory", Truncate.GLOB, explore!.permission).action,
      ).toBe("allow");
      expect(
        Permission.evaluate(
          "external_directory",
          path.join(Global.Path.tmp, "agent-work"),
          explore!.permission,
        ).action,
      ).toBe("allow");
    }),
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const build = yield* load((svc) => svc.get("build"));
      expect(
        Permission.evaluate("external_directory", Truncate.GLOB, build!.permission).action,
      ).toBe("allow");
      expect(
        Permission.evaluate("external_directory", Truncate.DIR, build!.permission).action,
      ).toBe("deny");
      expect(
        Permission.evaluate("external_directory", "/some/other/path", build!.permission).action,
      ).toBe("deny");
    }),
  {
    config: {
...
`,
      ...
      expect(Permission.evaluate("external_directory", target, build!.permission).action).toBe(
        "allow",
      );
    }),
  { git: true },
