...
describe("acp session state", () => {
  ...
  sessionTest.effect("fails required lookups with typed SessionNotFound", () =>
    Effect.gen(function* () {
      const error = yield* ACPSession.Service.use((session) => session.get("ses_missing")).pipe(
        Effect.flip,
      );
  ...
  sessionTest.effect("updates selected variant and mode independently", () =>
    Effect.gen(function* () {
      ...
      expect(yield* ACPSession.Service.use((session) => session.getVariant("ses_config"))).toBe(
        "high",
      );
      expect(yield* ACPSession.Service.use((session) => session.getMode("ses_config"))).toBe(
        "plan",
      );
  ...
  sessionTest.effect("records known message part metadata for delta routing", () =>
    Effect.gen(function* () {
      yield* ACPSession.Service.use((session) =>
        session.create({ id: "ses_parts", cwd: "/workspace" }),
      );
  ...
  sessionTest.effect("keeps repeated part ids distinct across messages", () =>
    Effect.gen(function* () {
      yield* ACPSession.Service.use((session) =>
        session.create({ id: "ses_duplicate_parts", cwd: "/workspace" }),
      );
      yield* ACPSession.Service.use((session) =>
        session.recordPartMetadata({
  ...
  sessionTest.effect("removing a session clears its known part metadata", () =>
    Effect.gen(function* () {
      yield* ACPSession.Service.use((session) =>
        session.create({ id: "ses_remove", cwd: "/workspace" }),
      );
      yield* ACPSession.Service.use((session) =>
        session.recordPartMetadata({
