...
describe("ACP service sessions", () => {
  const makeService = (
    ...
    const sdk = {
      config: {
        providers: () =>
          Promise.resolve({ data: { providers: [provider], default: { test: modelID } } }),
  ...
  it("loads a session and restores model variant and mode from messages", async () => {
    ...
    expect(result.configOptions?.find((option) => option.id === "effort")?.currentValue).toBe(
      "high",
    );
  ...
  it("lists sessions sorted by updated time with cursor support", async () => {
    ...
    const second = await Effect.runPromise(
      service.listSessions({ cwd: "/workspace", cursor: first.nextCursor }),
    );
  ...
  it("includes live ACP sessions before they appear in server-backed session list", async () => {
    ...
    const created = await Effect.runPromise(
      service.newSession({ cwd: "/workspace", mcpServers: [] }),
    );
  ...
  it("maps provider auth failures to auth-required request errors", async () => {
    const service = ACPService.make({
      sdk: {
        config: {
          providers: () =>
            Promise.reject({ name: "ProviderAuthError", data: { providerID: "test" } }),
          get: () => Promise.resolve({ data: {} }),
        },
        app: {
  ...
  it("does not cache failed directory snapshots", async () => {
    ...
    const sdk = {
      ...
      app: {
        agents: () =>
          Promise.resolve({
            data: [{ name: "build", mode: "primary", permission: [], options: {} }],
          }),
        skills: () => Promise.resolve({ data: [] }),
      },
      command: {
  ...
  it("uses the configured model as the new session default", async () => {
    const sdk = {
      ...
      session: {
        create: (input: { model?: { id?: string } }) =>
          Promise.resolve({ data: { id: input.model?.id } }),
        list: () => Promise.resolve({ data: [] }),
      },
      mcp: {
        add: () => Promise.resolve({ data: {} }),
    ...
    const sdk = {
      ...
      session: {
        ...
        messages: () => {
          ...
          return Promise.resolve({
            data: [
              { info: { role: "user", model: { providerID: "test", modelID: "second-model" } } },
            ],
          });
        },
      },
        ...
        Promise.resolve({
          data: {
            info: assistantInfo(
              ...
              {
                name: "APIError",
                data: { message: "Provider request failed", isRetryable: false },
              },
            ),
          },
        }),
...
function flattenSelectOptions(
  ...
  return (
    option?.options.flatMap((item): SessionConfigSelectOption[] =>
