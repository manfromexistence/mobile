...
describe("acp.error", () => {
  ...
  test("includes safe validation details", () => {
    expect(
      ACPError.toRequestError(new ACPError.SessionNotFoundError({ sessionId: "ses_123" })),
    ).toMatchObject({
      code: -32602,
      data: { sessionId: "ses_123" },
    });
    expect(
      ACPError.toRequestError(new ACPError.InvalidModelError({ modelId: "gpt-missing" })),
    ).toMatchObject({
      code: -32602,
  ...
  test("maps auth required to the SDK auth error", () => {
    const requestError = ACPError.toRequestError(
      new ACPError.AuthRequiredError({ providerId: "anthropic" }),
    );
