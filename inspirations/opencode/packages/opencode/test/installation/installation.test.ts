    ...
    testEffect(
      ...
      Effect.gen(function* () {
        ...
        expect(pnpmCalls).toContain(
          `https://registry.npmjs.org/opencode-ai/${InstallationChannel}`,
        );
      }),
    );

    testEffect(testLayer(() => jsonResponse({ version: "2.3.4" }))).effect(
      ...
      testLayer(
        ...
        (cmd, args) => {
          ...
          if (
            ...
            args.includes("anomalyco/tap/opencode")
          )
            return "";
          if (cmd === "brew" && args.includes("--formula") && args.includes("opencode"))
            return "opencode";
          return "";
        },
      ),
