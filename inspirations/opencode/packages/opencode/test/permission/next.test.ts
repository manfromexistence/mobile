  ...
  Effect.gen(function* () {
    ...
    return yield* Effect.gen(function* () {
      ...
      Effect.timeoutOrElse({
        duration: "1 second",
        orElse: () =>
          Effect.fail(new Error(`timed out waiting for ${count} pending permission request(s)`)),
      }),
    );
  });
...
test("fromConfig - expands tilde to home directory", () => {
  ...
  expect(result).toEqual([
    { permission: "external_directory", pattern: `${os.homedir()}/projects/*`, action: "allow" },
  ]);
});
...
test("fromConfig - expands $HOME without trailing slash", () => {
  ...
  expect(result).toEqual([
    { permission: "external_directory", pattern: os.homedir(), action: "allow" },
  ]);
});
...
test("fromConfig - does not expand tilde in middle of path", () => {
  ...
  expect(result).toEqual([
    { permission: "external_directory", pattern: "/some/~/path", action: "allow" },
  ]);
});
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      if (Exit.isFailure(ea))
        expect(Cause.squash(ea.cause)).toBeInstanceOf(PermissionV1.RejectedError);
      if (Exit.isFailure(eb))
        expect(Cause.squash(eb.cause)).toBeInstanceOf(PermissionV1.RejectedError);
    }),
  { git: true },
);
...
it.live("permission requests stay isolated by directory", () =>
  Effect.gen(function* () {
    ...
    yield* store.provide(
      { directory: one },
      reply({ requestID: onePending[0].id, reply: "reject" }),
    );
    yield* store.provide(
      { directory: two },
      reply({ requestID: twoPending[0].id, reply: "reject" }),
    );
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      const exit = yield* reply({
        requestID: PermissionV1.ID.make("per_unknown"),
        reply: "once",
      }).pipe(Effect.exit);
      expect(Exit.isFailure(exit)).toBe(true);
      if (Exit.isFailure(exit)) {
