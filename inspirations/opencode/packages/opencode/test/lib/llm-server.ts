...
function responseToolDone(
  tool: { id: string; item: string; name: string; args: string },
  seq: number,
) {
  return {
    type: "response.output_item.done",
...
function flow(item: Sse) {
  ...
  for (const part of [...item.head, ...item.tail]) {
    ...
    const delta =
      choice && "delta" in choice && choice.delta && typeof choice.delta === "object"
        ? choice.delta
        : undefined;
    ...
    if (delta && "tool_calls" in delta && Array.isArray(delta.tool_calls)) {
      for (const tool of delta.tool_calls) {
        ...
        const fn =
          "function" in tool && tool.function && typeof tool.function === "object"
            ? tool.function
            : undefined;
        if (
          "id" in tool &&
...
function send(item: Sse) {
  ...
  const body: Stream.Stream<Uint8Array, unknown> = wait
    ? Stream.concat(
        head,
        Stream.fromEffect(Effect.promise(() => wait)).pipe(Stream.flatMap(() => tail)),
      )
...
export class TestLLMServer extends Context.Service<TestLLMServer, TestLLMServer.Service>()(
  "@test/LLMServer",
) {
  static readonly layer = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      return TestLLMServer.of({
        ...
        pushMatch: Effect.fn("TestLLMServer.pushMatch")(function* (
          match: Match,
          ...input: (Item | Reply)[]
        ) {
        ...
        toolMatch: Effect.fn("TestLLMServer.toolMatch")(function* (
          match: Match,
          name: string,
          input: unknown,
        ) {
        ...
        hold: Effect.fn("TestLLMServer.hold")(function* (
          value: string,
          wait: PromiseLike<unknown>,
        ) {
