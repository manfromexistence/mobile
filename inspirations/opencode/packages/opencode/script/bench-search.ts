...
for (const q of FILE_QUERIES) {
  ...
  console.log(
    `[picker] fileSearch "${q}": ${(performance.now() - t).toFixed(1)}ms (${count} results)`,
  );
}
...
console.log(
  `[Search] init file (runtime + picker + scan): ${(performance.now() - tRuntime).toFixed(1)}ms`,
);
...
console.log(
  `[Search] init grep (content index warmup):    ${(performance.now() - tGrepWarmup).toFixed(1)}ms`,
);
...
for (const q of FILE_QUERIES) {
  ...
  console.log(
    `[Search.find] "${q}": ${(performance.now() - t).toFixed(1)}ms (${r.length} results)`,
  );
}
...
for (const q of GREP_QUERIES) {
  ...
  console.log(
    `[Search.grep] "${q}": ${(performance.now() - t).toFixed(1)}ms (${r.length} matches)`,
  );
}
