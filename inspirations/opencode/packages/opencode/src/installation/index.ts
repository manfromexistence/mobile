      ...
      latest: Effect.fn("Installation.latest")(function* (installMethod?: Method) {
        ...
        if (detectedMethod === "brew") {
          ...
          if (formula.includes("/")) {
            ...
            const info = yield* Schema.decodeUnknownEffect(Schema.fromJsonString(BrewInfoV2))(
              infoJson,
            );
...
export const node = LayerNode.make({
  service: Service,
  layer: layer,
  deps: [httpClient, AppProcess.node],
});

const { runPromise } = makeRuntime(Service, AppNodeBuilder.build(node));

export const latest = (...args: Parameters<Interface["latest"]>) =>
  runPromise((s) => s.latest(...args));
export const method = () => runPromise((s) => s.method());
export const upgrade = (...args: Parameters<Interface["upgrade"]>) =>
