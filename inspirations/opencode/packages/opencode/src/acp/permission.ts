...
function permissionTitle(toolName: string, input: ToolInput) {
  ...
  switch (tool) {
    case "external_directory":
      return (
        stringValue(input.description) ?? stringValue(input.command) ?? stringValue(input.parentDir)
      );
...
function permissionLocations(toolName: string, input: ToolInput): ToolCallLocation[] {
  ...
  if (files.length) {
    return Array.from(
      new Set(
        files.flatMap((file) =>
          [file.filePath, file.movePath].filter((path): path is string => !!path),
        ),
      ),
      (path) => ({ path }),
    );
...
function selectedReply(result: RequestPermissionResponse): Reply {
  ...
  if (result.outcome.optionId === "once" || result.outcome.optionId === "always")
