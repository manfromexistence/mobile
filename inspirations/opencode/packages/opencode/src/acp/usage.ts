...
export interface ContextLimitLoaderInterface {
  readonly providers: (
    directory: string,
  ) => Effect.Effect<Record<ProviderV2.ID, Provider.Info>, unknown>;
}
...
export interface Interface {
  ...
  readonly latestAssistantMessage: (
...
export const messageLoaderLayer = (sdk: SDK) =>
  Layer.succeed(MessageLoader, messageLoaderFromSDK(sdk));
...
export function latestAssistantMessage(
  ...
  return messages
    .filter(
      (message): message is { readonly info: AssistantMessage } =>
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const limits = yield* SynchronizedRef.make(
      new Map<string, Effect.Effect<number | undefined>>(),
    );
          ...
          const next = yield* Effect.cached(
            contextLimitLoader.providers(input.directory).pipe(
              Effect.map((providers) =>
                findContextLimit(providers, input.providerID, input.modelID),
              ),
              Effect.catch((error) =>
                Effect.logError("failed to get providers for usage context limit", {
                  error: error,
...
export const node = makeGlobalNode({
  service: Service,
  layer,
  deps: [messageLoaderNode, contextLimitLoaderNode],
});
