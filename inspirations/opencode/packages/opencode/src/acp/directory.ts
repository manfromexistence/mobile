...
export const variants = (snapshot: Snapshot, model: DefaultModel) =>
  snapshot.variantsByModel[modelKey(model)];
    ...
    variantsByModel: Object.fromEntries(
      Object.values(input.providers).flatMap((provider) =>
        Object.values(provider.models).flatMap((model) =>
          model.variants
            ? [[modelKey({ providerID: provider.id, modelID: model.id }), model.variants]]
            : [],
        ),
      ),
    ),
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const snapshots = yield* SynchronizedRef.make(
      new Map<string, Effect.Effect<Snapshot, ACPError.Error>>(),
    );
