...
const emptyPatch = (file: string) =>
  formatPatch(structuredPatch(file, file, "", "", "", "", { context: 0 }));

const nums = (list: Git.Stat[]) =>
  new Map(
    list.map(
      (item) => [item.file, { additions: item.additions, deletions: item.deletions }] as const,
    ),
  );
...
const splitGitPatch = (patch: Git.Patch) => {
  ...
  const chunks = starts.map((start, index) =>
    patch.text.slice(start, starts[index + 1] ?? patch.text.length),
  );
...
const track = Effect.fnUntraced(function* (
  ...
  if (!ref)
    return yield* files(git, cwd, ref, yield* git.status(cwd), new Map(), emptyBatch(), options);
...
export class PatchApplyError extends Schema.TaggedErrorClass<PatchApplyError>()(
  "VcsPatchApplyError",
  {
  ...
  Effect.gen(function* () {
    ...
    const state = yield* InstanceState.make<State>(
      Effect.fn("Vcs.state")(function* (ctx) {
        ...
        const [current, root] = yield* Effect.all(
          [git.branch(ctx.directory), git.defaultBranch(ctx.directory)],
          {
            concurrency: 2,
          },
        );
    ...
    return Service.of({
      ...
      diffRaw: Effect.fn("Vcs.diffRaw")(function* () {
        ...
        const [hasHead, status] = yield* Effect.all(
          [git.hasHead(ctx.directory), git.status(ctx.directory)],
          {
            concurrency: 2,
          },
        );
        ...
        const untracked = yield* Effect.forEach(
          ...
          (item) =>
            git.patchUntracked(ctx.directory, item.file).pipe(Effect.map((patch) => patch.text)),
        );
