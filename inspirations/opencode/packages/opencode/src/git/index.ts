...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const mergeBase = Effect.fn("Git.mergeBase")(function* (
      cwd: string,
      base: string,
      head = "HEAD",
    ) {
    ...
    const show = Effect.fn("Git.show")(function* (
      cwd: string,
      ref: string,
      file: string,
      prefix = "",
    ...
    const diff = Effect.fn("Git.diff")(function* (cwd: string, ref: string) {
      const list = nuls(
        yield* text(
          ["diff", "--no-ext-diff", "--no-renames", "--name-status", "-z", ref, "--", "."],
          { cwd },
        ),
      );
    ...
    const stats = Effect.fn("Git.stats")(function* (cwd: string, ref: string) {
      return nuls(
        yield* text(["diff", "--no-ext-diff", "--no-renames", "--numstat", "-z", ref, "--", "."], {
          cwd,
        }),
      ).flatMap((item) => {
    ...
    const patchAll = Effect.fn("Git.patchAll")(function* (
      cwd: string,
      ref: string,
      options?: PatchOptions,
    ) {
