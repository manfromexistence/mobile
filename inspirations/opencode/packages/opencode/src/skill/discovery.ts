...
const layer: Layer.Layer<Service, never, FSUtil.Service | Path.Path | HttpClient.HttpClient> =
  Layer.effect(
    Service,
    Effect.gen(function* () {
      ...
      const pull = Effect.fn("Discovery.pull")(function* (url: string) {
        ...
        const data = yield* HttpClientRequest.get(index).pipe(
          ...
          Effect.catch((err) =>
            Effect.logError("failed to fetch index", { url: index, error: err }).pipe(
              Effect.as(null),
            ),
          ),
        );
            ...
            Effect.gen(function* () {
              ...
              if (version === undefined || current === version) {
                yield* Effect.forEach(
                  skill.files,
                  (file) =>
                    download(new URL(file, `${host}/${skill.name}/`).href, path.join(root, file)),
                ...
                yield* Effect.gen(function* () {
                  ...
                  Effect.catch((error) =>
                    Effect.logError("failed to refresh skill", { skill: skill.name, error }),
                  ),
                  Effect.ensuring(
...
export const node = LayerNode.make({
  service: Service,
  layer: layer,
  deps: [FSUtil.node, path, httpClient],
});
