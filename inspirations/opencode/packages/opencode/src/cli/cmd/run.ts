...
function inline(info: Inline) {
  const suffix = info.description
    ? UI.Style.TEXT_DIM + ` ${info.description}` + UI.Style.TEXT_NORMAL
    : "";
...
export const RunCommand = effectCmd({
  ...
  directory: (args) =>
    args.dir && !args.attach ? path.resolve(process.cwd(), args.dir) : process.cwd(),
  builder: (yargs: Argv) =>
    yargs
      .positional("message", {
  ...
  handler: Effect.fn("Cli.run")(function* (args) {
    ...
    yield* Effect.promise(async () => {
      ...
      const replay =
        args.replay === false ? false : args.replay || args["replay-limit"] !== undefined;
          ...
          const content = await (async () => {
            ...
            try {
              ...
              if (!opened.isFile() || Number(opened.size) > ATTACH_FILE_MAX_BYTES) {
                UI.error(
                  `Cannot attach local file larger than 10 MiB or a special file: ${filePath}`,
                );
                process.exit(1);
              }
      ...
      async function execute(sdk: OpencodeClient) {
            ...
            if (event.type === "message.part.updated") {
              ...
              if (part.type === "reasoning" && part.time?.end && thinking) {
                ...
                if (process.stdout.isTTY) {
                  UI.empty();
                  UI.println(
                    `${UI.Style.TEXT_DIM}\u001b[3m${line}\u001b[0m${UI.Style.TEXT_NORMAL}`,
                  );
                  UI.empty();
                  continue;
                }
        ...
        const cwd = args.attach
          ? (directory ?? sess.directory ?? (await current(sdk)))
          : (directory ?? root);
