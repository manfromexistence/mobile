...
class Service extends Context.Service<Service, TransportService>()(
  ...
  return Layer.fresh(
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const recoverQuestion = Effect.fn("RunStreamTransport.recoverQuestion")(function* (
          partID: string,
        ) {
          ...
          try {
            while (!closed && !abort.signal.aborted && !input.footer.isClosed) {
              ...
              const questions = yield* Effect.promise(() => input.sdk.question.list()).pipe(
                Effect.map((item) =>
                  (item.data ?? []).filter((request) => request.sessionID === input.sessionID),
                ),
                Effect.orElseSucceed(() => []),
              );
              if (state.data.questions.length > 0 || !state.data.tools.has(partID)) {
        ...
        const replayMessages = () =>
          Effect.promise(() =>
            ...
            Effect.flatMap((item) =>
              item.error ? Effect.fail(item.error) : Effect.succeed(item.data ?? []),
            ),
          );
        ...
        const bootstrap = Effect.fn("RunStreamTransport.bootstrap")(function* () {
          ...
          if (replay) {
            ...
            for (const commit of replay.commits) {
              ...
              yield* Effect.promise(() => input.footer.idle()).pipe(
        ...
        const complete = Effect.fn("RunStreamTransport.complete")(function* (
          next: Wait,
          fallback: boolean,
        ) {
        ...
        const poll = Effect.fn("RunStreamTransport.poll")(function* (
          next: Wait,
          signal: AbortSignal,
        ) {
        ...
        const applyEvent = Effect.fn("RunStreamTransport.applyEvent")(function* (event: Event) {
          ...
          const prev =
            event.type === "message.part.updated" ? listSubagentTabs(state.subagent) : undefined;
        ...
        const replayOnResize: (next: SessionResizeReplayInput) => Effect.Effect<boolean> =
          Effect.fn("RunStreamTransport.replayOnResize")(function* (
            ...
            const source = yield* Effect.all([replayMessages(), replayRequests()], {
              concurrency: "unbounded",
            }).pipe(Effect.exit);
        ...
        const runPromptTurn = Effect.fn("RunStreamTransport.runPromptTurn")(function* (
          ...
          yield* poll(item, turn.signal).pipe(
            Effect.forkIn(scope, { startImmediately: true }),
            Effect.asVoid,
          );
          ...
          const send =
            next.prompt.mode === "shell"
                  ...
                  Effect.andThen(
                    resolveShellAgent(next.agent)
                      ...
                      .pipe(
                        ...
                        Effect.flatMap(() =>
                          Deferred.succeed(item.done, undefined).pipe(Effect.ignore),
                        ),
                        Effect.catch((error) =>
                          Deferred.fail(item.done, error).pipe(Effect.ignore),
                        ),
                        Effect.forkIn(scope, { startImmediately: true }),
                        Effect.asVoid,
                      ),
              ...
              : command
                ? Effect.sync(() => {
                    input.trace?.write("send.command", {
                      sessionID: input.sessionID,
                        ...
                        input.sdk.session.command(
                          {
                            ...
                            agent: next.agent,
                            model: next.model
                              ? `${next.model.providerID}/${next.model.modelID}`
                              : undefined,
                            variant: next.variant,
                            command: command.name,
                            ...
                            parts: [
                              ...
                              ...next.prompt.parts.filter(
                                (item): item is Extract<RunPromptPart, { type: "file" }> =>
                                  item.type === "file",
                              ),
                            ],
                          },
        ...
        const selectSubagent = Effect.fn("RunStreamTransport.selectSubagent")(
