...
export function pickVariant(
  model: RunInput["model"],
  input: RunSession | SessionMessages,
): string | undefined {
  ...
  return Layer.fresh(
    Layer.effect(
      ...
      Effect.gen(function* () {
        ...
        const resolveSavedVariant = Effect.fn("RunVariant.resolveSavedVariant")(function* (
          model: RunInput["model"],
        ) {
          if (!model) {
            return undefined;
...
export function createVariantRuntime(fs = AppNodeBuilder.build(FSUtil.node)): VariantRuntime {
  ...
  return {
    resolveSavedVariant: (model) =>
      runtime.runPromise((svc) => svc.resolveSavedVariant(model)).catch(() => undefined),
    saveVariant: (model, variant) =>
      runtime.runPromise((svc) => svc.saveVariant(model, variant)).catch(() => {}),
  };
}
