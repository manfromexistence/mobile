...
export type SessionMessages = NonNullable<
  Awaited<ReturnType<RunInput["sdk"]["session"]["messages"]>>["data"]
>;
...
export function messagePrompt(msg: SessionMessages[number]): RunPrompt {
  ...
  for (const part of msg.parts) {
    if (part.type === "file") {
      const next = part.source?.text
        ? structuredClone(part.source.text)
        : take("@" + fileName(part.url, part.filename));
    ...
    const span = part.source
      ? structuredClone(part.source)
      : (take("@" + part.name) ?? add("@" + part.name));
    used.push({ start: span.start, end: span.end });
    parts.push({
      type: "agent",
      name: part.name,
