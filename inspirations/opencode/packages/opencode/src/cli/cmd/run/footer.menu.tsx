...
function moveOffset(
  value: number,
  input: { count: number; limit: number; selected: number; dir: -1 | 1 },
) {
  const max = maxOffset(input.count, input.limit);
...
export function RunFooterMenu(props: {
  ...
  createEffect(() => {
    ...
    const selected = all.findIndex(
      (item) => item.type === "item" && item.index === props.selected(),
    );
    ...
    const dir =
      props.selected() === previous + 1 ? 1 : props.selected() === previous - 1 ? -1 : undefined;
