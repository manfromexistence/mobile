...
function fallbackInline(ctx: ToolFrame): ToolInline {
  const title =
    ...
    (Object.keys(ctx.input).length > 0 ? JSON.stringify(ctx.input) : "Unknown");
...
function runGlob(p: ToolProps<typeof GlobTool>): ToolInline {
  ...
  const description =
    matches === undefined ? suffix : `${suffix}${suffix ? " · " : ""}${count(matches, "match")}`;
  return {
    icon: "✱",
    title,
...
function runBatch(p: ToolProps): ToolInline {
  ...
  return {
    icon: "#",
    title:
      text(p.frame.state.title) ||
      (calls > 0 ? `Batch ${calls} tool${calls === 1 ? "" : "s"}` : "Batch"),
    mode: "block",
    body: p.frame.status === "completed" ? text(p.frame.state.output) : undefined,
  };
}
...
export function toolFrame(commit: StreamCommit, raw: string): ToolFrame {
  ...
  return {
    ...
    input: dict(state.input),
    meta:
      commit.part?.state && "metadata" in commit.part.state ? dict(commit.part.state.metadata) : {},
    state,
