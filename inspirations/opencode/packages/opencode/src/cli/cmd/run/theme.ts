...
function indexedPalette(
  ...
  size: number = Math.max(colors.palette.length, 16),
): RGBA[] {
...
function map(
  ...
  const statusAccentBase =
    footerMode === "dark"
      ? tint(footerBackground, rgba("#ffffff"), 0.06)
      : tint(statusBase, rgba("#000000"), 0.04);
...
export const RUN_THEME_FALLBACK: RunTheme = {
  ...
  splash: {
    ...
    leftShadow: splashShadow(
      fallbackSplashIndexed,
      RGBA.fromValues(0, 0, 0, 0),
      fallbackSplashLeft,
      0.14,
    ),
    rightShadow: splashShadow(
      fallbackSplashIndexed,
      RGBA.fromValues(0, 0, 0, 0),
      fallbackSplashRight,
      0.14,
