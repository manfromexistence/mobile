...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    const config = Effect.fn("RunBoot.config")(() =>
      Effect.promise(() => loadConfig().catch(() => undefined)),
    );
    ...
    const resolveSessionInfo = Effect.fn("RunBoot.resolveSessionInfo")(function* (
      ...
      const session = yield* Effect.promise(() =>
        resolveSession(sdk, sessionID).catch(() => undefined),
      );
      if (!session) {
...
export async function resolveModelInfo(
  ...
  return runtime
    .runPromise((svc) => svc.resolveModelInfo(sdk, directory, model))
    .catch(() => emptyModelInfo());
}
...
export async function resolveSessionInfo(
  ...
  return runtime
    .runPromise((svc) => svc.resolveSessionInfo(sdk, sessionID, model))
