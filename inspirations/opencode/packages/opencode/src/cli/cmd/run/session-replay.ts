...
function isShellSyntheticUser(message: SessionMessages[number]) {
  ...
  return (
    ...
    message.parts.some(
      (part) => part.type === "text" && part.synthetic && part.text === SHELL_SYNTHETIC_USER_TEXT,
    )
  );
}

function isShellSyntheticAssistant(
  message: SessionMessages[number],
  shellParents: ReadonlySet<string>,
) {
  return (
...
function summaryMessageIDs(messages: SessionMessages): ReadonlySet<string> {
  const shellParents = new Set(
    messages.filter(isShellSyntheticUser).map((message) => message.info.id),
  );
  ...
  return rows.reduce((out, local) => {
      ...
      ? out.findIndex(
          (commit) =>
            ...
            (local.after.partID
              ...
              : commit.messageID === local.after.messageID),
        )
      : -1;
    const anchored =
      exact !== -1
    ...
    if (anchored !== -1) {
      ...
      if (
        commit &&
        visible &&
        commit.text.startsWith(visible) &&
        commit.text.length > visible.length
      ) {
        return [
    ...
    const after = out.findIndex(
      (commit) => commit.kind === "user" && commit.messageID === row.messageID,
    );
    if (after !== -1) {
