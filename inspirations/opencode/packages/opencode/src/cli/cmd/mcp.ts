...
function configuredServers(config: ConfigV1.Info) {
  return Object.entries(config.mcp ?? {}).filter((entry): entry is [string, McpConfigured] =>
...
function listState() {
  return Effect.gen(function* () {
    ...
    const stored = yield* Effect.all(
      Object.fromEntries(
        configuredServers(config).map(([name]) => [name, mcp.hasStoredTokens(name)]),
      ),
...
export const McpListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.list")(function* () {
    ...
    for (const [name, serverConfig] of servers) {
      ...
      const typeHint =
        serverConfig.type === "remote" ? serverConfig.url : serverConfig.command.join(" ");
      prompts.log.info(
...
export const McpAuthCommand = effectCmd({
  ...
  "mcp": {
    ...
    const authStatus =
      auth[serverName] ?? (yield* MCP.Service.use((mcp) => mcp.getAuthStatus(serverName)));
...
export const McpAuthListCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.mcp.auth.list")(function* () {
    ...
    for (const [name, serverConfig] of servers) {
      ...
      prompts.log.info(
        `${icon} ${name} ${UI.Style.TEXT_DIM}${statusText}\n    ${UI.Style.TEXT_DIM}${url}`,
      );
    }
    ...
    yield* Effect.promise(async () => {
      ...
      if (entry?.tokens) {
        ...
        if (entry.tokens.expiresAt) {
          ...
          prompts.log.info(
            `  Expires: ${expiresDate.toISOString()} ${isExpired ? "(EXPIRED)" : ""}`,
          );
        }
