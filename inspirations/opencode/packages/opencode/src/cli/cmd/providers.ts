...
const handlePluginAuth = Effect.fn("Cli.providers.pluginAuth")(function* (
  ...
  const index = yield* Effect.gen(function* () {
    ...
    const match = plugin.auth.methods.findIndex(
      (x) => x.label.toLowerCase() === methodName.toLowerCase(),
    );
    if (match === -1) {
      return yield* fail(
...
export const ProvidersLoginCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.login")(function* (args) {
    ...
    if (args.url) {
      ...
      yield* Effect.orDie(
        authSvc.set(url, { type: "wellknown", key: wellknown.auth.env, token: token.trim() }),
      );
    ...
    if (provider === "other") {
      ...
      if (customPlugin && customPlugin.auth) {
        const handled = yield* handlePluginAuth(
          { auth: customPlugin.auth! },
          provider,
          args.method,
        );
        if (handled) return;
...
export const ProvidersLogoutCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.providers.logout")(function* (args) {
    ...
    UI.empty();
    const credentials: Array<[string, Auth.Info]> = Object.entries(
      yield* Effect.orDie(authSvc.all()),
    );
