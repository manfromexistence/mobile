...
const run = Effect.fn("Cli.debug.agent.body")(function* (
  ...
  if (!agent) {
    process.stderr.write(
      `Agent ${agentName} not found, run '${basename(process.execPath)} agent list' to get an agent list` +
        EOL,
    );
    return yield* fail("", 1);
  }
...
const getAvailableTools = Effect.fn("Cli.debug.agent.getAvailableTools")(function* (
  agent: Agent.Info,
) {
