...
export const PrCommand = effectCmd({
  ...
  handler: Effect.fn("Cli.pr")(function* (args) {
    ...
    const checkout = yield* Effect.promise(() =>
      Process.run(["gh", "pr", "checkout", `${prNumber}`, "--branch", localBranchName, "--force"], {
        nothrow: true,
      }),
    );
    if (checkout.code !== 0) {
    ...
    if (prInfoResult.code === 0 && prInfoResult.text.trim()) {
      ...
      if (prInfo?.isCrossRepository && prInfo.headRepository && prInfo.headRepositoryOwner) {
        ...
        if (!remotes.split("\n").includes(remoteName)) {
          yield* git.run(
            ["remote", "add", remoteName, `https://github.com/${forkOwner}/${forkName}.git`],
            {
              cwd: worktree,
            },
          );
        ...
        if (sessionMatch) {
          ...
          if (importResult.code === 0) {
            const sessionIdMatch = importResult.text
              .trim()
              .match(/Imported session: ([a-zA-Z0-9_-]+)/);
            if (sessionIdMatch) {
