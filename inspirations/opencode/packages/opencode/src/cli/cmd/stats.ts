        ...
        for (const message of messages) {
          if (message.info.role === "assistant") {
            ...
            if (message.info.tokens) {
              ...
              sessionModelUsage[modelKey].tokens.cache.write +=
...
export function displayStats(stats: SessionStats, toolLimit?: number, modelLimit?: number) {
  ...
  const medianTokensPerSession = isNaN(stats.medianTokensPerSession)
    ? 0
    : stats.medianTokensPerSession;
  ...
  if (modelLimit !== undefined && Object.keys(stats.modelUsage).length > 0) {
    const sortedModels = Object.entries(stats.modelUsage).sort(
      ([, a], [, b]) => b.messages - a.messages,
    );
    const modelsToDisplay =
      modelLimit === Infinity ? sortedModels : sortedModels.slice(0, modelLimit);
