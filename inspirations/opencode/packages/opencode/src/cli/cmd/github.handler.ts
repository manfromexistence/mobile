...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: {
  ...
  yield* Effect.promise(async () => {
    ...
    const isCommentEvent = ["issue_comment", "pull_request_review_comment"].includes(
      context.eventName,
    );
    ...
    const gitStatus = (args: string[]) =>
      Effect.runPromise(gitSvc.run(args, { cwd: ctx.worktree }));
    ...
    async function checkoutForkBranch(pr: GitHubPullRequest) {
      ...
      await gitRun([
        ...
        "add",
        "fork",
        `https://github.com/${pr.headRepository.nameWithOwner}.git`,
      ]);
