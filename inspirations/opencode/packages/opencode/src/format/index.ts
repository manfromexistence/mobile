    ...
    const state = yield* InstanceState.make(
      Effect.fn("Format.state")(function* (ctx) {
        ...
        async function getFormatter(ext: string) {
          const matching = Object.values(formatters).filter((item) =>
        ...
        if (cfg.formatter !== true) {
          for (const [name, item] of Object.entries(cfg.formatter)) {
            ...
            if (
              ["ruff", "uv"].includes(name) &&
              (cfg.formatter.ruff?.disabled || cfg.formatter.uv?.disabled)
            ) {
            ...
            formatters[name] = {
              ...
              enabled:
                builtIn && !info.command
                  ? builtIn.enabled
                  : async (_context) => info.command ?? false,
            };
          }
        }
