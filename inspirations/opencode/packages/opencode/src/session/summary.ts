...
export interface Interface {
  ...
  readonly diff: (input: { sessionID: SessionID; messageID?: MessageID }) => Effect.Effect<
    Snapshot.FileDiff[]
  >;
  readonly computeDiff: (input: { messages: SessionV1.WithParts[] }) => Effect.Effect<
    Snapshot.FileDiff[]
  >;
}
  ...
  Effect.gen(function* () {
    ...
    const diff = Effect.fn("SessionSummary.diff")(function* (input: {
      ...
      const message = (yield* sessions
        ...
        .pipe(Effect.orDie)).find((item) => item.info.id === input.messageID);
