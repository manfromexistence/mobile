        ...
        return run.promise(
          Effect.gen(function* () {
            ...
            const content = JSON.stringify(
              { resourceTemplates: filtered.map(formatMcpResourceTemplate) },
              null,
              2,
            );
            const truncated = yield* truncate.output(content, {}, input.agent);
      ...
      execute(args, opts) {
        return run.promise(
          Effect.gen(function* () {
            ...
            if (!content)
              throw new Error(`Failed to read MCP resource: ${parsed.server}/${parsed.uri}`);
    ...
    item.execute = (args, opts) =>
      run.promise(
        Effect.gen(function* () {
          ...
          const result: Awaited<ReturnType<NonNullable<typeof execute>>> = yield* Effect.gen(
            function* () {
...
function formatMcpResource(resource: MCP.Resource) {
  const result = Object.fromEntries(
    Object.entries(resource).filter((entry) => entry[0] !== "client"),
  );
...
function formatMcpResourceTemplate(template: Record<string, unknown> & { client: string }) {
  const result = Object.fromEntries(
    Object.entries(template).filter((entry) => entry[0] !== "client"),
  );
