...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("SessionProcessor.create")(function* (input: Input) {
      ...
      const isFilePart = (value: unknown): value is SessionV1.FilePart =>
        Schema.is(SessionV1.FilePart)(value);
      ...
      const toolResultOutput = (
        ...
        if (isRecord(value.result.value) && typeof value.result.value.output === "string") {
          return {
            title:
              typeof value.result.value.title === "string" ? value.result.value.title : value.name,
        ...
        return {
          title: value.name,
          metadata:
            value.result.type === "json" && isRecord(value.result.value) ? value.result.value : {},
          output:
      ...
      const cleanup = Effect.fn("SessionProcessor.cleanup")(function* () {
        ...
        for (const toolCallID of Object.keys(ctx.toolcalls)) {
          ...
          const end = Date.now();
          const metadata =
            "metadata" in part.state && isRecord(part.state.metadata) ? part.state.metadata : {};
          yield* session.updatePart({
            ...part,
            state: {
              ...part.state,
