...
export interface Interface {
  ...
  readonly fork: (input: { sessionID: SessionID; messageID?: MessageID }) => Effect.Effect<
    Info,
    NotFound
  >;
  ...
  readonly messages: (input: { sessionID: SessionID; limit?: number }) => Effect.Effect<
    SessionV1.WithParts[],
    NotFound
  >;
...
> = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const patch = (sessionID: SessionID, info: Patch) =>
      Effect.gen(function* () {
        ...
        const next = {
          ...
          permission:
            info.permission === null ? undefined : (info.permission ?? current.permission),
        } as Info;
    ...
    const setMetadata = Effect.fn("Session.setMetadata")(function* (
      input: typeof SetMetadataInput.Type,
    ) {
