...
const contentPart = (part: unknown) => {
  if (!isRecord(part))
    throw new Error("Native LLM request adapter only supports object content parts");
...
const messages = (input: readonly ModelMessage[]) => {
  const system = input.flatMap((message) =>
    message.role === "system" ? [SystemPart.make(message.content)] : [],
  );
...
const baseURL = (input: Provider.Model | RequestInput) =>
  "model" in input
    ? (input.baseURL ?? (input.model.api.url || undefined))
    : input.api.url || undefined;

const requireBaseURL = (model: Provider.Model, url: string | undefined) => {
  if (url) return url;
  throw new Error(
    `Native LLM request adapter requires a base URL for ${model.providerID}/${model.id}`,
  );
};
...
export const model = (input: Provider.Model | RequestInput, headers?: Record<string, string>) => {
  ...
  const options = {
    ...
    ...(url ? { baseURL: url } : {}),
    headers:
      Object.keys({ ...model.headers, ...headers }).length === 0
        ? undefined
  ...
  if (model.api.npm === "@ai-sdk/azure")
    return Azure.configure({ ...options, baseURL: requireBaseURL(model, url) }).responses(
      model.api.id,
    );
  if (model.api.npm === "@ai-sdk/anthropic")
    return Anthropic.configure(options).model(model.api.id);
  if (model.api.npm === "@ai-sdk/google") return Google.configure(options).model(model.api.id);
  if (model.api.npm === "@ai-sdk/amazon-bedrock")
    return AmazonBedrock.configure(options).model(model.api.id);
  ...
  if (model.api.npm === "@openrouter/ai-sdk-provider")
    return OpenRouter.configure(options).model(model.api.id);
