...
export function toLLMEvents(
  ...
  switch (event.type) {
    ...
    case "tool-error":
      return Effect.sync(() => {
        const name =
          state.toolNames[event.toolCallId] ?? ("toolName" in event ? event.toolName : "unknown");
        delete state.toolNames[event.toolCallId];
        return [
          LLMEvent.toolError({
            id: event.toolCallId,
    ...
    case "raw":
      return Effect.sync(() => {
        state.copilotTotalNanoAiu =
          copilotTotalNanoAiu(event.rawValue) ?? state.copilotTotalNanoAiu;
        return [];
      });
