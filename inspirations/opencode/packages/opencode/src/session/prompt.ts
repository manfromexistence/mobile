  ...
  Effect.gen(function* () {
    ...
    const title = Effect.fn("SessionPrompt.ensureTitle")(function* (input: {
      ...
      const subtasks = firstUser.parts.filter(
        (p): p is SessionV1.SubtaskPart => p.type === "subtask",
      );
      const onlySubtasks =
    ...
    const createUserMessage = Effect.fn("SessionPrompt.createUserMessage")(function* (
      ...
      ) => Effect.Effect<Draft<SessionV1.Part>[]> = Effect.fn("SessionPrompt.resolveUserPart")(
        function* (part) {
          if (part.type === "file") {
              ...
              if (Exit.isSuccess(exit)) {
                ...
                for (const c of items) {
                  ...
                  if ("text" in c && typeof c.text === "string" && c.text) {
                    ...
                    const filename =
                      "uri" in c && typeof c.uri === "string" ? c.uri : part.filename;
    ...
    const runLoop: (sessionID: SessionID) => Effect.Effect<SessionV1.WithParts> = Effect.fn(
      "SessionPrompt.run",
        ...
        if (
          ...
          const orphan = lastAssistantMsg?.parts.find(
            (part): part is SessionV1.ToolPart =>
              part.type === "tool" && isOrphanedInterruptedTool(part),
          );
          if (orphan) {
    ...
    const loop: (input: LoopInput) => Effect.Effect<SessionV1.WithParts> = Effect.fn(
      "SessionPrompt.loop",
    ...
    const shell: (input: ShellInput) => Effect.Effect<SessionV1.WithParts, Session.BusyError> =
    ...
    const command = Effect.fn("SessionPrompt.command")(function* (input: CommandInput) {
      ...
      if (shellMatches.length > 0) {
        ...
        const results = yield* Effect.promise(() =>
          Promise.all(
            shellMatches.map(
              async ([, cmd]) => (await Process.text([cmd], { shell: sh, nothrow: true })).text,
            ),
          ),
        );
        let index = 0;
