...
export const projectHandlers = HttpApiBuilder.group(InstanceHttpApi, "project", (handlers) =>
  Effect.gen(function* () {
    ...
    const initGit = Effect.fn("ProjectHttpApi.initGit")(function* () {
      ...
      if (
        next.id === ctx.project.id &&
        next.vcs === ctx.project.vcs &&
        next.worktree === ctx.project.worktree
      )
        return next;
    ...
    const directories = Effect.fn("ProjectHttpApi.directories")(
      (ctx: { params: { projectID: ProjectV2.ID } }) =>
