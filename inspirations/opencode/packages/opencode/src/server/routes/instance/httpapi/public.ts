...
function matchLegacyOpenApi(input: Record<string, unknown>) {
  ...
  for (const [path, item] of Object.entries(spec.paths ?? {})) {
    for (const method of ["get", "post", "put", "delete", "patch"] as const) {
      ...
      if (operation.requestBody) {
        ...
        if (path === "/experimental/workspace" && method === "post") {
          ...
          if (properties?.branch)
            properties.branch = { anyOf: [properties.branch, { type: "null" }] };
      ...
      if (
        (path === "/event" || path === "/global/event" || path === "/api/event") &&
        method === "get"
      ) {
...
function applyLegacySchemaOverrides(spec: OpenApiSpec) {
  ...
  if (schemas.Command?.properties?.template)
    schemas.Command.properties.template = { type: "string" };
  ...
  const variants =
    typeof model === "object" ? model.properties?.variants?.additionalProperties : undefined;
...
function nullable(schema: OpenApiSchema): OpenApiSchema {
  if (flattenOptions(schema.anyOf ?? schema.oneOf)?.some((item) => item.type === "null"))
    return schema;
...
function normalizeLegacyErrorResponses(operation: OpenApiOperation) {
  ...
  if (
    ...
    isBuiltInErrorResponse(operation.responses["404"], "NotFound")
  ) {
...
function normalizeLegacyOperation(operation: OpenApiOperation, path: string, method: string) {
  ...
  if (
    (path !== "/session/{sessionID}/message" && path !== "/session/{sessionID}/command") ||
    method !== "post"
  )
    return;
...
function isLegacyBadRequestResponse(response: OpenApiResponse) {
  return (
    isBuiltInErrorResponse(response, "BadRequest") || isRefResponse(response, "InvalidRequestError")
  );
}
...
function isEmptyObjectUnion(schema: OpenApiSchema) {
  ...
  return (
    options?.length === 2 && options.some(isBareObjectSchema) && options.some(isBareArraySchema)
  );
}
