...
export const ptyHandlers = HttpApiBuilder.group(InstanceHttpApi, "pty", (handlers) =>
  Effect.gen(function* () {
    ...
    const unregister = registerDisposer((directory) =>
      Effect.runPromise(
        locations.invalidate(Location.Ref.make({ directory: AbsolutePath.make(directory) })),
      ),
    );
    ...
    const create = Effect.fn("PtyHttpApi.create")(function* (ctx: {
      payload: typeof Pty.CreateInput.Type;
    }) {
    ...
    const connectToken = Effect.fn("PtyHttpApi.connectToken")(function* (ctx: {
      ...
      if (
        ...
        return yield* new ApiError.PtyForbiddenError({
  ...
  Effect.gen(function* () {
    ...
    return handlers.handleRaw(
      ...
      Effect.fn("PtyHttpApi.connect")(function* (ctx: {
        ...
        const query = Schema.decodeUnknownOption(CursorQuery)(
          yield* HttpServerRequest.ParsedSearchParams,
        );
        if (Option.isNone(query)) return HttpServerResponse.empty({ status: 400 });
        const ticket = new URL(ctx.request.url, "http://localhost").searchParams.get(
          PTY_CONNECT_TICKET_QUERY,
        );
        if (ticket) {
        ...
        const parsedCursor =
          query.value.cursor === undefined ? undefined : Number(query.value.cursor);
        const cursor =
        ...
        const registered = yield* WebSocketTracker.register(
          write(WebSocketTracker.SERVER_CLOSING_EVENT()),
        );
        if (!registered) {
        ...
        const attachment = yield* pty(
          ...
          Effect.catchTags({
            "Pty.NotFoundError": () =>
              closeAccepted(new Socket.CloseEvent(4404, "session not found")).pipe(
                Effect.as(undefined),
              ),
            "Pty.ExitedError": () =>
              closeAccepted(new Socket.CloseEvent(4404, "session not found")).pipe(
