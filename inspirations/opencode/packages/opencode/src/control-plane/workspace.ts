...
export class SyncHttpError extends Schema.TaggedErrorClass<SyncHttpError>()(
...
export class SyncTimeoutError extends Schema.TaggedErrorClass<SyncTimeoutError>()(
...
export class SyncAbortedError extends Schema.TaggedErrorClass<SyncAbortedError>()(
  "WorkspaceSyncAbortedError",
  {
    message: Schema.String,
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
            ...
            Effect.gen(function* () {
              ...
              if (payload.type === "sync" && payload.syncEvent) {
                const failed = yield* events
                  .replay(payload.syncEvent, { publish: true, ownerID: space.id })
                  .pipe(
                    Effect.as(false),
      ...
      const discovered = yield* Effect.forEach(
        ...
        ([type, adapter]) =>
          WorkspaceAdapterRuntime.list(adapter).pipe(
            Effect.catchCause((error) =>
              Effect.logWarning("workspace adapter list failed", { type, error }).pipe(
                Effect.as([]),
              ),
            ),
          ),
    ...
    const remove = Effect.fn("Workspace.remove")(function* (id: WorkspaceV2.ID) {
      ...
      yield* Effect.forEach(
        sessions.filter(
          (sessionInfo) => !sessionInfo.parentID || !sessionIDs.has(sessionInfo.parentID),
        ),
        (sessionInfo) =>
          session
            .remove(sessionInfo.id)
...
function waitUntilSynced(input: {
  ...
  return Effect.suspend(() =>
    waitEvent({
      ...
      Effect.flatMap(
        (done): Effect.Effect<void, unknown> => (done ? Effect.void : waitUntilSynced(input)),
      ),
    ),
  );
}
...
function synced(
  ...
  return db
    ...
    .pipe(
      Effect.orDie,
      Effect.map((rows) => {
        const done = Object.fromEntries(rows.map((row) => [row.id, row.seq])) as Record<
          string,
          number
        >;
