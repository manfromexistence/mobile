...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
    ...
    Effect.gen(function* () {
      ...
      const validated = parsed.attention?.sounds
        ? {
            ...
            attention: {
              ...parsed.attention,
              sounds: resolveHostAttentionSoundPaths(
                path.dirname(configFilepath),
                parsed.attention.sounds,
              ),
            },
          }
        : parsed;
  ...
  const mergeFile = (acc: Acc, file: string) =>
    Effect.gen(function* () {
      ...
      const plugins = ConfigPlugin.deduplicatePluginOrigins([
        ...acc.plugin_origins,
        ...data.plugin.map((spec) => ({
          spec: spec as ConfigPlugin.Origin["spec"],
          scope,
          source: file,
        })),
      ]);
  ...
  const dirs = unique(directories).filter(
    (dir) => dir.endsWith(".opencode") || dir === Flag.OPENCODE_CONFIG_DIR,
  );
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const pluginOrigins = Effect.fn("TuiConfig.pluginOrigins")(() =>
      Effect.succeed(data.pluginOrigins),
    );
