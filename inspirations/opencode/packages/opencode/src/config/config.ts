  ...
  Effect.gen(function* () {
    ...
    const fetchRemoteJson = Effect.fnUntraced(function* <S extends Schema.Top>(
      ...
      const response = yield* HttpClient.filterStatusOk(withTransientReadRetry(http))
        ...
        .pipe(
          Effect.catch((error) =>
            Effect.die(new Error(`failed to fetch remote config from ${url}: ${String(error)}`)),
          ),
        );
      const body = yield* response.text.pipe(
        Effect.catch((error) =>
          Effect.die(new Error(`failed to read remote config from ${url}: ${String(error)}`)),
        ),
      );
      ...
      return yield* Schema.decodeEffect(Schema.fromJsonString(schema))(body).pipe(
        Effect.catch((error) =>
          Effect.die(new Error(`failed to decode remote config from ${url}: ${String(error)}`)),
        ),
      );
    });
    ...
    const loadGlobal = Effect.fnUntraced(function* (env?: Record<string, string>) {
      ...
      result = mergeConfig(
        result,
        yield* loadFile(path.join(Global.Path.config, "config.json"), env),
      );
      result = mergeConfig(
        result,
        yield* loadFile(path.join(Global.Path.config, "opencode.json"), env),
      );
      result = mergeConfig(
        result,
        yield* loadFile(path.join(Global.Path.config, "opencode.jsonc"), env),
      );
    ...
    const loadInstanceState = Effect.fn("Config.loadInstanceState")(
      function* (ctx: InstanceContext) {
        ...
        for (const [key, value] of Object.entries(auth)) {
          if (value.type === "wellknown") {
            ...
            const remoteConfig = mergeConfig(
              isRecord(wellknown.config) ? wellknown.config : {},
              fetchedConfig,
            );
        ...
        if (!Flag.OPENCODE_DISABLE_PROJECT_CONFIG) {
          for (const file of yield* ConfigPaths.files("opencode", ctx.directory, ctx.worktree).pipe(
            Effect.orDie,
          )) {
        ...
        if (Flag.OPENCODE_CONFIG_DIR) {
          yield* Effect.logDebug("loading config from OPENCODE_CONFIG_DIR", {
        ...
        if (Flag.OPENCODE_PERMISSION) {
          try {
            ...
            yield* Effect.logWarning("OPENCODE_PERMISSION contains invalid JSON, skipping", {
              err,
            });
          }
        }
    ...
    const update = Effect.fn("Config.update")(function* (config: Info) {
      ...
      yield* fs
        .writeFileString(
          file,
          JSON.stringify(mergeDeep(writable(existing), writable(config)), null, 2),
        )
        .pipe(Effect.orDie);
    });
