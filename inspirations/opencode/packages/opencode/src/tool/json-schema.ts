  ...
  if (Array.isArray(schema.anyOf)) {
    ...
    const nonFinite = withoutNull.filter(
      (item) =>
        ...
        item.enum.every((entry) => isNonFiniteNumber(entry)),
    );
...
function isEmptyStructUnion(items: unknown[]) {
  return (
    items.length === 2 &&
    items.some(
      (item) => isRecord(item) && item.type === "object" && item.properties === undefined,
    ) &&
