...
const layer: Layer.Layer<Service, never, FSUtil.Service | AppProcess.Service | Config.Service> =
  Layer.effect(
    Service,
    Effect.gen(function* () {
      ...
      const state = yield* InstanceState.make<State>(
        Effect.fn("Snapshot.state")(function* (ctx) {
          ...
          const ignore = Effect.fnUntraced(function* (files: string[]) {
            ...
            const checkIgnorePaths = files.map((item) =>
          ...
          const seed = Effect.fnUntraced(function* () {
            ...
            if (yield* exists(sourceIndex)) {
              yield* fs
                .copyFile(sourceIndex, path.join(state.gitdir, "index"))
          ...
          const add = Effect.fnUntraced(function* () {
            ...
            if (ignored.size > 0) {
              ...
              yield* Effect.logInfo("removing gitignored files from snapshot", {
            ...
            return yield* locked(
              Effect.gen(function* () {
                ...
                const single = Effect.fnUntraced(function* (op: (typeof ops)[number]) {
                  ...
                  const result = yield* git(
                    [...core, ...args(["checkout", op.hash, "--", op.file])],
                    {
                      cwd: state.worktree,
                    },
                  );
                ...
                for (let i = 0; i < ops.length; ) {
                  ...
                  if (tree.code !== 0) {
                    yield* Effect.logInfo(
                      "batched ls-tree failed, falling back to single-file revert",
                      {
                        hash: first.hash,
                        files: run.length,
                      },
                  ...
                  if (list.length) {
                    ...
                    const result = yield* git(
                      [
                        ...core,
                        ...args(["checkout", first.hash, "--", ...list.map((item) => item.file)]),
                      ],
                      {
                        cwd: state.worktree,
                      },
                    ...
                    if (result.code !== 0) {
                      yield* Effect.logInfo(
                        "batched checkout failed, falling back to single-file revert",
                        {
                          hash: first.hash,
                          files: list.length,
                        },
              ...
              Effect.gen(function* () {
                ...
                const show = Effect.fnUntraced(function* (row: Row) {
                  ...
                  return yield* Effect.all(
                    [
                      git([...cfg, ...args(["show", `${from}:${row.file}`])]).pipe(
                        Effect.map((item) => item.text),
                      ),
                      git([...cfg, ...args(["show", `${to}:${row.file}`])]).pipe(
                        Effect.map((item) => item.text),
                      ),
                    ],
                    { concurrency: 2 },
                ...
                const load = Effect.fnUntraced(
                  ...
                  Effect.scoped,
                  Effect.catch(() =>
                    Effect.succeed<Map<string, { before: string; after: string }> | undefined>(
                      undefined,
                    ),
                  ),
                );
          ...
          yield* cleanup().pipe(
            Effect.catchCause((cause) =>
              Effect.logError("cleanup loop failed", { cause: Cause.pretty(cause) }),
            ),
