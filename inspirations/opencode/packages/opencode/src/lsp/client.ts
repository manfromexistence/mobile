...
export class InitializeError extends Schema.TaggedErrorClass<InitializeError>()(
  "LSPInitializeError",
  {
...
export async function create(input: {
  ...
  function documentPullState() {
    ...
    return {
      documentIdentifiers: [
        ...new Set(
          documentRegistrations.flatMap(
            (registration) => registration.registerOptions?.identifier ?? [],
          ),
        ),
      ],
  ...
  function workspacePullState() {
    ...
    return {
      workspaceIdentifiers: [
        ...new Set(
          workspaceRegistrations.flatMap(
            (registration) => registration.registerOptions?.identifier ?? [],
          ),
        ),
      ],
      ...
      async open(request: { path: string }) {
        request.path = Filesystem.normalizePath(
          path.isAbsolute(request.path)
            ? request.path
            : path.resolve(input.directory, request.path),
        );
