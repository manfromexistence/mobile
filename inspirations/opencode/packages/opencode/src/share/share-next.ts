  ...
  Effect.gen(function* () {
    ...
    const state: InstanceState.InstanceState<State> = yield* InstanceState.make<State>(
      Effect.fn("ShareNext.state")(function* (_ctx) {
        ...
        const watch = <D extends EventV2.Definition>(
          ...
          events.listen((event) => {
            if (event.type !== def.type || event.location?.directory !== _ctx.directory)
              return Effect.void;
        ...
        yield* watch(MessageV2.Event.Updated, (data) =>
          Effect.gen(function* () {
            ...
            yield* sync(info.sessionID, [
              { type: "message", data: structuredClone(info) as SDK.Message },
            ]);
    ...
    const full = Effect.fn("ShareNext.full")(function* (sessionID: SessionID) {
      ...
      const models = yield* Effect.forEach(
        ...
        (item) =>
          provider.getModel(ProviderV2.ID.make(item.providerID), ModelV2.ID.make(item.modelID)),
        { concurrency: 8 },
      );
      ...
      yield* sync(sessionID, [
        ...
        ...messages.flatMap((item) =>
          item.parts.map((part) => ({ type: "part" as const, data: part })),
        ),
