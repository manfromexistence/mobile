...
function connectionLimitError(event: Record<string, unknown>) {
  if (
    ...
    return;
  return new Error(
    typeof event.error.message === "string" ? event.error.message : CONNECTION_LIMIT_REACHED_CODE,
  );
}
...
export function withoutInternalHeaders<T extends { headers?: HeadersInit }>(
  ...
  if (Array.isArray(init.headers)) {
    return {
      ...init,
      headers: init.headers.filter((item) => item[0].toLowerCase() !== TITLE_HEADER),
    };
  }
  ...
  return {
    ...
    headers: Object.fromEntries(
      Object.entries(init.headers).filter(([key]) => key.toLowerCase() !== TITLE_HEADER),
    ),
  };
}
