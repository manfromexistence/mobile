...
function imgMsg(msg: any): boolean {
  ...
  return content.some(
    (part: any) =>
      ...
      part.text === MessageV2.SYNTHETIC_ATTACHMENT_PROMPT,
  );
}
      ...
      async models(provider, ctx) {
        if (ctx.auth?.type !== "oauth") {
          ...
          return Object.fromEntries(
            Object.entries(provider.models).map(([id, model]) => [id, fix(model, base())]),
          );
        }
          ...
          .catch((error) => {
            ...
            return Object.fromEntries(
              Object.entries(provider.models).map(([id, model]) => [
                id,
                fix(model, base(auth.enterpriseUrl)),
              ]),
            );
          });
      },
              ...
              try {
                ...
                if (body?.messages) {
                  ...
                  const hasNonToolCalls =
                    ...
                    last.content.some((part: any) => part?.type !== "tool_result");
                  return {
                    isVision: body.messages.some(
                      (item: any) =>
                        Array.isArray(item?.content) &&
