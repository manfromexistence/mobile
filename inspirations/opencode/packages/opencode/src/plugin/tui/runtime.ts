...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const mtime = stat
      ? Math.floor(typeof stat.mtimeMs === "bigint" ? Number(stat.mtimeMs) : stat.mtimeMs)
      : undefined;
...
async function syncPluginThemes(plugin: PluginEntry) {
  ...
  const install = createThemeInstaller(
    plugin.load.origin,
    ...
    finish: async (loaded, origin, retry) => {
      const mod = await Promise.resolve()
        .then(
          () =>
            readV1Plugin(
              loaded.mod as Record<string, unknown>,
              loaded.spec,
              "tui",
            ) as TuiPluginModule,
        )
...
async function addPluginBySpec(state: RuntimeState | undefined, raw: string) {
  ...
  const ready = await resolveExternalPlugins([cfg], () => TuiConfig.waitForDependencies()).catch(
    (error) => {
...
export async function init(input: {
  ...
  if (loaded) {
    if (dir !== cwd) {
      throw new Error(
        `TuiPluginRuntime.init() called with a different working directory. expected=${dir} got=${cwd}`,
      );
    }
    return loaded;
  }
...
export async function dispose() {
  ...
  if (task)
    await task.catch((error) =>
      fail("failed to finish loading tui plugins during disposal", { error }),
    );
  const state = runtime;
  runtime = undefined;
  if (!state) return;
