      ...
      Effect.fn("Plugin.state")(function* (ctx) {
            ...
            report: {
              ...
              error(candidate, _retry, stage, error, resolved) {
                ...
                if (stage === "install") {
                  ...
                  publishPluginError(
                    `Failed to install plugin ${parsed.pkg}@${parsed.version}: ${message}`,
                  );
                  return;
                }
        ...
        for (const load of loaded) {
          ...
          yield* Effect.tryPromise({
            ...
            Effect.tapError((error) =>
              Effect.logError("failed to load plugin", { path: load.spec, error }),
            ),
            Effect.catch(() => {
        ...
        const unsubscribe = yield* events.listen((event) => {
          ...
          return Effect.sync(() => {
            for (const hook of hooks) {
              void hook["event"]?.({
                event: { id: event.id, type: event.type, properties: event.data } as any,
              });
            }
          });
        });
