...
async function startOAuthServer(): Promise<void> {
  ...
  oauthServer = createServer((req, res) => {
    ...
    if (req.method === "GET" && url.pathname === OAUTH_REDIRECT_PATH) {
      ...
      res.end(
        OauthCallbackPage.bootstrap({ tokenPath: OAUTH_TOKEN_PATH, provider: "DigitalOcean" }),
      );
      return;
    }
  ...
  try {
    ...
    return parsed.flatMap((r) =>
      r && typeof r.name === "string"
        ? [{ name: r.name, uuid: r.uuid, description: r.description }]
        : [],
    );
  } catch {
    return [];
