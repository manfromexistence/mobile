...
export function createTuiAttention(input: {
  ...
  function soundCandidates(name: TuiAttentionSoundName) {
    return [
      input.config.attention.sounds[name],
      currentPack().sounds[name],
      BUILTIN_PACK.sounds[name],
    ].filter(
      (item, index, list): item is string =>
        typeof item === "string" && list.indexOf(item) === index,
    );
  }
    ...
    async notify(request) {
      try {
        ...
        const requestedNotification =
          typeof request.notification === "object" ? request.notification : undefined;
        const notificationSkip = focusSkip(requestedNotification?.when ?? "blurred", focus);
        const notificationRequested =
          input.config.attention.notifications && request.notification !== false;
        ...
        const soundSkip =
          volume === undefined ? undefined : focusSkip(requestedSound?.when ?? "always", focus);
        const soundName =
          requestedSound?.name && Schema.is(AttentionSoundName)(requestedSound.name)
            ? requestedSound.name
            : "default";
        const sound =
          volume === undefined || soundSkip ? false : await playSound(soundName, volume);
