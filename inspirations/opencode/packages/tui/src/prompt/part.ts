...
export function stripPromptPartIDs<
  Part extends { id: string; messageID: string; sessionID: string },
>(part: Part) {
...
function isPastedTextPart(
  part: unknown,
): part is { type: "text"; text: string; source: { text: { value: string } } } {
  ...
  return Boolean(
    text && typeof text === "object" && "value" in text && typeof text.value === "string",
  );
}

export function expandTrackedPastedText(
  text: string,
  ...
  return ranges
    ...
    .reduce(
      (result, part) =>
        displaySlice(result, 0, part.start) + part.text + displaySlice(result, part.end),
      text,
    );
}
