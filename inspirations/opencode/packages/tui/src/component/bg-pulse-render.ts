...
export class GoUpsellArtPainter {
  ...
  private rebuildGeometry(frameBuffer: OptimizedBuffer, rgb: boolean) {
    ...
    if (geometryChanged) {
      ...
      this.logoY = Math.max(
        0,
        Math.min(
          Math.max(0, height - LOGO_HEIGHT),
          Math.round((height - LOGO_HEIGHT) / 2) + LOGO_TOP_BIAS,
        ),
      );
      ...
      this.reach =
        Math.hypot(Math.max(centerX, width - centerX), Math.max(centerY, height - centerY) * 2) +
        TAIL;
  ...
  private drawCached(frameBuffer: OptimizedBuffer, rgb: boolean) {
    ...
    const frame =
      this.frameCache[Math.floor((this.elapsed / PERIOD) * CACHE_FRAME_COUNT) % CACHE_FRAME_COUNT];
    if (frame) {
