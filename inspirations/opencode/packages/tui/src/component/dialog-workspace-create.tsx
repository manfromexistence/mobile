...
export function recentConnectedWorkspaces<
...
export async function openWorkspaceSelect(input: {
  ...
  input.dialog.replace(() => (
    <DialogWorkspaceSelect adapters={adapters} onSelect={input.onSelect} />
  ));
}
...
export async function confirmWorkspaceFileChanges(input: {
  ...
  const status = await input.sdk.client.vcs
    .status({ workspace: input.sourceWorkspaceID })
    .catch(() => undefined);
...
export function DialogWorkspaceSelect(props: {
  ...
  const omittedWorkspaceID = createMemo(() =>
    route.data.type === "session" ? project.workspace.current() : undefined,
  );
