...
export function DialogSessionList() {
    ...
    dialog.replace(() => (
      <DialogSessionDeleteFailed
        ...
        onDelete={async () => {
          ...
          const result = await sdk.client.experimental.workspace.remove({
  ...
  const options = createMemo(() => {
    ...
    const current = currentSessionID();
    const displayOrder =
      current && sessionMap.has(current) && !order.includes(current) ? [...order, current] : order;
          ...
          onTrigger: async (option) => {
            if (toDelete() === option.value) {
              ...
              const status = session?.workspaceID
                ? project.workspace.status(session.workspaceID)
                : undefined;
