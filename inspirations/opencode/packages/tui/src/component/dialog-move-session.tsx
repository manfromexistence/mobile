...
export function DialogMoveSession(props: DialogMoveSessionProps) {
  ...
  const currentCheckout = createMemo(() => {
    if (projectContext.project() === props.projectID)
      return projectContext.instance.path().worktree;
  ...
  const options = createMemo<DialogSelectOption<MoveSessionSelection | undefined>[]>(() => {
    ...
    if (directories.loading && !data && !current)
      return [{ title: "Loading project directories...", value: undefined }];
    const roots = [...(data ?? [])];
    if (current && !roots.some((item) => item.directory === current))
      roots.unshift({ directory: current });
    roots.sort((a, b) => {
    ...
    const subdirectories = sync.data.session
      ...
      .filter(
        (item): item is { location: string; root: ProjectDirectory } => item.root !== undefined,
      );

    const list = [
      ...roots.map((root) => ({ location: root.directory, root })),
      ...subdirectories,
    ].toSorted((a, b) => {
  ...
  const current = createMemo(() => {
    ...
    if (replacement)
      return { type: "directory", directory: replacement, subdirectory: false } as const;
            ...
            : [
                ...
                },
                {
                  ...
                  disabled: (option) => {
                    ...
                    return !directoryData()?.find((item) => item.directory === value.directory)
                      ?.strategy;
                  },
                  onTrigger: remove,
                },
