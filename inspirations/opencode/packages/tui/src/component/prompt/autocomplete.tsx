...
function extractLineRange(input: string) {
  ...
  const endLine =
    lineMatch[2] && startLine < Number(lineMatch[2]) ? Number(lineMatch[2]) : undefined;
...
export function Autocomplete(props: {
  ...
  const options = createMemo((prev: AutocompleteOption[] | undefined) => {
    ...
    if (store.visible === "@" && referenceMatchValue) {
      return referenceAliasesValue.filter(
        (item) => item.display === `@${referenceMatchValue.name}`,
      );
    }
      ...
      onInput(value) {
        if (store.visible) {
          if (
            ...
            props
              .input()
              .getTextRange(store.index, props.input().cursorOffset)
              .match(/\s/) ||
