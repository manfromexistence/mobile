...
function getEditorRangeLabel(selection: EditorSelection["ranges"][number]) {
  ...
  if (selection.selection.start.line === selection.selection.end.line)
    return `#${selection.selection.start.line}`;
...
export function Prompt(props: PromptProps) {
  ...
  const usage = createMemo(() => {
    ...
    const model = sync.data.provider.find((item) => item.id === last.providerID)?.models[
      last.modelID
    ];
  ...
  useBindings(() => {
    return {
      ...
      enabled: (() => {
        cursorVersion();
        return (
          inputTarget() !== undefined && store.mode === "shell" && input?.visualCursor.offset === 0
        );
      })(),
      bindings: [
        {
  ...
  useBindings(() => {
    return {
      ...
      enabled: (() => {
        cursorVersion();
        return (
          inputTarget() !== undefined && !props.disabled && !auto()?.visible && input !== undefined
        );
      })(),
      commands: [
        {
  ...
  async function submitInner() {
    ...
    const workspaceStatus = workspaceID
      ? (project.workspace.status(workspaceID) ?? "error")
      : undefined;
    ...
    if (sessionID == null) {
      ...
      const workspaceID =
        selectedWorkspace?.type === "existing" ? selectedWorkspace.workspaceID : undefined;
  ...
  const modelMetaAlpha = createFadeIn(
    () => !!local.agent.current() && store.mode === "normal",
    animationsEnabled,
  );
              ...
              ref={(r: TextareaRenderable) => {
                ...
                Object.assign(r, {
                  getClipboardText: (text: string) =>
                    expandPastedTextPlaceholders(text, store.prompt.parts),
                });
                setInputTarget(r);
                      ...
                      <Show when={store.mode === "normal"}>
                        <box flexDirection="row" gap={1}>
                          ...
                          <text
                            flexShrink={0}
                            fg={fadeColor(
                              leader() ? theme.textMuted : theme.text,
                              modelMetaAlpha(),
                            )}
                          >
        ...
        <box width="100%" flexDirection="row" justifyContent="space-between">
                    ...
                    {(() => {
                      ...
                      if (item.type === "new") {
                        ...
                        return (
                          <>
                            Workspace{" "}
                            <span style={{ fg: theme.textMuted }}>(new {item.workspaceType})</span>
                          </>
                        );
                      }
                      return (
          ...
          <Show when={status().type !== "retry"}>
            <box gap={2} flexDirection="row">
              <Show
                when={editorContextLabelState() !== "none" ? editorFileLabelDisplay() : undefined}
              >
                {(file) => (
                  <text
