...
function DiffViewer(props: { api: TuiPluginApi }) {
  ...
  const [viewOverride, setViewOverride] = createSignal<DiffView | undefined>(
    storedView(props.api.kv.get(KV_VIEW)),
  );
  ...
  const [pendingPatchScrollFileIndex, setPendingPatchScrollFileIndex] = createSignal<
    number | undefined
  >();
  ...
  const currentPatchFileIndex = () => {
    ...
    return (
      entries.findLast((entry) => entry.contentY <= viewportContentY)?.fileIndex ??
      entries[0]?.fileIndex
    );
  };
  ...
  const jumpRelativePatchFile = (offset: number) => {
    ...
    const next = movePatchFileIndex(
      patchFileIndexes(),
      selectedFileIndex() ?? activePatchFileIndex(),
      offset,
    );
    if (singlePatch()) {
  ...
  const jumpRelativeHunk = (offset: -1 | 1) => {
    ...
    const selectedIndex =
      selected?.scrollTop === patchScroll.scrollTop
        ? hunks.findIndex(
            (hunk) =>
              hunk.fileIndex === selected.fileIndex && hunk.hunkIndex === selected.hunkIndex,
          )
        : -1;
    const next =
      selectedIndex !== -1
    ...
    setSelectedHunk({
      ...
      scrollTop: patchScroll.scrollTop,
    });
  };

  const highlightedPatchFileIndex = () =>
  ...
  const toggleSelectedFileTreeRow = () => {
    ...
    setExpandedFileNodes((expanded) =>
      toggleFileTreeDirectory(fileTree(), expanded, highlightedFileNode()),
    );
  };
                      ...
                      {(entry, index) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            >
                              <text fg={reviewed() ? theme().textMuted : theme().text}>
                                {entry.file.file}
                              </text>
                              <box flexGrow={1} />
                              ...
                              {(patch) => (
                                <box border={patchLeftBorder()} borderColor={theme().border}>
                                  <diff
                                    ...
                                    view={view()}
                                    filetype={
                                      reviewed() ? PLAIN_TEXT_FILETYPE : filetype(entry.file.file)
                                    }
                                    ...
                                    addedBg={
                                      reviewed() ? theme().backgroundElement : theme().diffAddedBg
                                    }
                                    removedBg={
                                      reviewed() ? theme().backgroundElement : theme().diffRemovedBg
                                    }
                                    addedSignColor={
                                      reviewed() ? theme().textMuted : theme().diffHighlightAdded
                                    }
                                    removedSignColor={
                                      reviewed() ? theme().textMuted : theme().diffHighlightRemoved
                                    }
    ...
    commands: [
      {
        ...
        run() {
          api.route.navigate(ROUTE, {
            mode: "git",
            sessionID:
              "params" in api.route.current ? api.route.current.params?.sessionID : undefined,
