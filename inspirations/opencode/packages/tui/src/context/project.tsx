  ...
  init: () => {
    ...
    async function sync() {
      ...
      batch(() => {
        ...
        setStore(
          "project",
          "mainDir",
          directories?.data?.findLast((item) => item.strategy === undefined)?.directory,
        );
      });
    }
    ...
    async function syncWorkspace() {
      ...
      const next = Object.fromEntries(
        (status?.data ?? []).map((item) => [item.workspaceID, item.status]),
      );
    ...
    sdk.event.on("event", (event) => {
      if (event.payload.type === "workspace.status") {
        setStore(
          "workspace",
          "status",
          event.payload.properties.workspaceID,
          event.payload.properties.status,
        );
      }
    });
