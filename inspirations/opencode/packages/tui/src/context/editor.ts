...
export const { use: useEditorContext, provider: EditorContextProvider } = createSimpleContext({
  ...
  init: (props: { integration?: EditorIntegration; WebSocketImpl?: typeof WebSocket }) => {
    ...
    const port =
      parsedPort && Number.isInteger(parsedPort) && parsedPort > 0 && parsedPort <= 65535
        ? parsedPort
        : undefined;
    const zedTerminal =
    ...
    const connect = () => {
      ...
      if (!connection) {
        ...
        zedSelection ??= editor
          ...
          .then((result) => {
            ...
            const decoded =
              result.type === "selection" ? decodeEditorSelection(result.selection) : Option.none();
      ...
      current.addEventListener("message", (event) => {
        ...
        const selection =
          message.method === "selection_changed"
            ? decodeEditorSelection(message.params)
            : Option.none();
        ...
        const mention =
          message.method === "at_mentioned" ? decodeEditorMention(message.params) : Option.none();
        ...
        const initialize =
          method === "initialize" ? decodeEditorServerInfo(message.result) : Option.none();
