      ...
      latestReasoning(assistant: SessionMessageAssistant | undefined, reasoningID: string) {
        return assistant?.content.findLast(
          (item): item is SessionMessageAssistantReasoning =>
            item.type === "reasoning" && item.id === reasoningID,
        );
      },
    };
    ...
    const result = {
      ...
      location: {
        ...
        async refresh(ref?: LocationRef) {
          const response = await sdk.client.v2.location.get(
        ...
        command: {
          ...
          async refresh(ref?: LocationRef) {
            const result = await sdk.client.v2.command.list(
              { location: locationQuery(ref) },
              { throwOnError: true },
            );
            const key = locationKey(result.data.location);
        ...
        model: {
          ...
          async refresh(ref?: LocationRef) {
            const result = await sdk.client.v2.model.list(
              { location: locationQuery(ref) },
              { throwOnError: true },
            );
            const key = locationKey(result.data.location);
        ...
        provider: {
          ...
          async refresh(ref?: LocationRef) {
            const result = await sdk.client.v2.provider.list(
              { location: locationQuery(ref) },
              { throwOnError: true },
            );
            const key = locationKey(result.data.location);
        ...
        skill: {
          ...
          async refresh(ref?: LocationRef) {
            const result = await sdk.client.v2.skill.list(
              { location: locationQuery(ref) },
              { throwOnError: true },
            );
            const key = locationKey(result.data.location);
