...
export function Session() {
  ...
  const [showAssistantMetadata, _setShowAssistantMetadata] = kv.signal(
  ...
  const [showGenericToolOutput, setShowGenericToolOutput] = kv.signal(
                          ...
                          return (
                            ...
                            >
                              ...
                              >
                                ...
                                <text fg={theme.textMuted}>
                                  <span style={{ fg: theme.text }}>{redoShortcut()}</span> or /redo
                                  to restore
                                </text>
        ...
        setPreLayoutSiblingMargin(el, (previous) => {
          return props.separate ||
            (previous instanceof BoxRenderable &&
              (previous.height > 1 || alwaysSeparate.has(previous)))
            ? 1
            : 0;
        });
      }}
  ...
  return (
    ...
    >
      ...
      <Show when={numberValue(props.metadata.count)}>
        ({numberValue(props.metadata.count)}{" "}
        {numberValue(props.metadata.count) === 1 ? "match" : "matches"})
      </Show>
    </InlineTool>
  );
}
  ...
  return (
    ...
    >
      ...
      <Show when={numberValue(props.metadata.matches)}>
        ({numberValue(props.metadata.matches)}{" "}
        {numberValue(props.metadata.matches) === 1 ? "match" : "matches"})
      </Show>
    </InlineTool>
  );
}
  ...
  return (
    ...
    >
      ...
      <Show when={numberValue(props.metadata.numResults)}>
        ({numberValue(props.metadata.numResults)} results)
      </Show>
    </InlineTool>
  );
}
...
function Task(props: ToolProps) {
  ...
  const content = createMemo(() => {
    ...
    if (isRunning() && retrying) {
      content.push(
        `↳ ${formatSubagentRetry(retrying.attempt, Locale.truncate(retrying.message, 80))}`,
      );
...
function ApplyPatch(props: ToolProps) {
  ...
  function title(file: {
    ...
    if (file.type === "move")
      return "# Moved " + pathFormatter.format(file.filePath) + " → " + file.relativePath;
...
export function parseApplyPatchFiles(value: unknown) {
  ...
  return value.flatMap((item) => {
    ...
    if (!type || !relativePath || !filePath || patch === undefined || deletions === undefined)
      return [];
    return [
      { type, relativePath, filePath, patch, deletions, movePath: stringValue(file.movePath) },
    ];
  });
}
...
export function parseDiagnostics(value: unknown, filePath: string) {
  ...
  return diagnostics
    .flatMap((item) => {
      ...
      if (diagnostic?.severity !== 1 || line === undefined || character === undefined || !message)
        return [];
