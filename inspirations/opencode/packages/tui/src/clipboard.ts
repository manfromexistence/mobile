...
export async function read() {
  ...
  if (platform() === "win32" || release().includes("WSL")) {
    ...
    const image = await command("powershell.exe", [
      "-NonInteractive",
      "-NoProfile",
      "-command",
      script,
...
function getCopyMethod() {
  return (copyMethod ??= (async () => {
    ...
    const native = copyCommand(platform(), Boolean(process.env.WAYLAND_DISPLAY), (name) =>
      Boolean(which(name)),
    );
    if (native?.[0] === "osascript") {
      return async (text: string) => {
        const escaped = text.replace(/\\/g, "\\\\").replace(/"/g, '\\"');
        await command("osascript", ["-e", `set the clipboard to "${escaped}"`]).catch(
          () => undefined,
        );
      };
    }
