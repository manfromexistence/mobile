...
describe("diff viewer file tree utilities", () => {
  test("builds a nested tree with deduplicated directories and file indexes", () => {
    ...
    expect(
      tree.nodes.filter((node) => node.kind === "directory" && node.name === "src"),
    ).toHaveLength(1);
    expect(
      tree.nodes.filter((node) => node.kind === "directory" && node.name === "config"),
    ).toHaveLength(1);
    expect(
      tree.nodes.filter((node) => node.kind === "directory" && node.name === "session"),
    ).toHaveLength(1);
    expect(
      tree.nodes
  ...
  test("sorts root-level files without creating directories", () => {
    ...
    expect(flattenFileTree(tree).map((row) => row.name)).toEqual([
      "alpha.ts",
      "beta.ts",
      "zeta.ts",
    ]);
  ...
  test("collapses expanded unary children under the first visible directory id", () => {
    ...
    const packages = tree.nodes.find(
      (node) => node.kind === "directory" && node.name === "packages",
    )!;

    expect(flattenFileTree(tree, new Set()).map((row) => row.name)).toEqual([
  ...
  test("moves selection across visible rows and clamps to bounds", () => {
    const rows = flattenFileTree(
      buildFileTree([{ file: "src/config/tui.ts" }, { file: "README.md" }]),
    );
  ...
  test("moves directory selection to first visible child", () => {
    const rows = flattenFileTree(
      buildFileTree([{ file: "src/config/tui.ts" }, { file: "src/session/index.ts" }]),
    );
  ...
  test("moves collapsed chain selection to first visible child", () => {
    ...
    const packages = rows.find(
      (row) => row.kind === "directory" && row.name === "packages/opencode/src",
    )!;
  ...
  test("selects a file tree node and expands its parents for a patch file", () => {
    ...
    expect([...selection!.expandedNodes].map((id) => tree.nodes[id]!.name)).toEqual([
      "session",
      "src",
    ]);
  ...
  test("toggles only selected directory expansion", () => {
    ...
    expect(flattenFileTree(tree, collapsed).map((row) => row.name)).toEqual([
      "src/config",
      "README.md",
    ]);
