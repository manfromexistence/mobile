...
test("defines package-owned plugin specs and attention sound names", () => {
  ...
  expect(decodePlugin(["example-plugin", { enabled: true }])).toEqual([
    "example-plugin",
    { enabled: true },
  ]);
...
test("validates config constraints", () => {
  ...
  expect(decodeInfo({ attention: { sounds: { unknown: "sound.wav" } } })).toEqual({
    attention: { sounds: {} },
  });
});
...
test("preserves an explicit undo binding when suspend is unsupported", () => {
  const config = resolve(
    { keybinds: { input_undo: "ctrl+u", terminal_suspend: "ctrl+s" } },
