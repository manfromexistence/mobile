  ...
  try {
    ...
    resolveMessages(
      json([
        {
          info: assistant,
          parts: [{ id: partID, sessionID, messageID, type: "text", text: "hydrated" }],
        },
      ]),
    );
    await hydrate;
...
test("hydration does not clear text streamed before it starts", async () => {
  ...
  try {
    ...
    await wait(
      () =>
        sync.data.part[messageID]?.[0]?.type === "text" && sync.data.part[messageID][0].text !== "",
    );
    const hydrate = sync.session.sync(sessionID);
    await wait(() => requested);
    resolveMessages(
      json([
        { info: assistant, parts: [{ id: partID, sessionID, messageID, type: "text", text: "" }] },
      ]),
    );
    await hydrate;
...
test("live messages merged during hydration retain the 100 message window", async () => {
  ...
  try {
    ...
    emit(
      global({ id: "evt_live", type: "message.updated", properties: { sessionID, info: live } }),
    );
    await wait(
      () => sync.data.message[sessionID]?.some((message) => message.id === live.id) ?? false,
    );
    resolveMessages(
      json(
...
test("a message removed during hydration does not regain stale parts", async () => {
  ...
  try {
    ...
    await wait(() => requested);
    emit(
      global({ id: "evt_removed", type: "message.removed", properties: { sessionID, messageID } }),
    );
