...
export function compile<Id extends string, Groups extends HttpApiGroup.Any>(
  ...
  HttpApi.reflect(api, {
    ...
    onEndpoint({ endpoint, errors, group, middleware }) {
      ...
      const sourcePayloads = Array.from(endpoint.payload.values()).flatMap(
        ({ schemas }) => schemas,
      );
      if (sourcePayloads.length > 1) {
        throw new GenerationError({ reason: `Multiple payload schemas: ${name}` });
      }
      const payloads = sourcePayloads.map(
        (schema) => normalizeTransport(schema, "payload", endpoint, name)!,
      );
      ...
      endpoints.push({
        ...
        operation: {
          ...
          inputMode:
            inputs.length === 0
              ? "none"
              : inputs.every((field) => field.optional)
                ? "optional"
                : "required",
  ...
  for (const group of groups) {
    ...
    for (const name of names) {
      if (publicNames.has(name))
        throw new GenerationError({ reason: `Client name collision: ${name}` });
...
export function emitEffect(contract: Contract): Output {
  const endpoint = contract.groups
    ...
    .find((endpoint) => !endpoint.effectPortable);
...
export function emitPromise(
  contract: Contract,
  options?: {
    readonly outputTypes?: Readonly<
      Record<string, { readonly name: string; readonly import: string }>
    >;
  },
): Output {
  ...
  return {
    ...
    files: [
      ...
      {
        path: "client.ts",
        content: renderPromiseClient(groups).replace(
...
function assertPromiseEndpoint(endpoint: Endpoint) {
  ...
  if (
    payload !== undefined &&
    (payloadEncoding?._tag ??
      (HttpMethod.hasBody(endpoint.endpoint.method) ? "Json" : "FormUrlEncoded")) !== "Json"
  ) {
  ...
  for (const error of endpoint.errors) {
    if (declaredErrorFields(error.schema) === undefined) {
      throw new GenerationError({
        reason: `Promise error must have a literal discriminator: ${name}`,
      });
    }
...
function renderImportedEffectFiles(
  ...
  const adapters = groups.map((group, groupIndex) => {
    const rawGroup = group.endpoints[0]?.topLevel
      ? "RawClient"
      : `RawClient[${JSON.stringify(group.sourceIdentifier)}]`;
  ...
  const usesStream = groups.some((group) =>
    group.endpoints.some((item) => item.operation.success === "stream"),
  );
  ...
  const httpApiImport =
    projection === undefined ? 'import { HttpApiClient } from "effect/unstable/httpapi"\n' : "";
    ...
    const fields = error.fields
      .map(
        ([name, schema, optional]) =>
          `readonly ${JSON.stringify(name)}${optional ? "?" : ""}: ${typeOf(schema)}`,
      )
      .join("; ");
      ...
      group.endpoints.flatMap((endpoint) => {
        ...
        return [
          ...(endpoint.operation.inputMode === "none"
            ? []
            : [`export type ${prefix}Input = { ${input} }`]),
...
function renderPromiseClient(groups: ReadonlyArray<Group>) {
  const imports = groups.flatMap((group) =>
    group.endpoints.flatMap((endpoint) => {
      const prefix = promiseTypePrefix(group.identifier, endpoint.operation.name);
      return [
        ...(endpoint.operation.inputMode === "none" ? [] : [`${prefix}Input`]),
        `${prefix}Output`,
      ];
    }),
  );
...
function structuralType(schema: Schema.Top) {
  ...
  if (
    document.artifacts.some(
      (artifact) =>
        artifact._tag !== "Import" ||
        artifact.importDeclaration !== 'import type * as Brand from "effect/Brand"',
    ) ||
  ...
  const expand = (type: string, seen = new Set<string>()): string => {
    for (const [reference, value] of references) {
      ...
      type = type.replace(
        ...
        `(${expand(value, new Set([...seen, reference]))})`,
      );
    }
    return type;
  };
...
function promisePath(path: string, input: ReadonlyArray<InputField>) {
  if (path.includes("*"))
    throw new GenerationError({ reason: `Unsupported Promise path wildcard: ${path}` });
  const fields = new Set(
    input.filter((field) => field.source === "params").map((field) => field.name),
  );
  ...
  const template = segments
    .map((segment) => {
      ...
      if (!fields.has(name))
        throw new GenerationError({ reason: `Missing path parameter: ${name}` });
...
function sameEncoding(left: SchemaAST.AST, right: SchemaAST.AST): boolean {
  ...
  if (
    left.encoding?.some((link, index) => {
      ...
      return (
        other === undefined ||
        link.transformation !== other.transformation ||
        !sameEncoding(link.to, other.to)
      );
    })
  )
    return false;
  if (!sameChecks(left.checks, right.checks) || !sameContext(left.context, right.context))
    return false;
  if (SchemaAST.isSuspend(left) && SchemaAST.isSuspend(right))
    return sameEncoding(left.thunk(), right.thunk());
  ...
  if (SchemaAST.isObjects(left) && SchemaAST.isObjects(right)) {
    return (
      ...
      left.propertySignatures.every((field, index) =>
        sameEncoding(field.type, right.propertySignatures[index].type),
      ) &&
...
function sameChecks(
  ...
  return left.every((check, index) => {
    ...
    return (
      ...
      sameChecks(check.checks, other.checks)
    );
  });
}
  ...
  return Effect.gen(function* () {
    ...
    for (const file of output.files) {
      if (!isSafeOutputPath(file.path))
        yield* new GenerationError({ reason: `Unsafe output path: ${file.path}` });
      const path = file.path.toLowerCase();
      if (normalizedPaths.has(path))
        yield* new GenerationError({ reason: `Duplicate output path: ${file.path}` });
    ...
    const previous = (yield* fs.exists(manifest))
      ? yield* fs.readFileString(manifest).pipe(
          ...
          Effect.mapError(
            () => new GenerationError({ reason: `Invalid generated file manifest: ${manifest}` }),
          ),
        )
      : [];
      ...
      (file) =>
        fs.exists(join(directory, file.path)).pipe(
          Effect.flatMap((exists) =>
            exists ? fs.stat(join(directory, file.path)) : Effect.succeed(undefined),
          ),
          Effect.flatMap((info) =>
      ...
      (file) =>
        Effect.tryPromise({
          try: () =>
            ...
            new GenerationError({ reason: `Failed to format ${file.path}: ${String(error)}` }),
        }).pipe(
          Effect.flatMap((content) => fs.writeFileString(join(directory, file.path), content)),
        ),
      { concurrency: 8, discard: true },
    );
...
export function generate<Id extends string, Groups extends HttpApiGroup.Any>(
  ...
  return Effect.try({
    try: () => emitEffect(compile(api)),
      ...
      error instanceof GenerationError ? error : new GenerationError({ reason: String(error) }),
...
function metadataPortable(ast: SchemaAST.AST, seen: Set<SchemaAST.AST>): boolean {
  ...
  if (SchemaAST.isDeclaration(ast))
    return ast.typeParameters.every((item) => metadataPortable(item, seen));
...
function generationPortable(generation: unknown): boolean {
  ...
  const namespace =
    typeof value.importDeclaration === "string"
      ...
      : undefined;
  return (
    value.runtime.startsWith("Schema.") ||
    (namespace !== undefined && value.runtime.startsWith(`${namespace}.`))
  );
}
...
function declaredErrorFields(schema: Schema.Top) {
  ...
  const key = fields.propertySignatures.find(
    (field) => field.name === "_tag" || field.name === "name",
  )?.name;
...
function renderGroup(group: Group, groupIndex: number) {
  ...
  const endpointSources = group.endpoints.map((operation, endpointIndex) => {
    ...
    const payloads = endpointPayloads.map(
      (schema, index) => addSlot(schema, `${prefix}Payload${index}`)!,
    );
    const success = renderSuccess(successes[0], `${prefix}Success`);
    const errorSlots = errors.map(
      (error, index) => addSlot(error.schema, `${prefix}Error${index}`)!,
    );
    const options = [
    ...
    const declared = [
      ...errorSlots,
      ...(success.streamError === undefined ? [] : [success.streamError]),
    ];
    const declaredSchema =
      declared.length === 0
        ? "Schema.Never"
        : `Schema.Union([${declared.map((slot) => slot.name).join(", ")}])`;
...
function renderSchemas(slots: ReadonlyArray<Slot>) {
  ...
  const expanded = [
    ...slots.map((slot, index) =>
      classes.has(index) ? { name: slot.name, schema: Schema.Never } : slot,
    ),
...
function renderClient(groups: ReadonlyArray<Group>) {
  const imports = groups
    .map(
      (group, index) =>
        `import { adaptGroup${index}, Group${index} } from ${JSON.stringify(`./${group.module}`)}`,
    )
    .join("\n");
