...
const Endpoint1Query = Schema.Struct({
...
class Endpoint2Error0Class extends Schema.TaggedErrorClass<Endpoint2Error0Class>("Missing")(
  "Missing",
  {
...
export const Group0 = HttpApiGroup.make("session", { topLevel: false })
  ...
  .add(
    HttpApiEndpoint.make("GET")("list", "/session", {
      query: Endpoint1Query,
      success: Endpoint1Success,
    }),
  )
  .add(
...
const Endpoint0 = (raw: RawGroup) => () =>
  raw["health"]({}).pipe(Effect.mapError(mapEndpoint0Error));
...
const Endpoint1 = (raw: RawGroup) => (input?: Endpoint1Input) =>
  raw["list"]({ query: { archived: input?.["archived"] } }).pipe(
...
const Endpoint3 = (raw: RawGroup) => (input: Endpoint3Input) =>
  raw["interrupt"]({ params: { sessionID: input["sessionID"] } }).pipe(
    Effect.mapError(mapEndpoint3Error),
  );
