...
function compile<Id extends string, Groups extends HttpApiGroup.Any>(
...
describe("HttpApiCodegen.generate", () => {
  test("compiles one contract for Promise and Effect emitters", () => {
    ...
    expect(promiseClient).toContain(
  ...
  test("allows Promise outputs to use an authoritative imported wire type", () => {
    const contract = compileContract(
      api(
        HttpApiEndpoint.get("events", "/event", {
  ...
  test("projects imported endpoint constants into a generated API", () => {
    ...
    expect(client).toContain(
      'const Api = HttpApi.make("generated").add(HttpApiGroup.make("session").add(SessionGet))',
    );
  });
  ...
  test("separates hosted and consumer group names", () => {
    ...
    expect(contract.groups[0]?.endpoints[0]?.operation).toMatchObject({
      group: "sessions",
      name: "get",
    });
  });
  ...
  test("supports explicit public endpoint names", () => {
    const source = HttpApi.make("test").add(
      HttpApiGroup.make("server.permission")
        ...
        .add(
          HttpApiEndpoint.get("session.permission.list", "/session", { success: Schema.String }),
        ),
    );
  ...
  test("omits custom transport endpoints", () => {
    ...
    expect(contract.groups[0]?.endpoints.map((endpoint) => endpoint.endpoint.name)).toEqual([
      "pty.get",
    ]);
  });
    ...
    const source = HttpApi.make("test")
      .add(
        HttpApiGroup.make("first").add(
          HttpApiEndpoint.get("one", "/one", { success: Schema.String }),
        ),
      )
      .add(
        HttpApiGroup.make("second").add(
          HttpApiEndpoint.get("two", "/two", { success: Schema.String }),
        ),
      );

    expect(() =>
      compileContract(source, { groupNames: { first: "same", second: "same" } }),
    ).toThrow("Client group name collision: same");
  });
  ...
  test("uses the unqualified endpoint name for the public client", () => {
    ...
    expect(effect).toContain(
      'const adaptGroup0 = (raw: RawClient["session"]) => ({ "get": Endpoint0_0(raw) })',
    );
  ...
  test("preserves optional keys in Promise error types", () => {
    ...
    const output = emitPromise(
      compileContract(
        api(
          HttpApiEndpoint.get("get", "/session", { success: Schema.String, error: OptionalError }),
        ),
      ),
    );
  ...
  test("preserves reflected default error statuses", () => {
    ...
    const output = emitPromise(
      compileContract(
        api(
          HttpApiEndpoint.get("get", "/session", { success: Schema.String, error: MissingStatus }),
        ),
      ),
    );

    expect(output.files.find((file) => file.path === "client.ts")?.content).toContain(
      "declaredStatuses: [500]",
    );
  });
  ...
  test("inlines non-recursive references in Promise wire types", () => {
    const Referenced = Schema.Struct({ value: Schema.String }).annotate({
  ...
  test("rejects Promise transports that are not implemented", () => {
    ...
    expect(() =>
      emitPromise(
        compileContract(api(HttpApiEndpoint.get("read", "/file/*", { success: Schema.String }))),
      ),
  ...
  test("executes an emitted Promise GET through fetch", async () => {
    ...
    try {
      await Promise.all(
        output.files.map((file) => Bun.write(join(directory, file.path), file.content)),
      );
  ...
  test("rejects with declared tagged errors and exports a type guard", async () => {
    ...
    try {
      ...
      const error = await client.session
    ...
    const output = compile(
      HttpApi.make("test").add(
        HttpApiGroup.make("session").add(
          HttpApiEndpoint.get("get", "/session/:sessionID", { success: Schema.String }),
        ),
      ),
    );
  ...
  test("emits one client module per HttpApi group", () => {
    const source = HttpApi.make("test")
      .add(
        HttpApiGroup.make("session").add(
          HttpApiEndpoint.get("get", "/session", { success: Schema.String }),
        ),
      )
      .add(
        HttpApiGroup.make("tool").add(
          HttpApiEndpoint.get("list", "/tool", { success: Schema.String }),
        ),
      );
  ...
  test("uses no argument when an operation has no input fields", () => {
    const output = compile(
      api(HttpApiEndpoint.get("health", "/health", { success: Schema.String })),
    );
  ...
  test("preserves non-default empty response statuses", () => {
    const output = compile(
      api(HttpApiEndpoint.post("create", "/session", { success: HttpApiSchema.Created })),
    );
  ...
  test("rejects custom validation checks without portable metadata", () => {
    const Positive = Schema.Number.check(
      Schema.makeFilter((value) => (value > 0 ? undefined : "positive")),
    );

    expect(() =>
      compile(api(HttpApiEndpoint.get("get", "/session", { success: Positive }))),
    ).toThrow("Unportable schema: session.get.success");
  });
  ...
  test("rejects spoofed and aborted validation checks", () => {
    ...
    expect(() =>
      compile(api(HttpApiEndpoint.get("spoofed", "/session", { success: Spoofed }))),
    ).toThrow("Unportable schema: session.spoofed.success");
    expect(() =>
      compile(api(HttpApiEndpoint.get("aborted", "/session", { success: Aborted }))),
    ).toThrow("Unportable schema: session.aborted.success");
  });
  ...
  test("rejects altered wire-side schemas even when the codec transformation is canonical", () => {
    ...
    if (typeof replaceEncoding !== "function")
      throw new Error("Expected SchemaAST.replaceEncoding");
    ...
    expect(() =>
      compile(api(HttpApiEndpoint.get("get", "/session", { success: Altered }))),
    ).toThrow("Effect schema requires authoritative import: session.get");
  });

  test("rejects lexical generation and annotation values", () => {
    const Generated = Schema.declare(
      (input): input is string => typeof input === "string",
    ).annotate({
      generation: { runtime: "LocalOnly", Type: "string" },
    });
    const Annotated = Schema.declare(
      (input): input is string => typeof input === "string",
    ).annotate({
      custom: () => "local",
    });
    ...
    expect(() =>
      compile(api(HttpApiEndpoint.get("generated", "/session", { success: Generated }))),
    ).toThrow("Unportable schema: session.generated.success");
    expect(() =>
      compile(api(HttpApiEndpoint.get("annotated", "/session", { success: Annotated }))),
    ).toThrow("Unportable schema: session.annotated.success");
  });
  ...
  test("preserves errors from server-only middleware", () => {
    ...
    const output = compile(
      api(
        HttpApiEndpoint.get("get", "/session", { success: Schema.String }).middleware(
          Authorization,
        ),
      ),
    );
  ...
  test("supports every HttpApi method through the generic constructor", () => {
    const output = compile(
      api(HttpApiEndpoint.make("TRACE")("trace", "/trace", { success: Schema.String })),
    );

    expect(output.files.find((file) => file.path === "session.ts")?.content).toContain(
      'HttpApiEndpoint.make("TRACE")',
    );
  });
  ...
  test("uses safe unique module paths without changing public group identifiers", () => {
    const output = compile(
      HttpApi.make("test")
        ...
        .add(
          HttpApiGroup.make("GROUP-0").add(
            HttpApiEndpoint.get("list", "/session", { success: Schema.String }),
          ),
        ),
    );

    expect(output.files.slice(0, 2).map((file) => file.path)).toEqual([
      "group-0.ts",
      "GROUP-0-1.ts",
    ]);
  ...
  test("reserves support module names case-insensitively", () => {
    const output = compile(
      HttpApi.make("test")
        .add(
          HttpApiGroup.make("client").add(
            HttpApiEndpoint.get("get", "/client", { success: Schema.String }),
          ),
        )
        .add(
          HttpApiGroup.make("INDEX").add(
            HttpApiEndpoint.get("get", "/index", { success: Schema.String }),
          ),
        ),
    );
  ...
  test("keeps searching when a reserved-name fallback is also occupied", () => {
    const output = compile(
      HttpApi.make("test")
        .add(
          HttpApiGroup.make("client-1").add(
            HttpApiEndpoint.get("first", "/first", { success: Schema.String }),
          ),
        )
        .add(
          HttpApiGroup.make("client").add(
            HttpApiEndpoint.get("second", "/second", { success: Schema.String }),
          ),
        ),
    );
        ...
        HttpApi.make("test")
          .add(
            HttpApiGroup.make("status").add(
              HttpApiEndpoint.get("get", "/nested", { success: Schema.String }),
            ),
          )
          .add(
  ...
  test("emits a usable raw type for top-level groups", () => {
    ...
    expect(output.files[0]?.content).toContain(
      "type RawGroup = HttpApiClient.Client<typeof Group0",
    );
  });
  ...
  it.effect("reports compiler failures in the generate Effect", () =>
    Effect.gen(function* () {
      ...
      if (error instanceof GenerationError)
        expect(error.reason).toBe("Unportable schema: session.get.success");
