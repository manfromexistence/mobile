...
} from "../../utils/visual-stability/invariant";
...
test("reports visible blanking, label reversal, and overlap", () => {
  const issues = analyzeVisualStability(
    trace([
      frame(
        0,
        region({ label: "Exploring", opacity: 1, bottom: 40 }),
        region({ top: 40, bottom: 60 }),
      ),
      frame(
        16,
        region({ label: "Explored", opacity: 0.2, bottom: 50 }),
        region({ top: 49, bottom: 69 }),
      ),
      frame(
        32,
        region({ label: "Exploring", opacity: 1, bottom: 50 }),
        region({ top: 50, bottom: 70 }),
      ),
    ]),
    { flow: ["changing", "following"] },
  );
...
test("reports duplicate regions and unexpected remounts", () => {
  const issues = analyzeVisualStability(
    trace([
      frame(0, region({ node: 1 })),
      frame(16, region({ node: 2, count: 2 })),
      frame(32, region({ node: 2 })),
    ]),
...
test("reports visible anchor movement while allowing virtual scrollbar movement", () => {
  const issues = analyzeVisualStability(
    trace([
      {
        at: 0,
        regions: { anchor: region({ top: 100, bottom: 120 }) },
        viewport: { ...viewport(100), scrollTop: 40 },
      },
      {
        at: 16,
        regions: { anchor: region({ top: 100, bottom: 120 }) },
        viewport: { ...viewport(120), scrollTop: 60 },
      },
    ]),
    { fixed: ["anchor"] },
  );
...
test("analyzes each marked event independently", () => {
  ...
  expect(analyzeVisualStability(input, { maxPositionReversals: 0 })).toContain(
    "changing top reversed 1 times",
  );
  expect(
...
test("reports cross-event motion reversals by default", () => {
  ...
  expect(analyzeVisualStabilityByMarker(input, { maxPositionReversals: 0 })).toContain(
    "changing top reversed 1 times",
  );
});
  ...
  const issues = analyzeVisualStability(
    {
      ...
      samples: [
        {
          at: 0,
          regions: { thinking: region(), error: region({ present: false, visible: false }) },
        },
        {
          at: 16,
          regions: {
        ...
        {
          at: 32,
          regions: { thinking: region({ present: false, visible: false }), error: region() },
        },
      ],
    },
...
test("evaluates the typed invariant algebra over explicit observations", () => {
  ...
  const invalid = { type: "required", regions: ["missing"] } satisfies VisualInvariant<
  ...
  expect(
    analyzeVisualObservations(
      [frame(0, region({ bottom: 50 }), region({ top: 49, bottom: 69 }))],
      plan,
    ),
  ).toEqual(["changing overlapped following by 1px at 0ms"]);
});
