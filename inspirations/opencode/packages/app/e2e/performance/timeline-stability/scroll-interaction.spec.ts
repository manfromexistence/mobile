...
test("does not reverse visible rows when the user wheels during shell remeasurement", async ({
  page,
}, testInfo) => {
  ...
  const timeline = await setupTimeline(page, {
    messages: [
      ...
      userMessage(),
      assistantMessage(
        [shell(shellID, "running"), textPart(followingID, "Following wheel interaction")],
        {
          completed: false,
        },
      ),
...
test("does not pull a keyboard-scrolled user during shell remeasurement", async ({
  page,
}, testInfo) => {
  ...
  const timeline = await setupTimeline(page, {
    messages: [
      ...
      userMessage(),
      assistantMessage(
        [shell(shellID, "running"), textPart(followingID, "Following keyboard interaction")],
        {
          completed: false,
        },
      ),
  ...
  await page.waitForFunction(() => {
    ...
    return new Promise<boolean>((resolve) => {
      ...
      requestAnimationFrame(() =>
        requestAnimationFrame(() => resolve(Math.abs(root.scrollTop - top) <= 0.5)),
      );
    });
  });
...
test("tracks keyboard scrolling from a focused timeline descendant", async ({ page }, testInfo) => {
  ...
  const timeline = await setupTimeline(page, {
    messages: [
      ...history(12),
      userMessage(),
      assistantMessage([shell(shellID, "completed", lines(5))]),
    ],
...
test("does not claim keyboard scrolling owned by a nested scrollable", async ({ page }) => {
  ...
  await scroller.evaluate(
    (element) => (element.scrollTop = Math.min(300, element.scrollHeight - element.clientHeight)),
  );
  ...
  await expect
    .poll(() => scroller.evaluate((element) => element.scrollTop))
    .toBeLessThan(nonOverflowBefore);
});

test("jump to latest lands on stable final rows after offscreen growth", async ({
  page,
}, testInfo) => {
  ...
  const timeline = await setupTimeline(page, {
    messages: [
      ...history(20),
      userMessage(),
      assistantMessage([shell(shellID, "running"), textPart(followingID, "Latest visible row")], {
        completed: false,
      }),
    ],
    settings: { shellToolPartsExpanded: true },
...
test("handles a single row taller than the viewport", async ({ page }, testInfo) => {
  ...
  const timeline = await setupTimeline(page, {
    messages: [
      userMessage(),
      assistantMessage([shell(shellID, "running"), textPart(followingID, "After tall row")], {
        completed: false,
      }),
    ],
    settings: { shellToolPartsExpanded: true },
  ...
  return Array.from({ length: count }, (_, index) => {
    ...
    return [
      ...
      assistantMessage(
        [
          textPart(
            `prt_${String(index).padStart(4, "0")}_scroll`,
            `History ${index}. ${"content ".repeat(30)}`,
          ),
        ],
        {
