...
test.describe("timeline visual lifecycle stability", () => {
  test("streams empty, short, and long parallel shells to staggered completion", async ({
    page,
  }, testInfo) => {
    ...
    const assistant = assistantMessage(
      [...initial, textPart(followingID, "Following all parallel shells.")],
      {
        completed: false,
      },
    );
    ...
    await reportVisualStability(
      ...
      visualPlan(
        ...
        [
          {
            type: "required",
            regions: ["prt_shell_empty", "prt_shell_short", "prt_shell_long", "following"],
          },
    ...
    await expect(
      page.locator(`[data-timeline-part-id="${ids[2]}"] [data-slot="bash-pre"]`),
    ...
    await expect(short.locator('[data-slot="collapsible-trigger"]')).toHaveAttribute(
      "aria-expanded",
      "false",
    );
    await timeline.send(
      partUpdated(textPart("prt_late_sibling", "A later sibling rerender.")),
      250,
    );
    await expect(short.locator('[data-slot="collapsible-trigger"]')).toHaveAttribute(
      "aria-expanded",
  ...
  test("replaces thinking with streamed reasoning and text without a blank visible turn", async ({
    ...
    await timeline.send(
      partUpdated(reasoningPart(reasoningID, "## Planning\n\nChecking the visible timeline.")),
      160,
    );
