    ...
    updates: [
      {
        ...
        output: Array.from(
          { length: 20 },
          (_, index) => `\u001b[32mline ${index}\u001b[0m ${"wide-".repeat(30)}`,
        ).join("\r\n"),
        delay: 240,
      },
      {
        ...
        output: Array.from(
          { length: 20 },
          (_, index) => `line ${index} ${"wide-".repeat(30)}`,
        ).join("\n"),
        delay: 500,
      },
    ],
  ...
  test(`keeps rows stable for shell ${profile.name}`, async ({ page }, testInfo) => {
    ...
    const timeline = await setupTimeline(page, {
      messages: [
        userMessage(),
        assistantMessage(
          [shell(shellID, "running"), textPart(followingID, "Following shell row")],
          {
            completed: false,
          },
        ),
...
test("keeps following row stable when a collapsed shell receives 50 lines", async ({
  page,
}, testInfo) => {
  ...
  const timeline = await setupTimeline(page, {
    messages: [
      userMessage(),
      assistantMessage(
        [shell(shellID, "running"), textPart(followingID, "Following collapsed shell")],
        {
          completed: false,
        },
      ),
...
test("keeps rows stable when a running shell becomes an error", async ({ page }, testInfo) => {
  ...
  const timeline = await setupTimeline(page, {
    messages: [
      userMessage(),
      assistantMessage(
        [shell(shellID, "running", lines(10)), textPart(followingID, "Following failed shell")],
        {
          completed: false,
        },
      ),
...
test("keeps rows stable when later text arrives before shell output", async ({
  page,
}, testInfo) => {
  ...
  await startVisualProbe(page, regions);
  await timeline.send(
    partUpdated(textPart(followingID, "Later assistant content arrived before shell output.")),
    240,
  );
