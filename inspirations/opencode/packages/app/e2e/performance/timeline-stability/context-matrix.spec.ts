...
test("appends context operations while the group is expanded", async ({ page }, testInfo) => {
  ...
  await timeline.send(
    partUpdated(toolPart("prt_append_02_glob", "glob", "running", inputs.glob)),
    180,
  );
  await timeline.send(
    partUpdated(toolPart("prt_append_03_grep", "grep", "completed", inputs.grep)),
    240,
  );
  await timeline.send(
    partUpdated(toolPart("prt_append_04_list", "list", "completed", inputs.list)),
    500,
  );
  ...
  await expect(
    page.locator(
      '[data-timeline-part-ids^="prt_append_01_read"] [data-slot="collapsible-trigger"]',
    ),
  ).toHaveAttribute("aria-expanded", "true");
});

test("splits and merges context groups when a middle text part changes", async ({
  page,
}, testInfo) => {
  ...
  await expect(
    page.locator('[data-timeline-part-ids="prt_split_01_read,prt_split_03_glob"]'),
  ).toBeVisible();
...
test("removing the first context member replaces the group once without overlapping following content", async ({
  ...
  await expect(original.locator('[data-slot="collapsible-trigger"]')).toHaveAttribute(
    "aria-expanded",
    "true",
  );
  const regions = defineVisualRegions({
  ...
  await expect(
    page.locator(
      `[data-timeline-part-ids="${ids.slice(1).join(",")}"] [data-slot="collapsible-trigger"]`,
    ),
  ).toHaveAttribute("aria-expanded", "true");
});
