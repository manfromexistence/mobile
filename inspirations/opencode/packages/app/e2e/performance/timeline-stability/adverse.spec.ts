...
test.describe("timeline adverse visual stability", () => {
  test("does not pull a scrolled-away user while an active shell grows", async ({
    page,
  }, testInfo) => {
    ...
    await scroller.evaluate((element) => {
      element.dispatchEvent(
        new WheelEvent("wheel", { bubbles: true, cancelable: true, deltaY: -450 }),
      );
  ...
  test("preserves an explicit shell state across virtualization", async ({ page }) => {
    ...
    await scroller.evaluate((element) => {
      element.dispatchEvent(
        new WheelEvent("wheel", { bubbles: true, cancelable: true, deltaY: -1_000 }),
      );
      element.scrollTop = 0;
    });
    await page.waitForTimeout(300);
    const trigger = page.locator(
      `[data-timeline-part-id="${targetID}"] [data-slot="collapsible-trigger"]`,
    );
  ...
  test("keeps narrow viewport rows ordered during long shell growth", async ({
    page,
  }, testInfo) => {
    ...
    const timeline = await setupTimeline(page, {
      messages: [
        ...
        assistantMessage(
          [
            shell(shellID, "running"),
            textPart(followingID, "A narrow following row that wraps across lines."),
          ],
          {
            completed: false,
          },
  ...
  test("keeps visible rows ordered while resizing desktop to narrow and back", async ({
    page,
  }, testInfo) => {
    ...
    await setupTimeline(page, {
      messages: [
        ...
        assistantMessage([
          ...
          textPart(
            followingID,
            "Following responsive timeline content that wraps on narrow screens.",
          ),
        ]),
      ],
      settings: { shellToolPartsExpanded: true },
...
function wideLines(count: number) {
  return Array.from(
    { length: count },
    (_, index) => `line ${index + 1} ${"wide-output-".repeat(20)}`,
  ).join("\n");
}
