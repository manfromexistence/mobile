...
for (const deviceScaleFactor of [1, 1.25]) {
  test(`keeps shell growth ordered at device scale ${deviceScaleFactor}`, async ({
    page,
  }, testInfo) => {
    ...
    const timeline = await setupTimeline(page, {
      messages: [
        userMessage(),
        assistantMessage(
          [shell(shellID, "running"), textPart(followingID, "Following scaled shell")],
          {
            completed: false,
          },
        ),
  ...
  test(`keeps shell and status transitions ordered with reduced motion ${reducedMotion}`, async ({
    ...
    const timeline = await setupTimeline(page, {
      messages: [
        userMessage(),
        assistantMessage(
          [shell(shellID, "running"), textPart(followingID, "Following motion profile")],
          {
            completed: false,
          },
        ),
