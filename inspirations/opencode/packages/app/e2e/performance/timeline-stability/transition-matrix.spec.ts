...
test("keeps unchanged siblings stable while a middle part is inserted and removed", async ({
  page,
}, testInfo) => {
  ...
  const timeline = await setupTimeline(page, {
    messages: [
      userMessage(),
      assistantMessage(
        [textPart(firstID, "First stable row"), textPart(lastID, "Last stable row")],
        {
          completed: false,
        },
      ),
  ...
  const regions = defineVisualRegions({
    first: {
      selector: `[data-timeline-part-id="${firstID}"]`,
      closest: '[data-timeline-row="AssistantPart"]',
    },
    last: {
      selector: `[data-timeline-part-id="${lastID}"]`,
      closest: '[data-timeline-row="AssistantPart"]',
    },
  });
...
test("streams text through growth, canonical replacement, and completion", async ({
  page,
}, testInfo) => {
  const textID = "prt_text_reconcile";
  const followingID = "prt_text_reconcile_following";
  const assistant = assistantMessage(
    [textPart(textID, "Starting"), textPart(followingID, "Following text row")],
    {
      completed: false,
    },
  );
  ...
  const regions = defineVisualRegions({
    text: {
      selector: `[data-timeline-part-id="${textID}"]`,
      closest: '[data-timeline-row="AssistantPart"]',
    },
    following: {
  ...
  await timeline.send(
    partUpdated(textPart(textID, "Canonical replacement with a shorter final paragraph.")),
    200,
  );
...
test("inserts a completed question between stable rows", async ({ page }, testInfo) => {
  ...
  await timeline.send(
    partUpdated(
      toolPart(questionID, "question", "completed", input, { metadata: { answers: [["Yes"]] } }),
    ),
    600,
  );
...
test("replaces thinking with an assistant error without a blank turn", async ({
  page,
}, testInfo) => {
  ...
  await expect(page.locator('[data-timeline-row="Error"]')).toContainText(
    "Provider failed visibly",
  );
  const trace = await stopVisualProbe<keyof typeof regions>(page);
  await reportVisualStability(
    testInfo,
...
test("reducer-hardening: removes a historical turn one message at a time without moving a visible lower anchor twice", async ({
  ...
  await timeline.send(
    event("message.removed", { sessionID: "ses_timeline_stability", messageID: removeUserID }),
    500,
  );
  const trace = await stopVisualProbe<keyof typeof regions>(page);
  await reportVisualStability(
