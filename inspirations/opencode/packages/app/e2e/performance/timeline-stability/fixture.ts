...
export type TimelineEvent = DeepReadonly<
  Omit<GlobalEvent, "payload"> & { payload: TimelinePayload }
>;
...
export type PartSeed<Owner extends Message["role"]> = OwnedPart<Owner> extends infer Candidate
...
export function event(
  type: TimelinePayload["type"],
  properties: TimelinePayload["properties"],
): TimelineEvent {
...
export function validateTimelineMessages(input: readonly TimelineMessage[]): TimelineMessage[] {
  ...
  messages.forEach((message) => {
    ...
    if (message.info.role === "assistant" && !users.has(message.info.parentID))
      throw new Error(
        `Timeline assistant ${message.info.id} must reference a parent user in the fixture`,
      );
    message.parts.forEach((part) => {
      if (part.sessionID !== message.info.sessionID || part.messageID !== message.info.id)
        throw new Error(
          `Timeline part ${part.id} ownership does not match message ${message.info.id}`,
        );
      if (message.info.role === "user" && !["text", "file", "agent", "subtask"].includes(part.type))
        throw new Error(
          `Timeline user message ${message.info.id} cannot own ${part.type} part ${part.id}`,
        );
      if (message.info.role === "assistant" && ["agent", "subtask"].includes(part.type))
        throw new Error(
          `Timeline assistant message ${message.info.id} cannot own ${part.type} part ${part.id}`,
        );
...
export function status(type: SessionStatus["type"], attempt = 1) {
  return event("session.status", {
    sessionID,
    status:
      type === "retry" ? { type, attempt, message: "Rate limited", next: 1700000010000 } : { type },
  });
}
...
export function userMessage(
  ...
  const seeds = parts ?? [
    userText("Build the timeline stability matrix.", { id: `prt_${id}_text` }),
  ];
  const message = {
    info: {
      id,
    ...
    info: {
      ...
      time: {
        ...
        ...(input.completed === false
          ? {}
          : { completed: (input.created ?? 1700000001000) + 1_000 }),
      },
...
export function textPart(
  id: string,
  text: string,
): Extract<PartSeed<"assistant">, { type: "text" }> {
...
export function reasoningPart(
  id: string,
  text: string,
): Extract<PartSeed<"assistant">, { type: "reasoning" }> {
...
export function shell(
  ...
  if (state === "running")
    return toolPart(
      ...
      state,
      { command },
      { title: command, metadata: { command, output } },
    );
  if (state === "error")
    return toolPart(
      id,
      ...
      state,
      { command },
      { error: output || undefined, metadata: { command, output } },
    );
  return toolPart(
    id,
    "bash",
    state,
    { command },
    { title: command, output, metadata: { command, output } },
  );
}
