...
test.describe("timeline tool state stability", () => {
  test("moves lightweight tools through pending, running, and completed without replacing rows", async ({
    ...
    const initial = [
      ...
      toolPart(todoID, "todowrite", "pending", {
        todos: [{ content: "Hidden", status: "pending" }],
      }),
    ...
    const timeline = await setupTimeline(page, {
      ...
      sessions: [
        session(),
        session({ id: childID, parentID: sessionID, title: "Inspect timeline" }),
      ],
      cpuRate: 4,
    });
    ...
    for (const [index, id] of ["skill", "webfetch", "custom", "task", "websearch"].entries()) {
      ...
      const metadata =
        key === "task" ? { sessionId: childID } : key === "websearch" ? { provider: "exa" } : {};
      const output = key === "websearch" ? "Result https://example.com/result" : "Completed";
      await timeline.send(
        partUpdated(
          toolPart(`prt_state_${key}`, names[key], "completed", inputs[key], { metadata, output }),
        ),
    ...
    await timeline.send(
      partUpdated(
        toolPart(questionID, "question", "completed", questionInput(), {
          metadata: { answers: [["Keep it stable"]] },
        }),
      ),
      350,
    );
    ...
    await expect(page.locator(`[data-timeline-part-id="${questionID}"]`)).toContainText(
      "Keep it stable",
    );
    await expect(page.locator(`[data-timeline-part-id="${todoID}"]`)).toHaveCount(0);
    await expect(
      page.locator(`a[href$="/session/${childID}"]`, {
        has: page.locator('[data-component="task-tool-card"]'),
      }),
    ).toBeVisible();
  ...
  test("keeps an expanded mixed context group stable through staggered completion and error", async ({
    ...
    const timeline = await setupTimeline(page, {
      messages: [
        userMessage(),
        assistantMessage([...context, textPart("prt_ctx_following", "Following context")], {
          completed: false,
        }),
      ],
      cpuRate: 4,
    ...
    await expect(group.locator('[data-slot="collapsible-trigger"]')).toHaveAttribute(
      "aria-expanded",
      "true",
    );
    ...
    for (const [index, delay] of [90, 260, 70, 380].entries()) {
      await timeline.send(
        partUpdated(toolPart(ids[index]!, tools[index]!, "running", inputs[index]!)),
        delay,
      );
    }
    ...
    await expect(group.locator('[data-component="tool-status-title"]')).toHaveAttribute(
      "aria-label",
      "Explored",
    );
    ...
    await timeline.send(
      partUpdated(textPart("prt_ctx_late_sibling", "Later sibling content")),
      200,
    );
    await expect(group.locator('[data-slot="collapsible-trigger"]')).toHaveAttribute(
      "aria-expanded",
...
function toolRegion(id: string) {
  return {
    selector: `[data-timeline-part-id="${id}"]`,
    closest: '[data-timeline-row="AssistantPart"]',
  };
}
