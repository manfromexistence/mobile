...
test("adds a task child-session link without replacing the task row", async ({
  page,
}, testInfo) => {
  ...
  const timeline = await setupTimeline(page, {
    messages: [
      userMessage(),
      assistantMessage([toolPart(taskID, "task", "running", input)], { completed: false }),
    ],
...
test("changes generic tool arguments without replacing the row", async ({ page }, testInfo) => {
  ...
  const regions = defineVisualRegions({
    tool: {
      selector: `[data-timeline-part-id="${toolID}"]`,
  ...
  await timeline.send(
    partUpdated(
      toolPart(toolID, "mcp_probe", "running", { target: "two", count: 2, mode: "deep" }),
    ),
    200,
  );
  await timeline.send(
    partUpdated(
      toolPart(toolID, "mcp_probe", "completed", { target: "two", count: 2, mode: "deep" }),
    ),
    400,
  );
  const trace = await stopVisualProbe<keyof typeof regions>(page);
