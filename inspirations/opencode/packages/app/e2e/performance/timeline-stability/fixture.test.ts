...
describe("timeline fixture validation", () => {
  ...
  test("rejects duplicate IDs and orphan assistants", () => {
    expect(() => validateTimelineMessages([userMessage(), userMessage()])).toThrow(
      /duplicate message ID/,
    );
    expect(() =>
      validateTimelineMessages([
        userMessage(),
        assistantMessage([], { parentID: "msg_missing_parent" }),
      ]),
    ).toThrow(/parent user/);
  });

  test("assigns deterministic event IDs", () => {
    const first = event("session.status", {
      sessionID: "ses_timeline_stability",
      status: { type: "busy" },
    });
    const second = event("session.status", {
      sessionID: "ses_timeline_stability",
      status: { type: "idle" },
    });
