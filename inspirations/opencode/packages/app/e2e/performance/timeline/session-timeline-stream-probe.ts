...
export async function installTimelineStreamProbe(
  ...
  await page.evaluate(
    ({ textPartID, finalIndex, profileVisual, minimal, markerPattern, fragmentCount }) => {
      ...
      (
        window as Window & { __timelineStreamBenchmark?: TimelineProbeState }
      ).__timelineStreamBenchmark = state;
      ...
      const describe = (element: Element) => {
        ...
        const part =
          element.closest<HTMLElement>("[data-timeline-part-id]")?.dataset.timelinePartId ??
          "unknown";
        const block = element
        ...
        records.forEach((record) => {
          record.addedNodes.forEach((node) => {
            if (
              ...
              node.matches("[data-timeline-key]") &&
              visible(node)
            ) {
            ...
            const added = [node, ...node.querySelectorAll(critical)].filter((element) =>
          ...
          record.removedNodes.forEach((node) => {
            ...
            removed.forEach((element) => {
              ...
              if (state.visibleSubtrees.get(key) === element)
                state.visibleSubtreeUnmounts.push(key);
            });
          });
        });
      };
      ...
      const currentPart = () =>
        root.querySelector<HTMLElement>(`[data-timeline-part-id="${textPartID}"]`);
      ...
      const sample = (now: number) => {
        ...
        setTimeout(() => {
          ...
          if (profileVisual) {
            ...
            state.maxPartTopMovement = Math.max(
              state.maxPartTopMovement,
              Math.abs(partTop - state.previousPartTop),
            );
            state.previousPartTop = partTop;
          }
          const visibleRow = [...root.querySelectorAll<HTMLElement>("[data-timeline-row]")].some(
            (element) => {
              const rect = element.getBoundingClientRect();
              return rect.bottom > viewport.top && rect.top < viewport.bottom;
            },
          ...
          if (profileVisual && duration > 33.34) {
            ...
            const index = complete
              ? finalIndex
              : Number(
                  content.match(new RegExp(markerPattern, "g"))?.at(-1)?.match(/\d+/)?.[0] ?? -1,
                );
            state.slowFrames.push({
              ...
              codeBlocks:
                livePart?.querySelectorAll('[data-component="markdown-code"]').length ?? 0,
...
export function startTimelineStreamProbe(page: Page) {
  return page.evaluate(() => {
    const state = (window as Window & { __timelineStreamBenchmark?: TimelineProbeState })
...
export function streamProgress(content: string) {
  const index = Number(
    content.match(new RegExp(STREAM_MARKER_PATTERN, "g"))?.at(-1)?.match(/\d+/)?.[0] ?? -1,
  );
  return {
    index,
...
export async function collectTimelineStreamMetrics(
  ...
  return page.evaluate(({ textPartID, finalIndex, navigations }) => {
    ...
    if (!state)
      throw new Error(
        `missing streaming benchmark state after navigation: ${JSON.stringify(navigations)}`,
      );
    ...
    const busyFrames =
      busyStart === undefined || busyEnd === undefined
        ? []
        : state.frames.filter(
            (_, index) => state.frameAt[index]! >= busyStart && state.frameAt[index]! <= busyEnd,
          );
    ...
    return {
      ...
      deltasPerSecond: Number.isFinite(completionObservedMs)
        ? finalIndex / (completionObservedMs / 1_000)
        : null,
      rafGapSamples: state.frames.length,
      rafCallbackRate: duration ? (state.frames.length * 1000) / duration : 0,
      observedProgressWindowRafCallbackRate: busyDuration
        ? (busyFrames.length * 1000) / busyDuration
        : null,
