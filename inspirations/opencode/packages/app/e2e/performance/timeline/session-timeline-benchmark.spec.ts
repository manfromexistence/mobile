...
benchmark.describe("performance: session timeline streaming", () => {
  benchmark(
    "streams assistant text without remounting or oscillating",
    async ({ page, report }) => {
  ...
  benchmark(
    "streams assistant text in v2 with review diffs and pane closed",
    async ({ page, report }) => {
      benchmark.setTimeout(Number(process.env.TIMELINE_COMPLETION_TIMEOUT_MS ?? 420_000) + 60_000);
      const result = await runTimelineStreamBenchmark(page, {
...
benchmark.describe("performance: review pane", () => {
  benchmark("loads v2 review diffs and switches active files", async ({ page, report }) => {
    ...
    const switches = [];
    for (const diff of diffs.slice(1, 4))
      switches.push(await measureReviewNextFile(page, diff.file));
...
async function runTimelineStreamBenchmark(page: Page, options: TimelineStreamOptions) {
  ...
  const reviewPane =
    options.reviewPane && diffs ? await measureReviewPaneLoad(page, diffs[0]!.file) : undefined;
  if (reviewPane) await fixture.waitForStableGeometry();

  const profile = await startTimelineProfile(page, { cpuThrottle, profileCPU });
  await installTimelineStreamProbe(page, {
    textPartID,
  ...
  await page.evaluate((input) => {
    ...
    const paneState = () => {
      ...
      const diffLines = viewers.reduce(
        (sum, viewer) =>
          sum +
          (viewer.shadowRoot?.querySelectorAll("[data-line]").length ??
...
async function collectReviewPaneProbe(page: Page) {
  await page.waitForFunction((streak) => {
    const samples = (window as Window & { __reviewPaneProbe?: ReviewPaneProbe }).__reviewPaneProbe
      ?.samples;
...
function summarizeReviewPaneSamples(samples: ReviewPaneSample[]) {
  ...
  return {
    ...
    stableReadyObservedMs:
      stableIndex === -1 ? null : samples[stableIndex + reviewReadyStreak - 1]!.observedAtMs,
