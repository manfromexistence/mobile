...
export function layoutShiftSample(
  entry: Pick<PerformanceEntry, "startTime"> & { value: number },
  started: number,
) {
...
export function summarizeCachedRepaintTrace(trace: CachedRepaintTrace) {
  ...
  return {
    ...
    wrongDestinationSamples: trace.samples.filter(
      (sample) => category(sample) === "wrongDestination",
    ).length,
    ...
    mountedMin: trace.samples.length
      ? Math.min(...trace.samples.map((sample) => sample.mounted))
      : 0,
