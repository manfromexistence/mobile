  ...
  const summarize = (matches: (sample: NavigationMilestoneSample) => boolean) => {
    ...
    const stable = samples.findIndex(
      (sample, index) =>
        ...
        matches(sample) &&
        matches(samples[index + 1]!) &&
        matches(samples[index + 2]!),
    );
    return {
...
export async function measureNavigationMilestones(
  ...
  await page.evaluate(
    ({ triggerSelector, milestones }) => {
      ...
      (
        window as Window & { __navigationMilestones?: NavigationMilestoneProbe }
      ).__navigationMilestones = {
        samples,
        stop: () => {
          running = false;
        },
  ...
  const samples = await page.evaluate(() => {
    const probe = (window as Window & { __navigationMilestones?: NavigationMilestoneProbe })
      .__navigationMilestones!;
    probe.stop();
