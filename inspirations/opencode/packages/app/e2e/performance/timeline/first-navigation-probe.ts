...
export async function measureFirstNavigation(
  ...
  await page.evaluate(
    ({ href, destinationPath, sourceSelector, destinationSelector, contentSelector }) => {
      ...
      (
        window as Window & { __firstNavigationProbe?: FirstNavigationProbe }
      ).__firstNavigationProbe = {
        samples,
        stop: () => {
          running = false;
        },
  ...
  await page.waitForFunction(() => {
    const samples = (window as Window & { __firstNavigationProbe?: FirstNavigationProbe })
      .__firstNavigationProbe?.samples;
    if (!samples) return false;
    return (
      samples.length >= 3 &&
      samples.slice(-3).every((sample) => sample.destination && !sample.source)
    );
  });
  const samples = await page.evaluate(() => {
    const probe = (window as Window & { __firstNavigationProbe?: FirstNavigationProbe })
      .__firstNavigationProbe!;
    probe.stop();
