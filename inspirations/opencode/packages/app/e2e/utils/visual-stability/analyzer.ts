...
export function analyzeVisualObservations<RegionName extends string>(
  ...
  const names = [
    ...new Set(observations.flatMap((sample) => Object.keys(sample.regions) as RegionName[])),
  ];
  ...
  const fixed = invariants.filter(
    (invariant): invariant is Extract<VisualInvariant<RegionName>, { type: "fixed" }> =>
      invariant.type === "fixed",
  );
  const opacity = invariants.filter(
    (invariant): invariant is Extract<VisualInvariant<RegionName>, { type: "opacity" }> =>
      invariant.type === "opacity",
  );
  ...
  const motion = invariants.filter(
    (invariant): invariant is Extract<VisualInvariant<RegionName>, { type: "motion" }> =>
      invariant.type === "motion",
  );
  ...
  for (const name of new Set(required)) {
    if (!observations.some((sample) => sample.regions[name]?.visible))
      issues.push(`${name} never rendered`);
  }
  for (const invariant of continuousAny) {
    if (
      !invariant.regions.some((name) =>
        observations.some((sample) => sample.regions[name]?.visible),
      )
    )
  ...
  for (const name of names) {
    const samples = observations.flatMap((observation) => {
      ...
      const clipped =
        ...
        (region.bottom <= observation.viewport.top || region.top >= observation.viewport.bottom);
    ...
    if (unique.has(name)) {
      ...
      if (duplicate)
        issues.push(`${name} appeared ${duplicate.count} times at ${Math.round(duplicate.at)}ms`);
    }
    if (stable.has(name)) {
      const identities = [
        ...new Set(visible.map((sample) => sample.node).filter((node) => node > 0)),
      ];
    ...
    for (const invariant of fixed.filter((invariant) => includes(invariant.regions, name))) {
      ...
      const movement = origin
        ? Math.max(0, ...visible.map((sample) => Math.abs(sample.top - origin.top)))
        : 0;
    ...
    if (continuity.some((invariant) => includes(invariant.regions, name))) {
      ...
      if (
        ...
        samples
          ...
          .some((sample) => !sample.visible && sample.inViewport)
      )
  ...
  if (invariants.some((invariant) => invariant.type === "acquire-bottom-anchor")) {
    ...
    if (!final || final.distanceFromBottom > 4)
      issues.push(
        `did not acquire bottom anchor${final ? ` (${final.distanceFromBottom}px away)` : ""}`,
      );
  }

  for (const invariant of continuousAny) {
    const active = observations.map((sample) =>
      invariant.regions.some((name) => sample.regions[name]?.visible),
    );
  ...
  for (const invariant of invariants.filter(
    ...
    for (const [before, after] of invariant.regions
      ...
      for (const sample of observations) {
        ...
        if (
          overlap > (invariant.overlapTolerance ?? 0.5) &&
          (!maximum || overlap > maximum.overlap)
        )
          maximum = { overlap, at: sample.at };
      }
      if (inverted)
        issues.push(`${before} rendered after ${after} at ${Math.round(inverted.at)}ms`);
      if (maximum)
        issues.push(
...
export function analyzeVisualTraceByMarker<RegionName extends string>(
  ...
  const required = [
    ...new Set(plan.markerRequired ?? regions(plan.invariants, "required")),
  ].flatMap((name) =>
  ...
  const windows = trace.markers.flatMap((marker, index) => {
    ...
    return analyzeVisualObservations(samples, {
      ...plan,
      perMarker: false,
      invariants: withoutRequired,
...
function regions<RegionName extends string, Type extends VisualInvariant<RegionName>["type"]>(
  ...
  return invariants.flatMap((invariant) =>
    invariant.type === type && "regions" in invariant && invariant.regions !== "all"
      ? [...invariant.regions]
      : [],
  ) as RegionName[];
}

function includes<RegionName extends string>(
  regions: readonly RegionName[] | "all",
  name: RegionName,
) {
