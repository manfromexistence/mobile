...
export async function reportVisualStability<RegionName extends string>(
  ...
  await writeFile(
    issuesPath,
    JSON.stringify(
      { issues, markers: result.markers, capturedFrameCount: result.frames.length },
      null,
      2,
    ),
  );
...
async function attachViolationFrames<RegionName extends string>(
  ...
  for (const [violation, target] of targets.entries()) {
    const nearest = result.frames.reduce(
      (best, frame, index) =>
        Math.abs(frame.at - target) < Math.abs(result.frames[best]!.at - target) ? index : best,
      0,
    );
    for (const [label, index] of [
      ...
      await testInfo.attach(
        `${name}-${violation + 1}-${label}-${Math.round(result.frames[index]!.at)}ms`,
        {
