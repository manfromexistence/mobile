...
export async function startVisualProbe<Regions extends Record<string, VisualRegionDefinition>>(
  ...
  const startedAtEpoch = await page.evaluate((regions) => {
    ...
    const sample = () => {
      ...
      setTimeout(() => {
        if (!running) return;
        const viewport = [...document.querySelectorAll<HTMLElement>(".scroll-view__viewport")].find(
            ...
            Object.entries(regions).map(([name, config]) => {
              ...
              return [
                name,
                {
                  ...
                  inViewport:
                    !viewportRect ||
                    (!!known && known.bottom > viewportRect.top && known.top < viewportRect.bottom),
                  cssHidden,
...
export async function stopVisualProbe<RegionName extends string = string>(
  ...
  return page
    ...
    .then(
      async (trace) =>
        ({ ...trace, frames: await stopCapture(page) }) as unknown as VisualProbeResult<RegionName>,
      async (error: unknown) => {
        await stopCapture(page);
        throw error;
      },
