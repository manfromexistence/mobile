...
export type SseTransport<T> = {
  ...
  burst(
    payloads: readonly T[],
    options?: readonly SseEventOptions[],
  ): Promise<SseDeliveryAcknowledgement[]>;
  split(
    payload: T,
    ...
    ({ server, retry }) => {
      ...
      const chunks = (bytes: Uint8Array, cuts?: readonly number[]) => {
        const boundaries = [...new Set(cuts ?? [])]
          ...
          .sort((a, b) => a - b);
        return [0, ...boundaries].map((start, index) =>
          bytes.slice(start, boundaries[index] ?? bytes.byteLength),
        );
      };
      ...
      const command = (input: BrowserCommand<unknown>) => {
        ...
        if (input.burst) {
          ...
          return encoded.map((item) =>
            acknowledge(connection, item.bytes.byteLength, 1, item.delivery.options?.id),
          );
        }
      ...
      const fetch = (input: RequestInfo | URL, init?: RequestInit) => {
        ...
        if (
          url.origin !== server ||
          (url.pathname !== "/global/event" && url.pathname !== "/event")
        )
  ...
  return {
    ...
    async waitForConnection(input = {}) {
      await page.waitForFunction(
        (after) => {
          const transport = (window as BrowserTransport).__testSseTransport;
          const connections = transport?.command({ type: "connections" }) as
            | SseConnectionRecord[]
            | undefined;
