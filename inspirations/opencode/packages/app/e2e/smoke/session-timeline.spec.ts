...
test.describe("smoke: session timeline", () => {
  ...
  test("keeps the visible message fixed while prepending history", async ({ page }) => {
    ...
    await mockOpenCodeServer(page, {
      ...
      messageDelay: 3_000,
      onMessages: (input) =>
        requests.push({ before: input.before, phase: input.phase, at: performance.now() }),
    });
      ...
      scroller.evaluate((element, keys) => {
        ...
        return Object.fromEntries(
          keys.map((key) => {
            ...
            return [
              key,
              Math.round((row.getBoundingClientRect().top - top) * devicePixelRatio) /
                devicePixelRatio,
            ];
          }),
        );
    ...
    await expect
      .poll(() => requests.some((request) => request.before && request.phase === "end"))
      .toBe(true);
    await waitForTimelineStable(page);
  ...
  test("paints cached session tabs at the latest message", async ({ page }) => {
    await mockOpenCodeServer(page, {
      ...
      pageMessages: (sessionID) => ({
        items: fixture.messages[sessionID as keyof typeof fixture.messages] ?? [],
      }),
    });
    await configureSmokePage(page, fixture.directory);
          ...
          setTimeout(() => {
            ...
            if (root) {
              ...
              samples.push({
                ...
                bottomError: bottom?.bottom - view.bottom,
              });
              if (
                !firstPaint &&
  ...
  test("paints a cold session tab at the latest message", async ({ page }) => {
        ...
        const sample = () => {
          ...
          if (root) {
            ...
            const spacer = root
              .querySelector<HTMLElement>('[data-timeline-row="bottom-spacer"]')
              ?.getBoundingClientRect();
            const messages = [...root.querySelectorAll<HTMLElement>("[data-message-id]")].filter(
              (element) => {
                const rect = element.getBoundingClientRect();
                return rect.bottom > view.top && rect.top < view.bottom;
              },
    ...
    await page.waitForFunction(() =>
      ...
      ).__coldTabSamples?.some((sample) => sample.destination),
    );
...
async function configureSmokePage(page: Page, directory: string) {
  ...
  await page.addInitScript(() => {
    ...
    const idsOf = (el: HTMLElement) =>
      [el.dataset.timelinePartId, ...(el.dataset.timelinePartIds?.split(",") ?? [])].filter(
        (id): id is string => !!id,
      );

    smoke.__timelineSmokeState = () => {
      const scroller = [...document.querySelectorAll<HTMLElement>(".scroll-view__viewport")].find(
        (el) => el.querySelector("[data-timeline-row], [data-session-title]"),
      );
      if (!scroller) {
        return {
          ids: [],
    ...
    const record = () => {
      for (const toast of document.querySelectorAll<HTMLElement>(
        ...
        if (text && !smoke.__timelineSmokeErrorToasts!.includes(text))
          smoke.__timelineSmokeErrorToasts!.push(text);
      }
...
async function expectCanScrollToStart(
  ...
  for (let attempt = 0; attempt < 600; attempt++) {
    ...
    if (!changed && current.signature === before.signature && current.scrollTop <= 1)
      unchangedAtTop++;
    ...
    (prev) =>
      new Promise<boolean>((resolve) => {
        ...
        scroller.dispatchEvent(
          new WheelEvent("wheel", { bubbles: true, cancelable: true, deltaY: -1, deltaMode: 0 }),
        );
        scroller.scrollTop = Math.max(
          0,
          scroller.scrollTop - Math.max(80, Math.round(scroller.clientHeight * 0.45)),
        );
...
function sampleSummary(samples: TraversalSample[]) {
  return samples
    .filter(
      (_, index) =>
        index % Math.max(1, Math.floor(samples.length / 8)) === 0 || index === samples.length - 1,
    )
    .map(
      (sample, index) =>
          ...
          requestAnimationFrame(() => {
            ...
            requestAnimationFrame(() =>
              resolve(
                !!a &&
                  a === b &&
                  b === ((window as SmokeWindow).__timelineSmokeState?.().signature ?? ""),
              ),
            );
          });
        });
...
function expectCompleteScroll(
  ...
  expect(
    state.scrollTop,
    `timeline should reach the start\n${sampleSummary(samples)}`,
  ).toBeLessThanOrEqual(1);
  expect(
