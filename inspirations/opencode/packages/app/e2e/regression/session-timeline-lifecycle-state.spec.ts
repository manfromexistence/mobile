...
for (const expanded of [false, true]) {
  test(`preserves shell user intent from a ${expanded ? "expanded" : "collapsed"} default`, async ({
    page,
  }) => {
    ...
    const trigger = page.locator(
      `[data-timeline-part-id="${id}"] [data-slot="collapsible-trigger"]`,
    );
...
test("transitions thinking and hidden reasoning through busy to idle", async ({ page }) => {
  ...
  const assistant = assistantMessage([reasoningPart(reasoningID, "## Inspecting stability")], {
    completed: false,
  });
  const timeline = await setupTimeline(page, {
