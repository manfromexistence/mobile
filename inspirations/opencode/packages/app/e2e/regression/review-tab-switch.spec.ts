...
test("keeps the v2 review pane mounted when switching session tabs in a workspace", async ({
  page,
}) => {
  await setup(page);
...
async function readProbe(page: Page) {
  return page
    .locator('#review-panel [data-component="session-review-v2"]')
  ...
  await page.addInitScript(
    ({ directory, server, sessions }) => {
      ...
      localStorage.setItem(
        ...
        JSON.stringify(
          sessions.map((sessionId: string) => ({ type: "session", server, sessionId })),
        ),
      );
    },
