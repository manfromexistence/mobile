...
test.describe("session timeline projection", () => {
  test("renders every admitted tool family and hides timeline-only exclusions", async ({
    page,
  }) => {
    const parts = [
      ...
      editPart("prt_edit"),
      toolPart("prt_write", "write", "completed", {
        filePath: "src/new.ts",
        content: "export const stable = true\n",
      }),
      patchPart("prt_patch"),
      toolPart("prt_todo", "todowrite", "completed", {
  ...
  test("projects gaps, dividers, assistant parts, and errors together", async ({ page }) => {
    const firstUser = userMessage(
      [
        userText(
          "The user made the following comment regarding lines 4 through 8 of src/a.ts: Keep this stable",
          {
            id: "prt_comment",
            synthetic: true,
            metadata: {
    ...
    const failed = assistantMessage(
      [{ id: "prt_after_abort", type: "text", text: "After interruption" }],
      {
        ...
        error: {
          ...
          data: {
            message: JSON.stringify({
              error: { type: "provider_error", message: "Visible provider failure" },
            }),
            isRetryable: false,
          },
        },
    ...
    const nextAssistant = assistantMessage(
      [{ id: "prt_second_text", type: "text", text: "Second response" }],
      {
  ...
  test("renders comment strips and historical diff summary overflow", async ({ page }) => {
    ...
    const nextUser = userMessage(undefined, {
      id: "msg_2000_diff_next_user",
  ...
  test("renders interruption independently when the turn is not compacted", async ({ page }) => {
    ...
    const rows = await page
      ...
      .evaluateAll((elements) =>
        elements.map((element) => element.getAttribute("data-timeline-row")),
      );
    expect(rows).toEqual(["AssistantPart", "TurnDivider", "AssistantPart"]);
  });

  test("renders user image, file attachment, file reference, and agent reference", async ({
    page,
  }) => {
    ...
    const parts: PartSeed<"user">[] = [
      ...
      },
      {
        ...
        source: {
          type: "file",
          path: "src/a.ts",
          text: { value: "@src/a.ts", start: 18, end: 27 },
        },
      },
      {
        id: "prt_user_agent",
