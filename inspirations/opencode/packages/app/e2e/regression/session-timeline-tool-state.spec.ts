...
test("updates expanded web search links without resetting expansion", async ({ page }) => {
  ...
  const timeline = await setupTimeline(page, {
    messages: [
      userMessage(),
      assistantMessage([
        toolPart(searchID, "websearch", "completed", input, { output: "https://example.com/one" }),
      ]),
    ],
  });
...
test("renders multiple question answers and preserves open state on answer updates", async ({
  page,
}) => {
  ...
  const timeline = await setupTimeline(page, {
    messages: [
      userMessage(),
      assistantMessage([
        toolPart(questionID, "question", "completed", input, {
