...
test.describe("regression: session timeline local row state", () => {
  test("keeps a manually collapsed tool collapsed when later assistant content streams", async ({
    page,
  }) => {
  ...
  test("does not remount an edit diff when sibling parts or diff counts update", async ({
    page,
  }) => {
    const events: EventPayload[] = [];
  ...
  test("keeps a sticky edit header aligned with a multi-hunk diff", async ({ page }) => {
    ...
    const after = [100, 300, 500, 700, 900].reduce(
      (result, index) =>
        result.replace(
          ...
          `export const value${index} = compute(${index})`,
        ),
      lines,
    );
    const part = {
    ...
    await expect
      .poll(() => wrapper.evaluate((element) => element.getBoundingClientRect().height))
      .toBeGreaterThan(500);
    ...
    expect(samples.every((sample) => Math.abs(sample.trigger - samples[0]!.trigger) <= 1)).toBe(
      true,
    );
