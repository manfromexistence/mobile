...
test("renders every tool error outcome without leaking hidden tools", async ({ page }) => {
  ...
  const parts = ordinary.map((tool, index) =>
    toolPart(`prt_error_${index}`, tool, "error", errorInput(tool), {
      error: `${tool} failed visibly`,
    }),
  );
  parts.push(
    ...
    toolPart("prt_question_error", "question", "error", questionInput(), {
...
test("transitions shell and question through running error outcomes", async ({ page }) => {
  ...
  await timeline.send(
    partUpdated(toolPart(shellID, "bash", "running", { command: "exit 1" })),
    120,
  );
  await timeline.send(
    partUpdated(toolPart(questionID, "question", "running", questionInput())),
    180,
  );
  await expect(page.locator(`[data-timeline-part-id="${questionID}"]`)).toHaveCount(0);
  await timeline.send(
    partUpdated(
      toolPart(shellID, "bash", "error", { command: "exit 1" }, { error: "Command exited 1" }),
    ),
    180,
  );
  await timeline.send(
    partUpdated(
      toolPart(questionID, "question", "error", questionInput(), {
        error: "The user dismissed this question",
      }),
    ),
    250,
  ...
  await expect(
    page.locator(`[data-timeline-part-id="${shellID}"] [data-kind="tool-error-card"]`),
  ).toBeVisible();
