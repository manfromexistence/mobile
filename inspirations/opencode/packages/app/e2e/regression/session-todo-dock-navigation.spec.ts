...
const activeTodos = [
  {
    id: "todo-1",
    content: "Receive todos in the active session",
    status: "completed",
    priority: "high",
  },
  {
    id: "todo-2",
    content: "Keep the dock visible across tabs",
    status: "completed",
    priority: "high",
  },
...
test("animates todo lifecycle without replaying it across session tabs", async ({ page }) => {
  ...
  await mockOpenCodeServer(page, {
    ...
    sessions: [
      session(sourceID, sourceTitle, 1700000000000),
  ...
  expect((await opening).some((sample) => sample.opacity > 0.05 && sample.opacity < 0.95)).toBe(
    true,
  );
  ...
  await expect(dock).toHaveCount(0);
  expect((await closing).some((sample) => sample.opacity > 0.05 && sample.opacity < 0.95)).toBe(
    true,
  );
  todos[sourceID] = [];
  ...
  await page.addInitScript(
    ({ directory, dirBase64, server, sessionIDs }) => {
      ...
      localStorage.setItem(
        ...
        JSON.stringify(
          sessionIDs.map((sessionId) => ({ type: "session", server, dirBase64, sessionId })),
        ),
      );
    },
  ...
  return page.evaluate(async (duration) => {
    ...
    while (performance.now() - start < duration) {
      ...
      const label = dock?.querySelector<HTMLElement>(
        '[data-action="session-todo-toggle"] span[aria-label]',
      );
      samples.push({
        present: !!dock,
