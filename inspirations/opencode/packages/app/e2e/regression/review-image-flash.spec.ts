...
test("clicking an image file in the v2 review pane does not blank the panel", async ({ page }) => {
  ...
  expect(
    bad,
    JSON.stringify(
      { bad: bad.slice(0, 8), first: trace.samples.slice(0, 8), last: trace.samples.slice(-4) },
      null,
      2,
    ),
  ).toEqual([]);
  ...
  await page.evaluate(() => {
    ...
    const sample = () => {
      const panel = document.querySelector<HTMLElement>(
        '#review-panel [data-component="session-review-v2"]',
      );
    ...
    (
      window as Window & { __reviewImageFlash?: { samples: typeof samples; startedAt: number } }
    ).__reviewImageFlash = {
      samples,
      startedAt,
    };
...
async function waitForReviewFlashProbe(page: Page, durationMs: number) {
  await page.waitForFunction((durationMs) => {
    const state = (
      window as Window & { __reviewImageFlash?: { samples: unknown[]; startedAt: number } }
    ).__reviewImageFlash;
...
async function collectReviewFlashProbe(page: Page) {
  return page.evaluate(() => {
    return (window as Window & { __reviewImageFlash?: { samples: unknown[]; startedAt: number } })
      .__reviewImageFlash!;
  }) as Promise<{
    startedAt: number;
    samples: Array<{
