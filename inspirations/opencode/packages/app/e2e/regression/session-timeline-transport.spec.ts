...
test("keeps one connection open while delivering multiple events", async ({ page }) => {
  ...
  const first = await timeline.transport.send(
    partUpdated(textPart("prt_transport_first", "first event")),
  );
  const second = await timeline.transport.send(
    partUpdated(textPart("prt_transport_second", "second event")),
  );
...
test("parses split JSON and a split multibyte code point", async ({ page }) => {
  ...
  const acknowledgement = await timeline.transport.split(payload, [
    9,
    multibyte + 1,
    multibyte + 2,
  ]);
