...
test("closing the active server's last tab opens the remaining server tab", async ({ page }) => {
  ...
  await expect
    .poll(() => requests.some((url) => url.startsWith(`${serverB}/session/${sessionB.id}`)))
    .toBe(true);
...
async function mockServers(page: Page, requests: string[]) {
  await page.route("**/*", async (route) => {
    ...
    if (directory && directory !== current.directory)
      return json(route, { name: "InvalidDirectory" }, 500);
    ...
    if (
      ...
      return json(route, []);
    if (
      ["/global/config", "/config", "/provider/auth", "/mcp", "/session/status"].includes(
        url.pathname,
      )
    )
      return json(route, {});
