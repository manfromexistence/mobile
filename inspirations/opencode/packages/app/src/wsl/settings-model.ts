...
export type AddServerRuntimeState =
  | "loading"
  | "pendingRestart"
  | "checking"
  | "unavailable"
...
export function addServerViewModel(input: {
  ...
  const visibleInstalledDistros = (state?.installed ?? []).filter(
    (item) => !isHiddenDistro(item.name),
  );
  ...
  const addableInstalledDistros = visibleInstalledDistros.filter(
    (item) => !existingServerDistros.has(item.name),
  );
  const selectedDistro = addServerSelectedDistro(
  ...
  const installableDistros = addServerInstallableDistros(
    visibleInstalledDistros,
    visibleOnlineDistros,
  );
  const filteredInstallableDistros = addServerFilteredInstallableDistros(
    installableDistros,
    input.catalogSearch,
  );
  ...
  return {
    ...
    distroStatuses: Object.fromEntries(
      addableInstalledDistros.flatMap((item) => {
        const status = addServerDistroStatus({
          state,
          name: item.name,
...
function addServerSelectedDistro(
  ...
  if (
    selected &&
    addableInstalledDistros.some((item) => item.name === selected && item.version !== 1)
  )
    return selected;
...
function addServerDistroStatus(input: {
  ...
  if (installed?.version === 1)
    return { label: { key: "wsl.onboarding.distroStatus.unsupported" }, tone: "muted" };
  const job = input.state?.job;
  const probe = input.state?.distroProbes[input.name];
  if (!probe) {
    if (
      input.probingAddable ||
      (job?.kind === "probe-addable" && job.distros.includes(input.name))
    ) {
  ...
  if (check.matchesDesktop === false)
    return { label: { key: "wsl.onboarding.updateOpencode" }, tone: "warning" };
  if (!check.resolvedPath)
    return { label: { key: "wsl.onboarding.distroStatus.opencodeMissing" }, tone: "warning" };
...
function addServerPrimaryButton(input: {
  ...
  const probingSelected =
    input.probingAddable && !addServerSelectedDistroSettled(input.state, input.selectedDistro);
  ...
  const installingOpencode =
    input.state?.job?.kind === "install-opencode" &&
    input.state.job.distro === input.selectedDistro;
  if (!ready || probingOpencode) {
    return {
      variant: "contrast",
      label: probingSelected
        ? { key: "wsl.onboarding.distroStatus.checking" }
        : { key: "wsl.server.add" },
      disabled: true,
      action: null,
  ...
  if (!addServerOpencodeReady(input.opencodeCheck)) {
    const update =
      !!input.opencodeCheck?.resolvedPath && input.opencodeCheck.matchesDesktop === false;
    return {
      variant: "neutral",
...
function addServerSelectedDistroSettled(
  state: WslServersState | undefined,
  selectedDistro: string | null,
) {
...
function addServerInstallableDistros(
  installedDistros: WslInstalledDistro[],
...
function addServerFilteredInstallableDistros(
  installableDistros: WslOnlineDistro[],
  search: string,
) {
  const query = search.trim();
  if (!query) return installableDistros;
  return fuzzysort
    .go(query, installableDistros, { keys: ["label", "name"] })
    .map((item) => item.obj);
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.opencodeChecks[item.name])
      return [`opencode:${item.name}`];
    return [];
  });
...
export function autoProbePlan(input: { state: WslServersState | undefined; busy: boolean }) {
  ...
  if (!input.state.runtime)
    return { key: "runtime", action: "probeRuntime" } satisfies AutoProbePlan;
...
export function addServerProbePlan(input: {
  ...
  if (auto)
    return { kind: "auto", key: `auto:${auto.key}`, plan: auto } satisfies AddServerProbePlan;
  const addable = addableProbePlan(input);
  if (addable)
    return {
      kind: "addable",
      key: `addable:${addable.key}`,
      plan: addable,
    } satisfies AddServerProbePlan;
}
