...
const useTerminalUiBindings = (input: {
  ...
  input.cleanups.push(() =>
    input.container.removeEventListener("pointerdown", input.handlePointerDown),
  );
...
export const Terminal = (props: TerminalProps) => {
  ...
  const start =
    typeof local.pty.cursor === "number" && Number.isSafeInteger(local.pty.cursor)
      ? local.pty.cursor
      : undefined;
  let cursor = start ?? 0;
  ...
  onMount(() => {
    const run = async () => {
      ...
      const open = async () => {
        ...
        const handleClose = (event: CloseEvent) => {
          ...
          retry(
            new Error(language.t("terminal.connectionLost.abnormalClose", { code: event.code })),
          );
        };
    ...
    void run().catch((err) => {
      ...
      showToast({
        ...
        description:
          err instanceof Error ? err.message : language.t("terminal.connectionLost.description"),
      });
      local.onConnectError?.(err);
    });
  });
