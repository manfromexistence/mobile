...
const FileTreeNodeV2 = (
  ...
  return (
    ...
    >
      {local.children}
      <span class="flex-1 min-w-0 text-12-medium whitespace-nowrap truncate">
        {local.node.name}
      </span>
      {(() => {
        const value = kind();
  ...
  const deeps = createMemo(() => {
    ...
    const push = (dir: string, lvl: number) => {
      ...
      const kids = file.tree
        .children(dir)
        .filter(
          (node) => node.type === "directory" && (file.tree.state(node.path)?.expanded ?? false),
        )
        .map((node) => node.path);
    ...
    <div data-component="file-tree-v2" class="group/file-tree-v2">
      <For each={nodes()}>
        {(node) => {
          ...
          const hasChildren = () =>
            visibleNodesForPath(node.path, file.tree.children, filter()).length > 0;
          return (
            <Switch>
              <Match when={node.type === "directory"}>
                <Collapsible
                  ...
                  onOpenChange={(open) =>
                    open
                      ? file.tree.expand(node.path, { list: false })
