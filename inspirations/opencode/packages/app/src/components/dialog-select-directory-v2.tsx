...
export function DialogSelectDirectoryV2(props: DialogSelectDirectoryV2Props) {
  ...
  >();
  const loads = createPriorityTaskQueue<
  ...
  const [suggestions] = createResource(input, async (value) => {
    ...
    return {
      query: value,
      items: Array.from(new Map(results.map((result) => [result.absolute, result])).values()).slice(
        0,
        8,
      ),
    };
  ...
  async function load(path: string, generation: number, eager = false) {
    ...
    if (!eager && advanceTreePreload(advanced, key)) {
      for (const directory of preloadTreeDirectories(key, nodes))
        void load(directory, generation, true);
    }
    return true;
  }
  ...
  async function navigate(path: string) {
    const value = policy.navigation(
      pickerAbsoluteInput(cleanPickerInput(path), home(), root() || start() || home()),
    );
    if (!value) return;
  ...
  function moveSuggestion(delta: -1 | 1) {
    ...
    setActiveSuggestion((current) =>
      nextSuggestionIndex(current, delta, currentSuggestions().length),
    );
  }
  ...
  return (
    <Dialog size="large" class="directory-picker-v2">
        ...
        <div class="directory-picker-v2-path" ref={pathArea}>
          <TextInputV2
            ...
            aria-activedescendant={
              activeSuggestion() >= 0
                ? `directory-picker-v2-suggestion-${activeSuggestion()}`
                : undefined
            }
      ...
      <DialogFooter>
        ...
        <ButtonV2
          variant="contrast"
          disabled={!policy.result(root(), selected(), rootValid())}
          onClick={resolve}
        >
