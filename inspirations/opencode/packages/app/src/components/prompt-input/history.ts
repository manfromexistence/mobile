...
function isPromptEqual(promptA: PromptHistoryStoredEntry, promptB: PromptHistoryStoredEntry) {
  ...
  for (let i = 0; i < entryA.prompt.length; i++) {
    ...
    if (partA.type === "text" && partA.content !== (partB.type === "text" ? partB.content : ""))
      return false;
    if (partA.type === "file") {
    ...
    if (partA.type === "agent" && partA.name !== (partB.type === "agent" ? partB.name : ""))
      return false;
    if (partA.type === "image" && partA.id !== (partB.type === "image" ? partB.id : ""))
      return false;
  }
