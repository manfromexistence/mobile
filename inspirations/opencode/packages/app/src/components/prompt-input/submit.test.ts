        ...
        optimistic: {
          add: (value: {
            ...
            optimisticSeeded.push(
              !!value.directory &&
                !!value.sessionID &&
                !!storedSessions[value.directory]?.find((item) => item.id === value.sessionID)
                  ?.title,
            );
          },
          remove: () => undefined,
...
describe("prompt submit worktree selection", () => {
  ...
  test("promotes drafts using the selected project's server", async () => {
    ...
    expect(promotedDrafts).toEqual([
      { draftID: "draft-1", server: "project-server", sessionId: "session-1" },
    ]);
  });
  ...
  test("seeds new sessions before optimistic prompts are added", async () => {
    ...
    expect(storedSessions["/repo/worktree-a"]).toEqual([
      { id: "session-1", title: "New session 1" },
    ]);
