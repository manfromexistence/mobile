...
test("preserves POSIX case while matching Windows drives case-insensitively", () => {
  ...
  expect(
    pickerMode("file", "//Server/Share/Repo").selection("//server/share/repo/src", "file.ts"),
  ).toBe("src/file.ts");
...
test("resolves relative input against the current picker root", () => {
  ...
  expect(pickerAbsoluteInput("~/.config", "/home/luke", "/home/luke/repo")).toBe(
    "/home/luke/.config",
  );
  expect(pickerAbsoluteInput("src", "C:/Users/luke", "C:/Users/luke/repo")).toBe(
...
test("scopes file autocomplete to the current browser root", () => {
  expect(pickerFileSearchQuery("/home/luke/repos", "/home/luke/repos/src/in", "/home/luke")).toBe(
    "src/in",
  );
...
test("returns absolute directories and relative files", () => {
  ...
  expect(selectedTreePath("/home/luke/repo/src", "index.ts", "file", "/home/luke/repo")).toBe(
    "src/index.ts",
  );
