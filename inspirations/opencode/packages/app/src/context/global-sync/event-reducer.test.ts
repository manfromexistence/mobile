...
describe("applyDirectoryEvent", () => {
  ...
  test("upserts and removes messages while clearing orphaned parts", () => {
    ...
    const [store, setStore] = createStore(
      baseState({
        message: {
          [sessionID]: [userMessage("msg_1", sessionID), userMessage("msg_3", sessionID)],
        },
  ...
  test("tracks permission and question request lifecycles", () => {
    ...
    const [store, setStore] = createStore(
      baseState({
        ...
        question: {
          [sessionID]: [questionRequest("q_1", sessionID), questionRequest("q_3", sessionID)],
        },
      }),
    );
    ...
    expect(store.question[sessionID]?.find((x) => x.id === "q_2")?.questions[0]?.header).toBe(
      "updated",
    );
  ...
  test("updates vcs branch in store and cache", () => {
    const [store, setStore] = createStore(
      baseState({ vcs: { branch: "main", default_branch: "main" } }),
    );
