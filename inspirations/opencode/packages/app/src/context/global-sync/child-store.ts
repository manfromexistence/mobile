...
export function createChildStoreManager(input: {
  ...
  function ensureChild(directory: string) {
    ...
    if (!children[key]) {
      ...
      if (!meta)
        throw new Error(input.translate("error.childStore.persistedProjectMetadataCreateFailed"));
      ...
      if (!icon)
        throw new Error(input.translate("error.childStore.persistedProjectIconCreateFailed"));
        ...
        createRoot((dispose) => {
          ...
          const child = createStore<State>({
            ...
            get provider() {
              ...
              if (providerQuery.data?.all.size === 0 && input.global.provider.all.size > 0)
  ...
  function enableMcp(
    directory: string,
    key: DirectoryKey,
    childStore: [Store<State>, SetStoreFunction<State>],
  ) {
  ...
  function projectMeta(directory: string, patch: ProjectMeta) {
    ...
    const commands = patch.commands
      ? { ...previous.commands, ...patch.commands }
      : previous.commands;
    const next = {
      ...previous,
      ...patch,
