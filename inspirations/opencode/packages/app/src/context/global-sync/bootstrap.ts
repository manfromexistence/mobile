...
export const loadReferencesQuery = (scope: ServerScope, directory: string, sdk: OpencodeClient) =>
  queryOptions<ReferenceInfo[]>({
    ...
    queryFn: () =>
      retry(() => sdk.v2.reference.list().then((x) => x.data?.data ?? [])).catch(() => []),
    placeholderData: [],
  });
...
export async function bootstrapDirectory(input: {
  ...
  const seededPath =
    input.global.path.directory === input.directory ? input.global.path : undefined;
    ...
    const slow = [
      ...
      () =>
        retry(() =>
          input.sdk.config
            .get()
            .then((x) => input.setStore("config", reconcile(x.data!, { merge: false }))),
        ),
      () =>
        retry(() =>
          input.sdk.session.status().then(async (x) => {
            ...
            await Promise.all(
              Object.keys(statuses).map((sessionID) =>
        ...
        retry(() =>
          input.sdk.permission.list().then((x) => {
            ...
            const warm = input.session
              ? Promise.all(ids.map((sessionID) => input.session!.resolve(sessionID))).then(
                  () => undefined,
                )
      ...
      () => Promise.resolve(input.loadSessions(input.directory)),
      input.mcp &&
        (() => input.queryClient.fetchQuery(loadMcpQuery(input.scope, input.directory, input.sdk))),
      input.mcp &&
        (() =>
          input.queryClient.fetchQuery(
            loadMcpResourcesQuery(input.scope, input.directory, input.sdk),
          )),
      () =>
        input.queryClient
          .fetchQuery(loadProvidersQuery(input.scope, input.directory, input.sdk))
          .catch((err) => {
