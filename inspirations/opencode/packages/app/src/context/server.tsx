...
export function createServerProjects<T extends ServerProjectState>(input: {
  ...
  return {
    ...
    close(directory: string) {
      ...
      const closed = [
        directory,
        ...currentClosed().filter((worktree) => pathKey(worktree) !== key),
...
export function nextServerAfterRemoval(
  ...
  const next =
    remaining.find((server) => ServerConnection.key(server) === fallback) ?? remaining[0];
...
export const { use: useServer, provider: ServerProvider } = createSimpleContext({
  ...
  init: (props: {
    ...
    const scope = (key = state.active) =>
      ServerScope.fromServerKey(key, props.canonicalLocalServer);
