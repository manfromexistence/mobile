...
function mergeOptimisticPage(page: MessagePage, items: OptimisticItem[]) {
  if (items.length === 0)
    return { ...page, observed: [] as { messageID: string; parts: Part[] }[] };
  ...
  for (const item of items) {
    ...
    const confirmed = result.found
      ? item.parts.filter(
          (part) => Binary.search(current ?? [], part.id, (value) => value.id).found,
        )
      : [];
...
export function createServerSession(client: OpencodeClient, options?: { retry?: typeof retry }) {
  ...
  const replaceMessages = (sessionID: string, messages: Message[]) => {
    ...
    const dropped = (data.message[sessionID] ?? []).filter(
  ...
  const replaceParts = (
    ...
    for (const item of items) {
      ...
      for (const part of fetched) {
        ...
        if (load?.carriedDeltaParts.get(item.id)?.has(part.id) && !preserveDelta)
  ...
  const applyMessagePage = (
    ...
    merged.observed.forEach((item) => {
      if (!load?.clearedMessageParts.has(item.messageID))
        confirmOptimistic(sessionID, item.messageID, item.parts);
    });
    ...
    switch (event.type) {
      ...
      case "session.diff": {
        ...
        setData(
          "session_diff",
          props.sessionID,
          reconcile(cleanDiffs(props.diff), { key: "file" }),
        );
        return;
      }
      case "todo.updated": {
      ...
      case "message.part.updated": {
        ...
        const missing =
          !messages || !Binary.search(messages, part.messageID, (message) => message.id).found;
      ...
      case "message.part.removed": {
        ...
        if (deltas?.size === 0)
          messageLoads.get(props.sessionID)?.deltaParts.delete(props.messageID);
        const load = messageLoads.get(props.sessionID);
      ...
      case "permission.asked": {
        ...
        if (result.found)
          setData("permission", permission.sessionID, result.index, reconcile(permission));
        if (!result.found)
          setData(
            "permission",
      ...
      case "question.asked": {
        ...
        if (result.found)
          setData("question", question.sessionID, result.index, reconcile(question));
        if (!result.found)
          setData(
            "question",
            question.sessionID,
    ...
    optimistic: {
      add(input: { sessionID: string; message: Message; parts: Part[] }) {
        ...
        if (!items)
          optimistic.set(
            input.sessionID,
            new Map([[input.message.id, { ...input, parts, confirmedParts: [] }]]),
          );
