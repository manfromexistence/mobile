...
describe("autoRespondsPermission", () => {
  ...
  test("uses a parent session's legacy auto-accept key", () => {
    ...
    expect(
      autoRespondsPermission({ root: true }, sessions, permission("child"), "/tmp/project"),
    ).toBe(true);
  });
  ...
  test("defaults to requiring approval when no lineage override exists", () => {
    ...
    expect(autoRespondsPermission(autoAccept, sessions, permission("child"), "/tmp/project")).toBe(
      false,
    );
  });
  ...
  test("inherits a parent session's false override", () => {
    ...
    expect(autoRespondsPermission(autoAccept, sessions, permission("child"), directory)).toBe(
      false,
    );
  });
