...
export const { use: useTabs, provider: TabsProvider } = createSimpleContext({
  ...
  init: () => {
    ...
    const [recent, setRecent, , recentReady] = persisted(
      Persist.window("tabs.recent"),
      createStore<RecentTab>({}),
    );
    ...
    const removeDraftPersisted = (draftID: string) => {
      for (const key of draftPersistedKeys())
        removePersisted(Persist.draft(draftID, key), platform);
    };
    ...
    const actions = {
      ...
      newDraft(draft: Omit<DraftTab, "type" | "draftID">, prompt?: string) {
        ...
        void startTransition(() => {
          ...
          navigate(
            prompt
              ? `${draftHref(draftID)}&prompt=${encodeURIComponent(prompt)}`
              : draftHref(draftID),
          );
        });
      },
      ...
      removeServer(key: ServerConnection.Key) {
        const drafts = store.flatMap((tab) =>
          tab.type === "draft" && tab.server === key ? [tab.draftID] : [],
        );
