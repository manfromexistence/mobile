...
export const ProjectDragOverlay = (props: {
  ...
  const project = createMemo(() =>
    props.projects().find((p) => p.worktree === props.activeProject()),
  );
  return (
    <Show when={project()}>
      {(p) => (
...
const ProjectTile = (props: {
  ...
  const unseenCount = createMemo(() =>
    props
      .dirs()
      .reduce((total, directory) => total + notification.project.unseenCount(directory), 0),
  );
...
const ProjectPreviewPanel = (props: {
  ...
  <div class="-m-3 p-2 flex flex-col w-72">
    ...
    <div class="px-4 pb-2 text-12-medium text-text-weak">
...
export const SortableProject = (props: {
  ...
  const selected = createMemo(
    () => props.ctx.currentProject()?.worktree === props.project.worktree,
  );
  ...
  const active = createMemo(
    () => state.menu || (preview() ? isHoverProject() : overlay() && isHoverProject()),
  );
  ...
  const projectStore = createMemo(
    () => serverSync().child(props.project.worktree, { bootstrap: false })[0],
  );
