...
describe("shouldFocusTerminalOnKeyDown", () => {
  test("skips pure modifier keys", () => {
    expect(
      shouldFocusTerminalOnKeyDown(new KeyboardEvent("keydown", { key: "Meta", metaKey: true })),
    ).toBe(false);
    expect(
      shouldFocusTerminalOnKeyDown(new KeyboardEvent("keydown", { key: "Control", ctrlKey: true })),
    ).toBe(false);
    expect(
      shouldFocusTerminalOnKeyDown(new KeyboardEvent("keydown", { key: "Alt", altKey: true })),
    ).toBe(false);
    expect(
      shouldFocusTerminalOnKeyDown(new KeyboardEvent("keydown", { key: "Shift", shiftKey: true })),
    ).toBe(false);
  });

  test("skips shortcut key combos", () => {
    expect(
      shouldFocusTerminalOnKeyDown(new KeyboardEvent("keydown", { key: "c", metaKey: true })),
    ).toBe(false);
    expect(
      shouldFocusTerminalOnKeyDown(new KeyboardEvent("keydown", { key: "c", ctrlKey: true })),
    ).toBe(false);
    expect(
      shouldFocusTerminalOnKeyDown(
  ...
  test("keeps plain typing focused on terminal", () => {
    ...
    expect(
      shouldFocusTerminalOnKeyDown(new KeyboardEvent("keydown", { key: "A", shiftKey: true })),
    ).toBe(true);
  });
});
  ...
  test("normalizes the effective file tab", () => {
    createRoot((dispose) => {
      ...
      const result = createSessionTabs({
        ...
        normalizeTab: (tab) =>
          tab.startsWith("file://") ? `norm:${tab.slice("file://".length)}` : tab,
      });
