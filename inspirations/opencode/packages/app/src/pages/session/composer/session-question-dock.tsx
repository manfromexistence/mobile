...
const cache = new Map<
  string,
  { tab: number; answers: QuestionAnswer[]; custom: string[]; customOn: boolean[] }
>();
...
export const SessionQuestionDock: Component<{ request: QuestionRequest; onSubmit: () => void }> = (
  props,
) => {
  const sdk = useSDK();
  ...
  const nav = (event: KeyboardEvent) => {
    ...
    const target =
      event.target instanceof HTMLElement
        ? event.target.closest('[data-slot="question-options"]')
        : undefined;
    if (store.editing) return;
            ...
            <div data-slot="question-header-actions">
              ...
              <button
                ...
                aria-label={language.t(
                  store.minimized ? "session.question.restore" : "session.question.minimize",
                )}
              >
