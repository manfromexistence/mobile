...
describe("persist localStorage resilience", () => {
  ...
  test("server workspace target preserves local storage and isolates remote storage", () => {
    ...
    const windows = Persist.serverWorkspace(
      "https://windows.example" as ServerScope,
      "/home/luke/repo",
      "prompt",
    );
    const debian = Persist.serverWorkspace(
      "https://debian.example" as ServerScope,
      "/home/luke/repo",
      "prompt",
    );
  ...
  test("server global target cannot collide when scope and key contain colons", () => {
    expect(Persist.serverGlobal("a:b" as ServerScope, "c")).not.toEqual(
      Persist.serverGlobal("a" as ServerScope, "b:c"),
    );
  });
});
