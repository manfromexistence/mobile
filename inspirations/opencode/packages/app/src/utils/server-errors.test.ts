...
describe("formatServerError", () => {
  ...
  test("returns provided string errors", () => {
    expect(formatServerError("Failed to connect to server", language.t)).toBe(
  ...
  test("falls back for unknown error objects and names", () => {
    expect(
      formatServerError({ name: "ServerTimeoutError", data: { seconds: 30 } }, language.t),
...
describe("isSessionNotFoundError", () => {
  ...
  test("rejects errors for other sessions and other 404 responses", () => {
    ...
    expect(
      isSessionNotFoundError(new Error(body.message, { cause: { body, status: 404 } }), "ses_tab"),
    ).toBe(false);
    expect(
      isSessionNotFoundError(
