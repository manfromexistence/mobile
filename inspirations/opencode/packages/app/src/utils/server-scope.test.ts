...
describe("ServerScope", () => {
  test("uses a stable local scope for the canonical sidecar", () => {
    expect(
      String(
        ServerScope.fromServerKey("sidecar" as Parameters<typeof ServerScope.fromServerKey>[0]),
      ),
    ).toBe("local");
  });
    ...
    expect(
      String(
        ServerScope.fromServerKey(
          "http://localhost:4096" as Parameters<typeof ServerScope.fromServerKey>[0],
        ),
      ),
...
describe("SessionStateKey", () => {
  ...
  test("extracts route keys from scoped and legacy state keys", () => {
    ...
    expect(String(SessionStateKey.route("https://debian.example\0cmVwbw/session-1"))).toBe(
      "cmVwbw/session-1",
    );
  });
});
