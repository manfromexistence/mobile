...
export function formatServerError(error: unknown, translate?: Translator, fallback?: string) {
  ...
  if (isConfigInvalidErrorLike(unwrapped))
    return parseReadableConfigInvalidError(unwrapped, translate);
  if (isProviderModelNotFoundErrorLike(unwrapped))
    return parseReadableProviderModelNotFoundError(unwrapped, translate);
...
export function parseReadableConfigInvalidError(
  ...
  const file =
    errorInput.data.path && errorInput.data.path !== "config" ? errorInput.data.path : "config";
  ...
  if (!msg)
    return tr(translator, "error.chain.configInvalid", `Config file at ${file} is invalid`, {
      path: file,
    });
  return tr(
    translator,
...
function parseReadableProviderModelNotFoundError(
  ...
  if (list.length) {
    ...
    return [
      body,
      tr(translator, "error.chain.didYouMean", `Did you mean: ${suggestions}`, { suggestions }),
      tail,
    ].join("\n");
  }
