...
describe("http-recorder", () => {
  ...
  test("redacts URL credentials", () => {
    expect(
      HttpRecorderInternal.redactUrl("https://user:password@example.test/path?safe=value"),
    ).toBe("https://%5BREDACTED%5D:%5BREDACTED%5D@example.test/path?safe=value");
  });
  ...
  test("records WebSocket frames in observed client/server order", async () => {
    ...
    let receive:
      | ((message: string | Uint8Array) => Effect.Effect<unknown, unknown, unknown> | void)
      | undefined;
    ...
    expect(
      JSON.parse(fs.readFileSync(path.join(directory, "websocket/record.json"), "utf8")),
    ).toMatchObject({
      interactions: [
        {
          transport: "websocket",
            ...
            Layer.provide(
              Layer.succeed(
                ...
                Socket.make({
                  ...
                  writer: Effect.succeed(() =>
                    Effect.die(new Error("unexpected live WebSocket write")),
                  ),
                }),
              ),
            ),
    ...
    const exit = await Effect.runPromise(
      Effect.gen(function* () {
        ...
        return yield* Effect.exit(
          socket.runRaw(() => {}, { onOpen: write(new Socket.CloseEvent(1000)) }),
        );
      }).pipe(
        Effect.scoped,
        Effect.provide(
  ...
  test("replay returns recorded responses in order for identical requests", async () => {
    await runWith(
      ...
      {},
      Effect.gen(function* () {
        expect(yield* post("https://example.test/poll", { id: "job_1" })).toBe(
          '{"status":"pending"}',
        );
        expect(yield* post("https://example.test/poll", { id: "job_1" })).toBe(
          '{"status":"complete"}',
  ...
  test("concurrent replay claims each interaction once", async () => {
    const results = await runWith(
      ...
      Effect.all(
        [
          post("https://example.test/poll", { id: "job_1" }),
          post("https://example.test/poll", { id: "job_1" }),
        ],
  ...
  test("records to disk when the cassette is missing", async () => {
    ...
    using server = Bun.serve({
      port: 0,
      fetch: () =>
        new Response('{"reply":"recorded"}', { headers: { "content-type": "application/json" } }),
    });
  ...
  test("records concurrent requests in request-start order", async () => {
    ...
    try {
      const request = (name: string) =>
        Effect.gen(function* () {
          ...
          const response = yield* http.execute(
            HttpClientRequest.get(`http://127.0.0.1:${server.port}/${name}`),
          );
          return yield* response.text;
        });
      ...
      const cassette = JSON.parse(
        fs.readFileSync(path.join(directory, "concurrent-order.json"), "utf8"),
      );

      expect(completed).toEqual(["second", "first"]);
      expect(responses).toEqual(["first", "second"]);
      expect(
        cassette.interactions.map((interaction: Interaction) => interaction.request.url),
      ).toEqual([
  ...
  test("returns the live response while persisting its redacted snapshot", async () => {
    ...
    try {
      ...
      const cassette = JSON.parse(
        fs.readFileSync(path.join(directory, "live-response.json"), "utf8"),
      );

      expect(body).toBe('{"access_token":"live-secret","safe":true}');
      expect(cassette.interactions[0].response.body).toBe(
  ...
  test("records and replays arbitrary binary responses without changing bytes", async () => {
    ...
    try {
      ...
      const record = await Effect.runPromise(
        program.pipe(Effect.provide(HttpRecorder.http("binary", { directory }))),
      );
      await server.stop();
    ...
    const exit = await Effect.runPromise(
      Effect.exit(
        post(url, { ok: true }).pipe(
          Effect.provide(
            HttpRecorderInternal.cassetteLayer("unsafe-record", { directory, mode: "record" }),
          ),
        ),
      ),
    );
  ...
  test("rejects cassette paths outside the recordings directory", () => {
    ...
    expect(() => HttpRecorderInternal.hasCassetteSync("../outside", { directory })).toThrow(
      "Invalid cassette name",
    );
    expect(() => HttpRecorderInternal.hasCassetteSync("C:\\outside", { directory })).toThrow(
      "Invalid cassette name",
    );
  });
