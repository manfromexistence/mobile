...
export class Service extends Context.Service<Service, Interface>()(
  "@opencode-ai/http-recorder/Cassette",
) {}
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const recorded = new Map<
        string,
        { interactions: Interaction[]; findings: SecretFinding[] }
      >();
        ...
        Effect.gen(function* () {
          ...
          const nested = yield* Effect.forEach(entries, (entry) => {
            ...
            return fs.stat(full).pipe(
              Effect.flatMap((stat) =>
                stat.type === "Directory" ? walk(full) : Effect.succeed([full]),
              ),
          ...
          appendLock.withPermit(
            Effect.gen(function* () {
              ...
              yield* fs.writeFileString(temporary, formatCassette(cassette)).pipe(
                ...
                Effect.ensuring(
                  fs.remove(temporary, { force: true }).pipe(Effect.catch(() => Effect.void)),
                ),
                Effect.orDie,
              );
      ...
      append: (name, interaction, metadata) =>
        appendLock.withPermit(
          Effect.suspend(() => {
            ...
            const findings = [
              ...(accumulatedFindings.get(name) ?? []),
              ...secretFindings(interaction),
            ];
            const allFindings = metadata
              ? [...findings, ...secretFindings({ name, ...metadata })]
              : findings;
