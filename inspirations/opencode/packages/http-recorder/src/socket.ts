...
const assertEvent = (
  actual: WebSocketEvent,
  expected: WebSocketEvent | undefined,
  index: number,
  asJson: boolean,
) =>
  Effect.sync(() => {
    ...
    throw new Error(
      `WebSocket event ${index + 1}: expected ${safeText(expected)}, received ${safeText(actual)}`,
    );
  });
    ...
    return Socket.make({
      runRaw: (handler, runOptions) =>
        Effect.gen(function* () {
          ...
          if (occupied)
            return yield* Effect.die("Concurrent runs of a recorded WebSocket are not supported");
          yield* upstream
            .runRaw(
              (message) => {
                if (!Ref.getUnsafe(state.accepting))
          ...
          (write) => (message) =>
            writeLock.withPermit(
              Effect.gen(function* () {
                ...
                return yield* write(message).pipe(
          ...
          const claimed = yield* replay
            .claim((interaction, index) =>
              Effect.sync(() => {
                ...
                if (
                  interaction &&
                  JSON.stringify(canonicalizeJson(incoming)) ===
                    JSON.stringify(canonicalizeJson(interaction.open))
                )
                  return;
                throw new Error(
...
export const socket = (
  ...
  provideCassette(
    recordingLayer(name, { url: "" }, { ...options, compareClientMessagesAsJson: true }),
    options,
  );
