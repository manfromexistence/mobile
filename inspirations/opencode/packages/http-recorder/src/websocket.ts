...
const jsonOrText = (value: string) =>
  Option.match(decodeJson(value), { onNone: () => value, onSome: canonicalizeJson });

const assertClientEvent = (
  actual: string,
          ...
          const claimed = yield* replay
            .claim((interaction, index) =>
              Effect.sync(() => {
                ...
                if (
                  interaction &&
                  JSON.stringify(incoming) === JSON.stringify(canonicalizeJson(interaction.open))
                )
                  return;
            ...
            close: Effect.gen(function* () {
              ...
              if (used !== client.length)
                return yield* Effect.die(
                  new Error(
                    `WebSocket client frame count: expected ${client.length}, received ${used}`,
                  ),
                );
            }),
          };
