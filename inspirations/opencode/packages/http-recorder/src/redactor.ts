...
export const compose = (...redactors: ReadonlyArray<Partial<Redactor>>): Redactor => {
  ...
  return {
    request:
      requests.length === 0
        ? identity
        : (snapshot) => requests.reduce((acc, fn) => fn(acc), snapshot),
    response:
      responses.length === 0
        ? identity
        : (snapshot) => responses.reduce((acc, fn) => fn(acc), snapshot),
  };
};
...
export const requestHeaders = (options: HeaderOptions = {}): Partial<Redactor> => ({
  request: (snapshot) => ({
    ...
    headers: redactHeaders(
      snapshot.headers,
      options.allow ?? DEFAULT_REQUEST_HEADERS,
      options.redact,
    ),
  }),
});
...
export const responseHeaders = (options: HeaderOptions = {}): Partial<Redactor> => ({
  response: (snapshot) => ({
    ...
    headers: redactHeaders(
      snapshot.headers,
      options.allow ?? DEFAULT_RESPONSE_HEADERS,
      options.redact,
    ),
  }),
});
