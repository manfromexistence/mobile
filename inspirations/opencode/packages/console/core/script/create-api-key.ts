...
function parseArgs(argv: string[]) {
  ...
  for (let index = 0; index < argv.length; index++) {
    ...
    if (arg === "--balance-dollars")
      parsed.balanceDollars = Number(requiredValue(argv, ++index, arg));
    if (arg === "--allow-production") parsed.allowProduction = true;
  }
  if (!Number.isFinite(parsed.balanceDollars) || parsed.balanceDollars < 0)
    throw new Error("Invalid --balance-dollars");
  return parsed;
}
