...
if (!fromPrevPayment?.customerID)
  throw new Error(`Error: FROM workspace has no previous Stripe customer to revert to`);
...
const fromPrevPaymentMethods = await Billing.stripe().customers.listPaymentMethods(
