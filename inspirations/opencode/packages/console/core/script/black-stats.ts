  ...
  tx
    ...
    .where(
      and(
        ...
        sql`JSON_UNQUOTE(JSON_EXTRACT(subscription, '$.plan')) = ${plan}`,
      ),
    ),
);
  ...
  tx
    ...
    .where(
      and(
        ...
        sql`JSON_UNQUOTE(JSON_EXTRACT(enrichment, '$.plan')) = 'sub'`,
      ),
    )
...
const tokenMap = new Map<
  string,
  Map<number, { input: number; cacheRead: number; output: number }>
>();
...
const users = await Database.use((tx) =>
  tx
    ...
    .where(
      and(
        inArray(SubscriptionTable.workspaceID, workspaceIDs),
        sql`${SubscriptionTable.timeCreated} <= ${cutoff}`,
      ),
    ),
);
...
const rows = users
  .map((user) => {
    ...
    const modelRows = (modelMap.get(user.workspaceID) ?? [])
