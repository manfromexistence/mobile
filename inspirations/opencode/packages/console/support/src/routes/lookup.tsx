...
export default function LookupPage() {
  ...
  const data = createAsync(() =>
    identifier() ? getLookup(identifier()) : Promise.resolve(undefined),
  );
  ...
  return (
    <main data-page="support">
      ...
      <Show
        when={identifier()}
        fallback={<div data-empty>Provide an `identifier` query parameter.</div>}
      >
        <ErrorBoundary
          fallback={(err) => <div data-component="error">{(err as Error).message}</div>}
        >
