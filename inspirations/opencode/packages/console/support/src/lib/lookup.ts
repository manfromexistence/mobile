  ...
  const usage = await Database.use((tx) =>
    tx
      .select({
        ...
        freeRequests: sql<number>`SUM(CASE WHEN ${UsageTable.cost} = 0 THEN 1 ELSE 0 END)`.as(
          "free_requests",
        ),
        goRequests: sql<number>`SUM(CASE WHEN ${planExpr} = 'lite' THEN 1 ELSE 0 END)`.as(
          "go_requests",
        ),
        goCost:
          sql<number>`SUM(CASE WHEN ${planExpr} = 'lite' THEN ${UsageTable.cost} ELSE 0 END)`.as(
            "go_cost",
          ),
        apiRequests:
          sql<number>`SUM(CASE WHEN ${planExpr} IS NULL AND ${UsageTable.cost} > 0 THEN 1 ELSE 0 END)`.as(
            "api_requests",
          ),
        apiCost:
...
function formatLiteUsage(usage: {
