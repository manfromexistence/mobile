...
export async function loadChangelog(): Promise<ChangelogData> {
  const response = await fetch(
    "https://api.github.com/repos/anomalyco/opencode/releases?per_page=20",
    {
      headers: {
...
function parseHighlights(body: string): HighlightGroup[] {
  ...
  while ((match = regex.exec(body)) !== null) {
    ...
    const imgMatch = content.match(
      /<img\s+width="([^"]+)"\s+height="([^"]+)"\s+alt="[^"]*"\s+src="([^"]+)"/,
    );
    const videoMatch = content.match(
      /^\s*(https:\/\/github\.com\/user-attachments\/assets\/[a-f0-9-]+)\s*$/m,
    );
