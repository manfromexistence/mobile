...
export const queryGoReferralUsagePreview = query(
  async (workspaceID: string, referralID?: string) => {
    "use server";
...
function rewardActionKey(reward: GoReferralReward, hasActiveGo: boolean) {
  ...
  if (reward.status === "pending" || !hasActiveGo)
    return "workspace.referral.reward.action.subscribeUnlock" as const;
              ...
              <table data-slot="referrals-table-element">
                ...
                <tbody>
                  <For each={props.summary.rewards}>
                    {(reward) => {
                      const earnedAt = () =>
                        formatDate(reward.timeCreated, language.tag(language.locale()));
                      return (
                        <tr data-status={reward.status} data-source={reward.source}>
                          ...
                          <td data-slot="referral-source">
                            {i18n.t(rewardDescriptionKey(reward.source), {
                              email: reward.email ?? "",
                            })}
                          </td>
                          ...
                          <td data-slot="referral-action">
                            <button
                              type="button"
                              disabled={
                                reward.status !== "available" || !props.lite || submission.pending
                              }
          ...
          <div data-slot="modal-actions">
            ...
            >
              {submission.pending
                ...
                : i18n.t("workspace.referral.apply.confirmAction")}
            </button>
          </div>
        </div>
      </Modal>
