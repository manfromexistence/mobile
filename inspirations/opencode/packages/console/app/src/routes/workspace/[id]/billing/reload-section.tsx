      ...
      <div data-slot="section-title">
        ...
        <div data-slot="title-row">
          <Show
            ...
            fallback={
              <p>
                {i18n.t("workspace.reload.disabled.before")}{" "}
                <b>{i18n.t("workspace.reload.disabled.state")}</b>.{" "}
                {i18n.t("workspace.reload.disabled.after")}
              </p>
            }
          >
            <p>
              {i18n.t("workspace.reload.enabled.before")}{" "}
              <b>{i18n.t("workspace.reload.enabled.state")}</b>.{" "}
              {i18n.t("workspace.reload.enabled.middle")} <b>${billingInfo()?.reloadAmount}</b> (+$
              {processingFee()} {i18n.t("workspace.reload.processingFee")}){" "}
              {i18n.t("workspace.reload.enabled.after")} <b>${billingInfo()?.reloadTrigger}</b>.
            </p>
          </Show>
          <button data-color="primary" type="button" onClick={() => show()}>
            {billingInfo()?.reload
              ? i18n.t("workspace.reload.edit")
              : i18n.t("workspace.reload.enable")}
          </button>
        </div>
      </div>
        ...
        <form action={setReload} method="post" data-slot="create-form">
          ...
          <div data-slot="form-actions">
            ...
            <button type="submit" data-color="primary" disabled={setReloadSubmission.pending}>
              {setReloadSubmission.pending
                ? i18n.t("workspace.reload.saving")
                : i18n.t("workspace.reload.save")}
            </button>
          </div>
        </form>
      </Show>
          ...
          <div data-slot="reload-error">
            ...
            <form action={reload} method="post" data-slot="create-form">
              ...
              <button data-color="ghost" type="submit" disabled={reloadSubmission.pending}>
                {reloadSubmission.pending
                  ? i18n.t("workspace.reload.retrying")
                  : i18n.t("workspace.reload.retry")}
              </button>
            </form>
          </div>
        </div>
