...
function subscribe(email: string, fullName: string) {
  ...
  const payload: { email_address: string; fields?: Record<string, string> } = {
    email_address: email,
  };
  ...
  try {
    ...
    if (trap) {
      return Response.json(
        { success: true, message: dict["enterprise.form.success.submitted"] },
        { status: 200 },
      );
    }
    ...
    if (!body.name || !body.role || !body.email || !body.message) {
      return Response.json(
        { error: dict["enterprise.form.error.allFieldsRequired"] },
        { status: 400 },
      );
    }
    ...
    if (!emailRegex.test(body.email)) {
      return Response.json(
        { error: dict["enterprise.form.error.invalidEmailFormat"] },
        { status: 400 },
      );
    }
...
${body.phone ? `${body.phone}<br>` : ""}`.trim();
    ...
    if (!lead && !mail && !octopus) {
      ...
      return Response.json(
        { error: dict["enterprise.form.error.internalServer"] },
        { status: 500 },
      );
    }
