...
async function generate() {
  ...
  try {
    ...
    if (generated.length > 1)
      throw new Error(`Expected one generated migration, found ${generated.length}.`);
    const name = generated[0];
    if (name) {
      const target = path.join(tsDir, `${name}.ts`);
      if (await Bun.file(target).exists())
        throw new Error(`Database migration already exists: ${name}`);
      await Bun.write(
        target,
        await formatTypescript(
          renderMigration(
            name,
            await Bun.file(path.join(incremental, name, "migration.sql")).text(),
          ),
        ),
      );
...
async function check() {
  ...
  try {
    ...
    await drizzle(temporary, full, "schema");
    if (
      (await Bun.file(schema).text()) !==
      (await formatTypescript(renderSchema(await generatedSql(full))))
    ) {
      throw new Error(
        "Current database schema is stale. Run `bun script/migration.ts` from packages/core.",
      );
    }

    const migrations = await typescriptMigrations();
    if (
      (await Bun.file(registry).text()) !== (await formatTypescript(renderRegistry(migrations)))
    ) {
      throw new Error(
        "Database migration registry is stale. Run `bun script/migration.ts` from packages/core.",
      );
    }
  } finally {
    await fs.rm(temporary, { recursive: true, force: true });
...
async function generatedSql(directory: string) {
  ...
  if (generated.length !== 1)
    throw new Error(`Expected one full schema migration, found ${generated.length}.`);
