...
const admitted = (id: SessionMessage.ID) =>
  Database.Service.use(({ db }) => SessionInput.find(db, id));
...
describe("SessionV2.prompt", () => {
  ...
  it.effect("returns one recorded message to concurrent exact retries", () =>
    Effect.gen(function* () {
      ...
      const messages = yield* Effect.all([session.prompt(input), session.prompt(input)], {
  ...
  it.effect("returns an exact retry of a legacy projected queued prompt", () =>
    Effect.gen(function* () {
      ...
      expect(retried).toMatchObject({
        id: messageID,
        prompt: { text: "Historical queued prompt" },
      });
    ...
    Effect.gen(function* () {
      ...
      const failure = yield* session
        .prompt({
          id: messageID,
          sessionID,
          prompt: Prompt.make({ text: "Conflicting prompt" }),
          resume: false,
        })
        .pipe(Effect.flip);
