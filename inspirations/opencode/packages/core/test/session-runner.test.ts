              ...
              : [
                  SystemContext.make({
                    ...
                    load: systemLoadHook.pipe(
                      Effect.andThen(
                        Effect.sync(() =>
                          systemUnavailable ? SystemContext.unavailable : systemBaseline,
                        ),
                      ),
                    ),
                    baseline: String,
...
const fragmentID = (kind: FragmentKind, suffix: string) =>
...
describe("SessionRunnerLLM", () => {
  ...
  it.effect("retries the first provider turn after system context becomes available", () =>
    Effect.gen(function* () {
      ...
      if (Exit.isFailure(exit))
        expect(Cause.squash(exit.cause)).toBeInstanceOf(SystemContext.InitializationBlocked);
  ...
  it.effect("fails gracefully when a stored context snapshot cannot be decoded", () =>
    Effect.gen(function* () {
      ...
      expect(Exit.isFailure(exit)).toBe(true);
      if (Exit.isFailure(exit))
        expect(Cause.squash(exit.cause)).toBeInstanceOf(ContextSnapshotDecodeError);
      expect(requests).toHaveLength(0);
    }),
  );
  ...
  it.effect("interrupts overflow recovery while the summary provider is running", () =>
    Effect.gen(function* () {
      ...
      expect(requests).toHaveLength(2);
      expect(
        (yield* session.context(sessionID)).some((message) => message.type === "compaction"),
      ).toBe(false);
    }),
  );
  ...
  it.effect("projects reasoning and tool events without executing or continuing tools", () =>
    Effect.gen(function* () {
      ...
      yield* session.prompt({
        sessionID,
        prompt: Prompt.make({ text: "Use tools" }),
        resume: false,
      });
    ...
    Effect.gen(function* () {
      ...
      expect(yield* session.context(sessionID)).toMatchObject([
        { type: "user", text: "Run once" },
        {
          type: "assistant",
          finish: "stop",
          content: [{ type: "text", id: "text-once", text: "Once" }],
        },
      ]);
    }),
  );
  ...
  it.effect("runs different sessions concurrently", () =>
    Effect.gen(function* () {
      ...
      yield* session.prompt({
        sessionID,
        prompt: Prompt.make({ text: "Run first" }),
        resume: false,
      });
      yield* session.prompt({
        sessionID: otherSessionID,
        prompt: Prompt.make({ text: "Run second" }),
        resume: false,
      });
    ...
    Effect.gen(function* () {
      ...
      expect(yield* session.context(sessionID)).toMatchObject([
        ...
        },
        {
          type: "assistant",
          finish: "stop",
          content: [{ type: "text", id: "text-after-error", text: "Recovered" }],
        },
      ]);
    }),
  );
          ...
          content: [
            {
              ...
              state: {
                ...
                error: {
                  type: "unknown",
                  message: "Tool execution failed: unexpected tool defect",
                },
              },
            },
          ],
          ...
          content: [
            {
              ...
              state: { status: "completed", structured: { text: "settle" } },
            },
          ],
        },
      ]);
  ...
  it.effect("durably fails blocked local tools when a provider turn is interrupted", () =>
    Effect.gen(function* () {
      ...
      expect(yield* session.context(sessionID)).toMatchObject([
        ...
        {
          type: "assistant",
          content: [{ type: "tool", id: "call-before-interrupt", state: { status: "error" } }],
        },
      ]);
      requests.length = 0;
      responseStream = undefined;
  ...
  it.effect("forces a text response on an agent's configured final step", () =>
    Effect.gen(function* () {
      ...
      expect(yield* session.context(sessionID)).toMatchObject([
        ...
        {
          type: "assistant",
          content: [{ type: "tool", id: "call-terminal", state: { status: "completed" } }],
        },
        {
          type: "assistant",
  ...
  for (const kind of fragmentKinds) {
    ...
    it.effect(`durably closes partial ${kind} when the provider stream fails`, () =>
      verifyPartialFlushOnFailure(kind),
    );
          ...
          content: [
            {
              type: "tool",
              id: "call-parsed",
              state: { status: "error", input: { query: "hello" } },
            },
          ],
        },
      ]);
