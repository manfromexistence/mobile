...
describe("LocationServiceMap", () => {
      ...
      Effect.flatMap((dir) =>
        Effect.scoped(
          Effect.gen(function* () {
            ...
            expect(yield* locations.contextEffect(constructed)).toBe(
              yield* locations.contextEffect(decoded),
            );
          }),
        ),
      ),
  ...
  it.live("isolates location state while sharing location policy with catalog", () =>
    Effect.acquireRelease(
      ...
      (dirs) =>
        Effect.promise(() =>
          Promise.all(dirs.map((dir) => dir[Symbol.asyncDispose]())).then(() => undefined),
        ),
    ).pipe(
      Effect.flatMap(([blocked, allowed]) =>
        Effect.gen(function* () {
          ...
          expect(
            blockedState.providers.some((provider) => provider.id === ProviderV2.ID.make("test")),
          ).toBe(false);
          ...
          expect(
            allowedState.providers.some((provider) => provider.id === ProviderV2.ID.make("test")),
          ).toBe(true);
