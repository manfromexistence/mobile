  ...
  const http = Layer.succeed(
    ...
    HttpClient.make((request) =>
      Effect.sync(() => requests.push(request.url)).pipe(
        Effect.map(() => {
          const body =
            request.url === `${base}index.json` ? JSON.stringify({ skills }) : files[request.url];
...
describe("SkillDiscovery.pull", () => {
  ...
  test("rejects cross-origin file URLs without fetching files", async () => {
    const result = await pull([
      { name: "deploy", files: ["SKILL.md", "https://evil.example.test/outside.md"] },
    ]);
    try {
      expect(result.directories).toEqual([]);
      expect(result.requests).toEqual([`${base}index.json`]);
  ...
  test("downloads safe nested files under the skill root", async () => {
    ...
    try {
      ...
      expect(result.requests.toSorted()).toEqual(
        [
          `${base}index.json`,
          `${base}deploy/SKILL.md`,
          `${base}deploy/references/guide.md`,
        ].toSorted(),
      );
      expect(await fs.readFile(path.join(result.directories[0], "SKILL.md"), "utf8")).toBe(
        "# Deploy",
      );
      expect(
        await fs.readFile(path.join(result.directories[0], "references", "guide.md"), "utf8"),
      ).toBe("# Guide");
    } finally {
      await result.tmp[Symbol.asyncDispose]();
    }
