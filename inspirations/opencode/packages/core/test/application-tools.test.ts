...
const it = testEffect(
  AppNodeBuilder.build(
    LayerNode.group([ApplicationTools.node, ToolRegistry.node, ToolRegistry.toolsNode]),
...
describe("ApplicationTools", () => {
  it.effect("keeps the Core carrier opaque and executes its single handler", () =>
    Effect.gen(function* () {
      ...
      expect(contexts).toEqual([
        { sessionID, agent, assistantMessageID, toolCallID: "call-opaque" },
      ]);
    }),
  );
    ...
    Effect.gen(function* () {
      ...
      expect(
        yield* toolDefinitions(registry, [
          { action: "application_context", resource: "*", effect: "deny" },
        ]),
      ).toEqual([]);
      expect(
      ...
      expect(contexts).toEqual([
        { sessionID, agent, assistantMessageID, toolCallID: "call-denied" },
      ]);
    }),
  );
  ...
  it.effect("advertises and executes a scoped application tool with Session context", () =>
    Effect.gen(function* () {
      ...
      expect(contexts).toEqual([
        { sessionID, agent, assistantMessageID, toolCallID: "call-context" },
      ]);
    }),
  );
  ...
  it.effect("removes a tool before settling a call produced from an earlier definition", () =>
    Effect.gen(function* () {
      ...
      yield* applications
        .register({ contextual: contextual([]) })
  ...
  it.effect(
    "settles with the current same-name application tool and restores earlier registrations",
    () =>
      Effect.gen(function* () {
        const applications = yield* ApplicationTools.Service;
        const registry = yield* ToolRegistry.Service;
        ...
        expect((yield* toolDefinitions(registry)).map((tool) => tool.name)).toEqual(["contextual"]);
        yield* applications
          .register({ contextual: contextual(secondContexts) })
        ...
        expect(secondContexts).toEqual([
          { sessionID, agent, assistantMessageID, toolCallID: "call-second" },
        ]);
        expect(firstContexts).toEqual([
          { sessionID, agent, assistantMessageID, toolCallID: "call-first" },
        ]);
      }),
  );
  ...
  it.effect("keeps the Location tool when an application tool has the same name", () =>
    Effect.gen(function* () {
      ...
      expect(
        (yield* toolDefinitions(registry, [
          { action: "shared", resource: "*", effect: "deny" },
      ...
      expect(locationContexts).toEqual([
        { sessionID, agent, assistantMessageID, toolCallID: "call-shared" },
      ]);
