...
function real(value: string) {
  return Effect.promise(() => fs.realpath(value)).pipe(
...
describe("ProjectV2.resolve", () => {
  ...
  it.live("prefers normalized origin over root commit", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        initRepo(tmp.path, { commit: true, remote: "git@github.com:Acme/App.git" }),
      );
      const project = yield* ProjectV2.Service;
      ...
      expect(result.id).not.toBe(
        ProjectV2.ID.make(yield* Effect.promise(() => rootCommit(tmp.path))),
      );
      expect(result.directory).toBe(yield* real(tmp.path));
      expect(result.vcs?.type).toBe("git");
    }),
  ...
  it.live("normalizes ssh and https remotes to the same id", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        initRepo(ssh.path, { commit: true, remote: "git@github.com:owner/repo.git" }),
      );
      yield* Effect.promise(() =>
  ...
  it.live("ignores file remotes and falls back to root commit", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        initRepo(tmp.path, { commit: true, remote: `file://${tmp.path}` }),
      );
      const project = yield* ProjectV2.Service;
  ...
  it.live("returns previous cached id from common dir", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        initRepo(tmp.path, { commit: true, remote: "git@github.com:owner/repo.git" }),
      );
  ...
  it.live("does not write the cache while resolving", () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* Effect.promise(() => Bun.file(path.join(tmp.path, ".git", "opencode")).exists()),
      ).toBe(false);
    }),
  );
  ...
  it.live("linked worktree returns opened worktree directory and previous from common dir", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() => Bun.write(path.join(tmp.path, ".git", "opencode"), "old-id"));
      yield* Effect.promise(() =>
        $`git worktree add ${worktree} -b test-${Date.now()}`.cwd(tmp.path).quiet(),
      );
      const project = yield* ProjectV2.Service;
