    ...
    Effect.acquireUseRelease(
      ...
      (tmp) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(path.join(tmp.path, ".gitignore"), "node_modules/\n"),
          );
          yield* Effect.promise(() =>
            fs.writeFile(path.join(tmp.path, "node_modules", "pkg", "index.js"), "ignored\n"),
          );
          yield* Effect.promise(() =>
            fs.writeFile(path.join(tmp.path, "src", "index.js"), "included\n"),
          );
          ...
          expect(files.map((item) => item.path)).not.toContain(
            RelativePath.make("node_modules/pkg/index.js"),
          );
        }),
    ...
    Effect.acquireUseRelease(
      ...
      (tmp) =>
        Effect.gen(function* () {
          ...
          yield* Effect.promise(() =>
            fs.writeFile(path.join(tmp.path, ".opencode", "config"), "needle\n"),
          );
          yield* Effect.promise(() => fs.mkdir(path.join(tmp.path, ".git")));
          yield* Effect.promise(() =>
            fs.writeFile(path.join(tmp.path, ".git", "config"), "needle\n"),
          );
          ...
          expect(matches.map((item) => item.entry.path)).not.toContain(
