...
describe("Repository", () => {
  test("parses github shorthand and builds an explicit-root cache path", () => {
    ...
    expect(Repository.cachePath("/cache", reference)).toBe(
  ...
  test("rejects unsafe remote references and branches with typed errors", () => {
    ...
    expect(() => Repository.parseRemote("git@github.com:../../../etc/passwd")).toThrow(
  ...
  test("compares cache identity independent of input spelling", () => {
    ...
    expect(
      Repository.same(shorthand, Repository.parseRemote("https://github.com/owner/repo.git")),
    ).toBe(true);
