      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.info.watcher).toEqual({
      ...
      Effect.flatMap((tmp) =>
        Effect.gen(function* () {
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(documents[0]?.info.attachments).toEqual({
              image: { auto_resize: false, max_width: 1200 },
            });
            ...
            expect(documents[0]?.info.providers?.openai).toMatchObject({
              ...
              models: {
                model: {
                  ...
                  variants: [
                    { id: "high", body: { reasoning: { effort: "high", summary: "auto" } } },
                  ],
                },
              },
            });
        ...
        return Effect.gen(function* () {
          yield* Effect.promise(async () => {
            ...
            await Promise.all([
              fs.writeFile(
                path.join(tmp.path, "opencode.json"),
                JSON.stringify({ $schema: "outside" }),
              ),
              fs.writeFile(
                path.join(global, "opencode.json"),
                JSON.stringify({ $schema: "global" }),
              ),
              fs.writeFile(path.join(root, "opencode.json"), JSON.stringify({ $schema: "root" })),
              fs.writeFile(
                path.join(parent, "opencode.jsonc"),
                JSON.stringify({ $schema: "parent" }),
              ),
              fs.writeFile(
                path.join(directory, "opencode.json"),
                JSON.stringify({ $schema: "directory" }),
              ),
              fs.writeFile(
          ...
          return yield* Effect.gen(function* () {
            ...
            expect(
              entries.map((entry) => (entry.type === "document" ? entry.info.$schema : entry.path)),
            ).toEqual([
              "global",
              AbsolutePath.make(global),
              "root",
