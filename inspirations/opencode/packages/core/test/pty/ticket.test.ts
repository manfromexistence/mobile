...
const itExpiring = testEffect(
  LayerNode.compile(PtyTicket.node, [
    [PtyTicket.node, Layer.effect(PtyTicket.Service, PtyTicket.make(5))],
  ]),
);
...
describe("PTY websocket tickets", () => {
  ...
  it.live("rejects tickets scoped to a different request", () =>
    Effect.gen(function* () {
      ...
      expect(yield* tickets.consume({ ptyID, directory: "/tmp/b", ticket: issued.ticket })).toBe(
        false,
      );
      expect(yield* tickets.consume({ ptyID, directory: "/tmp/a", ticket: issued.ticket })).toBe(
        true,
      );
    }),
  );
