  ...
  const unsubscribe = yield* source.listen((event) => {
    if (event.type === Pty.Event.Created.type)
      Queue.offerUnsafe(events, {
        type: "created",
        id: (event.data as typeof Pty.Event.Created.data.Type).info.id,
      });
    if (event.type === Pty.Event.Exited.type)
      Queue.offerUnsafe(events, {
        type: "exited",
        id: (event.data as typeof Pty.Event.Exited.data.Type).id,
      });
    if (event.type === Pty.Event.Deleted.type)
      Queue.offerUnsafe(events, {
        type: "deleted",
        id: (event.data as typeof Pty.Event.Deleted.data.Type).id,
      });
    return Effect.void;
  });
...
const createPty = Effect.fn("PtySessionTest.createPty")(function* (
  ...
  return yield* Effect.acquireRelease(
    pty.create({
      ...
      args,
      cwd: "/tmp",
      env: { TERM: "xterm-256color", OPENCODE_TERMINAL: "1" },
    }),
...
const attachCollecting = Effect.fn("PtySessionTest.attachCollecting")(function* (
  id: PtyID,
  cursor?: number,
) {
  const pty = yield* Pty.Service;
...
const waitForOutput = (output: Queue.Queue<string>, text: string) =>
  Effect.gen(function* () {
    ...
    Effect.timeoutOrElse({
      duration: "5 seconds",
      orElse: () =>
        Effect.fail(new Error(`timeout waiting for output containing ${JSON.stringify(text)}`)),
    }),
  );
  ...
  ptyTest("notifies attachments with the exit code and rejects attach after exit", () =>
    Effect.gen(function* () {
      ...
      const result = yield* pty
        .attach(info.id, { onData: () => {}, onEnd: () => {} })
        .pipe(Effect.exit);
      expect(Exit.isFailure(result)).toBe(true);
      if (Exit.isFailure(result))
        expect(Cause.squash(result.cause)).toMatchObject({
          _tag: "Pty.ExitedError",
          ptyID: info.id,
        });
    }),
  );
          ...
          Effect.succeed(
            configuredShell
              ? [
                  new Config.Document({
                    type: "document",
                    info: new Config.Info({ shell: configuredShell }),
                  }),
                ]
              : [],
          ),
