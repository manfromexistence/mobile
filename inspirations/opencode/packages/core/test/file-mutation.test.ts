  ...
  it.live("preserves exactly one BOM for text writes and normalizes created text", () =>
    withTmp((directory) =>
      Effect.gen(function* () {
        ...
        const preserved = yield* (yield* LocationMutation.Service).resolve({
          path: "preserved.txt",
        });
        ...
        yield* files.writeTextPreservingBom({
          target: created,
          content: "\uFEFF\uFEFF\uFEFFcreated",
        });

        expect(yield* Effect.promise(() => fs.readFile(preservedPath, "utf8"))).toBe("\uFEFFafter");
        expect(yield* Effect.promise(() => fs.readFile(created.canonical, "utf8"))).toBe(
          "\uFEFFcreated",
        );
      }).pipe(provide(directory)),
    ),
      ...
      Effect.gen(function* () {
        ...
        expect(
          yield* (yield* FileMutation.Service)
            .create({ target, content: "replacement" })
            .pipe(Effect.flip),
        ).toMatchObject({
    ...
    withTmp((directory) =>
      withTmp((outside) =>
        Effect.gen(function* () {
          ...
          const result = yield* (yield* FileMutation.Service).write({
            target,
            content: "external",
          });
      ...
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          const first = yield* files
            .write({ target: firstPlan, content: "first" })
            .pipe(Effect.forkChild);
          yield* Deferred.await(firstStarted);
          const second = yield* files
            .write({ target: secondPlan, content: "second" })
            .pipe(Effect.forkChild);
    ...
    withTmp((directory) =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          const first = yield* files
            .writeIfUnchanged({ target, expected, content: "first" })
            .pipe(Effect.forkChild);
          yield* Deferred.await(firstStarted);
          const second = yield* files
      ...
      Effect.gen(function* () {
        ...
        expect(
          yield* (yield* FileMutation.Service)
            .writeIfUnchanged({
              target,
              expected: new TextEncoder().encode("older"),
              content: "replacement",
            })
            .pipe(Effect.flip),
    ...
    Effect.gen(function* () {
      ...
      return FSUtil.Service.of({
        ...
        writeWithDirs: (target, content, mode) =>
          run(filesystem.writeWithDirs(target, content, mode), target),
        writeFile: (target, content, options) =>
          run(filesystem.writeFile(target, content, options), target),
