...
function read(file: string) {
  return Effect.promise(() => fs.readFile(file, "utf8")).pipe(
...
describe("Git worktrees", () => {
  it.live("creates, lists, and removes linked worktrees", () =>
    Effect.gen(function* () {
      ...
      expect(
        (yield* git.worktree.list(repo)).some((entry) => entry.directory.endsWith("-git-worktree")),
      ).toBe(true);
      const linked = yield* git.repo.discover(worktree);
      expect(linked?.worktree).toBe(
        AbsolutePath.make(yield* Effect.promise(() => fs.realpath(worktree))),
      );
