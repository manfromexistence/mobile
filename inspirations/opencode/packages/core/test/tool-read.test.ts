...
const reader = Layer.succeed(
  ...
  ReadToolFileSystem.Service.of({
    inspect: () =>
      resolveFailure === undefined ? Effect.succeed(resolvedType) : Effect.die(resolveFailure),
  ...
  PermissionV2.Service.of({
    assert: (input) =>
      Effect.sync(() => {
        ...
        Effect.andThen(
          allow ? Effect.void : Effect.fail(new PermissionV2.DeniedError({ rules: [] })),
        ),
      ),
  ...
  LocationMutation.Service.of({
    resolve: (input) => {
      if (input.path === missingPath)
        return Effect.fail(
          new LocationMutation.PathError({ path: input.path, reason: "non_directory_ancestor" }),
        );
...
const readLayer = (imageLayer: Layer.Layer<Image.Service>) =>
  AppNodeBuilder.build(
    LayerNode.group([ToolRegistry.node, ToolRegistry.toolsNode, ReadTool.node]),
    [
...
describe("ReadTool", () => {
  ...
  it.effect("registers, authorizes, and reads through the location filesystem", () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* toolDefinitions(registry, [{ action: "read", resource: "*", effect: "deny" }]),
      ).toEqual([]);
      expect(
      ...
      expect(assertions).toMatchObject([
        { sessionID, action: "read", resources: ["README.md"], save: ["*"] },
      ]);
  ...
  it.effect("rejects oversized images when resizing is disabled", () =>
    Effect.gen(function* () {
      ...
      const source = new photon.PhotonImage(
        new Uint8Array(Array.from({ length: 16 * 4 }, () => 255)),
        16,
        1,
      );
      ...
      if (result.type === "error")
        expect(result.value).toContain("exceeding configured limits 4x2000");
    }),
  );
    ...
    Effect.gen(function* () {
      ...
      configEntries = [
        new Config.Document({
          ...
          info: new Config.Info({
            attachments: new ConfigAttachments.Info({
              image: new ConfigAttachments.Image({ max_width: 4 }),
            }),
          }),
        }),
      ];
      ...
      const resized = photon.PhotonImage.new_from_byteslice(
        Buffer.from(media.uri.split(",")[1] ?? "", "base64"),
      );
  ...
  it.effect("lists a bounded directory page through read", () =>
    Effect.gen(function* () {
      ...
      expect(assertions).toMatchObject([
        { sessionID, action: "read", resources: ["src"], save: ["*"] },
      ]);
