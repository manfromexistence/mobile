...
describe("WebSearchTool provider selection", () => {
  test("rejects out-of-range numeric controls", () => {
    ...
    expect(() =>
      decode({ query: "x", contextMaxCharacters: WebSearchTool.MAX_CONTEXT_CHARACTERS + 1 }),
    ).toThrow();
  });
  ...
  test("supports an explicit operational override", () => {
    expect(
      WebSearchTool.selectProvider(
        sessionID,
        { enableExa: false, enableParallel: false },
        "parallel",
      ),
    ).toBe("parallel");
    expect(
      WebSearchTool.selectProvider(sessionID, { enableExa: false, enableParallel: false }, "exa"),
    ).toBe("exa");
  });

  test("prefers Parallel when both explicit flags are enabled", () => {
    expect(WebSearchTool.selectProvider(sessionID, { enableExa: true, enableParallel: true })).toBe(
      "parallel",
    );
  });
  ...
  test("prefers Exa when only its explicit flag is enabled", () => {
    expect(
      WebSearchTool.selectProvider(sessionID, { enableExa: true, enableParallel: false }),
    ).toBe("exa");
  });
});
...
describe("WebSearchTool MCP response parser", () => {
  test("parses plain JSON-RPC responses", async () => {
    expect(await Effect.runPromise(WebSearchTool.parseResponse(payload("search results")))).toBe(
      "search results",
    );
  });
