          ...
          .pipe(
            Effect.tap((content) =>
              Effect.sync(() => reads++).pipe(
                Effect.andThen(Effect.suspend(() => afterRead(target, content))),
              ),
            ),
          ),
      writeWithDirs: (target, content, mode) =>
        Effect.sync(() => writes.push(target)).pipe(
          Effect.andThen(fs.writeWithDirs(target, content, mode)),
        ),
      writeFile: (target, content, options) =>
        Effect.sync(() => writes.push(target)).pipe(
          Effect.andThen(fs.writeFile(target, content, options)),
        ),
...
describe("EditTool", () => {
        ...
        return Effect.promise(() => fs.writeFile(target, "before")).pipe(
          Effect.andThen(
            withTool(tmp.path, (registry) =>
              executeTool(
                registry,
                call({ path: target, oldString: "before", newString: "after" }),
              ),
            ),
          ),
  ...
  it.live("approves an explicit external absolute path before edit", () =>
    Effect.acquireUseRelease(
      ...
      ([active, outside]) =>
        Effect.promise(() =>
          Promise.all([active[Symbol.asyncDispose](), outside[Symbol.asyncDispose]()]).then(
            () => undefined,
          ),
        ),
    ),
        ...
        Effect.gen(function* () {
          ...
          expect(
            yield* withTool(active.path, (registry) =>
              executeTool(
                registry,
                call({ path: external, oldString: "before", newString: "after" }),
              ),
            ),
          ).toEqual({
            type: "error",
  ...
  it.live(
    "denied edit reads no target content and does not disclose whether oldString matches",
    () =>
      Effect.acquireUseRelease(
        Effect.promise(() => tmpdir()),
        (tmp) => {
            ...
            withTool(tmp.path, (registry) =>
              Effect.gen(function* () {
                ...
                expect(
                  yield* executeTool(
                    registry,
                    call({ path: "matches.txt", oldString: "", newString: "after" }),
                  ),
                ).toEqual({
                  type: "error",
                ...
                expect(
                  yield* executeTool(
                    registry,
                    call({ path: "matches.txt", oldString: "missing", newString: "after" }),
                  ),
                ).toEqual({
                  type: "error",
                  value:
                ...
                expect(
                  yield* executeTool(
                    registry,
                    call({ path: "matches.txt", oldString: "same", newString: "after" }),
                  ),
                ).toEqual({
                  type: "error",
                  value:
        ...
        return Effect.promise(() => fs.writeFile(target, "same same same")).pipe(
          Effect.andThen(
            withTool(tmp.path, (registry) =>
              settleTool(
                registry,
                call({ path: "all.txt", oldString: "same", newString: "after", replaceAll: true }),
              ),
            ),
          ),
        ...
        return Effect.promise(() => fs.writeFile(target, "\uFEFFbefore\r\nrest\r\n")).pipe(
          Effect.andThen(
            withTool(tmp.path, (registry) =>
              executeTool(
                registry,
                call({ path: "windows.txt", oldString: "before\nrest", newString: "after\nrest" }),
              ),
            ),
          ),
          Effect.andThen(() => Effect.promise(() => fs.readFile(target, "utf8"))),
          Effect.tap((content) =>
            Effect.sync(() => expect(content).toBe("\uFEFFafter\r\nrest\r\n")),
          ),
        );
      },
      (tmp) => Effect.promise(() => tmp[Symbol.asyncDispose]()),
  ...
  it.live("rejects an in-place content change after matching but before conditional commit", () =>
    Effect.acquireUseRelease(
      ...
      (tmp) => {
        ...
        afterRead = () =>
          reads === 1 ? Effect.promise(() => fs.writeFile(target, "newer\n")) : Effect.void;
        return Effect.promise(() => fs.writeFile(target, "before\n")).pipe(
          Effect.andThen(
            withTool(tmp.path, (registry) =>
              executeTool(
                registry,
                call({ path: "concurrent.txt", oldString: "before", newString: "after" }),
              ),
            ),
          ),
          Effect.andThen((result) =>
...
test("keeps the locked edit schema, semantics docstring, and deferred TODOs visible", async () => {
  const source = (
    await fs.readFile(new URL("../src/tool/edit.ts", import.meta.url), "utf8")
  ).replaceAll("\r\n", "\n");
