...
const toolLayer = (replacements: LayerNode.Replacements = []) =>
  AppNodeBuilder.build(
    LayerNode.group([ToolRegistry.node, ToolRegistry.toolsNode, WebFetchTool.node]),
    [
...
describe("WebFetchTool helpers", () => {
  test("defaults format and rejects invalid timeout controls", () => {
    ...
    expect(decode({ url: "https://example.com" })).toEqual({
      url: "https://example.com",
      format: "markdown",
    });
    expect(() => decode({ url: "https://example.com", timeout: 0 })).toThrow();
    expect(() =>
      decode({ url: "https://example.com", timeout: WebFetchTool.MAX_TIMEOUT_SECONDS + 1 }),
    ).toThrow();
  });
...
describe("WebFetchTool registration", () => {
      ...
      expect(assertions).toMatchObject([
        {
          ...
          resources: [url],
          save: ["*"],
          metadata: { url, format: "text", timeout: 4 },
        },
      ]);
      expect(requests).toMatchObject([
        { url, headers: { accept: expect.stringContaining("text/plain;q=1.0") } },
      ]);
    }),
  );
  ...
  it.effect("rejects non-HTTP schemes before permission or transport", () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* executeTool(registry, call({ url: "file:///etc/passwd", format: "text" })),
      ).toEqual({
        type: "error",
  ...
  it.effect("converts HTML to requested markdown and text", () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* executeTool(registry, call({ url: "https://1.1.1.1", format: "markdown" })),
      ).toEqual({
        type: "text",
        value: "# Hello\n\nworld",
      });
      expect(
        yield* executeTool(registry, call({ url: "https://1.1.1.1", format: "text" })),
      ).toEqual({
        type: "text",
        value: "Helloworld",
      });
  ...
  it.effect("rejects declared and streamed oversized bodies", () =>
    Effect.gen(function* () {
        ...
        Effect.succeed(
          new Response("small", {
            headers: {
              ...
              "content-length": String(WebFetchTool.MAX_RESPONSE_BYTES + 1),
            },
          }),
        );
      expect(
        yield* executeTool(registry, call({ url: "https://1.1.1.1/declared", format: "text" })),
      ).toEqual({
        type: "error",
      ...
      respond = () =>
        Effect.succeed(
          new Response("x".repeat(WebFetchTool.MAX_RESPONSE_BYTES + 1), {
            headers: { "content-type": "text/plain" },
          }),
        );
      expect(
        yield* executeTool(registry, call({ url: "https://1.1.1.1/streamed", format: "text" })),
      ).toEqual({
        type: "error",
  ...
  it.effect("keeps images and files unsupported until typed settlement can carry attachments", () =>
    Effect.gen(function* () {
      ...
      const registry = yield* ToolRegistry.Service;
      respond = () =>
        Effect.succeed(new Response("png", { headers: { "content-type": "image/png" } }));
      expect(
        yield* executeTool(registry, call({ url: "https://1.1.1.1/image", format: "html" })),
      ).toEqual({
        type: "error",
      ...
      respond = () =>
        Effect.succeed(new Response("pdf", { headers: { "content-type": "application/pdf" } }));
      expect(
        yield* executeTool(registry, call({ url: "https://1.1.1.1/file", format: "html" })),
      ).toEqual({
        type: "error",
