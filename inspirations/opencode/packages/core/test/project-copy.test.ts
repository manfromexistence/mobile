...
const it = testEffect(
  AppNodeBuilder.build(
    LayerNode.group([ProjectCopy.node, Database.node, EventV2.node, ProjectDirectories.node]),
  ),
);
...
describe("ProjectCopy", () => {
  ...
  it.effect("rejects duplicate strategies and reports unavailable ids", () =>
    Effect.gen(function* () {
      ...
      expect(yield* copy.register(strategy).pipe(Effect.flip)).toBeInstanceOf(
      ...
      if (error instanceof ProjectCopy.StrategyUnavailableError)
        expect(error.strategy).toBe(unavailable);
    }),
  );
  ...
  it.live("requires force to remove a dirty git worktree", () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* Effect.promise(() => Bun.file(path.join(created.directory, "dirty.txt")).exists()),
      ).toBe(true);
  ...
  it.live("preserves copies whose stored strategy is unavailable", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.addFinalizer(() =>
        Effect.promise(() => fs.rm(unavailable, { recursive: true, force: true })),
      );
      yield* input.db
        .insert(ProjectDirectoryTable)
    ...
    Effect.gen(function* () {
      ...
      expect(
        yield* Effect.promise(() =>
          fs.stat(path.join(parent, "copy")).then((item) => item.isDirectory()),
        ),
      ).toBe(true);
      expect(
        yield* Effect.promise(() =>
          fs.stat(path.join(parent, "copy-2")).then((item) => item.isDirectory()),
        ),
      ).toBe(true);
      ...
      yield* Effect.promise(() =>
        Promise.all(
          Array.from({ length: 10 }, (_, index) =>
            fs.mkdir(path.join(parent, index === 0 ? "copy" : `copy-${index + 1}`), {
              recursive: true,
            }),
          ),
        ),
  ...
  it.live("refresh discovers and prunes an externally managed git worktree", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        $`git worktree add --detach ${target} HEAD`.cwd(input.root.path).quiet(),
      );
      yield* input.db
        .insert(ProjectDirectoryTable)
      ...
      expect(yield* copy.refresh({ projectID: input.projectID })).toEqual({
        updated: [discovered],
        removed: [],
      });
      ...
      yield* Effect.promise(() =>
        $`git worktree remove --force ${target}`.cwd(input.root.path).quiet(),
      );
      expect(yield* copy.refresh({ projectID: input.projectID })).toEqual({
        updated: [],
        removed: [discovered],
  ...
  it.live("refresh ignores stale git worktree registrations", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        $`git worktree add --detach ${stale} HEAD`.cwd(input.root.path).quiet(),
      );
  ...
  it.live("refresh ignores existing directories that are no longer git checkouts", () =>
    Effect.gen(function* () {
      const input = yield* setup();
      yield* Effect.promise(() =>
        fs.rm(path.join(input.sourceDirectory, ".git"), { recursive: true }),
      );
      const copy = yield* ProjectCopy.Service;
  ...
  it.live("refresh removes missing ordinary checkouts", () =>
    Effect.gen(function* () {
      ...
      expect(yield* stored(input.projectID)).not.toContainEqual({
        directory: missing,
        strategy: null,
      });
    }),
