  ...
  Effect.gen(function* () {
    ...
    return FSUtil.Service.of({
      ...
      remove: (target, options) => {
        ...
        if (
          blockRemoveTarget &&
          path.basename(target) === blockRemoveTarget &&
          removeStarted &&
          releaseRemove
        )
...
describe("ApplyPatchTool", () => {
              ...
              Effect.gen(function* () {
                ...
                expect(
                  yield* Effect.promise(() =>
                    fs.readFile(path.join(tmp.path, "nested/new.txt"), "utf8"),
                  ),
                ).toBe("created\n");
  ...
  it.live(
    "approves an external directory and the batch before reading external update content",
    () =>
      Effect.acquireUseRelease(
  ...
  it.live("rejects an add target that appears during permission approval", () =>
    Effect.acquireUseRelease(
      ...
      (tmp) => {
        ...
        afterEditApproval = () =>
          Effect.promise(() => fs.writeFile(target, "winner\n")).pipe(Effect.orDie);
        return withTool(tmp.path, (registry) =>
          Effect.gen(function* () {
            expect(
              yield* executeTool(
