...
describe("DatabaseMigration", () => {
  ...
  if (process.platform === "linux") {
    test("declared schema has no ungenerated migrations", async () => {
      const result =
        await $`bun ${fileURLToPath(new URL("../script/migration.ts", import.meta.url))} --check`
          .quiet()
          .nothrow();
  ...
  test("applies tracked migrations to an empty database", async () => {
    await run(
      Effect.gen(function* () {
        ...
        expect(
          yield* db.get(
            sql`SELECT name FROM sqlite_master WHERE type = 'table' AND name = 'session_input'`,
          ),
  ...
  test("backfills existing Context Epoch rows to the build agent", async () => {
    await run(
      Effect.gen(function* () {
        ...
        expect(
          yield* db.get(
            sql`SELECT agent FROM session_context_epoch WHERE session_id = 'ses_existing'`,
          ),
        ).toEqual({
          agent: "build",
        });
  ...
  test("keeps legacy credential fields nullable", async () => {
    await run(
      Effect.gen(function* () {
        ...
        expect(
          yield* db.get(
            sql`SELECT connector_id, method_id, active FROM credential WHERE id = 'current'`,
          ),
  ...
  test("resets beta history and rebuilds event-sourced Session input storage", async () => {
    await run(
      Effect.gen(function* () {
        ...
        yield* db.run(
          sql`CREATE INDEX event_aggregate_type_seq_idx ON event (aggregate_id, type, seq)`,
        );
        yield* db.run(
        ...
        yield* db.run(
          sql`CREATE INDEX session_message_session_seq_idx ON session_message (session_id, seq)`,
        );
        yield* db.run(
        ...
        expect(
          (yield* db.all<{ name: string }>(sql`PRAGMA table_info(session_input)`)).map(
            (column) => column.name,
          ),
        ).toEqual([
          "id",
        ...
        expect(
          (yield* db.all<{ name: string; unique: number }>(
            sql`PRAGMA index_list(session_message)`,
          )).find((index) => index.name === "session_message_session_seq_idx"),
        ...
        expect(
          (yield* db.all<{ name: string; unique: number }>(
            sql`PRAGMA index_list(session_input)`,
          )).filter((index) =>
            [
    ...
    await run(
      Effect.gen(function* () {
        ...
        yield* EventV2.Service.use((service) =>
          ...
          Effect.provide(
            AppNodeBuilder.build(LayerNode.group([EventV2.node, SessionProjector.node]), [
  ...
  test("resets incompatible projected Session messages before adding sequence order", async () => {
    await run(
      Effect.gen(function* () {
        ...
        expect(yield* db.get(sql`SELECT id, seq FROM session_message`)).toEqual({
          id: "fresh_projection",
          seq: 7,
        });
      }),
  ...
  test("runs session usage backfill in order with schema changes", async () => {
    await run(
      Effect.gen(function* () {
        ...
        yield* db.run(
          sql`CREATE TABLE message (id text PRIMARY KEY, session_id text NOT NULL, data text NOT NULL)`,
        );
  ...
  test("normalizes Windows storage paths and leaves POSIX paths untouched", async () => {
    await run(
      Effect.gen(function* () {
        const db = yield* makeDb;
        yield* db.run(
          sql`CREATE TABLE project (id text PRIMARY KEY, worktree text NOT NULL, sandboxes text NOT NULL)`,
        );
        yield* db.run(
          sql`CREATE TABLE session (id text PRIMARY KEY, directory text NOT NULL, path text)`,
        );
        ...
        yield* db.run(
          sql`INSERT INTO project (id, worktree, sandboxes) VALUES (${"global"}, ${"/"}, ${"[]"})`,
        );
        yield* db.run(
        ...
        expect(
          yield* db.get(sql`SELECT worktree, sandboxes FROM project WHERE id = 'win'`),
        ).toEqual({
        ...
        expect(
          yield* db.get(sql`SELECT worktree, sandboxes FROM project WHERE id = 'unc'`),
        ).toEqual({
      ...
      Effect.gen(function* () {
        ...
        expect(
          (yield* db
            .select()
            .from(SessionTable)
            .where(eq(SessionTable.path, "packages\\api"))
            .get())?.id,
        ).toBe(sessionID);
  ...
  test("imports existing drizzle migration state", async () => {
    await run(
      Effect.gen(function* () {
        ...
        expect(yield* db.get(sql`SELECT id FROM migration`)).toEqual({
  ...
  test("accepts the temporary replacement session metadata migration id", async () => {
    await run(
      Effect.gen(function* () {
        ...
        yield* db.run(
          sql`INSERT INTO migration (id, time_completed) VALUES ('20260530232709_lovely_romulus', 1)`,
        );
