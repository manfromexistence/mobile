...
describe("Integration", () => {
  ...
  it.effect("reveals the previous registration when an override closes", () =>
    Effect.gen(function* () {
      ...
      yield* integrations
        .transform((editor) =>
          editor.update(id, (integration) => (integration.name = "OpenAI Override")),
        )
  ...
  it.effect("registers and overrides methods independently", () =>
    Effect.gen(function* () {
      ...
      expect((yield* integrations.get(integrationID))?.methods[0]).toMatchObject({
        label: "ChatGPT Override",
      });
  ...
  it.effect("keeps code attempts open when the code is missing and closes them on cancel", () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* integrations.attempt.complete({ attemptID: attempt.attemptID }).pipe(Effect.flip),
  ...
  it.effect("expires abandoned OAuth attempts", () =>
    Effect.gen(function* () {
      ...
      const attempt = yield* integrations.connection.oauth({ integrationID, methodID, inputs: {} });
      expect(attempt.time.expires - attempt.time.created).toBe(
