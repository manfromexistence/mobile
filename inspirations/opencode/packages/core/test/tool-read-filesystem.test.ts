...
describe("ReadToolFileSystem", () => {
  ...
  it.effect("fails with a typed filesystem error when directory listing fails", () =>
    Effect.gen(function* () {
      ...
      if (error instanceof FSUtil.FileSystemError)
        expect(error.method).toBe("readDirectoryEntries");
    }),
  );
  ...
  it.effect("reports binary and malformed UTF-8 content as typed errors", () =>
    Effect.gen(function* () {
      ...
      const binaryError = yield* ReadToolFileSystem.read(fs, binary, "archive.dat").pipe(
        Effect.flip,
      );
      const malformedError = yield* ReadToolFileSystem.read(fs, malformed, "malformed.txt").pipe(
        Effect.flip,
      );
  ...
  it.effect("reports out-of-range pagination as a typed error", () =>
    Effect.gen(function* () {
      ...
      const error = yield* ReadToolFileSystem.read(fs, file, "short.txt", { offset: 2 }).pipe(
        Effect.flip,
      );
