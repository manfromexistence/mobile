...
describe("SystemContext", () => {
  it.effect("stores the canonical JSON encoding of the loaded value", () =>
    Effect.gen(function* () {
      ...
      expect((yield* SystemContext.initialize(context)).snapshot["core/date"].value).toBe(
    ...
    Effect.gen(function* () {
      ...
      const context = SystemContext.combine([
        ...
        stringContext({
          key: "core/location",
          value: "/repo",
          baseline: (value) => `Directory: ${value}`,
        }),
      ]);
  ...
  it.effect("retains admitted snapshots while a source is temporarily unavailable", () =>
    Effect.gen(function* () {
      ...
      expect(yield* SystemContext.replace(context, previous)).toEqual({
    ...
    Effect.gen(function* () {
      expect(
        yield* SystemContext.reconcile(SystemContext.empty, {
          "core/instructions": {
            value: "contents",
            removed: "Instructions removed; stop applying them.",
          },
        }),
      ).toEqual({
        _tag: "Updated",
  ...
  it.effect("rejects empty model-visible renderings", () =>
    Effect.gen(function* () {
      ...
      expect(Exit.isFailure(exit)).toBe(true);
      if (Exit.isFailure(exit))
        expect(Cause.pretty(exit.cause)).toContain("rendered an empty baseline");
    }),
  );
  ...
  it.effect("blocks an incompatible replacement while another admitted source is unavailable", () =>
    Effect.gen(function* () {
      ...
      expect(yield* SystemContext.reconcile(context, previous)).toEqual({
        _tag: "ReplacementBlocked",
      });
      expect(yield* SystemContext.replace(context, previous)).toEqual({
        _tag: "ReplacementBlocked",
  ...
  it.effect("requires namespaced durable snapshot keys", () =>
    Effect.sync(() => {
      ...
      expect(Object.keys(decodeSnapshot({ "core/date": { value: "date" } }))).toEqual([
        "core/date",
      ]);
