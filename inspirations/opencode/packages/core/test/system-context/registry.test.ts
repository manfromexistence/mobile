...
describe("SystemContextRegistry", () => {
  it.effect("loads empty system context when there are no entries", () =>
    Effect.gen(function* () {
      ...
      expect(yield* SystemContext.initialize(yield* registry.load())).toEqual({
        baseline: "",
        snapshot: {},
      });
    }),
  ...
  it.effect("propagates entry producer failures", () =>
    Effect.gen(function* () {
      ...
      yield* registry.register({
        key: SystemContext.Key.make("test/failure"),
        load: Effect.die(failure),
      });
    ...
    Effect.gen(function* () {
      ...
      if (Exit.isFailure(exit)) {
        ...
        expect(Cause.squash(exit.cause)).toMatchObject({
          key: SystemContext.Key.make("test/duplicate"),
        });
      }
    }),
  );
  ...
  it.effect("rejects duplicate entry keys", () =>
    Effect.gen(function* () {
      ...
      const exit = yield* registry
        .register(entry("test/duplicate", "second", "test/other"))
        .pipe(Effect.exit);
