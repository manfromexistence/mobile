    ...
    (tmp) => {
      ...
      const configured = config
        ? Layer.succeed(
            ...
            Config.Service.of({
              entries: () =>
                Effect.succeed([new Config.Document({ type: "document", info: config })]),
            }),
          )
        : Layer.empty;
...
describe("ToolOutputStore", () => {
  it.live("bounds the provider-facing text channel with one managed file", () =>
    withStore(({ store, fs }) =>
      Effect.gen(function* () {
        ...
        expect(Buffer.byteLength(result.output.content[0].text)).toBeLessThanOrEqual(
  ...
  it.live(
    "preserves native media and structured metadata without applying a settlement media limit",
    () =>
      withStore(({ store }) =>
        Effect.gen(function* () {
    ...
    withStore(({ root, store, fs }) =>
      Effect.gen(function* () {
        ...
        if (Exit.isFailure(exit))
          expect(Option.getOrUndefined(Cause.findErrorOption(exit.cause))?._tag).toBe(
  ...
  it.live("does not encode ignored structured metadata when projected content exists", () =>
    withStore(({ store }) =>
      Effect.gen(function* () {
        const output = {
          structured: { value: 1n },
          content: [{ type: "text" as const, text: "readable text" }],
        };
