...
describe("BackgroundJob", () => {
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.forEach(Array.from({ length: 100 }), (_, index) =>
        Effect.gen(function* () {
          ...
          expect(yield* jobs.extend({ id: job.id, run: Effect.succeed(`second-${index}`) })).toBe(
            true,
          );
  ...
  it.live(
    "interrupts live work without promising settlement after the owning process-local scope closes",
    () =>
      Effect.gen(function* () {
