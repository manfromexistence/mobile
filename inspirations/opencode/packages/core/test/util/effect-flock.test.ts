      ...
      yield* Effect.scoped(
        Effect.gen(function* () {
          ...
          const json = yield* Effect.promise(() =>
            readJson<{ token?: unknown; pid?: unknown; hostname?: unknown; createdAt?: unknown }>(
              file,
            ),
          );
  ...
  it.live(
    ...
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        fs.chmod(dir, 0o700).then(() => fs.rm(tmp, { recursive: true, force: true })),
      );
    }),
  );
      ...
      Effect.promise(async () => {
        ...
        try {
          const out = await Promise.all(
            Array.from({ length: n }, () =>
              run({ key: "eflock:stress", dir, done, active, holdMs: 30 }),
            ),
          );
