...
const retentionFailure = new ToolOutputStore.StorageError({
  operation: "write",
...
describe("ToolRegistry", () => {
  it.effect("filters disabled tools with edit aliases and ordered wildcard precedence", () =>
    Effect.gen(function* () {
      ...
      const names = (rules: Parameters<ToolRegistry.Interface["materialize"]>[0]) =>
        toolDefinitions(service, rules).pipe(
  ...
  it.effect("propagates retention failures through settlement", () =>
    Effect.gen(function* () {
      ...
      if (Exit.isFailure(exit))
        expect(Option.getOrUndefined(Cause.findErrorOption(exit.cause))).toBe(retentionFailure);
    ...
    Effect.gen(function* () {
      ...
      yield* service.register({
        context: Tool.make({
          ...
          output: Schema.Struct({ ok: Schema.Boolean }),
          execute: (_, context) =>
            Effect.sync(() => contexts.push(context)).pipe(Effect.as({ ok: true })),
        }),
      });
      yield* executeTool(service, {
        sessionID,
    ...
    Effect.gen(function* () {
      ...
      yield* service.register({
        transformed: Tool.make({
          ...
          execute: ({ value }) =>
            Effect.sync(() => executed.push(value)).pipe(Effect.as({ value })),
  ...
  it.effect("rejects only the replaced name from a multi-tool provider turn", () =>
    Effect.gen(function* () {
      ...
      expect((yield* materialized.settle(call("second"))).result).toEqual({
        type: "text",
        value: "second",
      });
    }),
  ...
  integrated.effect(
    "rejects a Location call after removal reveals an application registration",
    () =>
      Effect.gen(function* () {
        const applications = yield* ApplicationTools.Service;
        const service = yield* ToolRegistry.Service;
  ...
  it.effect("keeps captured execution running after registration mutation", () =>
    Effect.gen(function* () {
      ...
      expect(yield* Fiber.join(settlement)).toMatchObject({
