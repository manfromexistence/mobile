  ...
  describe("run", () => {
      ...
      Effect.gen(function* () {
        ...
        if (Exit.isFailure(exit)) {
          ...
          if (reason && reason._tag === "Fail") {
            ...
            expect((reason.error as AppProcess.AppProcessError).message).toContain(
    ...
    it.effect(
      ...
      Effect.gen(function* () {
        ...
        const okZero = yield* svc
          .run(cmd("-e", "process.exit(0)"))
          .pipe(Effect.flatMap(requireZeroOrOne));
        expect(okZero.exitCode).toBe(0);
        const okOne = yield* svc
    ...
    it.effect(
      ...
      Effect.gen(function* () {
        const svc = yield* AppProcess.Service;
        const result = yield* svc.run(cmd("-e", "process.stdout.write('0123456789')"), {
          maxOutputBytes: 5,
        });
        expect(result.exitCode).toBe(0);
    ...
    it.effect(
      ...
      Effect.gen(function* () {
        const svc = yield* AppProcess.Service;
        const result = yield* svc.run(cmd("-e", "process.stderr.write('0123456789')"), {
          maxErrorBytes: 5,
        });
        expect(result.exitCode).toBe(0);
        expect(result.stdoutTruncated).toBe(false);
        ...
        Effect.acquireUseRelease(
          ...
          (directory) => {
            ...
            return Effect.gen(function* () {
              const svc = yield* AppProcess.Service;
              const exit = yield* Effect.exit(
                svc.run(cmd("-e", script), { timeout: "250 millis" }),
              );
              expect(Exit.isFailure(exit)).toBe(true);
  ...
  describe("run with stdin option", () => {
    ...
    it.effect(
      ...
      Effect.gen(function* () {
        ...
        const command = ChildProcess.make(NODE, ["-e", script], {
          env: { FEED: "envset" },
          extendEnv: true,
        });
