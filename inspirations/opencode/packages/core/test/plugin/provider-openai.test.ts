...
describe("OpenAIPlugin", () => {
  it.effect("registers browser and headless ChatGPT OAuth methods", () =>
    Effect.gen(function* () {
      yield* addPlugin();
      expect(
        (yield* (yield* Integration.Service).get(Integration.ID.make("openai")))?.methods,
      ).toEqual([
        {
  ...
  it.effect("disables gpt-5-chat-latest during catalog transforms", () =>
    Effect.gen(function* () {
      ...
      yield* addPlugin();
      expect(
        required(yield* catalog.model.get(ProviderV2.ID.openai, ModelV2.ID.make("gpt-5"))).enabled,
      ).toBe(true);
      expect(
        required(
          yield* catalog.model.get(ProviderV2.ID.openai, ModelV2.ID.make("gpt-5-chat-latest")),
        ).enabled,
      ).toBe(false);
    }),
  );
      ...
      expect(
        required(
          yield* catalog.model.get(
            ProviderV2.ID.make("custom-openai"),
