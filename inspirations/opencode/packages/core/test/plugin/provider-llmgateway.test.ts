...
const addPlugin = Effect.fn(function* () {
  ...
  yield* LLMGatewayPlugin.effect(host).pipe(
...
describe("LLMGatewayPlugin", () => {
  it.effect("is registered so legacy referer headers can be applied", () =>
    Effect.sync(() =>
      expect(ProviderPlugins.map((item) => item.id)).toContain(PluginV2.ID.make("llmgateway")),
    ),
  );
  ...
  it.effect("applies legacy referer headers only to enabled llmgateway", () =>
    Effect.gen(function* () {
      ...
      yield* addPlugin();
      expect(
        (yield* catalog.provider.get(ProviderV2.ID.make("llmgateway")))?.request.headers,
      ).toEqual({
        Existing: "value",
