...
function cloudflareLanguage(sdk: unknown, modelID = "@cf/model") {
  return (
    sdk as { languageModel: (id: string) => { config: CloudflareConfig; provider: string } }
...
function cloudflareURL(sdk: unknown, modelID = "@cf/model") {
  return cloudflareLanguage(sdk, modelID).config.url({
...
describe("CloudflareWorkersAIPlugin", () => {
    ...
    withEnv({ CLOUDFLARE_ACCOUNT_ID: "acct", CLOUDFLARE_API_KEY: "key" }, () =>
      Effect.gen(function* () {
        ...
        const provider = required(
          yield* catalog.provider.get(ProviderV2.ID.make("cloudflare-workers-ai")),
        );
        const sdk = yield* aisdk.runSDK({
          model: ModelV2.Info.make({
            ...ModelV2.Info.empty(
              ProviderV2.ID.make("cloudflare-workers-ai"),
              ModelV2.ID.make("@cf/model"),
            ),
  ...
  it.effect("preserves a configured endpoint URL instead of deriving one from account ID", () =>
    withEnv({ CLOUDFLARE_ACCOUNT_ID: "acct" }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin();
        expect(
          required(yield* catalog.provider.get(ProviderV2.ID.make("cloudflare-workers-ai"))).api,
        ).toEqual({
          type: "aisdk",
          package: "test-provider",
          url: "https://proxy.example/v1",
  ...
  it.effect(
    "uses env API key over auth or configured API key and keeps the Cloudflare User-Agent",
    () =>
      withEnv({ CLOUDFLARE_ACCOUNT_ID: "acct", CLOUDFLARE_API_KEY: "env-key" }, () =>
        Effect.gen(function* () {
          ...
          const headers = yield* Effect.promise(() =>
            Promise.resolve(cloudflareHeaders(result.sdk)),
          );
          expect(headers.authorization).toBe("Bearer env-key");
          expect(headers.custom).toBe("header");
          expect(headers["user-agent"]).toMatch(
            /^opencode\/.* cloudflare-workers-ai \(.+\) ai-sdk\/openai-compatible\//,
          );
        }),
      ),
  );
