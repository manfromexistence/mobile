...
describe("CerebrasPlugin", () => {
  it.effect("applies the legacy integration header", () =>
    Effect.gen(function* () {
      ...
      yield* addPlugin();
      expect(
        (yield* catalog.provider.get(ProviderV2.ID.make("cerebras")))?.request.headers,
      ).toEqual({
        Existing: "1",
  ...
  it.effect("ignores non-Cerebras providers", () =>
    Effect.gen(function* () {
      const catalog = yield* Catalog.Service;
      yield* catalog.transform((catalog) =>
        catalog.provider.update(ProviderV2.ID.make("groq"), () => {}),
      );
      yield* addPlugin();
  ...
  it.effect("creates a bundled Cerebras SDK with the model provider ID as the SDK name", () =>
    Effect.gen(function* () {
      ...
      expect(result.sdk.languageModel("llama-4-scout-17b-16e-instruct").provider).toBe(
        "custom-cerebras",
      );
    }),
  );
