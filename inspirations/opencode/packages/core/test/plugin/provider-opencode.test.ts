...
describe("OpencodePlugin", () => {
  it.effect("registers account and service account methods", () =>
    Effect.gen(function* () {
      yield* addPlugin();
      expect(
        (yield* (yield* Integration.Service).get(Integration.ID.make("opencode")))?.methods,
      ).toEqual([
        {
    ...
    Effect.acquireUseRelease(
      ...
      ({ authorization, release, server }) =>
        Effect.gen(function* () {
          ...
          expect(
            yield* (yield* Integration.Service).get(Integration.ID.make("remote")),
          ).toBeUndefined();

          const model = required(
            yield* catalog.model.get(ProviderV2.ID.make("remote"), ModelV2.ID.make("model")),
          );
          ...
          expect(
            required(
              yield* catalog.model.get(ProviderV2.ID.make("remote"), ModelV2.ID.make("disabled")),
            ).enabled,
          ).toBe(false);
          expect(
            yield* catalog.model.get(ProviderV2.ID.make("remote"), ModelV2.ID.make("stale")),
          ).toBeDefined();
  ...
  it.effect("uses a public key and disables paid models without credentials", () =>
    withEnv({ OPENCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin();
        expect(
          required(yield* catalog.provider.get(ProviderV2.ID.opencode)).request.body.apiKey,
        ).toBe("public");
        expect(
          required(yield* catalog.model.get(ProviderV2.ID.opencode, ModelV2.ID.make("paid")))
            .enabled,
        ).toBe(false);
      }),
    ),
  ...
  it.effect("keeps free models without credentials", () =>
    withEnv({ OPENCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        expect(
          required(yield* catalog.model.get(ProviderV2.ID.opencode, ModelV2.ID.make("free")))
            .enabled,
        ).toBe(true);
      }),
    ),
  ...
  it.effect("treats output-only cost as free without credentials", () =>
    withEnv({ OPENCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        expect(
          required(yield* catalog.model.get(ProviderV2.ID.opencode, ModelV2.ID.make("output-only")))
            .enabled,
        ).toBe(true);
      }),
    ),
  ...
  it.effect("ignores non-opencode providers and models", () =>
    withEnv({ OPENCODE_API_KEY: undefined }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin();
        expect(
          required(yield* catalog.provider.get(ProviderV2.ID.openai)).request.body.apiKey,
        ).toBeUndefined();
        expect(
          required(yield* catalog.model.get(ProviderV2.ID.openai, ModelV2.ID.make("paid"))).enabled,
        ).toBe(true);
      }),
    ),
  );
