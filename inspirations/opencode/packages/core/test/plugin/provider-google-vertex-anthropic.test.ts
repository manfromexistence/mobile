...
import {
  ...
  GoogleVertexPlugin,
} from "@opencode-ai/core/plugin/provider/google-vertex";
...
describe("GoogleVertexAnthropicPlugin", () => {
    ...
    withEnv(
      ...
      () =>
        Effect.gen(function* () {
          ...
          expect(
            (yield* catalog.provider.get(ProviderV2.ID.make("google-vertex-anthropic")))?.request
              .body.project,
          ).toBe("cloud-project");
          expect(
            (yield* catalog.provider.get(ProviderV2.ID.make("google-vertex-anthropic")))?.request
  ...
  it.effect("keeps configured project and location over env fallback", () =>
    withEnv({ GOOGLE_CLOUD_PROJECT: "env-project", GOOGLE_CLOUD_LOCATION: "env-location" }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin(GoogleVertexAnthropicPlugin);
        expect(
          (yield* catalog.provider.get(ProviderV2.ID.make("google-vertex-anthropic")))?.request.body
            .project,
        ).toBe("configured-project");
        expect(
          (yield* catalog.provider.get(ProviderV2.ID.make("google-vertex-anthropic")))?.request.body
            .location,
  ...
  it.effect("selects google-vertex Anthropic language models through V2 plugins", () =>
    Effect.gen(function* () {
      ...
      const language = languageResult.language as unknown as {
