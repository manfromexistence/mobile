...
describe("OpenRouterPlugin", () => {
  it.effect("is registered so legacy OpenRouter behavior can be applied", () =>
    Effect.sync(() =>
      expect(ProviderPlugins.map((item) => item.id)).toContain(PluginV2.ID.make("openrouter")),
    ),
  );
    ...
    Effect.gen(function* () {
      ...
      yield* catalog.transform((catalog) => {
        ...
        catalog.model.update(
          ...
          ModelV2.ID.make("openai/gpt-5-chat"),
          () => {},
        );
      ...
      expect(
        (yield* catalog.model.get(ProviderV2.ID.openrouter, ModelV2.ID.make("openai/gpt-5-chat")))
          ?.enabled,
      ).toBe(false);
      expect(
        (yield* catalog.model.get(ProviderV2.ID.openrouter, ModelV2.ID.make("openai/gpt-5")))
          ?.enabled,
      ).toBe(true);
      expect(
        (yield* catalog.model.get(ProviderV2.ID.openai, ModelV2.ID.make("openai/gpt-5-chat")))
          ?.enabled,
      ).toBe(true);
    }),
  );
  ...
  it.effect("does not disable gpt-5-chat-latest for non-OpenRouter providers", () =>
    Effect.gen(function* () {
      ...
      yield* catalog.transform((catalog) => {
        ...
        catalog.model.update(
          ProviderV2.ID.make("custom-openrouter"),
          ModelV2.ID.make("gpt-5-chat-latest"),
          () => {},
        );
      });
      yield* addPlugin();
      expect(
        (yield* catalog.model.get(
          ProviderV2.ID.make("custom-openrouter"),
          ModelV2.ID.make("gpt-5-chat-latest"),
        ))?.enabled,
      ).toBe(true);
