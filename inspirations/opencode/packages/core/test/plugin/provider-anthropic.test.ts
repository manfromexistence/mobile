...
describe("AnthropicPlugin", () => {
  it.effect("applies legacy beta headers", () =>
    Effect.gen(function* () {
      ...
      yield* addPlugin();
      expect(
        required(yield* catalog.provider.get(ProviderV2.ID.anthropic)).request.headers[
          "anthropic-beta"
        ],
      ).toBe("interleaved-thinking-2025-05-14,fine-grained-tool-streaming-2025-05-14");
      expect(
        required(yield* catalog.provider.get(ProviderV2.ID.anthropic)).request.headers.Existing,
      ).toBe("1");
    }),
  );
  ...
  it.effect("ignores non-Anthropic providers", () =>
    Effect.gen(function* () {
      ...
      yield* addPlugin();
      expect(
        required(yield* catalog.provider.get(ProviderV2.ID.openai)).request.headers[
          "anthropic-beta"
        ],
      ).toBeUndefined();
    }),
  ...
  it.effect(
    "uses the Anthropic provider ID as the SDK name for the bundled Anthropic provider",
    () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service;
        const aisdk = yield* AISDK.Service;
