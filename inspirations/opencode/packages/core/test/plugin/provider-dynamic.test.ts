...
const addPlugin = Effect.fn(function* (npm?: Npm.Interface) {
  ...
  yield* DynamicProviderPlugin.effect(host).pipe(
    Effect.provideService(Npm.Service, npm ?? (yield* Npm.Service)),
  );
});
...
describe("DynamicProviderPlugin", () => {
  it.effect("creates an SDK from a provider factory export", () =>
    Effect.gen(function* () {
      ...
      expect(result.sdk.languageModel("x")).toEqual({
        modelID: "x",
        options: { marker: "dynamic", name: "custom" },
      });
    }),
  );
  ...
  it.effect("loads npm packages through their resolved import entrypoint", () =>
    Effect.gen(function* () {
      ...
      expect(result.sdk.languageModel("x")).toEqual({
        modelID: "x",
        options: { marker: "npm", name: "npm-provider" },
      });
    }),
  );
    ...
    Effect.gen(function* () {
      ...
      const exit = yield* aisdk
        .language(
          ModelV2.Info.make({
            ...ModelV2.Info.empty(
              ProviderV2.ID.make("missing-entrypoint"),
