...
function captureAiGatewayOptions(options: Record<string, unknown>) {
  const nested =
    options.options && typeof options.options === "object"
...
describe("CloudflareAIGatewayPlugin", () => {
  ...
  it.effect(
    "passes legacy metadata, cache, log, and User-Agent values under the AI Gateway options key",
    () =>
  ...
  it.effect("does not create an SDK when account and gateway IDs are missing", () =>
    withEnv(
      cloudflareEnv({ CLOUDFLARE_ACCOUNT_ID: undefined, CLOUDFLARE_GATEWAY_ID: undefined }),
      () =>
        Effect.gen(function* () {
          resetCalls();
          const plugin = yield* PluginV2.Service;
