...
describe("GitLabPlugin", () => {
  it.effect(
    "creates SDKs with legacy default instance URL, token env, headers, and feature flags",
    () =>
      withEnv(
        {
        ...
        () =>
          Effect.gen(function* () {
            ...
            expect(
              String(
                (gitlabSDKOptions[0].aiGatewayHeaders as Record<string, string>)["User-Agent"],
              ),
            ).toContain("gitlab-ai-provider/test-version");
  ...
  it.effect(
    "keeps configured instance URL, apiKey, aiGatewayHeaders, and featureFlags over env/defaults",
    () =>
      withEnv(
        {
          GITLAB_INSTANCE_URL: "https://env.gitlab.example",
  ...
  it.effect("uses workflowChat for duo workflow models and preserves selectedModelRef", () =>
    Effect.gen(function* () {
      ...
      const result = yield* aisdk.runLanguage({
        model: ModelV2.Info.make({
          ...ModelV2.Info.empty(
            ProviderV2.ID.make("gitlab"),
            ModelV2.ID.make("duo-workflow-custom"),
          ),
          api: {
            id: ModelV2.ID.make("duo-workflow-custom"),
            type: "aisdk",
    ...
    Effect.gen(function* () {
      ...
      const result = yield* aisdk.runLanguage({
        model: ModelV2.Info.make({
          ...ModelV2.Info.empty(
            ProviderV2.ID.make("gitlab"),
            ModelV2.ID.make("duo-workflow-exact"),
          ),
          api: {
            id: ModelV2.ID.make("duo-workflow-exact"),
            type: "aisdk",
            package: "test-provider",
          },
        }),
      ...
      expect(calls).toEqual([
        [
          "duo-workflow-exact",
          { featureFlags: { configured: true }, workflowDefinition: undefined },
        ],
      ]);
      expect(result.language as unknown).toEqual({
        id: "duo-workflow-exact",
        options: calls[0]?.[1],
      });
    }),
  );
  ...
  it.effect("uses provider feature flags instead of request feature flags", () =>
    Effect.gen(function* () {
      ...
      expect(calls).toEqual([
        ["duo-workflow", { featureFlags: { configured: true }, workflowDefinition: undefined }],
      ]);
    }),
  );

  it.effect(
    "uses agenticChat with provider aiGatewayHeaders and feature flags for normal models",
    () =>
      Effect.gen(function* () {
        const plugin = yield* PluginV2.Service;
        const aisdk = yield* AISDK.Service;
          ...
          sdk: {
            ...
            agenticChat: (id: string, options: unknown) => {
              ...
              calls.push([
                id,
                {
                  aiGatewayHeaders: { ...selected.aiGatewayHeaders },
                  featureFlags: { ...selected.featureFlags },
                },
              ]);
            },
          },
          options: {
            aiGatewayHeaders: { fallback: "header" },
            featureFlags: { duo_agent_platform: true },
          },
        });
        expect(calls).toEqual([
          [
