  ...
  it.effect("resolves resourceName from env", () =>
    withEnv({ AZURE_RESOURCE_NAME: "from-env" }, () =>
      Effect.gen(function* () {
        ...
        yield* addPlugin();
        expect(
          required(yield* catalog.provider.get(ProviderV2.ID.azure)).request.body.resourceName,
        ).toBe("from-env");
      }),
    ),
  );
  ...
  it.effect("keeps explicit resourceName over env and ignores other providers", () =>
    withEnv({ AZURE_RESOURCE_NAME: "from-env" }, () =>
      Effect.gen(function* () {
        ...
        expect(
          required(yield* catalog.provider.get(ProviderV2.ID.openai)).request.body.resourceName,
        ).toBeUndefined();
      }),
    ),
  );
    ...
    Effect.gen(function* () {
      ...
      yield* aisdk.runLanguage({
        model: ModelV2.Info.make({
          ...
          api: {
            id: ModelV2.ID.make("messages-deployment"),
            type: "aisdk",
      ...
      yield* aisdk.runLanguage({
        model: ModelV2.Info.make({
          ...
          api: {
            id: ModelV2.ID.make("language-deployment"),
            type: "aisdk",
            package: "test-provider",
          },
        }),
