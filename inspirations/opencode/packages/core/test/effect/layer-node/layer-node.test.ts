...
class Database extends Context.Service<Database, { readonly name: string }>()(
  "test/GraphDatabase",
) {}
class Users extends Context.Service<Users, { readonly list: Effect.Effect<string[]> }>()(
  "test/GraphUsers",
) {}
class App extends Context.Service<App, { readonly run: Effect.Effect<string[]> }>()(
  "test/GraphApp",
) {}
...
describe("layer node", () => {
  ...
  test("preserves branch-specific implementations across roots", async () => {
    const firstValue = make({
      service: Value,
      layer: Layer.succeed(Value, Value.of({ value: "first" })),
      deps: [],
    });
    const secondValue = make({
      service: Value,
      layer: Layer.succeed(Value, Value.of({ value: "second" })),
      deps: [],
    });
  ...
  test("does not acquire an unused replacement", async () => {
    ...
    const other = make({
      service: Left,
      layer: Layer.succeed(Left, Left.of({ value: "other" })),
      deps: [],
    });
