...
class CycleB extends Context.Service<CycleB, { readonly directory: AbsolutePath }>()(
  "test/NodeBuildB",
) {}
...
describe("node build", () => {
  ...
  test("detects cycles through a replaced location service map", async () => {
    ...
    const map = Node.makeGlobalNode({
      service: LocationServiceMap.Service,
      layer: mapLayer,
      deps: [b],
    });
    expect(() =>
      AppNodeBuilder.build(LayerNode.group([a]), [[LocationServiceMap.node, map]]),
    ).toThrow("Cycle detected in layer tree");
  });
