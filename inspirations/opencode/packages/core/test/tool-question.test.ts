  ...
  PermissionV2.Service.of({
    assert: (input) =>
      Effect.sync(() => assertions.push(input)).pipe(
        Effect.andThen(
          deny ? Effect.fail(new PermissionV2.DeniedError({ rules: [] })) : Effect.void,
        ),
      ),
  ...
  QuestionV2.Service.of({
    ask: (input: QuestionV2.AskInput) =>
      Effect.sync(() => {
        ...
        Effect.andThen(
          reject ? Effect.fail(new QuestionV2.RejectedError()) : Effect.succeed([["Build"], []]),
        ),
      ),
    reply: () => Effect.die("unused"),
...
const it = testEffect(
  AppNodeBuilder.build(
    LayerNode.group([ToolRegistry.node, ToolRegistry.toolsNode, QuestionTool.node]),
    [
...
describe("QuestionTool", () => {
  it.effect("omits a denied built-in question and terminally settles a stale call", () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* toolDefinitions(registry, [{ action: "question", resource: "*", effect: "deny" }]),
      ).toEqual([]);
      expect(
