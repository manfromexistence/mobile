...
describe("CatalogV2", () => {
  ...
  it.effect("derives availability from active credentials without changing provider state", () => {
    ...
    return Effect.gen(function* () {
      ...
      expect(
        required(yield* catalog.provider.get(ProviderV2.ID.make("test"))).request.body,
      ).toEqual({});
      yield* credentials.create({
        integrationID,
        label: "Second",
  ...
  it.effect("derives availability from a provider's integration", () => {
    ...
    return Effect.gen(function* () {
      const catalog = yield* Catalog.Service;
      yield* (yield* Integration.Service).transform((editor) =>
        editor.update(integrationID, () => {}),
      );
      yield* catalog.transform((editor) =>
  ...
  it.effect("removes providers denied by policy after loading", () =>
    Effect.gen(function* () {
      ...
      yield* policy.load([
        new Policy.Info({ effect: "deny", action: "provider.use", resource: "blocked" }),
      ]);
      yield* catalog.transform((catalog) => {
        catalog.provider.update(providerID, () => {});
