...
describe("MoveSession", () => {
  it.live("moves session changes to another project directory", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.addFinalizer(() =>
        Effect.promise(() => fs.rm(destination, { recursive: true, force: true })).pipe(
          Effect.ignore,
        ),
      );
      yield* Effect.promise(() =>
        $`git worktree add --detach ${destination} HEAD`.cwd(root.path).quiet(),
      );
      ...
      expect(
        yield* Effect.promise(() => fs.readFile(path.join(moved, "tracked.txt"), "utf8")),
      ).toBe("changed\n");
      expect(
        yield* Effect.promise(() => fs.readFile(path.join(moved, "untracked.txt"), "utf8")),
      ).toBe("new\n");
      expect(
        yield* Effect.promise(() => fs.readFile(path.join(source, "tracked.txt"), "utf8")),
      ).toBe("initial\n");
      expect(
        yield* Effect.promise(() => Bun.file(path.join(source, "untracked.txt")).exists()),
      ).toBe(false);
      expect(
        yield* db
  ...
  it.live("moves within a checkout without transferring existing changes", () =>
    Effect.gen(function* () {
      ...
      expect(
        yield* Effect.promise(() => fs.readFile(path.join(source, "untracked.txt"), "utf8")),
      ).toBe("new\n");
      expect(
        yield* db
          .select({ directory: SessionTable.directory, path: SessionTable.path })
  ...
  it.live("moves nested session changes without cleaning unrelated files", () =>
    Effect.gen(function* () {
      ...
      yield* Effect.promise(() =>
        fs.writeFile(path.join(sourceDirectory, "tracked.txt"), "initial\n"),
      );
      yield* Effect.promise(() =>
        fs.writeFile(path.join(sourceDirectory, "staged.txt"), "initial\n"),
      );
      yield* Effect.promise(() =>
        $`git add packages/tracked.txt packages/staged.txt`.cwd(source).quiet(),
      );
      ...
      yield* Effect.promise(() =>
        $`git worktree add --detach ${destination} HEAD`.cwd(source).quiet(),
      );
      const moved = abs(
        path.join(yield* Effect.promise(() => fs.realpath(destination)), "packages"),
      );
      yield* Effect.promise(() =>
        fs.writeFile(path.join(sourceDirectory, "tracked.txt"), "changed\n"),
      );
      yield* Effect.promise(() =>
        fs.writeFile(path.join(sourceDirectory, "staged.txt"), "staged\n"),
      );
      yield* Effect.promise(() => $`git add packages/staged.txt`.cwd(source).quiet());
      yield* Effect.promise(() =>
        fs.writeFile(path.join(sourceDirectory, "untracked.txt"), "new\n"),
      );
      ...
      expect(yield* Effect.promise(() => fs.readFile(path.join(moved, "staged.txt"), "utf8"))).toBe(
        "staged\n",
      );
      expect(
        yield* Effect.promise(() => fs.readFile(path.join(moved, "untracked.txt"), "utf8")),
      ...
      expect(
        yield* Effect.promise(() => fs.readFile(path.join(sourceDirectory, "tracked.txt"), "utf8")),
      ).toBe("initial\n");
      expect(
        yield* Effect.promise(() => Bun.file(path.join(sourceDirectory, "untracked.txt")).exists()),
      ).toBe(false);
      expect(
        yield* Effect.promise(() => fs.readFile(path.join(sourceDirectory, "staged.txt"), "utf8")),
      ).toBe("staged\n");
      expect(
        yield* Effect.promise(() =>
          $`git status --porcelain -- packages/staged.txt`.cwd(source).text(),
        ),
