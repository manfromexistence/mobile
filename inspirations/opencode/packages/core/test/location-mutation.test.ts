...
describe("LocationMutation", () => {
    ...
    withTmp((directory) =>
      Effect.gen(function* () {
        const error = yield* Effect.flip(
          (yield* LocationMutation.Service).resolve({ path: "../outside.txt" }),
        );
  ...
  it.live("follows an in-location symlink using ordinary filesystem semantics", () =>
    withTmp((directory) =>
      Effect.gen(function* () {
        ...
        expect(
          yield* (yield* LocationMutation.Service).resolve({ path: "linked/new.txt" }),
        ).toMatchObject({
          canonical: path.join(
            yield* Effect.promise(() => fs.realpath(directory)),
            "actual",
            "new.txt",
          ),
  ...
  it.live(
    "requires external-directory authorization for an explicit external absolute target",
    () =>
      withTmp((directory) =>
        withTmp((outside) =>
          Effect.gen(function* () {
  ...
  test("ignores unknown mutation input fields", () => {
    ...
    expect(
      Schema.decodeUnknownSync(LocationMutation.ResolveInput)({
        path: "README.md",
        reference: "docs",
      }),
    ).toEqual({
