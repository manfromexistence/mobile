...
describe("BashTool", () => {
      ...
      (tmp) => {
        ...
        return withTool(tmp.path, (registry) =>
          Effect.gen(function* () {
            ...
            expect(
              yield* toolDefinitions(registry, [{ action: "bash", resource: "*", effect: "deny" }]),
            ).toEqual([]);
            ...
            expect(assertions).toMatchObject([
              { sessionID, action: "bash", resources: ["pwd"], save: ["pwd"] },
            ]);
          }),
        );
      },
    ...
    Effect.acquireUseRelease(
      ...
      ([active, outside]) =>
        Effect.gen(function* () {
          ...
          denyAction = "bash";
          yield* withTool(active.path, (registry) =>
            executeTool(registry, call({ command: "pwd" })),
          );
  ...
  it.live(
    "reports external command arguments as advisory warnings without enforcing approval",
    () =>
      Effect.acquireUseRelease(
        Effect.promise(() => Promise.all([tmpdir(), tmpdir()])),
        ([active, outside]) => {
  ...
  it.live("keeps non-zero exits useful", () =>
    Effect.acquireUseRelease(
      ...
      (tmp) => {
        ...
        return withTool(tmp.path, (registry) =>
          settleTool(registry, call({ command: "false" }, "call-overflow")),
        ).pipe(
          Effect.andThen((settled) =>
            Effect.sync(() => {
              expect(settled.output?.content[1]).toMatchObject({
  ...
  it.live("surfaces bounded process-capture truncation", () =>
    Effect.acquireUseRelease(
      ...
      (tmp) => {
        ...
        return withTool(tmp.path, (registry) =>
          settleTool(registry, call({ command: "verbose" })),
        ).pipe(
          Effect.andThen((settled) =>
            Effect.sync(() => {
  ...
  it.live("returns a useful timeout settlement", () =>
    Effect.acquireUseRelease(
      ...
      (tmp) => {
        ...
        return withTool(tmp.path, (registry) =>
          settleTool(registry, call({ command: "sleep 60", timeout: 10 })),
        ).pipe(
          Effect.andThen((settled) =>
            Effect.sync(() => {
              expect(settled.output?.content[1]).toMatchObject({
