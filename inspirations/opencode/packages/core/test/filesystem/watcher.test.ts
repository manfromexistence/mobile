...
function maybeNextUpdate<E>(
  ...
  return Effect.acquireUseRelease(
    wait(check),
    ({ deferred }) =>
      trigger.pipe(Effect.andThen(Deferred.await(deferred)), Effect.timeoutOption(timeout)),
      ...
      (directory) =>
        Effect.gen(function* () {
          ...
          for (const item of [
            ...
            expect(
              yield* nextUpdate(
                (event) => event.file === file && event.event === item.event,
                item.trigger,
              ),
            ).toEqual({
              file,
        ...
        Effect.gen(function* () {
          ...
          yield* noUpdate(
            ...
            fs
              ...
              .pipe(
                Effect.andThen(Effect.promise(() => $`git add .`.cwd(directory).quiet())),
                Effect.asVoid,
              ),
          );
        }),
        ...
        (directory) =>
          Effect.gen(function* () {
            ...
            yield* Effect.addFinalizer(() =>
              Effect.promise(() => fs.rm(actual, { recursive: true, force: true })),
            );
            yield* ready(directory);
            const head = path.join(directory, ".git", "HEAD");
            const branch = `watch-${Math.random().toString(36).slice(2)}`;
