...
describe("SessionV2.create", () => {
  ...
  it.effect("returns one recorded session to concurrent exact retries", () =>
    Effect.gen(function* () {
      ...
      const created = yield* Effect.all([session.create(input), session.create(input)], {
  ...
  it.effect("does not mask unrelated created projector defects", () =>
    Effect.gen(function* () {
      ...
      expect(yield* session.create({ id, location }).pipe(Effect.catchDefect(Effect.succeed))).toBe(
        defect,
      );
    }),
  );
    ...
    Effect.gen(function* () {
      ...
      const unavailable = (
        ...
        effect.pipe(
          Effect.flip,
          Effect.map((error) =>
            error instanceof SessionV2.OperationUnavailableError ? error.operation : "not-found",
          ),
        );

      expect(yield* unavailable(session.shell({ sessionID: created.id, command: "pwd" }))).toBe(
        "shell",
      );
      expect(yield* unavailable(session.skill({ sessionID: created.id, skill: "review" }))).toBe(
        "skill",
      );
    }),
  );
