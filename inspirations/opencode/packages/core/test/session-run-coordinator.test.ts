  ...
  it.effect("coalesces wakes received during active execution", () =>
    Effect.scoped(
      Effect.gen(function* () {
        ...
        yield* Deferred.await(firstStarted);
        yield* Effect.all(
          [coordinator.wake("session"), coordinator.wake("session"), coordinator.wake("session")],
          {
        ...
        const coordinator = yield* SessionRunCoordinator.make({
          drain: () =>
            Effect.sync(() => ++active).pipe(
              Effect.tap(() =>
                active === 2 ? Deferred.succeed(bothStarted, undefined) : Effect.void,
              ),
              Effect.andThen(Deferred.await(gate)),
            ),
        });
      ...
      Effect.gen(function* () {
        ...
        const coordinator = yield* SessionRunCoordinator.make<string, never>({
          drain: (key) =>
            Effect.sync(() => ++runs).pipe(
              Effect.tap((run) =>
                run < limit ? wake(key) : Deferred.succeed(completed, undefined),
              ),
              Effect.asVoid,
            ),
        });
