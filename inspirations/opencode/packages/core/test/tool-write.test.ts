        ...
        return withTool(tmp.path, (registry) =>
          Effect.gen(function* () {
            ...
            const settled = yield* settleTool(
              registry,
              call({ path: "src/new.txt", content: "created" }),
            );
            ...
            expect(
              yield* Effect.promise(() =>
                fs.readFile(path.join(tmp.path, "src", "new.txt"), "utf8"),
              ),
            ).toBe("created");
    ...
    Effect.acquireUseRelease(
      ...
      (tmp) => {
        ...
        return Effect.promise(() =>
          fs.writeFile(path.join(tmp.path, "existing.txt"), "before"),
        ).pipe(
          Effect.andThen(
            withTool(tmp.path, (registry) =>
              settleTool(registry, call({ path: "existing.txt", content: "after" })),
            ),
          ),
          Effect.andThen((settled) =>
            Effect.gen(function* () {
              expect(settled.result).toEqual({
                type: "text",
                value: "Wrote file successfully: existing.txt",
              });
              ...
              expect(
                yield* Effect.promise(() =>
                  fs.readFile(path.join(tmp.path, "existing.txt"), "utf8"),
                ),
              ).toBe("after");
  ...
  it.live("preserves exactly one BOM when overwriting existing files", () =>
    Effect.acquireUseRelease(
      ...
      (tmp) => {
        ...
        return Effect.promise(() =>
          Promise.all([
            fs.writeFile(preserved, "\uFEFFbefore"),
            fs.writeFile(deduplicated, "\uFEFFbefore"),
          ]),
        ).pipe(
          Effect.andThen(
            withTool(tmp.path, (registry) =>
              Effect.gen(function* () {
                ...
                expect(yield* Effect.promise(() => fs.readFile(preserved, "utf8"))).toBe(
                  "\uFEFFafter",
                );
                expect(yield* Effect.promise(() => fs.readFile(deduplicated, "utf8"))).toBe(
                  "\uFEFFafter",
                );
              }),
            ),
          ...
          Effect.andThen((result) =>
            Effect.gen(function* () {
              expect(result).toEqual({
                type: "text",
                value: "Created file successfully: absolute.txt",
              });
          ...
          Effect.andThen((settled) =>
            Effect.gen(function* () {
              const canonicalTarget = path.join(
                yield* Effect.promise(() => fs.realpath(outside.path)),
                "external.txt",
              );
              ...
              expect(assertions[0]).toMatchObject({
                resources: [
                  path
                    .join(yield* Effect.promise(() => fs.realpath(outside.path)), "*")
                    .replaceAll("\\", "/"),
                ],
              });
...
test("keeps the locked write schema, semantics docstring, and deferred UX TODOs visible", async () => {
  const source = (
    await fs.readFile(new URL("../src/tool/write.ts", import.meta.url), "utf8")
  ).replaceAll("\r\n", "\n");
