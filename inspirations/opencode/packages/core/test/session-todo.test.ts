...
const it = testEffect(
  AppNodeBuilder.build(LayerNode.group([Database.node, EventV2.node, SessionTodo.node])),
);
...
describe("SessionTodo", () => {
  it.effect("replaces persisted todos in order and publishes updates", () =>
    Effect.gen(function* () {
      ...
      yield* todos.update({
        sessionID,
        todos: [{ content: "replacement", status: "completed", priority: "medium" }],
      });
      expect(yield* todos.get(sessionID)).toEqual([
        { content: "replacement", status: "completed", priority: "medium" },
      ]);
