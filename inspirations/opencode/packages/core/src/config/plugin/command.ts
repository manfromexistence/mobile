            ...
            Effect.map((commands) => [
              {
                commands: Object.fromEntries(
                  commands.map((command) => [command.name, command.info]),
                ),
              },
            ]),
          );
  ...
  return Effect.gen(function* () {
    ...
    return yield* Effect.forEach(files.toSorted(), (filepath) =>
      fs.readFileStringSafe(filepath).pipe(
        Effect.map((content) =>
          content === undefined ? undefined : decode(directory, filepath, content),
        ),
      ...
      Effect.map((commands) =>
        commands.filter(
          (command): command is { name: string; info: ConfigCommand.Info } => command !== undefined,
        ),
      ),
    );
  });
...
function decode(directory: string, filepath: string, content: string) {
  ...
  const info = Option.getOrUndefined(
    decodeCommand({ ...markdown.data, template: markdown.content.trim() }),
  );
  if (!info) return;
  return {
    name: path
