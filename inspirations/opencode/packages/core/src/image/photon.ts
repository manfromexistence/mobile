...
export const make = Effect.gen(function* () {
  ...
  return Effect.fn("Image.Photon.normalize")(function* (
    ...
    try {
      ...
      const sizes = Array.from({ length: 32 }).reduce<Array<{ width: number; height: number }>>(
        (acc) => {
          ...
          const next =
            acc.length === 0
              ...
              : {
                  ...
                  height:
                    previous.height === 1 ? 1 : Math.max(1, Math.floor(previous.height * 0.75)),
                };
      ...
      for (const size of sizes) {
        ...
        try {
          const encoders: Array<readonly [mime: string, encode: () => Uint8Array]> = [
            ...
            ...JPEG_QUALITIES.map(
              (quality) => ["image/jpeg", () => resized.get_bytes_jpeg(quality)] as const,
            ),
          ];
