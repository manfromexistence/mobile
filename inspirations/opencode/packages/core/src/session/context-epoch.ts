...
export function prepare(
  ...
  return prepareOnce(db, events, context, sessionID).pipe(
...
const prepareOnce = Effect.fnUntraced(function* (
  ...
  const snapshot = yield* Schema.decodeUnknownEffect(SystemContext.Snapshot)(stored.snapshot).pipe(
    Effect.mapError(
      (error) => new ContextSnapshotDecodeError({ sessionID, details: String(error) }),
    ),
  );
  const replacementSeq =
    compaction !== undefined && compaction.seq > stored.baseline_seq ? compaction.seq : undefined;
