...
export interface Adapter {
  ...
  ) => Effect.Effect<SessionMessage.Assistant | undefined>;
...
export function memory(state: MemoryState): Adapter {
  ...
  const latestAssistantIndex = () =>
    state.messages.findLastIndex((message) => message.type === "assistant");
  const activeShellIndex = (callID: string) =>
    state.messages.findLastIndex(
      (message) => message.type === "shell" && message.callID === callID,
    );
...
export function update(adapter: Adapter, event: SessionEvent.Event) {
  ...
  const latestText = (assistant: DraftAssistant | undefined, textID: string) =>
    assistant?.content.findLast(
      (item): item is DraftText => item.type === "text" && item.id === textID,
    );
  ...
  const updateOwnedAssistant = (
    messageID: SessionMessage.ID,
    recipe: (draft: DraftAssistant) => void,
  ) =>
    Effect.gen(function* () {
