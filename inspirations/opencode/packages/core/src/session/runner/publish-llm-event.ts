...
const settledOutput = (value: ToolOutput | undefined, result: ToolResultValue): SettledOutput => {
  if (result.type === "error")
    return { error: { type: "unknown", message: message(result.value) } };
...
export const createLLMEventPublisher = (events: EventV2.Interface, input: Input) => {
  ...
  let stepSettlement:
    | { readonly finish: string; readonly tokens: ReturnType<typeof tokens> }
    | undefined;
  ...
  const assistantMessageIDForTool = (callID: string) => {
    const tool = tools.get(callID);
    return tool
      ? Effect.succeed(tool.assistantMessageID)
      : Effect.die(`Unknown tool call: ${callID}`);
  };
  ...
  const publish = Effect.fn("SessionRunner.publishLLMEvent")(function* (
    ...
    switch (event.type) {
      ...
      case "tool-call": {
        ...
        if (tool.name !== event.name)
          return yield* Effect.die(
            `Tool call name changed for ${event.id}: ${tool.name} -> ${event.name}`,
          );
      ...
      case "tool-result": {
        ...
        if (tool.name !== event.name)
          return yield* Effect.die(
            `Tool result name changed for ${event.id}: ${tool.name} -> ${event.name}`,
          );
        if (tool.settled) {
      ...
      case "tool-error": {
        ...
        if (tool.name !== event.name)
          return yield* Effect.die(
            `Tool error name changed for ${event.id}: ${tool.name} -> ${event.name}`,
          );
