...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getSession = Effect.fn("SessionRunner.getSession")(function* (
    ...
    const getContext = Effect.fn("SessionRunner.getContext")(function* (
      sessionID: SessionSchema.ID,
    ) {
        ...
        for (const tool of message.content) {
          ...
          yield* events.publish(SessionEvent.Tool.Failed, {
            ...
            provider: {
              ...
              ...(tool.provider?.metadata === undefined
                ? {}
    ...
    const continueAfterCompaction = (step: number) =>
      new TurnTransitionError({ _tag: "ContinueAfterCompaction", step });
    ...
    const runTurnAttempt = Effect.fn("SessionRunner.runTurn")(function* (
      ...
      const promptCacheKey = /^ses_[0-9a-f]{64}$/.test(session.id)
        ? session.id.slice(4)
        : session.id;
      ...
      return yield* Effect.uninterruptibleMask((restore) =>
        Effect.gen(function* () {
          ...
          const failure =
            stream._tag === "Failure"
              ? Option.getOrUndefined(Cause.findErrorOption(stream.cause))
              : undefined;
          if (
            recoverOverflow &&
          ...
          if (llmFailure && !publisher.hasProviderError()) {
            yield* withPublication(
              publisher.failUnsettledTools("Provider did not return a tool result", true),
            );
          ...
          if (settled._tag === "Failure" && !Cause.hasInterrupts(settled.cause)) {
            ...
            yield* withPublication(
              publisher.failUnsettledTools(`Tool execution failed: ${message}`),
            );
          }
    ...
    const runAfterOverflowCompaction: RunTurn = Effect.fnUntraced(
      function* (sessionID, promotion, step) {
