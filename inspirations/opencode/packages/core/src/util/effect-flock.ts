...
export namespace EffectFlock {
  ...
  export class LockTimeoutError extends Schema.TaggedErrorClass<LockTimeoutError>()(
    "LockTimeoutError",
    {
      key: Schema.String,
    },
  ...
  export class LockCompromisedError extends Schema.TaggedErrorClass<LockCompromisedError>()(
  ...
  const isPathGone = (e: PlatformError) =>
    e.reason._tag === "NotFound" || e.reason._tag === "Unknown";
  ...
  const layer: Layer.Layer<Service, never, Global.Service | FSUtil.Service> = Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const forceRemove = (target: string) =>
        fs.remove(target, { recursive: true }).pipe(Effect.ignore);
      ...
      const tryAcquireLockDir = (lockDir: string, key: string) =>
        Effect.gen(function* () {
          ...
          if (!created) {
            if (!(yield* isStale(lockDir, heartbeatPath, metaPath)))
      ...
      const acquire = Effect.fn("EffectFlock.acquire")(function* (key: string, dir?: string) {
        ...
        const handle = yield* Effect.acquireRelease(acquireHandle(lockfile, key), (handle) =>
          release(handle),
        );
