...
export type Draft = {
  ...
  model: {
    ...
    update: (
      ...
      modelID: ModelV2.ID,
      fn: (model: ModelV2.MutableInfo) => void,
    ) => void;
          ...
          model: {
            ...
            update: (providerID, modelID, fn) => {
              ...
              const model =
                ...
                (ModelV2.Info.empty(providerID, modelID) as ModelV2.MutableInfo);
    ...
    const result: Interface = {
      ...
      provider: {
        ...
        all: Effect.fn("CatalogV2.provider.all")(function* () {
          return Array.fromIterable(state.get().providers.values()).map(
        ...
        available: Effect.fn("CatalogV2.provider.available")(function* () {
          const active = new Map(
            (yield* integrations.list()).map((integration) => [integration.id, integration]),
          );
      ...
      model: {
        ...
        available: Effect.fn("CatalogV2.model.available")(function* () {
          const providers = new Set(
            (yield* result.provider.available()).map((provider) => provider.id),
          );
          return (yield* result.model.all()).filter(
            (model) => providers.has(model.providerID) && model.enabled,
          );
        }),
        ...
        default: Effect.fn("CatalogV2.model.default")(function* () {
          ...
          if (defaultModel) {
            ...
            if (
              provider &&
              (yield* result.provider.available()).some((item) => item.id === provider.id)
            ) {
        ...
        small: Effect.fn("CatalogV2.model.small")(function* (providerID) {
          ...
          if (providerID === ProviderV2.ID.opencode) {
            ...
            if (gpt5Nano?.enabled && gpt5Nano.status === "active")
          ...
          const candidates = pipe(
            ...
            Array.map((model) => ({
              ...
              small: SMALL_MODEL_RE.test(
                `${model.id} ${model.family ?? ""} ${model.name}`.toLowerCase(),
              ),
            })),
...
export const node = makeLocationNode({
  service: Service,
  layer,
  deps: [EventV2.node, Policy.node, Integration.node],
});
