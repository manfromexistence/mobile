...
export class CodeRequiredError extends Schema.TaggedErrorClass<CodeRequiredError>()(
      ...
      draft: (draft) => ({
        ...
        method: {
          ...
          update: (implementation) => {
            ...
            if (index === -1)
              current.methods.push(implementation.method as Types.DeepMutable<Method>);
    ...
    const settle = Effect.fnUntraced(function* (
      ...
      if (Exit.isSuccess(exit)) {
        const implementation = state
          .get()
          .integrations.get(result.integrationID)
          ?.implementations.get(result.methodID);
    ...
    return Service.of({
      ...
      connection: {
        ...
        resolve: Effect.fn("Integration.connection.resolve")(function* (connection) {
          ...
          const now = yield* Clock.currentTimeMillis;
          if (credential.value.expires > now + Duration.toMillis(Duration.minutes(5)))
        ...
        update: Effect.fn("Integration.connection.update")(function* (credentialID, updates) {
          ...
          if (credential) {
            yield* events.publish(Event.ConnectionUpdated, {
      ...
      attempt: {
        status: Effect.fn("Integration.attempt.status")(function* (attemptID) {
          ...
          if (attempt.status === "failed") {
            return {
              ...
              message: attempt.message ?? "Authorization failed",
              time: attempt.time,
            };
          }
        ...
        complete: Effect.fn("Integration.attempt.complete")(function* (input) {
          const attempt = yield* SynchronizedRef.modify(attempts, (current) => {
            ...
            if (match.authorization.mode === "code" && input.code === undefined)
          ...
          if (attempt.completing)
            return yield* Effect.die(`OAuth attempt already completing: ${input.attemptID}`);
          const callback =
