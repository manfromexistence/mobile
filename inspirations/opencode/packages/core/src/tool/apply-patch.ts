...
export const toModelOutput = (output: Output) =>
  [
    ...
    ...output.applied.map(
      (item) =>
        `${item.type === "add" ? "A" : item.type === "delete" ? "D" : "M"} ${item.resource}`,
    ),
  ].join("\n");
            ...
            execute: (input, context) => {
              ...
              return Effect.gen(function* () {
                ...
                if (!input.patchText.trim())
                  return yield* new ToolFailure({ message: "patchText is required" });
                const hunks = yield* Effect.try({
                  try: () => Patch.parse(input.patchText),
                  catch: (cause) =>
                    new ToolFailure({
                      message: `apply_patch verification failed: ${String(cause)}`,
                    }),
                });
                if (hunks.length === 0)
                  return yield* new ToolFailure({ message: "patch rejected: empty patch" });
                const move = hunks.find(
                  (hunk) => hunk.type === "update" && hunk.movePath !== undefined,
                );
                if (move)
                  return yield* new ToolFailure({
                    message: "apply_patch moves are not supported yet",
                  });
                ...
                const externalDirectories = new Map<
                  string,
                  LocationMutation.ExternalDirectoryAuthorization
                >();
