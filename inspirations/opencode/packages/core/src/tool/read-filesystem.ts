...
export class BinaryFileError extends Schema.TaggedErrorClass<BinaryFileError>()(
...
export class MalformedUtf8Error extends Schema.TaggedErrorClass<MalformedUtf8Error>()(
  "ReadTool.MalformedUtf8Error",
  {
    resource: Schema.String,
  },
) {
...
export class PathKindError extends Schema.TaggedErrorClass<PathKindError>()(
...
const decodeChunk = (resource: string, decoder: TextDecoder, bytes: Uint8Array) =>
  bytes.includes(0)
    ? Effect.fail(new BinaryFileError({ resource }))
    : decodeUtf8(resource, decoder, bytes);
...
export const inspect = Effect.fn("ReadTool.inspect")(function* (
  ...
  if (!type)
    return yield* Effect.fail(
      new PathKindError({ resource: input, expected: "a file or directory" }),
    );
  return type;
});
...
export const read = Effect.fn("ReadTool.read")(function* (
  ...
  return yield* Effect.scoped(
    Effect.gen(function* () {
      ...
      if (info.type !== "File")
        return yield* Effect.fail(new PathKindError({ resource, expected: "a file" }));
      ...
      if (mime) {
        if (info.size > MAX_MEDIA_INGEST_BYTES)
          return yield* Effect.fail(
            new MediaIngestLimitError({ resource, maximumBytes: MAX_MEDIA_INGEST_BYTES }),
          );
        const chunks = [first];
        let total = first.length;
        while (total <= MAX_MEDIA_INGEST_BYTES) {
          const chunk = yield* file.readAlloc(
            Math.min(64 * 1024, MAX_MEDIA_INGEST_BYTES + 1 - total),
          );
      ...
      if (
        startsWith(first, [0x25, 0x50, 0x44, 0x46]) ||
        extensions.has(path.extname(resource).toLowerCase())
      )
        return yield* Effect.fail(new BinaryFileError({ resource }));
      const paged =
        info.size > MAX_READ_BYTES || page.offset !== undefined || page.limit !== undefined;
      if (!paged) {
      ...
      const append = (input: string) => {
        ...
        const text =
          input.length > MAX_LINE_LENGTH
            ? input.slice(0, MAX_LINE_LENGTH) + MAX_LINE_SUFFIX
            : input;
  ...
  const entries = yield* Effect.forEach(
    ...
    (item) =>
      Effect.gen(function* () {
        ...
        const type =
          info?.type === "Directory" ? "directory" : info?.type === "File" ? "file" : undefined;
        if (!type) return;
