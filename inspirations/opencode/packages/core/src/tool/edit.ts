...
const convertToLineEnding = (text: string, ending: "\n" | "\r\n") =>
  ending === "\n"
    ...
    : normalizeLineEndings(text).replaceAll("\n", "\r\n");
...
const previewLines = (value: string, prefix: "+" | "-") => {
  ...
  const shown = lines
    .slice(0, 6)
    .map((line) => `${prefix}${line.length > 240 ? `${line.slice(0, 240)}...` : line}`);
          ...
          Tool.make({
            ...
            execute: (input, context) => {
              ...
              return Effect.gen(function* () {
                ...
                const target = yield* unableToEdit(
                  mutation.resolve({ path: input.path, kind: "file" }),
                );
