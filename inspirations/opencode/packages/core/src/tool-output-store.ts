...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const limits = Effect.fn("ToolOutputStore.limits")(function* () {
      ...
      const entries = yield* config.value
        .entries()
        .pipe(Effect.catch(() => Effect.succeed([] as Config.Entry[])));
    ...
    const bound = Effect.fn("ToolOutputStore.bound")(function* (input: BoundInput) {
      ...
      const contextual =
        input.output.content.length === 0
          ? yield* Effect.try({
              try: () =>
                JSON.stringify(input.output.structured, null, 2) ?? String(input.output.structured),
...
export const cleanupLayer = Layer.effectDiscard(
  Effect.gen(function* () {
    ...
    yield* store
      .cleanup()
      .pipe(Effect.repeat(Schedule.spaced(Duration.hours(1))), Effect.forkScoped);
  }),
);
