...
export interface Strategy {
  ...
  readonly list: (
    directory: AbsolutePath,
  ) => Effect.Effect<ListEntry[], Git.WorktreeError | DirectoryUnavailableError>;
}
  ...
  Effect.gen(function* () {
    ...
    const canonical = Effect.fnUntraced(function* (input: AbsolutePath) {
      ...
      if (!(yield* fs.isDir(resolved)))
        return yield* new DirectoryUnavailableError({ directory: input });
      return resolved;
    });
    ...
    const register = Effect.fn("ProjectCopy.register")(function* (strategy: Strategy) {
      if (registry.has(strategy.id))
        return yield* new DuplicateStrategyError({ strategy: strategy.id });
    ...
    const remove = Effect.fn("ProjectCopy.remove")(function* (input: RemoveInput) {
      ...
      const stored = yield* directories.get({
        projectID: input.projectID,
        directory: copyDirectory,
      });
    ...
    const refresh = Effect.fn("ProjectCopy.refresh")(function* (input: RefreshInput) {
      ...
      const changes = {
        updated: discovered
          .filter((_, index) => result.updated[index])
          .map((item) => item.directory),
