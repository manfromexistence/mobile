...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const create = Effect.fn("ProjectDirectories.create")(function* (
      input: CreateInput,
      tx?: Transaction,
    ) {
      const insert = (tx ?? db)
        ...
        input.behavior === "replace"
          ? insert.onConflictDoUpdate({
              ...
              setWhere: input.strategy
                ? or(
                    isNull(ProjectDirectoryTable.strategy),
                    ne(ProjectDirectoryTable.strategy, input.strategy),
                  )
    ...
    const remove = Effect.fn("ProjectDirectories.remove")(function* (
      input: RemoveInput,
      tx?: Transaction,
    ) {
      return (
