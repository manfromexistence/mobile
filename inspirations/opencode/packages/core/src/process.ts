...
export class AppProcessError extends Schema.TaggedErrorClass<AppProcessError>()("AppProcessError", {
  ...
  override get message() {
    const detail =
      this.stderr?.trim() ||
      (this.cause instanceof Error ? this.cause.message : this.cause && String(this.cause));
    ...
    const runCommand = (command: ChildProcess.Command, options?: RunOptions) => {
      ...
      const timed = options?.timeout
        ? Effect.timeoutOrElse(collect, {
            ...
            orElse: () =>
              Effect.fail(
                new AppProcessError({ command: description, cause: new Error("Timed out") }),
              ),
          })
        : collect;
          ...
          const tail = Stream.unwrap(
            Effect.gen(function* () {
              ...
              if (okExitCodes && okExitCodes.length > 0 && !okExitCodes.includes(code)) {
                ...
                return Stream.fail(
                  new AppProcessError({ command: description, exitCode: code, stderr }),
                );
              }
              return Stream.empty;
            }),
