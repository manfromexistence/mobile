...
export type Output<Item> = [Item] extends [never]
  ? never
  : Item extends Node<infer A, unknown, any>
    ? A
    : never;
export type Error<Item> = [Item] extends [never]
  ? never
  : Item extends Node<unknown, infer E, any>
    ? E
    : never;
type NodeTag<Item> = [Item] extends [never]
  ? undefined
  : Item extends Node<unknown, unknown, infer T>
    ? T
    : never;
...
type CheckReplacementErrors<SourceError, ReplacementError> = [
  Exclude<ReplacementError, SourceError>,
] extends [never]
  ? unknown
  : { readonly "New replacement errors": Exclude<ReplacementError, SourceError> };

type CheckReplacement<Item> = Item extends readonly [
  Node<infer A, infer E, infer T>,
  infer Replacement,
]
...
function rewriteReplacementDependencies(root: AnyNode, replacements: ReadonlyMap<string, AnyNode>) {
  ...
  const recur = (node: AnyNode, isRoot = false): AnyNode => {
    ...
    try {
      ...
      const result = dependencies.every(
        (dependency, index) => dependency === target.dependencies[index],
      )
        ? target
        : { ...target, dependencies };
