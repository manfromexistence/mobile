...
export class CloneFailedError extends Schema.TaggedErrorClass<CloneFailedError>()(
  "RepositoryCacheCloneFailedError",
  {
    repository: Schema.String,
    message: Schema.String,
  },
...
export class FetchFailedError extends Schema.TaggedErrorClass<FetchFailedError>()(
  "RepositoryCacheFetchFailedError",
  {
    repository: Schema.String,
    message: Schema.String,
  },
...
export class ResetFailedError extends Schema.TaggedErrorClass<ResetFailedError>()(
  "RepositoryCacheResetFailedError",
  {
    repository: Schema.String,
    message: Schema.String,
  },
...
export class LockFailedError extends Schema.TaggedErrorClass<LockFailedError>()(
  "RepositoryCacheLockFailedError",
  {
...
export const validateBranch = Effect.fn("RepositoryCache.validateBranch")(function* (
  branch: string,
) {
  return yield* Effect.try({
...
const layer: Layer.Layer<
  Service,
  never,
  FSUtil.Service | Git.Service | EffectFlock.Service | Global.Service
> = Layer.effect(
  Service,
  Effect.gen(function* () {
        ...
        return yield* flock
          .withLock(
            Effect.gen(function* () {
              ...
              const reuse = Boolean(
                existing && originReference && Repository.same(originReference, cloneTarget),
              );
              if (exists && !reuse) {
                yield* cacheOperation(
                  fs.remove(localPath, { recursive: true }),
                  "remove stale cache",
                  localPath,
                );
              }
              ...
              const currentBranch =
                reuse && existing ? yield* git.history.branch(existing) : undefined;
              ...
              if (status === "cloned") {
                yield* git.repo
                  ...
                  .pipe(
                    Effect.mapError(
                      (error) => new CloneFailedError({ repository, message: error.message }),
                    ),
                  );
              }
              ...
              if (status === "refreshed") {
                ...
                yield* git.sync
                  ...
                  .pipe(
                    Effect.mapError(
                      (error) => new FetchFailedError({ repository, message: error.message }),
                    ),
                  );
                ...
                yield* git.sync
                  ...
                  .pipe(
                    Effect.mapError(
                      (error) => new ResetFailedError({ repository, message: error.message }),
                    ),
                  );
              }
          ...
          .pipe(
            Effect.mapError((error) =>
              isError(error)
                ? error
                : new LockFailedError({ localPath, message: errorMessage(error) }),
            ),
          );
      }),
    });
...
function cacheOperation<A, E, R>(
  ...
  return effect.pipe(
    Effect.mapError(
      (error) => new CacheOperationError({ operation, path: target, message: errorMessage(error) }),
    ),
  );
}
