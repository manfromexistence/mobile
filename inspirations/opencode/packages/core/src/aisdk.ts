...
function prepareOptions(model: ModelV2.Info, pkg: string) {
  ...
  options.fetch = async (input: Parameters<typeof fetch>[0], init?: RequestInit) => {
    ...
    const chunkAbortCtl = signals.find(
      (item): item is AbortController => item instanceof AbortController,
    );
    const abortSignals = signals.map((item) =>
      item instanceof AbortController ? item.signal : item,
    );
...
function initError(providerID: ProviderV2.ID) {
  return Effect.catchCause((cause) =>
    Effect.fail(new InitError({ providerID, cause: Cause.squash(cause) })),
  );
}
    ...
    const service = Service.of({
      ...
      language: Effect.fn("AISDK.language")(function* (model) {
        ...
        const sdk =
          sdks.get(sdkKey) ??
          (yield* service
            .runSDK({ model, package: model.api.package, options })
        ...
        const result = yield* service
          .runLanguage({ model, sdk, options })
          .pipe(initError(model.providerID));
        const language = yield* Effect.sync(
          () => result.language ?? sdk.languageModel(model.api.id),
        ).pipe(initError(model.providerID));
