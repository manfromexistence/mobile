...
export interface Interface {
  ...
  readonly revert: {
    ...
    readonly clear: (
      sessionID: SessionSchema.ID,
    ) => Effect.Effect<void, NotFoundError | Snapshot.Error>;
  ...
  Effect.gen(function* () {
    ...
    const result = Service.of({
      ...
      list: Effect.fn("V2Session.list")(function* (input = {}) {
        ...
        const order =
          direction === "previous" ? (requestedOrder === "asc" ? "desc" : "asc") : requestedOrder;
        ...
        const rows = yield* (
          input.limit === undefined ? query.all() : query.limit(input.limit).all()
        ).pipe(Effect.orDie);
      ...
      events: (input) =>
        Stream.unwrap(
          ...
          Stream.filter((event): event is SessionEvent.DurableEvent =>
  ...
  Prompt.make({
    ...
    files: input.files?.map((file) => {
      ...
      return {
        ...file,
        mime:
          dataMime ?? (target.endsWith("/") ? "application/x-directory" : FSUtil.mimeType(target)),
      };
    }),
  });
