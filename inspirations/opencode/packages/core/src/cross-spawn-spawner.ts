...
export const make = Effect.gen(function* () {
  ...
  ): NodeChildProcess.IOType | undefined => (Sink.isSink(x) ? "pipe" : x);
  ...
  const killOne = (
    ...
    Effect.suspend(() => {
      ...
      return Effect.fail(
        toPlatformError("kill", new Error("Failed to kill child process"), command),
      );
    });
...
const layer: Layer.Layer<ChildProcessSpawner, never, FileSystem.FileSystem | Path.Path> =
