...
export interface Interface {
  ...
  readonly change: {
    readonly capture: (input: { repository: Repository; path: AbsolutePath }) => Effect.Effect<
      ChangeSet,
      PatchError
    >;
  ...
  readonly tree: {
    ...
    readonly checkout: (input: { repository: Repository; tree: TreeID }) => Effect.Effect<
      void,
      OperationError
    >;
  };
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const remoteHead = Effect.fn("Git.history.defaultRemoteBranch")(function* (
      ...
      if (result.exitCode !== 0) return undefined;
      return (
        result.text.trim().replace(new RegExp(`^refs/remotes/${remoteName}/`), "") || undefined
      );
    });
    ...
    const operation = Effect.fnUntraced(function* (
      ...
      const result = yield* execute(
        ...
        Effect.mapError(
          (cause) => new OperationError({ operation, directory, message: cause.message, cause }),
        ),
      );
      if (result.exitCode === 0) return;
    ...
    const fetch = Effect.fn("Git.sync.fetchRemotes")(function* (
      ...
      yield* operation("fetch", repository.worktree, [
        "fetch",
        "--all",
    ...
    const checkout = Effect.fn("Git.sync.checkoutRemoteBranch")(function* (
      ...
      yield* operation("checkout", repository.worktree, [
        ...
        ...(input.reset === false
          ? [input.branch]
          : ["-B", input.branch, `${remoteName}/${input.branch}`]),
      ]);
    });
    ...
    const repositoryOperation = Effect.fnUntraced(function* (
      ...
      return yield* new OperationError({
        ...
        directory: repository.worktree,
        message:
          result.stderr.toString("utf8").trim() || text.trim() || `Git ${operationName} failed`,
      });
    });
    ...
    const create = Effect.fn("Git.repo.create")(function* (input: {
      ...
      yield* fs
        .copyFile(
          path.join(input.seed.gitDirectory, "index"),
          path.join(input.gitDirectory, "index"),
        )
    ...
    const writeTree = Effect.fn("Git.tree.write")(function* (repository: Repository) {
      return TreeID.make(
        (yield* repositoryOperation("write_tree", repository, ["write-tree"])).text.trim(),
      );
    });
    ...
    const captureTree = Effect.fn("Git.tree.capture")(
      (input: {
        ...
        locked(
          input.repository,
          Effect.gen(function* () {
            yield* Effect.forEach(input.scopes, (scope) => refresh({ ...input, scope }), {
              discard: true,
            });
    ...
    const treeDiff = Effect.fn("Git.tree.diff")(function* (input: {
      ...
      return yield* Effect.forEach(paths, (file) =>
        Effect.gen(function* () {
          ...
          const status = statusText.startsWith("A")
            ? "added"
            : statusText.startsWith("D")
              ? "deleted"
              : "modified";
        ...
        locked(
          ...
          Effect.gen(function* () {
            ...
            return yield* Effect.gen(function* () {
              yield* repositoryOperation("diff", input.repository, ["read-tree", input.current], {
                env,
              });
              yield* Effect.forEach(
                input.files,
              ...
              const target = TreeID.make(
                (yield* repositoryOperation("diff", input.repository, ["write-tree"], {
                  env,
                })).text.trim(),
              );
          ...
          Effect.forEach(
            ...
            ([file, tree]) =>
              Effect.gen(function* () {
                ...
                yield* fs
                  .remove(path.join(input.repository.worktree, file), {
                    recursive: true,
                    force: true,
                  })
                  .pipe(
    ...
    const checkoutTree = Effect.fn("Git.tree.checkout")(
      (input: { repository: Repository; tree: TreeID }) =>
        locked(
          input.repository,
          Effect.gen(function* () {
    ...
    const capture = Effect.fn("Git.change.capture")(function* (input: {
      ...
      const scope =
        path.relative(input.repository.worktree, input.path).replaceAll("\\", "/") || ".";
      ...
      if (tracked.exitCode !== 0) {
        return yield* new PatchError({
          ...
          directory: input.path,
          message:
            tracked.stderr.trim() || tracked.text.trim() || "Failed to capture tracked changes",
        });
      }
      ...
      if (untracked.exitCode !== 0) {
        return yield* new PatchError({
          ...
          directory: input.path,
          message:
            untracked.stderr.trim() || untracked.text.trim() || "Failed to list untracked changes",
        });
      }
              ...
              : Effect.fail(
                  new PatchError({
                    ...
                    message:
                      result.stderr.trim() ||
                      result.text.trim() ||
                      `Failed to capture untracked change: ${file}`,
                  }),
                ),
          ),
        ),
    ...
    const discard = Effect.fn("Git.change.discard")(function* (input: {
      ...
      if (restore.exitCode !== 0) {
        return yield* new PatchError({
          ...
          directory: input.path,
          message:
            restore.stderr.trim() || restore.text.trim() || "Failed to restore tracked changes",
        });
      }
    ...
    const worktreeRun = Effect.fnUntraced(function* (
      ...
      return yield* new WorktreeError({
        ...
        message,
        forceRequired:
          operation === "remove" && /contains modified or untracked files|is dirty/i.test(message),
      });
    });
          ...
          (line, index) =>
            new Worktree({
              directory: AbsolutePath.make(
                resolvePath(repository.worktree, line.slice("worktree ".length).trim()),
              ),
...
function run(cwd: string, proc: AppProcess.Interface) {
  return (args: string[]) =>
    execute(
      cwd,
      proc,
    )(args).pipe(Effect.catch(() => Effect.succeed({ exitCode: 1, text: "", stderr: "" })));
}
