  ...
  effect: Effect.fn(function* (ctx) {
    yield* ctx.aisdk.sdk(
      Effect.fn(function* (evt) {
        ...
        const { createAiGateway } = yield* Effect.promise(() => import("ai-gateway-provider")).pipe(
          Effect.orDie,
        );
...
function gatewayConfig(options: Record<string, unknown>): GatewayConfig | undefined {
  ...
  const apiKey =
    process.env.CLOUDFLARE_API_TOKEN ?? process.env.CF_AIG_TOKEN ?? stringOption(options, "apiKey");
