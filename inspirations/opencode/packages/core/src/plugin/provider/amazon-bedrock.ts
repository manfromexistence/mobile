...
function resolveModelID(modelID: string, region: string | undefined) {
  ...
  if (regionPrefix === "eu") {
    ...
    const modelRequiresPrefix = ["claude", "nova-lite", "nova-micro", "llama3", "pixtral"].some(
  ...
  if (
    australia &&
    ["anthropic.claude-sonnet-4-5", "anthropic.claude-haiku"].some((item) => modelID.includes(item))
  ) {
    return `au.${modelID}`;
  }
...
export const AmazonBedrockPlugin = define({
  ...
  effect: Effect.fn(function* (ctx) {
    ...
    yield* ctx.aisdk.sdk(
      Effect.fn(function* (evt) {
        if (!["@ai-sdk/amazon-bedrock", "@ai-sdk/amazon-bedrock/mantle"].includes(evt.package))
          return;
        const options = { ...evt.options };
        const profile =
          typeof options.profile === "string" ? options.profile : process.env.AWS_PROFILE;
        const region =
    ...
    yield* ctx.aisdk.language(
      Effect.fn(function* (evt) {
        ...
        if (
          evt.model.api.type === "aisdk" &&
          evt.model.api.package === "@ai-sdk/amazon-bedrock/mantle"
        ) {
          evt.language = selectMantleModel(evt.sdk, evt.model.api.id);
          return;
        }
        const region =
          typeof evt.options.region === "string" ? evt.options.region : process.env.AWS_REGION;
