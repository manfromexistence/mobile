      ...
      Effect.gen(function* () {
        ...
        const transform =
          ...
          (callback: (draft: Draft) => Promise<void> | void) =>
            register(
              domain.transform((draft) => Effect.promise(() => Promise.resolve(callback(draft)))),
            );
        ...
        const context2: PluginContext = {
          ...
          aisdk: {
            sdk: (callback) =>
              register(
                host.aisdk.sdk((event) => Effect.promise(() => Promise.resolve(callback(event)))),
              ),
            language: (callback) =>
              register(
                host.aisdk.language((event) =>
          ...
          integration: {
            ...
            connection: {
              active: (id) =>
                Effect.runPromiseWith(context)(host.integration.connection.active(id)),
              resolve: (connection) =>
