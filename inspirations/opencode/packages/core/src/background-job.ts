...
export const make = Effect.gen(function* () {
        ...
        const result = yield* SynchronizedRef.modifyEffect(
          ...
          Effect.fnUntraced(function* (jobs) {
            ...
            if (existing?.info.status === "running") {
              return [{ info: snapshot(existing) }, jobs] as readonly [
                StartResult,
                Map<string, Active>,
              ];
            }
  ...
  const waitForPromotion: Interface["waitForPromotion"] = Effect.fn(
    "BackgroundJob.waitForPromotion",
  )(function* (id) {
    const job = (yield* SynchronizedRef.get(state.jobs)).get(id);
  ...
  const promote: Interface["promote"] = Effect.fn("BackgroundJob.promote")(function* (id) {
    ...
    if (result.info && result.promoted)
      yield* Deferred.succeed(result.promoted, result.info).pipe(Effect.ignore);
