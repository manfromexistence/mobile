...
export type Subscriber<D extends Definition = Definition> = (
  event: Payload<D>,
) => Effect.Effect<void>;
...
export const allBounded = (events: Interface, capacity: number) =>
  Effect.gen(function* () {
    ...
    yield* Effect.addFinalizer(() =>
      unsubscribe.pipe(Effect.andThen(Queue.shutdown(queue)), Effect.asVoid),
    );
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const observe = (event: Payload, observer: (event: Payload) => Effect.Effect<void>) =>
        Effect.suspend(() => observer(event)).pipe(
          Effect.catchCauseIf(
            ...
            (cause) =>
              Effect.logError("Event listener failed", {
                eventID: event.id,
      ...
      const subscribe = <D extends Definition>(definition: D): Stream.Stream<Payload<D>> =>
        ...
        ).pipe(Stream.map((event) => event as Payload<D>));
