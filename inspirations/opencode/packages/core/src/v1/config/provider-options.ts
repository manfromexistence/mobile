...
const openai: Lowerer = {
  ...
  request(options) {
    ...
    if (options.textVerbosity !== undefined) {
      result.text = {
        ...(isRecord(result.text) ? result.text : {}),
...
const anthropic: Lowerer = {
  ...
  request(options) {
    ...
    if (options.effort !== undefined || options.taskBudget !== undefined) {
      result.output_config = compactUnknown({
        effort: options.effort,
        task_budget: options.taskBudget,
      });
...
const google: Lowerer = {
  ...
  request(options) {
    const generationConfig = pick(options, [
      "thinkingConfig",
      "responseModalities",
      "mediaResolution",
      "imageConfig",
...
function snake(options: Options) {
  return Object.fromEntries(
    Object.entries(options).map(([key, value]) => [snakeKey(key), snakeValue(value)]),
  );
}
...
function snakeValue(value: unknown): unknown {
  ...
  return Object.fromEntries(
    Object.entries(value).map(([key, value]) => [snakeKey(key), snakeValue(value)]),
  );
}
