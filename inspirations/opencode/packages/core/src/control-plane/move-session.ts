...
export class ApplyChangesError extends Schema.TaggedErrorClass<ApplyChangesError>()(
...
export class Service extends Context.Service<Service, Interface>()(
  "@opencode/ControlPlaneMoveSession",
) {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const moveSession = Effect.fn("MoveSession.moveSession")(function* (input: Input) {
      ...
      const sourceRepository = moveChanges
        ? yield* git.repo.discover(current.location.directory)
        : undefined;
      ...
      if (patch) {
        ...
        if (!repository)
          return yield* new ApplyChangesError({ message: "Destination is not a Git repository" });
        yield* git.change
      ...
      yield* events.publish(SessionEvent.Moved, {
        ...
        subdirectory: RelativePath.make(
          path.relative(destination.directory, directory).replaceAll("\\", "/"),
        ),
