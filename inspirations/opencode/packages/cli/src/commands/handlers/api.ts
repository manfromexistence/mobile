...
export default Runtime.handler(
  ...
  Effect.fn("cli.api")(function* (input) {
    ...
    for (const header of input.header) {
      ...
      if (index < 1)
        return yield* Effect.fail(new Error(`Invalid header, expected name:value: ${header}`));
...
export function rawRequest(input: readonly string[]) {
  if (input.length !== 2 || !methods.has(input[0].toLowerCase()) || !input[1].startsWith("/"))
    return;
...
function resolveRequest(
  ...
  if (input.length !== 1)
    return Effect.fail(new Error("Expected an operation name or an HTTP method and path"));
  return Effect.tryPromise(async () => {
    const response = await fetch(new URL("/openapi.json", transport.url), {
