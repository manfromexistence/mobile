...
export type Input<Value> = Value extends Spec.Node<infer _Name, infer Command, infer _Commands>
  ? Input<Command>
...
export type Handlers<Node extends Spec.Any> = keyof Node["commands"] extends never
  ...
  : { readonly $?: Loader<Node> } & {
      readonly [Key in keyof Node["commands"]]: Handlers<Node["commands"][Key]>;
    };
...
export function run(
  ...
  return Command.run(provide(commands, handlers), options) as Effect.Effect<
    void,
    unknown,
    Command.Environment
  >;
