...
describe("H2: string property access reads as undefined (not a throw)", () => {
  ...
  test("the real MCP pattern: result is a JSON string, defensive read falls through", async () => {
    ...
    expect(
      await value(`const me = { result: '{"login":"x"}' }; return me.result?.login ?? me.result`),
    ).toBe('{"login":"x"}');
  });
...
describe("H6: object spread of null/undefined is a no-op", () => {
  ...
  test("spreading an absent argument merges cleanly", async () => {
    expect(await value(`function f(opts){ return { ...opts, a: 1 } } return f(undefined)`)).toEqual(
      { a: 1 },
    );
  });
...
describe("H1: NaN/Infinity flow as intermediates and normalize to null at the boundary", () => {
  test("guards run instead of the program crashing on a transient NaN", async () => {
    ...
    expect(await value(`const a = []; return a.length ? a.reduce((s,x)=>s+x,0)/a.length : 0`)).toBe(
      0,
    );
  });
  ...
  test("a non-finite value becomes null when it leaves the sandbox", async () => {
    ...
    expect(await value(`return { a: Number("x"), b: 2, c: [1/0] }`)).toEqual({
      a: null,
      b: 2,
      c: [null],
    });
...
describe("Error values and instanceof", () => {
  test("new Error carries name/message and is instanceof Error", async () => {
    expect(
      await value(`const e = new Error("boom"); return [e instanceof Error, e.name, e.message]`),
    ).toEqual([true, "Error", "boom"]);
  });

  test("Error without new behaves like new Error", async () => {
    expect(
      await value(`const e = Error("plain"); return [e instanceof Error, e.name, e.message]`),
    ).toEqual([true, "Error", "plain"]);
    expect(
      await value(`const e = new Error(); return [e.name, e.message, e instanceof Error]`),
    ).toEqual(["Error", "", true]);
  });
  ...
  test("interpreter runtime failures are caught as Error values", async () => {
    expect(await value(`try { JSON.parse("nope") } catch (e) { return e instanceof Error }`)).toBe(
      true,
    );
  ...
  test("caught failures carry the constructor name the real-JS failure would have", async () => {
    ...
    expect(
      await value(`try { "a".match("(") } catch (e) { return [e.name, e instanceof SyntaxError] }`),
  ...
  test("diagnostics without a specific real-JS analogue are named plain Error", async () => {
    expect(
      await value(`try { JSON.parse(5) } catch (e) { return [e.name, e instanceof Error] }`),
    ).toEqual(["Error", true]);
  });
  ...
  test("non-error thrown values are not instanceof Error", async () => {
    ...
    expect(
      await value(`try { throw { message: "shaped" } } catch (e) { return e instanceof Error }`),
    ).toBe(false);
  });

  test("plain data is never instanceof Error", async () => {
    expect(
      await value(`return [({}) instanceof Error, "s" instanceof Error, null instanceof Error]`),
  ...
  test("error values still serialize as plain { name, message } data", async () => {
    ...
    expect(await value(`try { throw new Error("m") } catch (e) { return Object.keys(e) }`)).toEqual(
      ["name", "message"],
    );
  });
  ...
  test("typeof Error is function; an unknown instanceof right-hand side is a catchable error", async () => {
    ...
    expect(await value(`try { return 1 instanceof 5 } catch (e) { return "caught" }`)).toBe(
      "caught",
    );
...
describe("array methods: splice, fill, copyWithin, keys/values/entries", () => {
  test("splice removes in place and returns the removed elements", async () => {
    expect(
      await value(`const a = [1,2,3,4]; const removed = a.splice(1, 2); return { removed, a }`),
    ).toEqual({
      removed: [2, 3],
      a: [1, 4],
    });
  ...
  test("splice inserts new elements at the cut", async () => {
    expect(await value(`const a = ["a","d"]; a.splice(1, 0, "b", "c"); return a`)).toEqual([
      "a",
      "b",
      "c",
      "d",
    ...
    expect(
      await value(`const a = [1,2,3]; const removed = a.splice(1, 1, "x"); return { removed, a }`),
    ).toEqual({
      removed: [2],
      a: [1, "x", 3],
    });
  ...
  test("splice with one argument removes to the end; negative start counts back", async () => {
    expect(
      await value(`const a = [1,2,3]; const removed = a.splice(1); return { removed, a }`),
    ).toEqual({
      removed: [2, 3],
      a: [1],
    });
...
describe("string methods: localeCompare, normalize, trim aliases", () => {
  test("localeCompare orders strings for sorting", async () => {
    expect(await value(`return ["b","a","c"].sort((x, y) => x.localeCompare(y))`)).toEqual([
      "a",
      "b",
      "c",
    ]);
  ...
  test("an invalid normalize form is a clear catchable error", async () => {
    expect(
      await value(`try { "x".normalize("nope"); return "no" } catch (e) { return e.message }`),
    ).toContain('"NFC"');
  });
...
describe("compound assignment matches its binary operator", () => {
  ...
  test("string += object/array matches x = x + obj", async () => {
    expect(
      await pair(`let x = "a"; x += { b: 1 }; return x`, `let x = "a"; x = x + { b: 1 }; return x`),
    ).toBe("a[object Object]");
    expect(
      await pair(`let x = "a"; x += [1, 2]; return x`, `let x = "a"; x = x + [1, 2]; return x`),
    ).toBe("a1,2");
  });
