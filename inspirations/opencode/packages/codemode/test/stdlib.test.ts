...
describe("Date", () => {
  ...
  test("string parsing round-trips", async () => {
    expect(await value(`return new Date("2024-01-02T03:04:05.000Z").getTime()`)).toBe(
      1704164645000,
    );
  ...
  test("date arithmetic and comparison use the time value", async () => {
    expect(await value(`const a = new Date(1000); const b = new Date(3000); return b - a`)).toBe(
      2000,
    );
    expect(await value(`const a = new Date(1000); const b = new Date(3000); return a < b`)).toBe(
      true,
    );
  ...
  test("dates serialize to ISO strings at the boundary, direct and nested", async () => {
    ...
    expect(await value(`return JSON.stringify({ d: new Date(0) })`)).toBe(
...
describe("RegExp", () => {
  ...
  test("string match: non-global carries index, global lists all matches", async () => {
    expect(await value(`const m = "a1b22".match(/\\d+/); return [m[0], m.index]`)).toEqual([
      "1",
      1,
    ]);
  ...
  test("replaceAll without the g flag is a catchable error", async () => {
    expect(
      await value(`try { "a".replaceAll(/a/, "b"); return "no" } catch { return "caught" }`),
    ).toBe("caught");
  });
  ...
  test("new RegExp constructs from strings; invalid patterns are catchable", async () => {
    ...
    expect(await value(`try { new RegExp("("); return "no" } catch { return "caught" }`)).toBe(
      "caught",
    );
  ...
  test("missing g-flag errors say how to fix the call", async () => {
    expect((await error(`return "aa".replaceAll(/a/, "b")`)).message).toContain(
      "write /a/g, or use String.replace",
    );
    expect((await error(`return "aa".matchAll(/a/)`)).message).toContain(
...
describe("Map", () => {
  ...
  test("Object.fromEntries(map) and Array.from(map)", async () => {
    expect(await value(`return Object.fromEntries(new Map([["a", 1], ["b", 2]]))`)).toEqual({
      a: 1,
      b: 2,
    });
...
describe("stdlib integration", () => {
  ...
  test("instanceof recognizes the stdlib value types", async () => {
    ...
    expect(await value(`return [new Map() instanceof Set, "s" instanceof Date]`)).toEqual([
      false,
      false,
    ]);
    expect(
