...
export type CodeModeOptions<Tools extends Record<string, unknown> = {}> = Omit<
...
const promiseStatics = new Set<PromiseMethodName>([
  "all",
  "allSettled",
  "race",
  "resolve",
...
const coerceToString = (value: unknown): string => {
  ...
  if (typeof value === "object") {
    return Array.isArray(value)
      ? value
          .map((item) => (item === null || item === undefined ? "" : coerceToString(item)))
          .join(",")
      : "[object Object]";
  }
  return String(value);
  ...
  switch (name) {
    ...
    case "fromEntries": {
      ...
      if (args[0] instanceof SandboxMap) {
        const out: Record<string, unknown> = Object.create(null);
        for (const [key, item] of (args[0] as SandboxMap).map.entries())
  ...
  private formatConsoleValue(value: unknown, seen: Set<object>, depth: number): string {
    ...
    try {
      ...
      return `{${Object.entries(value)
        .map(
          ([key, item]) =>
            `${JSON.stringify(key)}:${this.formatConsoleValue(item, seen, depth + 1)}`,
        )
        .join(",")}}`;
    } finally {
      seen.delete(value);
...
export const make = <const Tools extends Record<string, unknown> = {}>(
  ...
  const executeProgram = (code: string) =>
    executeWithLimits<Tools>({ ...options, code }, limits, discovery.searchIndex);
