...
const renderSchema = (
  ...
  if (Array.isArray(schema.type)) {
    return schema.type
      .map((item) => renderSchema({ type: item }, ctx, depth + 1, seen))
      .join(" | ");
  }
  ...
  if (schema.type === "array")
    return `Array<${renderSchema(schema.items ?? {}, ctx, depth + 1, seen)}>`;
  if (schema.type === "object" || schema.properties) {
    ...
    const indexType =
      additional && typeof additional === "object"
        ? renderSchema(additional, ctx, depth + 1, seen)
        : undefined;
 ...
 */
export const make = <
  I extends ToolSchema,
  const O extends ToolSchema | undefined = undefined,
  R = never,
>(
