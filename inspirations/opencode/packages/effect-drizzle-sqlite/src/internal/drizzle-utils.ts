...
export function getViewSelectedFieldsRuntime(view: SQLiteViewBase) {
  return (
    view as unknown as Record<symbol, { selectedFields: Record<string, unknown>; name: string }>
  )[ViewBaseConfig];
}
...
export function orderSelectedFields<TColumn extends Column>(
  ...
  return Object.entries(fields).flatMap(([name, field]) => {
    ...
    if (is(field, Table))
      return orderSelectedFields(getTableColumnsRuntime(field as SQLiteTable), path);
    return orderSelectedFields(field as Record<string, unknown>, path);
  }) as SelectedFieldsOrdered;
}

export function mapUpdateSet<TTable extends SQLiteTable>(
  table: TTable,
  ...
  return Object.fromEntries(
    entries.map(([key, value]) => [
      ...
      is(value, SQL) || is(value, Column)
        ? value
        : new Param(value, getTableColumnsRuntime(table)[key]),
    ]),
  ) as UpdateSet;
}
  ...
  columns.forEach((column, columnIndex) => {
    const decoder = (
      is(column.field, Column)
        ...
        : is(column.field, SQL)
          ? (
              column.field as unknown as {
                decoder: { mapFromDriverValue(value: unknown): unknown };
              }
            ).decoder
          : is(column.field, Subquery)
            ? (
                column.field._.sql as unknown as {
                  decoder: { mapFromDriverValue(value: unknown): unknown };
                }
              ).decoder
            : (
                column.field.sql as unknown as {
                  decoder: { mapFromDriverValue(value: unknown): unknown };
                }
              ).decoder
    ) as {
