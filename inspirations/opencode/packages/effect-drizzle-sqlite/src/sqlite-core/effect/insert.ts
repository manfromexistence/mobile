...
export type SQLiteEffectInsertExecute<T extends AnySQLiteEffectInsert> =
...
> {
  ...
  values(
    ...
    return new SQLiteEffectInsertBase(
      this.table,
      mappedValues,
      this.session,
      this.dialect,
  ...
  select(
    ...
    if (
      !is(select, SQL) &&
      !haveSameKeys(getTableColumnsRuntime(this.table), select._.selectedFields)
    ) {
      throw new Error(
