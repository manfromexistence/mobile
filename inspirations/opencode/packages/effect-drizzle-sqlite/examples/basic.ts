...
const mapStoreError = (message: string) => (cause: unknown) =>
  new UserStoreError({ message, cause });
...
class UserStore extends Context.Service<UserStore, UserStoreShape>()(
      ...
      return UserStore.of({
        migrate: Effect.fn("UserStore.migrate")(function* () {
          yield* EffectDrizzleSqlite.migrate(db, {
            ...
            Effect.mapError(
              (cause) => new UserStoreError({ message: "Failed to migrate users", cause }),
            ),
          );
        }),
