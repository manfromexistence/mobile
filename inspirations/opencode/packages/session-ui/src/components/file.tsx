...
function createSharedVirtualStrategy(
  host: () => HTMLDivElement | undefined,
  enabled: () => boolean,
): VirtualStrategy {
...
function diffMouseSide(node: HTMLElement) {
  ...
  if (type === "change-addition" || type === "change-additions")
    return "additions" satisfies DiffSelectionSide;
...
function ViewerShell(props: {
  ...
  return (
    ...
    >
      ...
      <div
        ref={(el) => (props.viewer.overlay = el)}
...
function TextViewer<T>(props: TextFileProps<T>) {
  ...
  const applySelection = (range: SelectedLineRange | null) => {
    ...
    if (
      !root.querySelector(`[data-line="${start}"]`) ||
      !root.querySelector(`[data-line="${end}"]`)
    ) {
      current.setSelectedLines(null);
      return true;
    }
...
function DiffViewer<T>(props: DiffFileProps<T>) {
  ...
  const lineFromMouseEvent = (event: MouseEvent): MouseHit =>
    mouseHit(event, findDiffLineNumber, diffMouseSide);
  ...
  const largeOptions = {
    ...
    FileDiffOptions<T>,
    "lineDiffType" | "maxLineDiffLength" | "tokenizeMaxLineLength"
  >;
  ...
  createEffect(() => {
    ...
    const reset =
      ...
      (instanceVirtualizer !== virtualizer ||
        ...
        (virtualizer !== undefined &&
          (instanceVirtualHunkSeparators !== opts.hunkSeparators || targetChanged)));
    const forceRender =
      !reset && instance !== undefined && !areOptionsEqual(instance.options, opts);
  ...
  return (
    <ViewerShell mode="diff" viewer={viewer} class={local.class} classList={local.classList} />
  );
}
