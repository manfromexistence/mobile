...
export function sameGroups(
  a: readonly PartGroup[] | undefined,
  b: readonly PartGroup[] | undefined,
) {
  if (a === b) return true;
  if (!a || !b) return false;
  if (a.length !== b.length) return false;
  ...
  const part = createMemo(
    () =>
      new Map(
        props.messages.map(
          (message) =>
            [message.id, index(list(data.store.part?.[message.id], emptyParts))] as const,
        ),
      ),
  );
          ...
          <Index each={props.parts}>
            {(partAccessor) => {
              ...
              const running = createMemo(
                () =>
                  partAccessor().state.status === "pending" ||
                  partAccessor().state.status === "running",
              );
              return (
...
export function UserMessageDisplay(props: {
  ...
  const files = createMemo(
    () => (props.parts?.filter((p) => p.type === "file") as FilePart[]) ?? [],
  );
  ...
  const agents = createMemo(
    () => (props.parts?.filter((p) => p.type === "agent") as AgentPart[]) ?? [],
  );
...
function HighlightedText(props: { text: string; references: FilePart[]; agents: AgentPart[] }) {
  ...
  return (
    <For each={segments()}>
      {(segment) => <span data-highlight={segment.type}>{segment.text}</span>}
    </For>
  );
}
...
PART_MAPPING["tool"] = function ToolPartDisplay(props) {
  ...
  const controlledOpen = () =>
    props.onToolOpenChange ? (props.toolOpen ?? props.defaultOpen) : undefined;
        ...
        <Show when={showCopy()}>
          <div data-slot="text-part-copy-wrapper" data-interrupted={interrupted() ? "" : undefined}>
            <MessageActionButton
              ...
              onClick={handleCopy}
              aria-label={
                copied() ? i18n.t("ui.message.copied") : i18n.t("ui.message.copyResponse")
              }
            />
            <Show when={meta()}>
              <span
...
ToolRegistry.register({
  ...
  render(props) {
    ...
    const href = createMemo(() =>
      sessionLink(childSessionId(), location.pathname, data.sessionHref),
    );
          ...
          <div data-slot="basic-tool-tool-info-main">
            <Show when={running()}>
              <span
                ...
                style={{ color: tone() ?? "var(--icon-interactive-base)" }}
              >
    ...
    const fileCompProps = createMemo(() => {
      try {
        ...
        if (source) {
          ...
          if (fileDiff)
            return { fileDiff, hunkSeparators: fileDiff.isPartial ? "simple" : "line-info-basic" };
        }
      } catch {}
                            ...
                            <Show when={props.deferContent === false || visible()}>
                              <div data-component="apply-patch-file-diff">
                                <Dynamic
                                  ...
                                  hunkSeparators={
                                    file.view.fileDiff.isPartial ? "simple" : "line-info-basic"
                                  }
                                  onRendered={props.onContentRendered}
                                />
                              </div>
