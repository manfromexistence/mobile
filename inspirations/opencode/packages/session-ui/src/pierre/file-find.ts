...
function clearHighlightFind() {
  const api = (globalThis as { CSS?: { highlights?: { delete: (name: string) => void } } }).CSS
    ?.highlights;
  if (!api) return;
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const scan = (root: ShadowRoot, value: string) => {
    ...
    const cols = Array.from(
      root.querySelectorAll("[data-content] [data-line], [data-column-content]"),
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    const api = (globalThis as unknown as { CSS?: { highlights?: any }; Highlight?: any }).CSS
      ?.highlights;
  ...
  createEffect(() => {
    for (const el of overlayScroll())
      makeEventListener(el, "scroll", scheduleOverlay, { passive: true });
  });
