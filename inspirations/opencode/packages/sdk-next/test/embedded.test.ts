...
test("embedded client uses the real router and handlers", async () => {
  ...
  const { AbsolutePath, Agent, Location, Model, OpenCode, Prompt, Provider, Session, Tool } =
    await import("../src");
  const sessionID = Session.ID.make(`ses_embedded_${crypto.randomUUID()}`);
  const model = Model.Ref.make({
    id: Model.ID.make("embedded"),
  ...
  try {
    const program = Effect.gen(function* () {
      ...
      const prompted = yield* opencode.sessions.events({ sessionID }).pipe(
        Stream.filter(
          (event) => event.type === "session.next.prompted" && event.data.messageID === wake.id,
        ),
        Stream.runHead,
      ...
      const modelMessage = Option.fromNullishOr(
        context.find((message) => message.type === "model-switched"),
  ...
  try {
    const program = Effect.gen(function* () {
      ...
      yield* opencode.sessions.prompt({
        sessionID,
        prompt: Prompt.make({ text: "Observe this input" }),
      });

      const event = yield* Deferred.await(prompted).pipe(Effect.timeout("4 seconds"));
      expect(event.durable).toEqual(
        expect.objectContaining({ aggregateID: sessionID, seq: expect.any(Number) }),
      );
    });
    await Effect.runPromise(Effect.scoped(program));
  } finally {
...
test("independent embedded hosts do not share live notifications", async () => {
  ...
  try {
    const program = Effect.gen(function* () {
      ...
      const observe = (ready: Latch.Latch, event: Latch.Latch) =>
        Stream.runForEach((notification: OpenCodeEvent) =>
          notification.type === "server.connected"
            ? ready.open
            : notification.type === "session.next.agent.switched" &&
      ...
      expect(Option.isNone(yield* secondEvent.await.pipe(Effect.timeoutOption("100 millis")))).toBe(
        true,
      );
    });
    await Effect.runPromise(Effect.scoped(program));
