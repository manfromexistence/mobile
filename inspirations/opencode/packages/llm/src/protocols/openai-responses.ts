...
const encodeWebSocketMessage = Schema.encodeSync(
  Schema.fromJsonString(OpenAIResponsesWebSocketMessage),
);
...
const hostedToolItemID = (part: ToolResultPart) => {
  const openai = part.providerMetadata?.openai;
  return ProviderShared.isRecord(openai) &&
    typeof openai.itemId === "string" &&
    openai.itemId.length > 0
    ? openai.itemId
    : undefined;
...
const lowerToolResultContentItem = Effect.fn("OpenAIResponses.lowerToolResultContentItem")(
  ...
  for (const message of request.messages) {
    ...
    for (const part of message.content) {
      if (!ProviderShared.supportsContent(part, ["tool-result"]))
        return yield* ProviderShared.unsupportedContent("OpenAI Responses", "tool", [
          "tool-result",
        ]);
      input.push({
...
const openaiMetadata = (metadata: Record<string, unknown>): ProviderMetadata => ({
  openai: metadata,
});
...
const onReasoningSummaryPartAdded = (
  state: ParserState,
...
const onReasoningSummaryPartDone = (
  state: ParserState,
  event: OpenAIResponsesEvent,
): StepResult => {
  if (!event.item_id || event.summary_index === undefined) return [state, NO_EVENTS];
  ...
  if (isReasoningItem(item)) {
    ...
    if (reasoningItem) {
      const lifecycle = Object.entries(reasoningItem.summaryParts)
        ...
        .reduce(
          (lifecycle, entry) =>
            Lifecycle.reasoningEnd(lifecycle, events, `${item.id}:${entry[0]}`, providerMetadata),
          state.lifecycle,
        );
...
const decodeWebSocketMessage = ProviderShared.validateWith(
  ...
  Effect.gen(function* () {
    if (!ProviderShared.isRecord(body))
      return yield* ProviderShared.invalidRequest(
        "OpenAI Responses WebSocket body must be a JSON object",
      );
