...
type Provider = {
  ...
  readonly validate?: (
    env: Env,
  ) => Effect.Effect<string | undefined, unknown, HttpClient.HttpClient>;
};
...
const PROVIDERS: ReadonlyArray<Provider> = [
  {
    ...
    validate: (env) =>
      validateBearer("https://api.openai.com/v1/models", Redacted.make(env.OPENAI_API_KEY)),
  },
  {
    id: "anthropic",
    label: "Anthropic",
  ...
  },
  {
    ...
    validate: (env) =>
      validateBearer("https://api.groq.com/openai/v1/models", Redacted.make(env.GROQ_API_KEY)),
  },
  {
    id: "openrouter",
    label: "OpenRouter",
  ...
  },
  {
    ...
    validate: (env) =>
      validateBearer("https://api.deepseek.com/models", Redacted.make(env.DEEPSEEK_API_KEY)),
  },
  {
    id: "togetherai",
    label: "TogetherAI",
    ...
    validate: (env) =>
      validateBearer("https://api.together.xyz/v1/models", Redacted.make(env.TOGETHER_AI_API_KEY)),
  },
  {
    id: "mistral",
    label: "Mistral",
    ...
    validate: (env) =>
      validateBearer("https://api.mistral.ai/v1/models", Redacted.make(env.MISTRAL_API_KEY)),
  },
  {
    id: "perplexity",
    label: "Perplexity",
    ...
    validate: (env) =>
      validateBearer("https://api.perplexity.ai/models", Redacted.make(env.PERPLEXITY_API_KEY)),
  },
  {
    id: "venice",
    label: "Venice",
    ...
    validate: (env) =>
      validateBearer("https://api.venice.ai/api/v1/models", Redacted.make(env.VENICE_API_KEY)),
  },
  {
    id: "cerebras",
    label: "Cerebras",
    ...
    validate: (env) =>
      validateBearer("https://api.cerebras.ai/v1/models", Redacted.make(env.CEREBRAS_API_KEY)),
  },
  {
    id: "deepinfra",
    label: "DeepInfra",
    ...
    validate: (env) =>
      validateBearer(
        ...
        Redacted.make(env.DEEPINFRA_API_KEY),
      ),
  },
  {
    id: "fireworks",
    ...
    validate: (env) =>
      validateBearer(
        ...
        Redacted.make(env.FIREWORKS_API_KEY),
      ),
  },
  {
    id: "baseten",
...
const envNames = Array.from(
  new Set(PROVIDERS.flatMap((provider) => provider.vars.map((item) => item.name))),
);
...
const printStatus = (providers: ReadonlyArray<Provider>, fileEnv: Env) => {
  prompts.note(
    providers.map((provider) => statusLine(provider, fileEnv)).join("\n\n"),
...
const promptVars = (provider: Provider) =>
  provider.vars.filter((item) => !item.optional || item.secret === false);
...
const validateChat = (input: {
  ...
  ProviderShared.jsonPost({
    ...
    headers: {
      ...input.headers,
      [input.tokenHeader ?? "authorization"]: `Bearer ${Redacted.value(input.token)}`,
    },
...
const validateProvider = Effect.fn("RecordingEnv.validateProvider")(function* (
  provider: Provider,
  env: Env,
) {
...
const prompt = <A>(run: () => Promise<A | symbol>) =>
  Effect.promise(run).pipe(Effect.map(exitIfCancel));

const chooseConfigurableProviders = Effect.fn("RecordingEnv.chooseConfigurableProviders")(
  function* (providers: ReadonlyArray<Provider>, fileEnv: Env) {
    ...
    const input = {
      ...
      validate: (input: string | undefined) => {
        ...
        return !input || input.length === 0
          ? "Leave blank by pressing Esc/cancel, or paste a value"
          : undefined;
      },
    };
...
const main = Effect.fn("RecordingEnv.main")(function* () {
  ...
  if (
    interactive &&
    (yield* prompt(() =>
      prompts.confirm({ message: "Validate credentials before saving?", initialValue: true }),
    ))
  ) {
  ...
  prompts.outro(
    "Keep .env.local local. Store shared team credentials in a password manager or vault.",
  );
});

await Effect.runPromise(
  main().pipe(Effect.provide(NodeFileSystem.layer), Effect.provide(FetchHttpClient.layer)),
);
