...
const decodeLLMRequest = Schema.decodeUnknownSync(
  LLMRequest as unknown as Schema.Decoder<LLMRequest>,
);
...
describe("llm schema", () => {
  ...
  test("finish constructors accept usage input", () => {
    expect(
      LLMEvent.stepFinish({ index: 0, reason: "stop", usage: { inputTokens: 1 } }).usage,
    ).toBeInstanceOf(Usage);
    expect(LLMEvent.finish({ reason: "stop", usage: { outputTokens: 2 } }).usage).toBeInstanceOf(
      Usage,
    );
  });
