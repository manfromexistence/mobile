...
Azure.configure({
  auth: RuntimeAuth.header("api-key", "azure-key"),
...
Azure.configure({
  resourceName: "resource",
  apiKey: "azure-key",
  auth: RuntimeAuth.header("api-key", "override"),
});
...
Cloudflare.CloudflareWorkersAI.configure({ accountId: "account", apiKey: "cf-key" }).model(
  "@cf/meta/llama",
);
...
GitHubCopilot.configure({ baseURL: "https://copilot.test", apiKey: "copilot-key" }).model(
  "gpt-4.1",
);
