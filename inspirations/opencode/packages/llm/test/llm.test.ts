...
describe("llm constructors", () => {
  ...
  test("keeps request options separate from route defaults", () => {
    const request = LLM.request({
      model: Model.make({
        ...
        route: chatRoute.with({
          ...
          http: {
            body: { metadata: { model: true } },
            headers: { "x-shared": "model" },
            query: { model: "1" },
          },
        }),
      }),
    ...
    expect(request.providerOptions).toEqual({
      openai: { store: true, metadata: { request: true } },
    });
  ...
  test("carries model defaults and compatibility through route model selection", () => {
    ...
    expect(request.model.defaults?.providerOptions).toEqual({
  ...
  test("builds tool choices from names and tools", () => {
    ...
    expect(ToolChoice.named("required")).toEqual(
      new ToolChoice({ type: "tool", name: "required" }),
    );
