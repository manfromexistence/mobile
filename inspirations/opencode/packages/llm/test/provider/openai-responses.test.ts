...
describe("OpenAI Responses route", () => {
                ...
                anyOf: [
                  ...
                  },
                  {
                    ...
                    properties: {
                      ...
                      limit: { type: "integer", maximum: 51200 },
                    },
                    required: ["resource"],
                  },
                ],
  ...
  it.effect("streams OpenAI Responses over WebSocket", () =>
    Effect.gen(function* () {
      ...
      expect(opened).toEqual([
        { url: "wss://api.openai.test/v1/responses", authorization: "Bearer test" },
      ]);
        ...
        Effect.provide(
          dynamicResponse((input) =>
            Effect.gen(function* () {
              const web = yield* HttpClientRequest.toWeb(input.request).pipe(Effect.orDie);
              expect(web.url).toBe(
                "https://opencode-test.openai.azure.com/openai/v1/responses?api-version=v1",
              );
  ...
  it.effect("loads OpenAI default auth from Effect Config", () =>
    LLMClient.generate(
      LLM.updateRequest(request, {
        model: OpenAI.configure({ baseURL: "https://api.openai.test/v1/" }).responses(
          "gpt-4.1-mini",
        ),
      }),
    ).pipe(
    ...
    Effect.gen(function* () {
      ...
      const prepared = yield* LLMClient.prepare<OpenAIResponses.OpenAIResponsesBody>(
        LLM.request({
          ...
          messages: [
            Message.assistant([
              ToolCallPart.make({ id: "call_1", name: "bash", input: { command: "sleep 10" } }),
            ]),
            Message.tool({
              id: "call_1",
              name: "bash",
    ...
    Effect.gen(function* () {
      ...
      expect(prepared.body).toMatchObject({
        input: [
          ...
          {
            role: "assistant",
            content: [{ type: "output_text", text: "It shows a small test image." }],
          },
          {
            role: "user",
            content: [
              { type: "input_text", text: "Check the weather in Paris before continuing." },
            ],
          },
          {
            type: "function_call",
          ...
          { role: "assistant", content: [{ type: "output_text", text: "Paris is 22 degrees." }] },
          {
            ...
            content: [
              {
                type: "input_text",
                text: "Continue from this conversation in one short sentence.",
              },
            ],
          },
        ],
  ...
  it.effect("maps OpenAI provider options to Responses options", () =>
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<OpenAIResponses.OpenAIResponsesBody>(
        LLM.request({
          model: OpenAI.configure({ baseURL: "https://api.openai.test/v1/", apiKey: "test" }).model(
            "gpt-5.2",
          ),
          prompt: "think",
          providerOptions: {
    ...
    Effect.gen(function* () {
      const prepared = yield* LLMClient.prepare<OpenAIResponses.OpenAIResponsesBody>(
        LLM.request({
          model,
          prompt: "hi",
          providerOptions: { openai: { include: ["bogus.thing"] } },
        }),
      );
                ...
                sseEvents(
                  {
                    type: "response.output_text.delta",
                    item_id: "msg_1",
                    delta: "Parser now round-trips reasoning.",
                  },
                  { type: "response.completed", response: { id: "resp_1" } },
                ),
                { headers: { "content-type": "text/event-stream" } },
          ...
          messages: [
            Message.assistant([
              ...
              }),
              {
                ...
                result: {
                  type: "json",
                  value: { type: "web_search_call", id: "ws_1", status: "completed" },
                },
                providerExecuted: true,
                providerMetadata: { openai: { itemId: "ws_1" } },
              },
    ...
    Effect.gen(function* () {
      const error = yield* LLMClient.prepare(
        LLM.request({
          ...
          model,
          messages: [
            Message.user({ type: "media", mediaType: "application/pdf", data: "AAECAw==" }),
          ],
        }),
      ).pipe(Effect.flip);

      expect(error.message).toContain(
        "OpenAI Responses does not support media type application/pdf",
      );
    }),
  );
  ...
  it.effect("emits provider-error events for mid-stream provider errors", () =>
    Effect.gen(function* () {
      const response = yield* LLMClient.generate(request).pipe(
        Effect.provide(
          fixedResponse(
            sseEvents({ type: "error", code: "rate_limit_exceeded", message: "Slow down" }),
          ),
        ),
      );
      ...
      expect(response.events).toEqual([
        { type: "provider-error", message: "rate_limit_exceeded: Slow down" },
      ]);
    }),
  );
  ...
  it.effect("falls back to error code when message is empty", () =>
    Effect.gen(function* () {
      const response = yield* LLMClient.generate(request).pipe(
        Effect.provide(
          fixedResponse(sseEvents({ type: "error", code: "internal_error", message: "" })),
        ),
      );
  ...
  it.effect("falls back to a stable default when both error and response are absent", () =>
    Effect.gen(function* () {
      ...
      expect(response.events).toEqual([
        { type: "provider-error", message: "OpenAI Responses stream error" },
      ]);
    }),
  );
  ...
  it.effect("falls back to a stable default when response.failed has no error payload", () =>
    Effect.gen(function* () {
      const response = yield* LLMClient.generate(request).pipe(
        Effect.provide(
          fixedResponse(sseEvents({ type: "response.failed", response: { id: "resp_failed_3" } })),
        ),
      );

      expect(response.events).toEqual([
        { type: "provider-error", message: "OpenAI Responses response failed" },
      ]);
    }),
  );
