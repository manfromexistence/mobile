...
const fixedBytes = (bytes: Uint8Array) =>
  fixedResponse(bytes.slice().buffer, {
...
describe("Bedrock Converse route", () => {
  ...
  it.effect("lowers chronological system updates to wrapped user text in order", () =>
    Effect.gen(function* () {
      ...
      expect(prepared.body.messages).toEqual([
        {
          role: "user",
          content: [{ text: "Before." }, { text: "<system-update>\nUpdate.\n</system-update>" }],
        },
      ...
      const prepared = yield* LLMClient.prepare(
        LLM.request({
          ...
          messages: [
            ...
            Message.assistant([
              ToolCallPart.make({ id: "tool_1", name: "lookup", input: { query: "weather" } }),
            ]),
  ...
  it.effect("decodes text-delta + messageStop + metadata usage from binary event stream", () =>
    Effect.gen(function* () {
      ...
      const response = yield* LLMClient.generate(baseRequest).pipe(
        Effect.provide(fixedBytes(body)),
      );
  ...
  it.effect("assembles streamed tool call input", () =>
    Effect.gen(function* () {
      const body = eventStreamBody(
        ...
        [
          "contentBlockDelta",
          { contentBlockIndex: 0, delta: { toolUse: { input: ':"weather"}' } } },
        ],
        ["contentBlockStop", { contentBlockIndex: 0 }],
      ...
      expect(prepared.body.messages).toEqual([
        {
          role: "assistant",
          content: [
            { reasoningContent: { reasoningText: { text: "Let me think.", signature: "sig_1" } } },
          ],
        },
      ]);
    }),
  ...
  it.effect("rejects requests with no auth path", () =>
    Effect.gen(function* () {
      ...
      const error = yield* LLMClient.generate(
        LLM.updateRequest(baseRequest, { model: unsignedModel }),
      ).pipe(
      ...
      expect(error.message).toContain(
        "Bedrock Converse requires either route bearer auth or AWS credentials",
      );
    }),
  );

  it.effect(
    "signs requests with SigV4 when AWS credentials are provided (deterministic plumbing check)",
    () =>
      Effect.gen(function* () {
  ...
  it.effect(
    "emits cachePoint markers after system, user-text, and assistant-text with cache hints",
    () =>
      Effect.gen(function* () {
  ...
  it.effect("rejects unsupported image media types", () =>
    Effect.gen(function* () {
      ...
      expect(error.message).toContain(
        "Bedrock Converse does not support image media type image/svg+xml",
      );
    }),
  );
    ...
    Effect.gen(function* () {
      const error = yield* LLMClient.prepare(
        LLM.request({
          ...
          messages: [
            Message.user([
              { type: "media", mediaType: "application/x-tar", data: "x", filename: "a.tar" },
            ]),
          ],
        }),
      ).pipe(Effect.flip);
      ...
      expect(error.message).toContain(
        "Bedrock Converse does not support media type application/x-tar",
      );
    }),
  );
