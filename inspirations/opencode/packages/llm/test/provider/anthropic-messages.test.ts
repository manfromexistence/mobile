...
import {
  continuationRequest,
  nativeAnthropicMessagesContinuation,
...
describe("Anthropic Messages route", () => {
  ...
  it.effect(
    "lowers chronological system updates natively for Claude Opus 4.8 with cache hints",
    () =>
      Effect.gen(function* () {
        ...
        expect(prepared.body.messages).toEqual([
          { role: "user", content: [{ type: "text", text: "Before." }] },
          {
            role: "system",
            content: [
              { type: "text", text: "Operator update.", cache_control: { type: "ephemeral" } },
            ],
          },
  ...
  it.effect(
    "lowers chronological system updates to wrapped user text for unsupported Anthropic models",
    () =>
      Effect.gen(function* () {
        const prepared = yield* LLMClient.prepare<AnthropicMessages.AnthropicMessagesBody>(
          LLM.request({
  ...
  it.effect("rejects non-text chronological system update content before send", () =>
    Effect.gen(function* () {
      const error = yield* LLMClient.prepare(
        LLM.request({
          ...
          messages: [
            ...
            Message.make({
              role: "system",
              content: { type: "media", mediaType: "image/png", data: "AAECAw==" },
            }),
          ],
        }),
      ).pipe(Effect.flip);
      ...
      expect(error.message).toContain(
        "Anthropic Messages system messages only support text content for now",
      );
    }),
  );
  ...
  it.effect("rejects a system update between a local tool call and its result", () =>
    Effect.gen(function* () {
      ...
      expect(error.message).toContain(
        "system updates cannot split a local tool call from its tool result",
      );
    }),
  );
      ...
      const prepared = yield* LLMClient.prepare<AnthropicMessages.AnthropicMessagesBody>(
        LLM.request({
          ...
          messages: [
            ...
            Message.assistant([
              ToolCallPart.make({ id: "call_1", name: "read", input: { filePath: "shot.png" } }),
            ]),
            Message.tool({
              id: "call_1",
              name: "read",
        ...
        LLM.request({
          ...
          messages: [
            ...
            Message.tool({
              ...
              resultType: "content",
              result: [
                { type: "file", uri: "data:image/jpeg;base64,/9j/AA==", mime: "image/jpeg" },
              ],
            }),
          ],
          cache: "none",
        ...
        LLM.request({
          ...
          messages: [
            ...
            Message.tool({
              ...
              resultType: "content",
              result: [
                { type: "file", uri: "data:audio/mpeg;base64,AAECAw==", mime: "audio/mpeg" },
              ],
            }),
          ],
          cache: "none",
        ...
        messages: [
          ...
          },
          {
            ...
            content: [
              {
                type: "thinking",
                thinking: "I inspected the previous turn.",
                signature: "sig_continuation_1",
              },
          ...
          {
            role: "user",
            content: [{ type: "text", text: "Check the weather in Paris before continuing." }],
          },
          {
            role: "assistant",
            content: [
          ...
          },
          {
            role: "user",
            content: [
              { type: "tool_result", tool_use_id: "call_weather_1", content: '{"temperature":22}' },
            ],
          },
          { role: "assistant", content: [{ type: "text", text: "Paris is 22 degrees." }] },
          {
            role: "user",
            content: [
              { type: "text", text: "Continue from this conversation in one short sentence." },
            ],
          },
        ],
      });
    ...
    Effect.gen(function* () {
      ...
      expect(prepared.body).toMatchObject({
        messages: [
          {
            role: "assistant",
            content: [{ type: "thinking", thinking: "thinking", signature: "sig_1" }],
          },
        ],
      });
    }),
  ...
  it.effect("emits provider-error events for mid-stream provider errors", () =>
    Effect.gen(function* () {
      ...
      expect(response.events).toEqual([
        { type: "provider-error", message: "overloaded_error: Overloaded" },
      ]);
    }),
  );
  ...
  it.effect("falls back to error type when no message is present", () =>
    Effect.gen(function* () {
      const response = yield* LLMClient.generate(request).pipe(
        Effect.provide(
          fixedResponse(
            sseEvents({ type: "error", error: { type: "overloaded_error", message: "" } }),
          ),
        ),
      );
  ...
  it.effect("falls back to a stable default when error payload is absent", () =>
    Effect.gen(function* () {
      ...
      expect(response.events).toEqual([
        { type: "provider-error", message: "Anthropic Messages stream error" },
      ]);
    }),
  );
  ...
  it.effect("decodes server_tool_use + web_search_tool_result as provider-executed events", () =>
    Effect.gen(function* () {
      ...
      const response = yield* LLMClient.generate(
        LLM.updateRequest(request, {
          tools: [
            { name: "web_search", description: "Web search", inputSchema: { type: "object" } },
          ],
        }),
      ).pipe(Effect.provide(fixedResponse(body)));
      ...
      expect(toolResult).toEqual({
        ...
        result: {
          type: "json",
          value: [{ type: "web_search_result", url: "https://example.com", title: "Example" }],
        },
        providerExecuted: true,
              ...
              return input.respond(
                sseEvents(
                  ...
                  },
                  {
                    type: "content_block_delta",
                    index: 0,
                    delta: { type: "text_delta", text: "An image." },
                  },
                  { type: "content_block_stop", index: 0 },
                  {
                    type: "message_delta",
      ...
      expect(prepared.body).toMatchObject({
        ...
        messages: [
          ...
          { role: "assistant", content: [{ type: "tool_use", id: "call_1", name: "lookup" }] },
          {
            role: "user",
            content: [
              { type: "tool_result", tool_use_id: "call_1", cache_control: { type: "ephemeral" } },
            ],
          },
        ],
      });
