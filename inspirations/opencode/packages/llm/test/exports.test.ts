...
describe("public exports", () => {
  ...
  test("provider barrels expose user-facing facades", () => {
    ...
    expect(
      CloudflareAIGateway.configure({ accountId: "fixture", gatewayApiKey: "fixture" }).model,
    ).toBeFunction();
    expect(CloudflareWorkersAI.configure).toBeFunction();
    expect(
      CloudflareWorkersAI.configure({ accountId: "fixture", apiKey: "fixture" }).model,
    ).toBeFunction();
    ...
    expect(XAI.configure({ apiKey: "fixture" }).responses("grok-4.3").route.id).toBe(
      "openai-responses",
    );
    expect(XAI.configure({ apiKey: "fixture" }).chat("grok-4.3").route.id).toBe(
      "openai-compatible-chat",
    );
    expect(
      GitHubCopilot.configure({ baseURL: "https://api.githubcopilot.test", apiKey: "fixture" })
        .model,
    ).toBeFunction();
    expect(
