    ...
    Effect.gen(function* () {
      if (city === "FAIL")
        return yield* new ToolFailure({
          message: `Weather lookup failed for ${city}`,
          error: weatherFailureCause,
        });
...
describe("LLMClient tools", () => {
  ...
  it.effect("projects encoded typed tool success into canonical model content", () =>
    Effect.gen(function* () {
      ...
      expect(calls).toEqual([
        { callID: "call_projected", parameters: { prefix: "count" }, output: { count: "2" } },
      ]);
    ...
    Effect.gen(function* () {
      ...
      expect(
        (yield* ToolRuntime.dispatch(
          { text },
          LLMEvent.toolCall({ id: "call_text", name: "text", input: {} }),
        )).output,
      ).toEqual({ structured: "hello", content: [{ type: "text", text: "hello" }] });
      expect(
        (yield* ToolRuntime.dispatch(
          { json },
          LLMEvent.toolCall({ id: "call_json", name: "json", input: {} }),
        )).output,
  ...
  it.effect("models canonical tool files with URIs", () =>
    Effect.sync(() => {
      ...
      expect(
        decode({ type: "file", uri: "data:image/png;base64,AAAA", mime: "image/png" }),
      ).toEqual({
        type: "file",
  ...
  it.effect("derives typed output schemas and preserves dynamic output schemas", () =>
    Effect.sync(() => {
      ...
      expect(
        Reflect.get(Reflect.get(typed?.outputSchema ?? {}, "properties") as object, "temperature"),
      ).toBeDefined();
    ...
    Effect.gen(function* () {
      ...
      const events = Array.from(
        yield* TestToolRuntime.runTools({
          ...
          Effect.provide(
            scriptedResponses([
              sseEvents(toolCallChunk("call_1", "screenshot", "{}"), finishChunk("tool_calls")),
            ]),
          ),
        ),
      );
  ...
  it.effect("executes tool calls for one step without looping by default", () =>
    Effect.gen(function* () {
      const layer = scriptedResponses([
        ...
        sseEvents(
          deltaChunk({ role: "assistant", content: "Should not run." }),
          finishChunk("stop"),
        ),
      ]);
      ...
      const events = Array.from(
        yield* TestToolRuntime.runTools({ request: baseRequest, tools: { contextual } }).pipe(
          ...
          Effect.provide(
            scriptedResponses([
              sseEvents(
                toolCallChunk("call_ctx", "contextual", '{"value":"x"}'),
                finishChunk("tool_calls"),
              ),
            ]),
          ),
      ...
      const events = Array.from(
        yield* LLMClient.stream(
          LLMRequest.update(baseRequest, {
            tools: toDefinitions({ get_weather: schema_only_weather }),
          }),
        ).pipe(Stream.runCollect, Effect.provide(layer)),
      );
  ...
  it.effect(
    "preserves provider metadata when folding streamed assistant content into follow-up history",
    () =>
      Effect.gen(function* () {
        const bodies: unknown[] = [];
        const layer = dynamicResponse((input) =>
