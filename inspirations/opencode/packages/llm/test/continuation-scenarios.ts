...
export const basicContinuation = [
  "system",
  "user-text",
  "assistant-text",
  "user-follow-up",
...
export function continuationRequest(input: {
  ...
  if (features.has("assistant-text"))
    firstAssistant.push({ type: "text", text: "It shows a small test image." });
  ...
  if (features.has("tool-call")) {
    ...
    messages.push(
      Message.assistant([
        ToolCallPart.make({ id: "call_weather_1", name: "get_weather", input: { city: "Paris" } }),
      ]),
    );
  }
  ...
  return LLM.request({
    id: input.id,
    model: input.model,
    system: features.has("system")
      ? "You are concise. Continue from the provided history."
      : undefined,
    messages,
