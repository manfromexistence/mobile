...
export const missingEnv = (names: ReadonlyArray<string>) =>
...
export const matchesSelected = (input: {
  ...
  if (providers.length > 0 && !providers.some((provider) => tags.includes(`provider:${provider}`)))
    return false;
  if (requiredTags.length > 0 && !requiredTags.every((tag) => tags.includes(tag))) return false;
  if (tests.length > 0 && !tests.some((test) => names.some((name) => name.includes(test))))
    return false;
  return true;
};
