...
const mergeOptions = (
  ...
  return {
    ...
    ...override,
    metadata:
      base.metadata || override.metadata ? { ...base.metadata, ...override.metadata } : undefined,
    redact:
      base.redact || override.redact
        ? {
            ...
            queryParameters: [
              ...(base.redact?.queryParameters ?? []),
              ...(override.redact?.queryParameters ?? []),
            ],
            jsonFields: [
              ...(base.redact?.jsonFields ?? []),
              ...(override.redact?.jsonFields ?? []),
            ],
          }
        : undefined,
  };
...
export const recordedTests = (options: RecordedTestsOptions) =>
  recordedEffectGroup<RecordedEnv, never, RecordedTestsOptions, RecordedCaseOptions>({
    ...
    options,
    cassetteExists: (cassette) =>
      HttpRecorderInternal.hasCassetteSync(cassette, { directory: FIXTURES_DIR }),
    layer: ({ cassette, metadata, options, caseOptions, recording }) => {
      ...
      const cassetteService = HttpRecorderInternal.Cassette.fileSystem({
      ...
      return Layer.mergeAll(deps, LLMClient.layer.pipe(Layer.provide(deps))).pipe(
