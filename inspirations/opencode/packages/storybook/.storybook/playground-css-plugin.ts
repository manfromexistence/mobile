  ...
  for (const edit of edits) {
    ...
    if (idx === -1) {
      results.push({
        ...
        prop: edit.prop,
        ok: false,
        error: `Anchor not found: ${edit.anchor.slice(0, 50)}`,
      });
      continue;
    }
    ...
    if (!match) {
      results.push({
        ...
        prop: edit.prop,
        ok: false,
        error: `Property "${edit.prop}" not found after anchor`,
      });
      continue;
    }
      ...
      server.middlewares.use((req: IncomingMessage, res: ServerResponse, next: () => void) => {
        ...
        req.on("end", () => {
          ...
          for (const edit of payload.edits) {
            ...
            const abs = roots
              .map((root) => path.resolve(root, edit.file))
          ...
          for (const [abs, edits] of grouped) {
            ...
            if (!fs.existsSync(abs)) {
              for (const e of edits)
                results.push({ file: name, prop: e.prop, ok: false, error: "File not found" });
              continue;
            }
            ...
            try {
              ...
              for (const e of edits)
                results.push({ file: name, prop: e.prop, ok: false, error: String(err) });
            }
          }
