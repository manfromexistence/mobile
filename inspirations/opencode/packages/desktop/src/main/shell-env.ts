...
export function loadShellEnv(shell: string, logger: ShellEnvLogger) {
  ...
  if (interactive.type === "Loaded") {
    logger.log(
      `[server] Loaded shell environment with -il (${Object.keys(interactive.value).length} vars)`,
    );
  ...
  if (login.type === "Loaded") {
    logger.log(
      `[server] Loaded shell environment with -l (${Object.keys(login.value).length} vars)`,
    );
    return login.value;
  }
