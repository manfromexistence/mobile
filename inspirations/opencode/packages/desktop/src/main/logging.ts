...
export async function exportDebugLogs() {
  ...
  if (restartNetLog) {
    await netLog
      .stopLogging()
      .catch((error) => write("network", "failed to stop net log", { error }));
  }
  ...
  try {
    ...
    if (restartNetLog) {
      await startNetLog().catch((error) =>
        write("network", "failed to restart net log", { error }),
      );
    }
  }
}
...
function collect(dir: string, prefix: string): Entry[] {
  ...
  const walk = (current: string) => {
    for (const entry of readdirSync(current)) {
      ...
      result.push({
        name: join(prefix, file.slice(dir.length + 1)).replace(/\\/g, "/"),
        path: file,
      });
    }
  };
