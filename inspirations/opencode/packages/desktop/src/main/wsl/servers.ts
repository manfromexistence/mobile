...
export function createWslServersController(
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    if (opencodeChecks.length) {
      setState({
        opencodeChecks: { ...state.opencodeChecks, ...Object.fromEntries(opencodeChecks) },
      });
    }
  };
  ...
  return {
    ...
    async installDistro(name: string) {
      await runJob(
        { kind: "install-distro", distro: name, startedAt: Date.now() },
        async (abort) => {
          const result = await installWslDistro(name, { signal: abort.signal });
          if (result.code !== 0) {
            const message =
              summarize(result.stderr || result.stdout) || `Failed to install distro: ${name}`;
            throw new Error(message);
          }
    ...
    async installOpencode(name: string) {
      await runJob(
        { kind: "install-opencode", distro: name, startedAt: Date.now() },
        async (abort) => {
          const result = await installWslOpencode(appVersion, name, { signal: abort.signal });
          if (result.code !== 0) {
            throw new Error(
              summarize(result.stderr || result.stdout) || "OpenCode installation failed",
            );
          }
...
function normalizePersistedServer(value: unknown): WslServerConfig[] {
  ...
  const distro =
    typeof record.distro === "string" && record.distro.length > 0 ? record.distro : null;
  if (!distro) return [];
  const id =
    typeof record.id === "string" && record.id.length > 0
      ? record.id
      : wslServerIdForDistro(distro);
  return [
    {
      id,
      distro,
