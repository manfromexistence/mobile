...
async function sign(url: string, key: string) {
  ...
  if (!res.ok) {
    throw new Error(
      `Failed to fetch file ${name}: ${res.status} ${res.statusText} (${asset?.url ?? url})`,
    );
  }
...
const add = async (
  data: Record<string, { url: string; signature: string }>,
  key: string,
...
if (yver && yver !== version)
  throw new Error(`latest.yml version mismatch: expected ${version}, got ${yver}`);
