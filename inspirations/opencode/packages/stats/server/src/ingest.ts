  ...
  static readonly layer: Layer.Layer<Ingest> = Layer.effect(
    ...
    Effect.sync(() => {
      ...
      const write = Effect.fn("Ingest.write")(function* (events: unknown[]) {
        ...
        if (failed > 0) {
          yield* Effect.logWarning(
            `lake ingest incomplete ${JSON.stringify({ records: counts.records, failed })}`,
          );
          return yield* Effect.fail(
            new IngestError({ message: "Failed to ingest all lake records", failed }),
          );
        }

        yield* Effect.logInfo(
          `lake ingest complete ${JSON.stringify({ records: counts.records, batches })}`,
        );
...
const putRecords: (
  ...
  const result = yield* Effect.tryPromise({
    try: () =>
      client.send(new PutRecordBatchCommand({ DeliveryStreamName: streamName, Records: records })),
    catch: (cause) =>
      new IngestError({
        message: "Failed to write lake records to Firehose",
    ...
    Effect.tapError(() =>
      Effect.logWarning(
        `firehose batch write failed ${JSON.stringify({ records: records.length, attempt })}`,
      ),
    ),
  );
  const failed =
