...
export function findModelCatalogEntry(catalog: ModelCatalog, model: string, lab?: string) {
  const normalizedId = lab
    ? `${catalogLabSlug(lab)}/${catalogSlug(model)}`
    : model.trim().toLowerCase();
  const leaf = catalogSlug(model);
  return (
    catalog.models.find((entry) => entry.id.toLowerCase() === normalizedId) ??
    catalog.models.find(
      (entry) => (lab ? entry.lab === catalogLabSlug(lab) : true) && entry.slug === leaf,
    ) ??
  ...
  return fetch(url)
    .then(
      (response): Promise<unknown> =>
        response.ok ? (response.json() as Promise<unknown>) : Promise.resolve(),
    )
    .catch(() => undefined);
}
...
function readCatalogCosts(payload: unknown) {
  ...
  const add = (model: unknown, provider?: string) => {
    ...
    if (provider && !id.includes("/"))
      costs.set(`${catalogSlug(provider)}/${catalogSlug(id)}`, cost);
  };
