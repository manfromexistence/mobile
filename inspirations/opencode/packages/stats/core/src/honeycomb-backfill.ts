...
type ModelAggregate = StatBaseAggregate & {
  provider: string;
  model: string;
  provider_model: string;
};
type ProviderAggregate = StatBaseAggregate & { provider: string };
type GeoAggregate = StatBaseAggregate & {
  provider: string;
  model: string;
  country: string;
  continent: string;
...
function printQueries(args: string[]) {
  ...
  if (only) {
    ...
    if (!item)
      fail(
        `Unknown --only ${only}. Expected one of: ${queries.map((query) => query.name).join(", ")}`,
      );
...
async function importFiles(args: string[]) {
  ...
  const opts = {
    ...parsed,
    files: mergeFiles(parsed.files, await discoverFiles(parsed.directories)),
  };
  if (!inputKeys.some((key) => opts.files[key]?.length))
    fail("No CSV or JSON import files were provided or discovered");
  const providerModelLookup = new Map([
    ...(await lookupRows(
      opts.files["model-provider-model-day"],
      "day",
      opts,
      modelProviderModelLookup,
    )),
    ...(await lookupRows(
      opts.files["model-provider-model-week"],
      "week",
      opts,
      modelProviderModelLookup,
    )),
    ...
    JSON.stringify(
      {
        inputs: Object.fromEntries(
          inputKeys.flatMap((key) =>
            opts.files[key]?.length ? [[key, opts.files[key].length]] : [],
          ),
        ),
...
function buildQueries(limit: number, tiers: string[]): QuerySpec[] {
  const daily = tiers.flatMap((tier) => [
    ...
    querySpec(
      "geo-day",
      tier,
      metricQuery(["date", "tier", "country", "continent"], limit, tierFilters(tier)),
    ),
    querySpec(
      "geo-model-day",
      tier,
  ...
  const weekly = tiers.flatMap((tier) => [
    ...
    querySpec(
      "geo-week",
      tier,
      metricQuery(["week", "tier", "country", "continent"], limit, tierFilters(tier)),
    ),
    querySpec(
      "geo-model-week",
      tier,
...
function metricRows<T extends StatBaseAggregate>(
  ...
  return readFiles(files).then((rows) =>
    rows.flatMap((row) => map(row, baseAggregate(row, grain, opts))),
  );
}
...
function mergeFiles(
  left: Partial<Record<ImportKey, string[]>>,
  right: Partial<Record<ImportKey, string[]>>,
) {
...
function modelProviderModelLookup(
  row: RawRow,
  grain: Grain,
  opts: ImportOptions,
): [string, string][] {
  ...
  return [
    [
      lookupKey({ ...base, dataset: opts.dataset, tier: tier(row), grain }, author, model(row)),
      value,
    ],
  ];
}
  ...
  return [
    {
      ...
      model: model(row),
      provider_model:
        providerModelLookup.get(lookupKey(base, author, model(row))) ?? providerModel(row),
    },
  ];
}
...
function geoContinentLookup(row: RawRow, grain: Grain, opts: ImportOptions): [string, string][] {
  ...
  if (!value) return [];
  return [
    [lookupKey({ ...base, dataset: opts.dataset, tier: tier(row), grain }, country(row)), value],
  ];
}
...
function baseAggregate(row: RawRow, grain: Grain, opts: ImportOptions): StatBaseAggregate {
  return {
    ...
    avg_output_tps: nullableNumber(row, "avg_output_tps", [
      "AVG(tps.output)",
      "AVG(stat_output_tps)",
    ]),
    success_count: integer(row, "success_count", [
      "SUM(success)",
      "SUM(is_success)",
      "SUM(stat_success)",
    ]),
...
function modelRowsFromAggregates(aggregates: ModelAggregate[]) {
  return rankModelRows([
    ...synthesizeAllTierRows(
      collapseRows(
        aggregates.filter((item) => item.grain === "week").map(toModelRow),
        modelDimensionKey,
      ),
      modelDimensionKey,
    ),
    ...synthesizeAllTierRows(
      collapseRows(
        aggregates.filter((item) => item.grain === "day").map(toModelRow),
        modelDimensionKey,
      ),
      modelDimensionKey,
    ),
...
function providerRowsFromAggregates(aggregates: ProviderAggregate[]) {
  return rankRowsWithMarketShare([
    ...synthesizeAllTierRows(
      collapseRows(
        aggregates.filter((item) => item.grain === "week").map(toProviderRow),
    ...
    ...synthesizeAllTierRows(
      collapseRows(
        aggregates.filter((item) => item.grain === "day").map(toProviderRow),
        providerDimensionKey,
      ),
      providerDimensionKey,
    ),
...
function geoRowsFromAggregates(aggregates: GeoAggregate[]) {
  return rankRowsWithMarketShare(
    [
      ...synthesizeAllTierRows(
        collapseRows(
          aggregates.filter((item) => item.grain === "week").map(toGeoRow),
          geoDimensionKey,
        ),
        geoDimensionKey,
      ),
      ...synthesizeAllTierRows(
        collapseRows(
          aggregates.filter((item) => item.grain === "day").map(toGeoRow),
          geoDimensionKey,
        ),
        geoDimensionKey,
      ),
...
function lookupKey(
  base: { grain: string; period_key: string; dataset: string; tier: string },
  ...dimension: string[]
) {
...
function provider(row: RawRow) {
  return statProvider(
    model(row),
    providerModel(row),
    cell(row, ["stat_provider_2", "stat_provider"]),
  );
}
...
function cell(row: RawRow, names: string[]) {
  ...
  return (
    names
      .flatMap((name) => [row[name], normalized.get(normalizeHeader(name))])
...
function parseWeek(row: RawRow) {
  ...
  const start = new Date(
    startOfIsoWeek(new Date(Date.UTC(year, 0, 4))).getTime() + (week - 1) * 7 * DAY_MS,
  );
...
function rowsFromJson(value: unknown): RawRow[] {
  ...
  if (!isRecord(value))
    fail("JSON imports must be an array of rows or an object with results/data/rows");
...
function rowsFromCsv(text: string): RawRow[] {
  const [headers, ...rows] = csvRecords(text).filter((row) =>
...
function parseFlags(args: string[]) {
  ...
  for (let index = 0; index < args.length; index++) {
    ...
    const nextFlag = args.findIndex(
      (value, valueIndex) => valueIndex > index && value.startsWith("--"),
    );
