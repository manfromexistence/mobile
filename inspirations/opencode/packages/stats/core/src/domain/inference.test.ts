...
describe("inference stat normalization", () => {
  ...
  test("model aggregates prefer provider.model and use normalized model", () => {
    ...
    expect(
      toModelAggregate(aggregate("deepseek-v4-flash-free", "not-public-provider")),
    ).toMatchObject([
      {
  ...
  test("provider aggregates never keep opencode as the provider", () => {
    expect(
      toProviderAggregate({ ...aggregate("big-pickle", "opencode"), provider_model: "gpt-5" }),
    ).toMatchObject([{ provider: "openai" }]);
    expect(toProviderAggregate(aggregate("big-pickle", "opencode"))).toMatchObject([
      { provider: "unknown" },
    ]);
  });
