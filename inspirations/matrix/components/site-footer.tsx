...
export function SiteFooter() {
  return (
    ...
    >
      <span
        className="absolute pointer-events-none inline-block [html[data-theme='dark']_&]:hidden -rotate-6 sm:p-3 size-[280px] md:size-[340px] max-md:bottom-28! md:top-1/2 md:-translate-y-1/2 right-1/2 translate-x-1/2 p-1.5 bg-background rounded-[76px] md:rounded-[86px]"
        aria-hidden="true"
      >
        <Image
          src="/icon.svg"
      ...
      <span
        className="absolute pointer-events-none hidden [html[data-theme='dark']_&]:inline-block -rotate-6 sm:p-3 size-[280px] md:size-[340px]  max-md:bottom-28! md:top-1/2 md:-translate-y-1/2 right-1/2 translate-x-1/2 p-1.5 bg-[#d0d0d0] shadow-[0_3px_4px_0px_rgba(255,255,255,1)_inset] rounded-[76px] md:rounded-[86px]"
        aria-hidden="true"
      >
        <Image
          src="/icon.svg"
      ...
      <span className="inline-flex items-center gap-2.5 text-3xl  absolute bottom-4 right-8 z-10">
        ...
        <a href={CREATOR_URL} target="_blank" className="">
          by shawn.
        </a>
      </span>
      <div className="flex flex-col  gap-2.5 text-xl md:text-3xl italic absolute top-8 left-8 z-10 pointer-events-auto">
        <span className="">
          <span className="font-semibold text-3xl md:text-5xl">@dotmatrix</span> - v1.5.0
        </span>
        <nav
      ...
      <span className="inline-flex flex-col items-end gap-2.5 md:text-3xl italic absolute bottom-4 md:top-6 md:right-8 left-6">
        <a href={CREATOR_URL} target="_blank" className="text-base">
          x.com
        </a>
        <a href={REPO_URL} target="_blank" className="text-base">
          github
        </a>
      </span>
    </footer>
