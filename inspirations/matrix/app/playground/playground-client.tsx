...
export function PlaygroundClient({ initialSlug, loaders }: PlaygroundClientProps) {
  ...
  const defaultSlug = loaders.some((loader) => loader.slug === initialSlug)
    ...
    : fallbackSlug;
  const defaultProps =
    LOADER_GALLERY_PREVIEW_PROPS[defaultSlug] ?? LOADER_GALLERY_PREVIEW_PROPS[fallbackSlug];
  ...
  const SelectedLoader =
    loaderComponentMap[selectedLoader?.slug ?? fallbackSlug] ??
    loaderComponentMap[fallbackSlug] ??
    DotMatrixIcon;
  const customColor =
