...
export function getGradientCode({
  ...
  if (tailwindVersion === "v3") {
    if (gradientType === "radial") {
      ...
      const color1 = formatRgbaForTailwind(gradientStops[0].color, gradientStops[0].alpha);
...
export function getRadialGradientCode({
  ...
  CodeGeneratorProps,
  "gradientStops" | "gradientSizeX" | "gradientSizeY" | "gradientPositionX" | "gradientPositionY"
>) {
  const color1 = formatRgbaForTailwind(gradientStops[0].color, gradientStops[0].alpha);
  const color2 = formatRgbaForTailwind(
    gradientStops[gradientStops.length - 1].color,
