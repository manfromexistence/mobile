  ...
  return (
    <>
      <p className="mb-4 text-muted-foreground text-sm">
        The gradient uses RGBA colors. Set Alpha to 0 for the first color to make it transparent.
      </p>
      ...
      <div className="mb-6 space-y-4">
        <div className="space-y-2 rounded-lg border p-2">
          ...
          <div className="space-y-2 ">
            ...
            <CustomSlider
              ...
              max={90}
              step={5}
              onValueChange={(value) => updateColorStop(0, { position: value[0] })}
            />
          </div>
        </div>
        ...
        <div className="space-y-2 rounded-lg border p-2 ">
          <RgbaPickerPopover
            ...
            onColorChange={(color) => updateColorStop(gradientStops.length - 1, { color })}
            onAlphaChange={(alpha) => updateColorStop(gradientStops.length - 1, { alpha })}
            label="Outer Color"
          />

          <div className="space-y-2 ">
            <Label>Position: {gradientStops[gradientStops.length - 1].position}%</Label>
            <CustomSlider
