...
export function PresetCard({
  ...
  return (
    ...
    >
      <div
        className={cn("relative mb-2 h-20 w-40 overflow-hidden rounded-md 2xl:h-24", cardClassName)}
      >
        {/* Base background */}
        <div className="absolute inset-0" style={{ backgroundColor: preset.config.bgColor }} />

        {/* Pattern layer */}
        {preset.config.patternType !== "none" && (
          <div className="absolute inset-0" style={getPatternStyle(preset.config)} />
        )}

        {/* Gradient layer */}
        {preset.config.useGradient && (
          <div className="absolute inset-0" style={getGradientStyle(preset.config)} />
        )}

        {/* Mask layer - simplified for preview */}
        {preset.config.useMask && (
          <div className="absolute inset-0" style={getMaskStyle(preset.config)} />
        )}
      </div>
      {!hideName && (
