...
export function ControlPanel({
      ...
      <div className="mb-4 space-y-2">
        ...
        <div className="grid grid-cols-3 gap-4">
          {["color1", "color2", "color3"].map((colorKey) => {
            const value = settings[colorKey as keyof ShaderGradientSettings] as string;
            return (
  ...
  const renderControl = (key: string, config: ControlConfig): JSX.Element | null => {
        ...
        return (
          <div className="mb-4 space-y-2" key={key}>
            <div className="flex justify-between">
              <Label htmlFor={key}>{formatLabel(key)}</Label>
              <span className="text-muted-foreground text-sm">{value as number}</span>
            </div>
            <CustomSlider
              id={key}
    ...
    <div className="space-y-6">
      {Object.entries(sections).map(([sectionKey, section]) => {
        const hasColorControls =
          sectionKey === "basic" &&
          Object.keys(section.controls).some((key) => ["color1", "color2", "color3"].includes(key));

        return (
          <div key={sectionKey} className={sectionClassNames[sectionKey] || ""}>
            <h2 className="mb-4 border-b pb-2 font-semibold text-lg">{section.title}</h2>
            <div className="space-y-2">
              ...
              {Object.entries(section.controls).map(([key, config]) => renderControl(key, config))}
            </div>
          </div>
        );
      })}
