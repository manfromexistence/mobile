...
export function ShapePreview({
  ...
  const handlePreviewMouseMove = (e: React.MouseEvent<HTMLDivElement>) => {
    ...
    const deltaX =
      (((e.clientX - startDragPos.x) / previewRef.current.clientWidth) * 100) / zoomLevel;
    const deltaY =
      (((e.clientY - startDragPos.y) / previewRef.current.clientHeight) * 100) / zoomLevel;
    // @ts-ignore
  ...
  return (
    <div
      ...
      style={{
        cursor: isDraggingPreview ? "grabbing" : zoomLevel > 1 ? "grab" : "default",
      }}
    >
      <div className="absolute inset-0 h-full w-full bg-[radial-gradient(#e9e9e9_1px,transparent_1px)] [background-size:16px_16px] dark:bg-[radial-gradient(#272727_1px,transparent_1px)]" />
      ...
      {editMode ? (
        <div className="relative h-full w-full">
          ...
          <svg
            ref={editorRef}
            viewBox={getViewBox()}
            className={cn("absolute inset-0 z-10 h-full w-full", selectedShape.className)}
          ...
          >
            {/* Path outline */}
            <path d={editPath} fill="none" stroke="currentColor" strokeWidth={0.01 / zoomLevel} />

            {/* Control lines */}
            {controlPoints.map((point, index) => {
              if (point.isControl && index > 0 && index < controlPoints.length - 1) {
                // Find the anchor points this control point belongs to
                let prevAnchor = -1;
                let _nextAnchor = -1;
        ...
        <div
          className={cn("h-full w-full overflow-hidden", selectedShape.className)}
          style={{
            transform: `scale(${zoomLevel}) translate(${-previewOffset.x}%, ${-previewOffset.y}%)`,
            transformOrigin: "center",
        ...
        >
          ...
          <figure
            className={cn("group h-full w-full transition-all duration-300")}
            style={{
      ...
      <div className="absolute right-1.5 bottom-1.5 z-50 flex w-fit cursor-default items-center gap-2 rounded-md border-0 p-2 shadow-md backdrop-blur-md">
        ...
        <Button
          ...
          size="sm"
          onClick={resetZoom}
          disabled={zoomLevel === 1 && previewOffset.x === 0 && previewOffset.y === 0}
          className="text-xs"
        >
