...
export function ShapeGrid({
  ...
  return (
    <div
      className={cn("grid grid-cols-3 gap-3 rounded-md bg-card-bg p-4 xl:grid-cols-4", className)}
    >
      {shapes.map((shape) => (
        <div key={shape.id} className="group relative h-fit">
          <Button
            variant={selectedShapeId === shape.id ? "default" : compact ? "outline" : "secondary"}
