...
export function EditControls({ historyIndex, onSave, onCancel }: EditControlsProps) {
  return (
    <div className="mt-4 space-y-4 p-2">
      <h3 className="font-medium text-lg">Edit Mode</h3>
      <p className="text-muted-foreground text-sm">
        Drag the control points to edit the shape. Blue points are anchor points, and red points are
      ...
      <div className="mt-2 text-muted-foreground text-xs">
        {historyIndex > 0 ? (
          <p>
            Press Ctrl+Z to undo changes ({historyIndex} {historyIndex === 1 ? "change" : "changes"}{" "}
            made)
          </p>
        ) : (
