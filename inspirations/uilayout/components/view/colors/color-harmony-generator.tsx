...
export function ColorHarmonyGenerator({ hexValue, setHexValue }: ColorHarmonyGeneratorProps) {
  ...
  const generateHarmony = (type: typeof harmonyType) => {
    ...
    switch (type) {
      case "complementary":
        ...
        break;
      case "analogous":
        colors = [getHarmonyColor(h, -30, s, l), hexValue, getHarmonyColor(h, 30, s, l)];
        break;
      case "triadic":
        colors = [hexValue, getHarmonyColor(h, 120, s, l), getHarmonyColor(h, 240, s, l)];
        break;
      case "tetradic":
        colors = [
          hexValue,
      ...
      case "split-complementary":
        colors = [hexValue, getHarmonyColor(h, 150, s, l), getHarmonyColor(h, 210, s, l)];
        break;
      case "monochromatic":
        colors = [
  ...
  const getHarmonyColor = (h: number, angle: number, s: number, l: number): string => {
  ...
  return (
    <Card>
      <CardHeader className="pb-3">
        <CardTitle className="font-medium text-lg">Color Harmony Generator</CardTitle>
      </CardHeader>
        ...
        >
          ...
          <TabsContent value="complementary" className="mt-2">
            <p className="text-muted-foreground text-sm">
              Complementary colors are opposite each other on the color wheel. They create high
              contrast and vibrant looks.
            </p>
          </TabsContent>

          <TabsContent value="analogous" className="mt-2">
            <p className="text-muted-foreground text-sm">
              Analogous colors are next to each other on the color wheel. They create harmonious,
              comfortable designs.
            </p>
          </TabsContent>
          ...
          <TabsContent value="triadic" className="mt-2">
            <p className="text-muted-foreground text-sm">
              Triadic colors are evenly spaced around the color wheel. They tend to be vibrant and
              balanced.
            </p>
          </TabsContent>
          ...
          <TabsContent value="tetradic" className="mt-2">
            <p className="text-muted-foreground text-sm">
              Tetradic (or square) colors form a rectangle on the color wheel. They offer rich color
              schemes with many possibilities.
            </p>
          </TabsContent>

          <TabsContent value="split-complementary" className="mt-2">
            <p className="text-muted-foreground text-sm">
              Split-complementary uses a base color and two colors adjacent to its complement. It
              provides strong visual contrast with less tension.
            </p>
          </TabsContent>

          <TabsContent value="monochromatic" className="mt-2">
            <p className="text-muted-foreground text-sm">
              Monochromatic colors are different shades and tints of the same hue. They create a
