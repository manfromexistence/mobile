  ...
  const detectAndConvertColor = () => {
    ...
    try {
      ...
      if (value.includes("hsla")) {
        ...
        if (matches && matches.length >= 4) {
          ...
          if (!Number.isNaN(h) && !Number.isNaN(s) && !Number.isNaN(l) && !Number.isNaN(a)) {
            ...
            const hex = rgbToHex(rgb.r, rgb.g, rgb.b);
            setHexValue(hex);
            toast.success(`Detected HSLA format: hsla(${h}, ${s}%, ${l}%, ${a})`);
            return;
          }
        }
      }
      ...
      if (value.includes("cmyk")) {
        ...
        if (matches && matches.length >= 4) {
          ...
          if (!Number.isNaN(c) && !Number.isNaN(m) && !Number.isNaN(y) && !Number.isNaN(k)) {
            ...
            const hex = rgbToHex(r, g, b);
            setHexValue(hex);
            toast.success(`Detected CMYK format: cmyk(${c}%, ${m}%, ${y}%, ${k}%)`);
            return;
          }
        }
      }
      ...
      if (value.includes(",")) {
        ...
        if (parts.length === 3 && parts.every((p) => !Number.isNaN(p) && p >= 0 && p <= 255)) {
      ...
      toast.success("Could not detect the color format. Please try a different format.");
    } catch (error) {
