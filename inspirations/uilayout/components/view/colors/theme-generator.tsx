...
export function ThemeGenerator({ hexValue }: ThemeGeneratorProps) {
  ...
  const [expandedSectionsJson, setExpandedSectionsJson] = useQueryState("expanded", {
  ...
  const [customLightColorsJson, setCustomLightColorsJson] = useQueryState("lightColors", {
    defaultValue: "{}",
    parse: (value) => {
      try {
        return JSON.parse(value);
  ...
  const [customDarkColorsJson, setCustomDarkColorsJson] = useQueryState("darkColors", {
    defaultValue: "{}",
    parse: (value) => {
      try {
        return JSON.parse(value);
  ...
  const baseTheme = useMemo(() => {
    return typeof baseThemeJson === "string" ? JSON.parse(baseThemeJson) : baseThemeJson;
  }, [baseThemeJson]);
  ...
  const hslToHexColor = useCallback((hslString: string) => {
    const [h, s, l] = hslString.split(" ").map((val) => Number.parseFloat(val.replace("%", "")));
    ...
    return (
      <div className="space-y-1.5">
        <div className="flex items-center justify-between">
          <Label htmlFor={`color-${colorKey}`} className="flex items-center gap-1 text-xs">
            {label}
            {isCustomized && (
              <Button
                variant="ghost"
          ...
          <div className="font-mono text-muted-foreground text-xs">{currentHslValue}</div>
        </div>
        <ColorPicker
    ...
    <div className="mt-20 space-y-6">
      ...
      <div className="flex gap-4 overflow-x-auto pb-4">
        {EXAMPLE_THEMES.map((theme, _index) => {
          ...
          return (
            <div key={theme.name} className="w-56 shrink-0 overflow-hidden rounded-md border">
              ...
              <div className="space-y-4">
                <div className="relative space-y-3 overflow-hidden rounded-lg border bg-main p-2">
                  ...
                  <div className="flex items-center justify-between p-1">
                    <h5 className="font-medium text-muted-foreground text-xs">Base Colors</h5>
                  ...
                  {BASE_COLORS.slice(0, expandedSections.base ? BASE_COLORS.length : 3).map(
                    (color) => (
                      <ColorPickerItem key={color.key} colorKey={color.key} label={color.label} />
                    ),
                  )}
                </div>
                ...
                <div className="relative space-y-3 overflow-hidden rounded-lg border bg-main p-2">
                  ...
                  <div className="flex items-center justify-between p-1 ">
                    <h5 className="font-medium text-muted-foreground text-xs">UI Colors</h5>
                    <div className="flex items-center gap-2">
                      <span className="text-muted-foreground text-xs">
                  ...
                  {UI_COLORS.slice(0, expandedSections.ui ? UI_COLORS.length : 3).map((color) => (
                    <ColorPickerItem key={color.key} colorKey={color.key} label={color.label} />
                  ))}
                </div>
                ...
                <div className="relative space-y-3 overflow-hidden rounded-lg border bg-main p-2">
                  ...
                  <div className="flex items-center justify-between p-1">
                    <h5 className="font-medium text-muted-foreground text-xs">Component Colors</h5>
                    <div className="flex items-center gap-2">
                      <span className="text-muted-foreground text-xs">
                      ...
                      <Switch
                        ...
                        onCheckedChange={() => toggleSectionExpansion("component")}
                      />
                    </div>
                  </div>
            ...
            style={
              {
                ...
                "--popover-foreground": themeData.themeVars["popover-foreground"],
                "--primary": themeData.themeVars.primary,
                "--primary-foreground": themeData.themeVars["primary-foreground"],
                "--secondary": themeData.themeVars.secondary,
                "--secondary-foreground": themeData.themeVars["secondary-foreground"],
                ...
                "--destructive-foreground": themeData.themeVars["destructive-foreground"],
