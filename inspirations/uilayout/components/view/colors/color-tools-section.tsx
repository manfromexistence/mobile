...
export function ColorToolsSection({ hexValue, setHexValue }: ColorToolsSectionProps) {
  const [activeTab, setActiveTab] = useState<"contrast" | "harmony" | "blindness" | "gradient">(
    "contrast",
  );
    ...
    <div className="space-y-4">
      ...
      <Tabs value={activeTab} onValueChange={(value) => setActiveTab(value as typeof activeTab)}>
        ...
        <TabsContent value="contrast" className="mt-4">
          <ColorContrastChecker initialForeground={hexValue} initialBackground="#FFFFFF" />
        </TabsContent>

        <TabsContent value="harmony" className="mt-4">
          <ColorHarmonyGenerator hexValue={hexValue} setHexValue={setHexValue} />
        </TabsContent>
        ...
        <TabsContent value="gradient" className="mt-4">
          <ColorGradientGenerator initialStartColor={hexValue} initialEndColor="#FFFFFF" />
        </TabsContent>
      </Tabs>
    </div>
  );
