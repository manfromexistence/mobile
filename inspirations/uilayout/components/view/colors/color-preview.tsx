            ...
            <div className="flex gap-1">
              <TooltipProvider>
                <Tooltip>
                  ...
                  <TooltipContent>
                    <p>{isFavorite(hexValue) ? "Remove from favorites" : "Add to favorites"}</p>
                  </TooltipContent>
                </Tooltip>
              </TooltipProvider>
              <TooltipProvider>
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 gap-2 text-xs">
          <div>
            ...
            {(() => {
              const ratio = getContrastRatio({ r: 255, g: 255, b: 255 }, rgbValues).toFixed(2);
              const level = getAccessibilityLevel(Number.parseFloat(ratio));
              return (
                <div className="flex items-center gap-1">
                  <span>{ratio}:1</span>
                  <Badge variant={level === "Fail" ? "destructive" : "outline"}>{level}</Badge>
                </div>
              );
            })()}
          </div>
