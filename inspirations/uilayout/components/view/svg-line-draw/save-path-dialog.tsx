...
export function SavePathDialog({
  ...
  const { addEditedPath, updateEditedPath, getEditedPathById } = useSavedEditedPathsStore();

  const isEditMode = Boolean(editPath);

  useEffect(() => {
    if (existingPathId) {
      const existing = isEditMode ? getEditedPathById(existingPathId) : getPathById(existingPathId);
      if (existing) {
    ...
    <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4">
      <div className="w-full max-w-md rounded-lg border bg-card-bg shadow-lg">
        <div className="flex items-center justify-between border-b p-4">
          <h2 className="font-semibold text-xl">{isUpdating ? "Update Path" : "Save Path"}</h2>
        ...
        <div className="flex justify-end gap-2 border-t p-4">
          ...
          <Button onClick={handleSave}>{isUpdating ? "Update Path" : "Save Path"}</Button>
        </div>
      </div>
    </div>
  );
