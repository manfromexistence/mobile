...
import { type SavedPath, useDrawPathsStore } from "@/store/use-draw-paths-store";
...
export function SavedPathsTab({
  ...
  const { savedPaths, deletePath, clearAllPaths } = useDrawPathsStore();
  ...
  const handleClearAll = () => {
    ...
    if (confirm("Are you sure you want to delete all saved paths? This cannot be undone.")) {
      clearAllPaths();
  ...
  const copyComponentCode = (id: string) => {
    const example = savedPaths.find((path) => path.id === id);
      ...
      <CardContent>
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
          {savedPaths.map((savedPath) => (
            <div key={savedPath.id} className="group relative rounded-xl border bg-card-bg p-1">
              <div
                className={cn(
