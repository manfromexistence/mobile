...
export function SvgEditor({
  ...
  const [controlPointsHistory, setControlPointsHistory] = useState<ControlPoint[][]>([]);
  ...
  useEffect(() => {
    if (path) {
      try {
        ...
        if (points.length === 0) {
          setError("Could not parse any points from the path. Please check the path data.");
        } else {
          setError(null);
        ...
        setError(`Error parsing path: ${error instanceof Error ? error.message : String(error)}`);
  ...
  const addControlPointsToAllAnchors = (points: ControlPoint[]): ControlPoint[] => {
  ...
  const startDrag = (index: number, e: React.MouseEvent) => {
    ...
    const pointType = controlPoints[index].isControl ? "control (red)" : "anchor (blue)";
  ...
  const handleMouseMove = (e: React.MouseEvent<SVGSVGElement>) => {
    ...
    setDebugInfo(`Dragging point ${draggedPointIndex} to ${newX.toFixed(2)}, ${newY.toFixed(2)}`);
    ...
    setControlPoints((prev) => {
      ...
      if (!currentPoint.isControl) {
        ...
        if (draggedPointIndex > 0 && newPoints[draggedPointIndex - 1].isControl) {
          newPoints[draggedPointIndex - 1].x += dx;
          newPoints[draggedPointIndex - 1].y += dy;

          // Check for a second control point
          if (draggedPointIndex > 1 && newPoints[draggedPointIndex - 2].isControl) {
  ...
  const _addPoint = (index: number) => {
    ...
    if (!p1.isControl && !p2.isControl) {
      ...
      setControlPointsHistory((prev) => [...prev.slice(0, historyIndex + 1), newPoints]);
      setHistoryIndex((prev) => prev + 1);
  ...
  const removePoint = (index: number) => {
    if (controlPoints.length <= 2 || index < 0 || index >= controlPoints.length) return;
  ...
  const handleWheel = (e: React.WheelEvent<SVGSVGElement>) => {
    if (e.shiftKey || e.metaKey) {
      ...
      const newPanOffsetX = mouseX - (mouseX - panOffset.x) * (newZoomLevel / zoomLevel);
      const newPanOffsetY = mouseY - (mouseY - panOffset.y) * (newZoomLevel / zoomLevel);
      ...
      <div className="flex h-full w-full max-w-full flex-col rounded-lg border bg-card-bg shadow-lg">
        <div className="flex items-center justify-between border-b p-2">
          <div>
            ...
            <p className="text-primary text-xs">Path length: {currentEditPath.length} characters</p>
        ...
        <div className="border-t p-0 px-4 py-2">
          <div className="you know the main work of this timeline is basically by default it has 24 hours and whenever users suppose I start working today and whenever I click start working then it will record that okay and it will start is a pointer I mean the what it called green things okay or blue pointer uh suppose I start working at 8 p.m sorry 8 a.m then it will start from these and when I finished it then it will suppose I finish it at 5 p.m then it will go there to 5 p.m so in 24 hours I start working at 8 p.m to 5 p.m so it will calculate all that and definitely it will add breakers whenever I take breaks okay so that's what that means0 text-primary/9Do text-sm">
            ...
            <p>
              ...
              <kbd className="rounded border bg-primary/20 px-1 py-0.5 text-xs">Ctrl+Wheel</kbd> to
              zoom.
              <kbd className="ml-2 rounded border bg-primary/20 px-1 py-0.5 text-xs">Ctrl+Z</kbd> to
              undo,
              <kbd className="ml-1 rounded border bg-primary/20 px-1 py-0.5 text-xs">Ctrl+Y</kbd> to
              redo.
            </p>
            {/* {zoomLevel !== 1 && (
    ...
    switch (command.toUpperCase()) {
      ...
      case "Q": // Quadratic bezier
        for (let i = 0; i < params.length; i += 4) {
          ...
          // Skip if any NaN
          if (Number.isNaN(x1) || Number.isNaN(y1) || Number.isNaN(x) || Number.isNaN(y)) continue;
...
export function pointsToSvgPath(points: ControlPoint[]): string {
  ...
  while (i < points.length) {
    if (
      ...
      // Cubic bezier curve
      path += ` C${points[i].x.toFixed(2)} ${points[i].y.toFixed(2)}, ${points[i + 1].x.toFixed(
        2,
      )} ${points[i + 1].y.toFixed(2)}, ${points[i + 2].x.toFixed(
        2,
      )} ${points[i + 2].y.toFixed(2)}`;
      i += 3;
    } else if (i + 1 < points.length && points[i].isControl && !points[i + 1].isControl) {
      // Quadratic bezier curve
      path += ` Q${points[i].x.toFixed(2)} ${points[i].y.toFixed(2)}, ${points[i + 1].x.toFixed(
        2,
