...
import { Minus, MousePointer, Pen, Pencil, Plus, Redo, Save, Trash2, Undo } from "lucide-react";
...
export function DrawingCanvas({
  ...
  const [hoverControlPoint, setHoverControlPoint] = useState<"control1" | "control2" | null>(null);
  ...
  useEffect(() => {
    ...
    if (points.length > 0) {
      ...
      for (let i = 1; i < points.length; i++) {
        ...
        if (point.controlPoint1 && prevPoint.controlPoint2) {
          ...
          ctx.quadraticCurveTo(point.controlPoint1.x, point.controlPoint1.y, point.x, point.y);
    ...
    if ((currentTool === "select" || showControlPoints) && points.length > 0) {
      points.forEach((point, index) => {
        ...
        ctx.fillStyle = isSelected ? "#4f46e5" : isHovered ? "#818cf8" : "#3b82f6";
        ctx.fill();
  ...
  useEffect(() => {
    ...
    for (let i = 1; i < points.length; i++) {
      ...
      if (point.controlPoint1 && prevPoint.controlPoint2) {
        ...
        ).toFixed(2)}, ${(point.x - offsetX).toFixed(2)} ${(point.y - offsetY).toFixed(2)}`;
      } else if (point.controlPoint1) {
        // Quadratic bezier curve with only current control point
  ...
  const drawGrid = (ctx: CanvasRenderingContext2D) => {
    ...
    const endX = Math.ceil((canvas.width - offset.x) / scale / gridSize) * gridSize;
    const endY = Math.ceil((canvas.height - offset.y) / scale / gridSize) * gridSize;
  ...
  const simplifyPath = (pathPoints: Point[], tolerance = 5): Point[] => {
    ...
    if (finalPoints[finalPoints.length - 1] !== pathPoints[pathPoints.length - 1]) {
  ...
  const douglasPeuckerSimplify = (points: Point[], epsilon: number): Point[] => {
    ...
    if (maxDistance > epsilon) {
      // Recursive call
      const firstHalf = douglasPeuckerSimplify(points.slice(0, maxDistanceIndex + 1), epsilon);
      const secondHalf = douglasPeuckerSimplify(points.slice(maxDistanceIndex), epsilon);
  ...
  const addCurvaturePoints = (originalPoints: Point[], simplifiedPoints: Point[]): Point[] => {
    ...
    for (let i = 0; i < simplifiedPoints.length - 1; i++) {
      ...
      const endIndex = originalPoints.findIndex((p) => p.x === endPoint.x && p.y === endPoint.y);
  ...
  const smoothPath = (pathPoints: Point[], smoothingFactor: number): Point[] => {
  ...
  const getClosestPointOnSegment = (p1: Point, p2: Point, p: Point) => {
    ...
    const t = Math.max(0, Math.min(1, ((p.x - p1.x) * dx + (p.y - p1.y) * dy) / d2));
    ...
    if (currentTool === "pencil") {
      ...
      if (points.length >= 2) {
        ...
        for (let i = 0; i < points.length - 1; i++) {
          ...
          const distance = Math.sqrt((closestOnSegment.x - x) ** 2 + (closestOnSegment.y - y) ** 2);
        ...
        if (closestDistance < 10 / scale && closestSegmentIndex >= 0) {
          ...
          const t =
            ((newPoint.x - p1.x) * dx + (newPoint.y - p1.y) * dy) / (segmentLength * segmentLength);
  ...
  const draw = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    if (currentTool === "pencil") {
      ...
      setPendingPoint({ x, y });
    } else if (currentTool === "select" || currentTool === "add" || currentTool === "remove") {
      ...
      if (isDrawing && selectedPointIndex !== null && draggingPointType !== null) {
        const updatedPoints = [...points];
        const selectedPoint = { ...updatedPoints[selectedPointIndex] };
  ...
  const stopDrawing = () => {
    if (currentTool === "pencil" && isDrawing && pencilPoints.length > 1) {
      ...
      // Clear raw points
      setPencilPoints([]);
    } else if (isDrawing && (currentTool === "select" || currentTool === "pen")) {
      ...
      <div className="h-full space-y-2">
        <div className="flex flex-wrap items-center justify-between">
          ...
          <ToggleGroup
            type="single"
            value={currentTool}
            onValueChange={(value) => value && setCurrentTool(value as DrawingTool)}
          >
        ...
        <p className="mt-1 text-muted-foreground text-xs">
          Zoom: Use mouse wheel, + and - keys, or the zoom buttons. Pan: Hold Space + drag. Reset
          view: Press 0.
        </p>
      </div>
    </TooltipProvider>
