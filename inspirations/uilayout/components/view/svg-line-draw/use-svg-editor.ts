...
import { addControlPointsToAllAnchors, parseSvgPath, pointsToSvgPath } from "./svg-path-utils";
import type { ControlPoint, EditorState } from "./types";

export function useSvgEditor(path: string, editorRef: RefObject<SVGSVGElement>) {
    ...
    if (path) {
      try {
        ...
        if (points.length === 0) {
          setState((prev) => ({
            ...prev,
            error: "Could not parse any points from the path. Please check the path data.",
            currentEditPath: path,
          }));
        } else {
  ...
  const startDrag = useCallback(
    (index: number, e: React.MouseEvent) => {
      e.stopPropagation();
      const pointType = state.controlPoints[index].isControl ? "control (red)" : "anchor (blue)";
      setState((prev) => ({
        ...prev,
    ...
    (e: React.MouseEvent<SVGSVGElement>) => {
      ...
      setState((prev) => {
        ...
        if (!currentPoint.isControl) {
          ...
          if (prev.draggedPointIndex > 0 && newPoints[prev.draggedPointIndex - 1].isControl) {
            newPoints[prev.draggedPointIndex - 1].x += dx;
            newPoints[prev.draggedPointIndex - 1].y += dy;

            // Check for a second control point
            if (prev.draggedPointIndex > 1 && newPoints[prev.draggedPointIndex - 2].isControl) {
  ...
  const endDrag = useCallback(() => {
    setState((prev) => {
      if (prev.draggedPointIndex !== -1) {
        ...
        const newHistory = prev.controlPointsHistory.slice(0, prev.historyIndex + 1);
    ...
    (e: React.WheelEvent<SVGSVGElement>) => {
      if (e.ctrlKey || e.metaKey) {
        ...
        setState((prev) => {
          ...
          const newZoomLevel = Math.min(Math.max(prev.zoomLevel * factor, 0.5), 5);

          // Calculate new pan offset to zoom toward/from mouse position
          const newPanOffsetX =
            mouseX - (mouseX - prev.panOffset.x) * (newZoomLevel / prev.zoomLevel);
          const newPanOffsetY =
            mouseY - (mouseY - prev.panOffset.y) * (newZoomLevel / prev.zoomLevel);
  ...
  const removePoint = useCallback((index: number) => {
    setState((prev) => {
      if (prev.controlPoints.length <= 2 || index < 0 || index >= prev.controlPoints.length)
        return prev;
