...
export function useGradientStops(
  ...
  const [gradientStops, setGradientStops] = useState<GradientStop[]>(initialStops);
  ...
  useEffect(() => {
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const newDragIndex = newStops.findIndex((stop) => stop === newStops[dragIndex]);
  ...
  const addColorStop = () => {
    ...
    const secondLastPosition = gradientStops[gradientStops.length - 2]?.position || 0;
    const newPosition = Math.min(Math.floor((lastPosition + secondLastPosition) / 2) + 10, 99);
