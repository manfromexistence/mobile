import { type ClipPathShape, INITIAL_CLIP_PATHS } from "@/components/view/clip-path/data";
...
interface ClipPathStore {
  ...
  addEditedShape: (originalId: string, newPath: string, className?: string) => void;
  ...
  persist(
    (set, get) => ({
      ...
      addEditedShape: (originalId, newPath, className) => {
        ...
        const originalShape = allShapes.find((shape) => shape.id === originalId);
            ...
            editedShapes: state.editedShapes.map((shape) =>
              shape.id === originalId
                ? {
                    ...shape,
                    path: newPath,
                    className: className !== undefined ? className : shape.className,
                  }
                : shape,
            ),
          }));
      ...
      deleteShape: (id) => {
        ...
        if (!shapeToDelete || (!shapeToDelete.isEdited && !shapeToDelete.isCustom)) {
          return;
        }

        // Determine if it's an edited or custom shape and update the appropriate array
        if (shapeToDelete.isEdited) {
          const newEditedShapes = editedShapes.filter((shape) => shape.id !== id);
          set({ editedShapes: newEditedShapes });
        } else if (shapeToDelete.isCustom) {
          const newCustomShapes = customShapes.filter((shape) => shape.id !== id);
