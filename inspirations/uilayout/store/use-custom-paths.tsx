...
type SvgStore = {
  ...
  updatePointInLine: (id: string, index: number, point: { x: number; y: number }) => void;
  deletePointFromLine: (id: string, index: number) => void;
  addControlPoint: (id: string, index: number, controlPoint: ControlPoint) => void;
  updateControlPoint: (id: string, index: number, controlPoint: Partial<ControlPoint>) => void;
...
export const useSvgStore = create<SvgStore>()(
  persist(
    (set, get) => ({
      ...
      updateLine: (id, line) =>
        set((state) => ({
          customLines: state.customLines.map((l) => (l.id === id ? { ...l, ...line } : l)),
        })),
      ...
      addPointToLine: (id, point) =>
        set((state) => ({
          customLines: state.customLines.map((line) =>
            line.id === id ? { ...line, points: [...line.points, point] } : line,
          ),
        })),
        ...
        set((state) => ({
          customLines: state.customLines.map((line) =>
            line.id === id
              ? {
                  ...line,
                  points: line.points.map((p, i) => (i === index ? { ...p, ...point } : p)),
                }
              : line,
          ),
        })),
      ...
      generatePathData: (id) => {
        ...
        for (let i = 1; i < line.points.length; i++) {
          ...
          if (controlPoint) {
            if (controlPoint.x2 !== undefined && controlPoint.y2 !== undefined) {
              // Cubic bezier curve
