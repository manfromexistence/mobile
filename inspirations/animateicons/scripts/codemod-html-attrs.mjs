...
const DIRS = [path.join(REPO_ROOT, "icons", "lucide"), path.join(REPO_ROOT, "icons", "huge")];
...
const transform = (source) => {
  ...
  //      → (line removed)
  const motionImportRe = /^import\s+type\s*\{([^}]*)\}\s*from\s*["']motion\/react["'];?\s*$/m;
  ...
  if (motionMatch) {
    ...
    if (inner.length === 0) {
      ...
      out = out.replace(motionImportRe, `import type { ${inner.join(", ")} } from "motion/react";`);
      changed = true;
    }
  }
  ...
  if (changed && /HTMLAttributes<HTMLDivElement>/.test(out)) {
    const reactValueImportRe = /^import\s*\{([^}]*)\}\s*from\s*["']react["'];?\s*$/m;
    const reactTypeImportRe = /^import\s+type\s*\{([^}]*)\}\s*from\s*["']react["'];?\s*$/m;
...
const main = async () => {
  ...
  for (const dir of DIRS) {
    const files = (await fs.readdir(dir)).filter((f) => f.endsWith("-icon.tsx"));
