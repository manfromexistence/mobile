...
export async function runSearch(query: string, opts: QueryOptions = {}): Promise<CatalogIcon[]> {
...
export async function runInfo(name: string, opts: QueryOptions = {}): Promise<InfoResult> {
  const catalog = await fetchCatalog({ registryBase: opts.registryBase });
  const res = resolveIcon(catalog, name);
  return {
    icon: res.match,
    candidates: [...res.ambiguous, ...res.suggestions].map((i) => i.registryName),
  };
}
