...
export function BrowseApp({ cwd, dir, registryBase, utilsImport, reportAdded }: AppProps) {
  const { exit } = useApp();
  ...
  const cursorIdx = results.length === 0 ? 0 : Math.min(cursor, results.length - 1);
  ...
  const doAdd = async () => {
    ...
    const names = selected.size > 0 ? Array.from(selected) : current ? [current.registryName] : [];
  ...
  for (let r = 0; r < page.length; r += cols) gridData.push(page.slice(r, r + cols));
          ...
          gridData.map((row, ri) => (
            <Box key={ri} flexDirection="row">
              {row.map((icon, ci) => {
                ...
                const fitted = trunc(label, COL_WIDTH - 1).padEnd(COL_WIDTH - 1);
                return (
                  <Box key={icon.registryName} width={COL_WIDTH}>
                    {isCursor ? (
                      <Text color="black" backgroundColor="cyan" wrap="truncate">
                        {fitted}
                      </Text>
                    ) : (
