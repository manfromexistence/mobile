...
function resolveLibrary(value: unknown): IconLibrary | undefined {
  ...
  if (value != null) {
    console.error(pc.red(`Unknown library "${String(value)}". Use lucide or huge.`));
    process.exit(1);
  }
  return undefined;
}
          ...
          if (a.skipped) {
            console.log(
              pc.yellow(`• skipped ${a.registryName} `) +
                pc.dim(`(${path.relative(cwd, a.file)} exists - use --overwrite)`),
            );
          } else {
            console.log(
              pc.green("✓ added ") + a.registryName + pc.dim(` → ${path.relative(cwd, a.file)}`),
            );
          }
        }
        for (const am of outcome.ambiguous) {
          console.log(
            pc.red(`✗ "${am.input}" is ambiguous.`) + ` Use one of: ${am.options.join(", ")}`,
          );
        }
        for (const nf of outcome.notFound) {
          const hint = nf.suggestions.length ? ` Did you mean: ${nf.suggestions.join(", ")}?` : "";
      ...
      if (wroteSomething && !hasUtils) {
        if ((options as { withUtils?: boolean }).withUtils) {
          ...
          if (!fs.existsSync(utilsPath)) {
            ...
            console.log(pc.green("✓ created ") + pc.dim(path.relative(cwd, utilsPath)));
          }
        } else {
          console.log(
            pc.dim(
...
cli
  ...
  .action(async (query: string, options: GlobalOptions & { library?: string; limit?: number }) => {
...
cli
  ...
  .action(async (options: GlobalOptions & { library?: string; limit?: number }) => {
