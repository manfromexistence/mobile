...
export default function NotFound() {
  ...
  useEffect(() => {
    if (reduced) return;
    const play = () =>
      refs.current.forEach((icon, i) => window.setTimeout(() => icon?.startAnimation(), i * 140));
    play();
  ...
  return (
    <div className="relative flex min-h-dvh items-center justify-center overflow-hidden px-4 py-16">
      <div aria-hidden="true" className="bg-grid pointer-events-none absolute inset-0 z-0" />
      <span
        aria-hidden="true"
      ...
      >
        <motion.div variants={itemVariants}>
          ...
          >
            ...
            <span className="text-textPrimary text-base font-semibold">AnimateIcons</span>
          </Link>
        </motion.div>
