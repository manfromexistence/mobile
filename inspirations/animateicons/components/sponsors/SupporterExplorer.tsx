...
const SupporterExplorer: React.FC<Props> = ({ supporters }) => {
  ...
  const filtered = useMemo(
    () => (filter === "all" ? supporters : supporters.filter((s) => s.source === filter)),
    [filter, supporters],
  );
  ...
  return (
    <div className="space-y-6">
      <div role="tablist" aria-label="Filter supporters by source" className="flex flex-wrap gap-2">
            ...
            >
              ...
              <span
                className={cn(
                  ...
                  active ? "bg-primary/20 text-primary" : "text-textMuted bg-white/5",
                )}
              >
                {counts[id]}
              </span>
