...
const SectionHeader: React.FC<Props> = ({ title, subtitle, spacing = "default", className }) => {
  return (
    <div className={cn(SPACING_CLASS[spacing], "text-center", className)}>
      <h2 className="text-textPrimary text-2xl font-semibold sm:text-3xl">{title}</h2>
      {subtitle ? <p className="text-textSecondary mt-3 text-sm">{subtitle}</p> : null}
    </div>
  );
};
