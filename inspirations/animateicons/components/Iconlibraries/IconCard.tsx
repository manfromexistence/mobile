...
const IconCard: React.FC<IconLibraryCardData> = ({ icons, description, title, count, href }) => {
    ...
    <article className="group/poster relative flex flex-col gap-8 py-4">
      ...
      <div className="flex items-end gap-4">
        ...
        <div className="text-textSecondary pb-2 text-sm leading-tight">
          <div className="text-textPrimary text-base font-semibold">{displayName}</div>
          <div>animated icons</div>
        </div>
      </div>

      {/* Supporting copy */}
      <p className="text-textSecondary max-w-md text-sm leading-relaxed">{description}</p>

      {/* Icon proof row */}
      <div className="flex flex-wrap items-center gap-x-5 gap-y-3">
        {previewIcons.map((Icon, index) => (
          <div key={index} className="text-textSecondary hover:text-primary transition-colors">
            <Icon size={24} />
          </div>
        ))}
      </div>
