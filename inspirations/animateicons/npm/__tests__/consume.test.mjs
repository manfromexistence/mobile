...
test("dist/ exists - package was built", async () => {
  assert.ok(await exists(DIST), "dist/ missing. Run `pnpm build` before this test.");
});
...
test("all declared export targets are emitted", async () => {
  ...
  for (const file of expected) {
    assert.ok(await exists(path.join(DIST, file)), `dist/${file} missing - broken exports map`);
  }
});

test("ESM: lucide subpath exports a known icon component", async () => {
  const mod = await import(path.join(DIST, "lucide.js"));
  assert.equal(typeof mod.BellRingIcon, "object", "BellRingIcon should be a forwardRef object");
...
test("expected icon counts: lucide ≥ 248, huge ≥ 33", async () => {
  ...
  assert.ok(lucideIcons.length >= 248, `expected ≥248 lucide icons, got ${lucideIcons.length}`);
  assert.ok(hugeIcons.length >= 33, `expected ≥33 huge icons, got ${hugeIcons.length}`);
});

test("top-level types-only entry exports IconHandle type", async () => {
  const dts = await fs.readFile(path.join(DIST, "index.d.ts"), "utf8");
  assert.match(dts, /IconHandle/, "index.d.ts should re-export IconHandle type");
});
