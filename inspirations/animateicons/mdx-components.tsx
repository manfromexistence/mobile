...
export function useMDXComponents(components: MDXComponents): MDXComponents {
  return {
    h1: ({ children }) => <h1 className="text-textPrimary text-2xl font-semibold">{children}</h1>,
    h2: ({ children, id }) => (
    ...
    p: ({ children }) => (
      <p className="text-textSecondary max-w-2xl text-sm leading-relaxed">{children}</p>
    ),
    a: ({ href, children }) => (
      <a
        href={href}
    ...
    code: ({ children }) => (
      <code className="bg-surface rounded px-1.5 py-0.5 font-mono text-xs">{children}</code>
    ),
    ul: ({ children }) => (
      <ul className="text-textSecondary ml-5 list-disc space-y-1 text-sm">{children}</ul>
    ),
    ol: ({ children }) => (
      <ol className="text-textSecondary ml-5 list-decimal space-y-1 text-sm">{children}</ol>
    ),
