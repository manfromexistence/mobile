...
describe("mapBmcSupporter", () => {
  ...
  it("falls back to 'Anonymous' when name is missing or whitespace", () => {
    expect(mapBmcSupporter({ ...baseRaw, supporter_name: null }).name).toBe("Anonymous");
    expect(mapBmcSupporter({ ...baseRaw, supporter_name: "   " }).name).toBe("Anonymous");
  });
  ...
  it("coerces amount to null when computed total is zero or negative", () => {
    expect(mapBmcSupporter({ ...baseRaw, support_coffees: 0 }).amount).toBeNull();
  });
