...
export function renderIconContent(item: RegistryItem, opts: RenderOptions = {}): string {
...
export function writeIcon(item: RegistryItem, opts: WriteOptions): WriteResult {
  ...
  const dir = path.isAbsolute(opts.targetDir) ? opts.targetDir : path.join(cwd, opts.targetDir);
