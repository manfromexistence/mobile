...
describe("searchIcons", () => {
  it("finds bell-ring by a keyword", async () => {
    const catalog = await loadCatalog();
    const names = searchIcons(catalog, "notification").map((i) => i.registryName);
...
describe("resolveIcon", () => {
  it("resolves a prefixed registry name", async () => {
    const catalog = await loadCatalog();
    expect(resolveIcon(catalog, "lu-bell-ring").match?.registryName).toBe("lu-bell-ring");
  });

  it("resolves a unique bare base name", async () => {
    const catalog = await loadCatalog();
    expect(resolveIcon(catalog, "bell-ring").match?.registryName).toBe("lu-bell-ring");
  });
  ...
  it("reports ambiguity for a base name in multiple libraries", async () => {
    ...
    expect(res.ambiguous.map((i) => i.registryName).sort()).toEqual(["hu-activity", "lu-activity"]);
  });
