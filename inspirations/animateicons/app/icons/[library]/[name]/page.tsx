...
const isLibrary = (v: string): v is LibraryKey => v === "lucide" || v === "huge";

const getList = (lib: LibraryKey) => (lib === "lucide" ? LUCIDE_ICON_LIST : HUGE_ICON_LIST);
...
const findRelatedIcons = (
  ...
  for (const candidate of list) {
    ...
    if (!candidate.name.startsWith(`${root}-`) && candidate.name !== root) continue;
    ...
    <div className="mx-auto w-full max-w-3xl px-4 py-10 lg:py-16">
      ...
      <div className="mb-6 flex items-center gap-3">
        ...
        >
          ...
          <span>/</span>
          <Link href={`/icons/${library}`} className="hover:text-textPrimary transition-colors">
            {libDisplay}
          </Link>
          <span>/</span>
      ...
      <header className="mb-8 flex flex-wrap items-end justify-between gap-3">
        <div>
          <h1 className="text-textPrimary text-3xl font-semibold sm:text-4xl">{componentName}</h1>
        ...
        <div className="text-textMuted flex items-center gap-2 text-[11px] tracking-wide uppercase">
          {item.category?.slice(0, 2).map((c) => (
            <span key={c} className="border-border/60 rounded-full border px-2 py-1">
              {c}
            </span>
          ))}
        </div>
      ...
      <IconDetailPlayground Icon={item.icon} componentName={componentName} library={library} />
      ...
      <section className="mt-8 space-y-3">
        <h2 className="text-textSecondary text-[11px] tracking-wide uppercase">Usage</h2>
        <div className={GLASS_CARD}>
          <span
            aria-hidden="true"
            className="pointer-events-none absolute inset-x-4 top-px h-px bg-gradient-to-r from-transparent via-white/15 to-transparent"
      ...
      {related.length > 0 && (
        <section className="mt-12 space-y-4">
          <h2 className="text-textSecondary text-[11px] tracking-wide uppercase">Related icons</h2>
