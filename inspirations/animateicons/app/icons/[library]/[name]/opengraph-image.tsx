...
const SHAPE_TAGS = ["path", "circle", "rect", "line", "ellipse", "polyline", "polygon"] as const;
...
const readIconShapes = async (library: LibraryKey, name: string): Promise<Shape[]> => {
  try {
    const filePath = path.join(process.cwd(), "icons", library, `${name}-icon.tsx`);
    ...
    const elementRe = new RegExp(`<(?:motion\\.|m\\.)?(${tagPattern})\\b([^>]*?)/?>`, "g");
...
export default async function OGImage({
  ...
  const list =
    validLibrary === "lucide" ? LUCIDE_ICON_LIST : validLibrary === "huge" ? HUGE_ICON_LIST : [];
  const item = list.find((i) => i.name === name);

  const componentName = item ? toComponentName(item.name) : "AnimateIcons";
  const libraryDisplay =
    validLibrary === "lucide" ? "Lucide" : validLibrary === "huge" ? "Huge" : "";

  const [iconShapes, logoDataUrl] = await Promise.all([
    validLibrary && item ? readIconShapes(validLibrary, name) : Promise.resolve([] as Shape[]),
    readLogoDataUrl(),
  ]);
    ...
    >
      ...
      >
        {logoDataUrl ? (
          ...
          <img src={logoDataUrl} width={44} height={44} alt="" style={{ display: "block" }} />
        ) : (
          <div
            style={{
              width: 44,
