...
function reducer(state: State, action: Action): State {
  switch (action.type) {
    case "SET_COPIED_CODE":
      return state.copiedCodeId === action.id ? state : { ...state, copiedCodeId: action.id };
    case "SET_COPIED_CLI":
      return state.copiedCliId === action.id ? state : { ...state, copiedCliId: action.id };
    case "SET_LOADING":
      return state.loadingId === action.id ? state : { ...state, loadingId: action.id };
    default:
      return state;
  }
}
...
export const IconTileProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
...
const useTileState = (): State => {
  ...
  if (!ctx) {
    throw new Error("useTileState/useIs* must be used within an IconTileProvider");
  }
  return ctx;
};
...
export const useIconTileDispatch = (): Dispatchers => {
  ...
  if (!ctx) {
    throw new Error("useIconTileDispatch must be used within an IconTileProvider");
  }
  return ctx;
};

/** Selector hooks - narrow the read to a single boolean per consumer. */
export const useIsCopiedCode = (tileId: string): boolean => useTileState().copiedCodeId === tileId;

export const useIsCopiedCli = (tileId: string): boolean => useTileState().copiedCliId === tileId;

export const useIsLoading = (tileId: string): boolean => useTileState().loadingId === tileId;
