      ...
      <div className="border-border/60 text-textPrimary relative w-full max-w-md overflow-hidden rounded-2xl border bg-gradient-to-b from-white/[0.03] to-white/[0.01] p-8 shadow-[0_8px_24px_-12px_rgba(0,0,0,0.6)] backdrop-blur">
        ...
        <div className="mb-7 flex flex-col items-center gap-4 text-center">
          ...
          <p className="text-textSecondary max-w-xs text-sm leading-relaxed">
            An unexpected error occurred while loading this page. Try again, and if it keeps
            happening let us know on GitHub.
          </p>
          {error.digest ? (
            <p className="text-textMuted font-mono text-[11px]">ref: {error.digest}</p>
          ) : null}
        </div>
        ...
        {process.env.NODE_ENV === "development" && (
          <div className="border-border/60 bg-bgDark/60 mt-6 rounded-xl border p-3">
            <p className="text-textSecondary mb-1 text-[11px] tracking-wide uppercase">Debug</p>
            <p className="text-textMuted font-mono text-xs break-all">{error.message}</p>
          </div>
        )}
      </div>
    </div>
