...
if (!content.includes('PopoverContent')) {
  content = content.replace('import { Button }', 'import { Popover, PopoverContent, PopoverTrigger } from "@/components/ui/popover";\nimport { Button }');
}
...
const inputDivSearch = 'className="pointer-events-none absolute right-0 bottom-0 left-0 z-20 flex flex-col items-center justify-end bg-gradient-to-t from-background via-background/95 to-transparent px-3 pt-20 pb-4 md:px-6 md:pb-6"';
const inputDivReplacement = 'className={cn("pointer-events-none absolute right-0 left-0 z-20 flex flex-col items-center px-3 md:px-6", messages.length === 0 ? "top-1/2 -translate-y-1/2 justify-center" : "bottom-0 justify-end bg-gradient-to-t from-background via-background/95 to-transparent pt-20 pb-4 md:pb-6")}';
