...
const moves = [
  ...
  // Step 4
  { from: 'components/daikanoid', to: 'features/portfolio/components/daikanoid' },
  { from: 'components/animated-icons', to: 'features/portfolio/components/animated-icons' },
...
if (fs.existsSync(duckFollowerDup)) {
    try { fs.rmSync(duckFollowerDup, { recursive: true, force: true }); } catch (e) {}
}
...
walkSync(srcDir, (filepath) => {
    if (/\.(ts|tsx|js|jsx|mdx)$/.test(filepath)) {
        ...
        content = content.replace(/(['"])@\/components\/screens\b/g, '$1@/features/dx/components/screens');
        content = content.replace(/(['"])@\/components\/browser\b/g, '$1@/features/dx/components/browser');
        
        content = content.replace(/(['"])@\/components\/daikanoid\b/g, '$1@/features/portfolio/components/daikanoid');
        content = content.replace(/(['"])@\/components\/animated-icons\b/g, '$1@/features/portfolio/components/animated-icons');
        content = content.replace(/(['"])@\/components\/eyes\b/g, '$1@/features/portfolio/components/eyes');
        
        content = content.replace(/(['"])@\/components\/duck-follower\b/g, '$1@/features/portfolio/components/duck-follower');
        
        content = content.replace(/(['"])@\/components\/react-bits\b/g, '$1@/components/vendor/react-bits');
