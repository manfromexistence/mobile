import { eachYearOfInterval, format, getYear, startOfYear, subDays } from "date-fns";
...
export async function ProfileActivityMosaicCover() {
  ...
  return (
    <ActivityMosaicCover activities={contributions} rowCount={rowCount} columnCount={columnCount} />
  );
}

function getYearRange(daysBack: number, now: Date = new Date()): number[] {
  if (!Number.isInteger(daysBack) || daysBack < 0) {
    throw new RangeError(`daysBack must be a non-negative integer, received: ${daysBack}`);
  }
...
function buildContributionGrid(activities: Activity[], cellCount: number): Activity[] {
  const contributionsSortedByDate = [...activities].sort((a, b) => b.date.localeCompare(a.date));

  const today = format(new Date(), "yyyy-MM-dd");
  const todayIndex = contributionsSortedByDate.findIndex((c) => c.date === today);
