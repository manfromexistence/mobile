...
const ActivityMosaicContext = createContext<ActivityMosaicContextType | null>(null);

const useActivityMosaic = () => {
  const context = useContext(ActivityMosaicContext);

  if (!context) {
    throw new Error("ActivityMosaic components must be used within a ActivityMosaic");
  }
...
const fillMissingDates = (activities: Activity[]): Activity[] => {
  ...
  const activityDateMap = new Map<string, Activity>(activities.map((a) => [a.date, a]));
...
const groupActivitiesByColumn = (
  ...
  return new Array(columnCount)
    .fill(undefined)
    .map((_, columnIndex) =>
      activitiesWithFilledDates.slice(columnIndex * rowCount, columnIndex * rowCount + rowCount),
    );
};
...
export const ActivityMosaic = ({
  ...
  return (
    ...
    >
      <div
        className={cn("flex items-center justify-center overflow-x-clip", className)}
        {...props}
      />
    </ActivityMosaicContext.Provider>
  );
...
export type ActivityMosaicGridProps = Omit<HTMLAttributes<SVGSVGElement>, "children"> & {
  className?: string;
  children: (props: {
    activity: Activity;
    rowIndex: number;
...
export const ActivityMosaicGrid = ({ className, children, ...props }: ActivityMosaicGridProps) => {
