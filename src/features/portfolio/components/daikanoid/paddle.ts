...
import { PADDLE_HEIGHT, PADDLE_SPEED, PADDLE_WIDTH, uncheckedClamp } from "./constants";
...
export class Paddle {
  ...
  constructor(p: p5, state: GameState) {
    ...
    state.canvas!.touchMoved((e: TouchEvent) => {
      const rect = (state.canvas!.elt as HTMLCanvasElement).getBoundingClientRect();
  ...
  show() {
    ...
    this.p.image(this.state.paddleImage!, this.x, this.y, PADDLE_WIDTH, PADDLE_HEIGHT);
