...
const SCALED_SPRITE_SIZE = (() => {
  const { frameWidth, frameHeight, renderedWidth, renderedHeight, gutter, animations } = CONFIG;
  ...
  return {
    width: (maxFrames * frameWidth + (maxFrames - 1) * gutter) * (renderedWidth / frameWidth),
    height: (maxRows * frameHeight + (maxRows - 1) * gutter) * (renderedHeight / frameHeight),
  };
})();
...
const ANIMATION_STYLES: Record<AnimationName, ReturnType<typeof generateAnimationStyle>> = {
...
function useDuckState() {
  ...
  const getRandomIdleAnimation = useCallback((): AnimationName => {
    const availableAnimations = IDLE_ANIMATIONS.filter((a) => a !== lastIdleAnimationRef.current);
