 ...
 */
function generateCrackLines(center: Point, viewW: number, viewH: number): CrackLine[] {
...
const CrackEffect = memo(function CrackEffect({ x, y, shards }: CrackEffectProps) {
  ...
  useEffect(() => {
    ...
    for (const [level, group] of byLevel) {
      timers.push(window.setTimeout(() => drawCrackLines(ctx, group), level * LINE_STAGGER_MS));
    }
...
export function GunshotMode({ active, onShoot }: GunshotModeProps) {
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      setCracks((prev) => [...prev, { id, x, y, shards: generateShards(x, y) }]);
      onShoot?.();
