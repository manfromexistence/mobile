...
const FLICKER = Float32Array.from({ length: FLICKER_SIZE }, () => 0.4 + Math.random() * 0.6);
...
function drawWaveFrame(
  ...
  if (strandAlpha > 0.01) {
    ...
    for (let i = 0; i < numStrands; i++) {
      ...
      ctx.bezierCurveTo(cp1X, cp1Y, cp2X, cp2Y, center.x + cos * outer, center.y + sin * outer);
      ctx.stroke();
    }
  }
...
const WaveEffect = memo(function WaveEffect({ x, y, pattern, rotation, scale }: WaveEffectProps) {
  ...
  return (
    ...
    >
      <canvas ref={canvasRef} className="absolute inset-0 h-full w-full" style={CANVAS_STYLE} />
