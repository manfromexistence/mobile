...
const _NUM_RINGS = 6
const _NUM_SPEED_LINES = 24
const _NUM_DOT_RINGS = 4
const _DOTS_PER_RING = 16
const NUM_WEB_STRANDS = 12
const _NUM_CONNECTING_WEBS = 8
const TAU = Math.PI * 2

// White sticky web palette
const WHITE = { r: 255, g: 255, b: 255 } as const
const _LIGHT_GRAY = { r: 220, g: 220, b: 230 } as const
...
function _easeOutQuart(t: number): number {
  return 1 - (1 - t) ** 4
}
...
function drawWaveFrame(
  ...
  if (strandAlpha > 0.01) {
    ...
    for (let i = 0; i < numStrands; i++) {
      ...
      const cp1X =
        center.x + Math.cos(a) * cp1Dist + Math.cos(perpAngle) * waveOffset1
      const cp1Y =
        center.y + Math.sin(a) * cp1Dist + Math.sin(perpAngle) * waveOffset1

      const cp2X =
        center.x + Math.cos(a) * cp2Dist + Math.cos(perpAngle) * waveOffset2
      const cp2Y =
        center.y + Math.sin(a) * cp2Dist + Math.sin(perpAngle) * waveOffset2
    ...
    for (let i = 0; i < numRings; i++) {
      ...
      for (let s = 0; s <= segments; s++) {
        ...
        const radiusVariation =
          Math.sin(angle * 3 + i * 0.5) * ringRadius * 0.08
        const waveVariation =
          Math.sin(angle * 5 + progress * 3) * ringRadius * 0.05
      ...
      if (i > 0) {
        ...
        for (let c = 0; c < connectDensity; c++) {
          ...
          const startRadiusVar =
            Math.sin(angle * 3 + (i - 1) * 0.5) * prevRingRadius * 0.08
          ...
          const cp1X =
            center.x +
            Math.cos(angle) * (startR + (midDist - startR) * 0.4) +
            Math.cos(perpAngle) * (curveBias * 0.5 + waveOffset)
          const cp1Y =
            center.y +
            Math.sin(angle) * (startR + (midDist - startR) * 0.4) +
            Math.sin(perpAngle) * (curveBias * 0.5 + waveOffset)

          const cp2X =
            center.x +
            Math.cos(angle) * (startR + (midDist - startR) * 0.7) +
            Math.cos(perpAngle) * (curveBias * 0.3 - waveOffset * 0.5)
          const cp2Y =
            center.y +
            Math.sin(angle) * (startR + (midDist - startR) * 0.7) +
...
const _GLITCH_BG: React.CSSProperties = { mixBlendMode: "screen" }
...
const WaveEffect = memo(function WaveEffect({
  x,
  y,
  pattern,
  rotation,
...
export function SpidermanWavesMode({
  active,
  onTrigger,
}: SpidermanWavesModeProps) {
  ...
  const handleClick = useCallback(
    (e: React.MouseEvent) => {
      ...
      setWaves((prev) => [...prev, { id, x, y, pattern, rotation, scale }])
      onTrigger?.()
