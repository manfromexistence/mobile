...
export function ScreenCarousel({
  ...
  const [resizeDirection, setResizeDirection] = useState<"left" | "right" | "both" | null>(null);
  ...
  useEffect(() => {
    ...
    const hasUninitialized = screens.some((s) => s.width === 0 || s.height === 0);
    const sizeChanged =
      Math.abs(containerSize.width - previousContainerSizeRef.current.width) > 10 ||
      Math.abs(containerSize.height - previousContainerSizeRef.current.height) > 10;
    ...
    if (hasUninitialized) {
      ...
      onScreensUpdate(
        screens.map((screen) => {
          const wasManuallyResized = manuallyResizedScreensRef.current.has(screen.id);
  ...
  }, [safeActiveIndex, containerSize.width, isDragging, isResizing, resizeDirection, screens, x]);
            ...
            >
              {isActive ? (
                <Resizable
                  ...
                  onResizeStart={() => {
                    console.log("onResizeStart - resizeSides:", resizeSidesRef.current);
                    setIsResizing(true);
                  ...
                  onResizeStop={(_e, _direction, _ref, d) => {
                    ...
                    console.log("onResizeStop - final resizeSides:", resizeSidesRef.current);
