...
import { type ParameterNode, type TypeNode, TypeTable } from "@/features/doc/components/type-table";
    ...
    const entries = item.entries.map(async (entry): Promise<[string, TypeNode]> => {
      ...
      return [
        entry.name,
        {
          ...
          example: tags.example ? await renderMarkdown!(tags.example) : undefined,
