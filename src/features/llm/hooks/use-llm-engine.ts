...
import type { LLMEngine, LLMEngineCapabilities, LLMChatOptions, LLMEngineType } from "../types"
...
export interface UseLLMEngineResult {
  ...
  chat: (options: LLMChatOptions) => Promise<string | void>
  unload: () => Promise<void>
}
...
export function useLLMEngine(): UseLLMEngineResult {
  ...
  const chat = useCallback(async (options: LLMChatOptions): Promise<string | void> => {
