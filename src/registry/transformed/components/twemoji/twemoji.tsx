...
export function Twemoji({ children, className, source = defaultSource }: TwemojiProps) {
...
function getEmojiCodePoint(rawText: string) {
  return toCodePoint(
    rawText.indexOf(ZERO_WIDTH_JOINER) < 0 ? rawText.replace(VARIATION_SELECTOR_16, "") : rawText,
  );
}
...
function toCodePoint(unicodeSurrogates: string, sep = "-") {
  ...
  while (i < unicodeSurrogates.length) {
    ...
    if (previous) {
      result.push((0x10000 + ((previous - 0xd800) << 10) + (code - 0xdc00)).toString(16));
      previous = 0;
