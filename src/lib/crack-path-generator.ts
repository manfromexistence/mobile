...
export function generateCrackClipPath(
  ...
  for (let i = 0; i < branches; i++) {
    ...
    const midX =
      centerX + Math.cos(angle) * (length / 2) + (Math.random() - 0.5) * 10
    const midY =
      centerY + Math.sin(angle) * (length / 2) + (Math.random() - 0.5) * 10
...
export function generateCrackSequence(
  ...
  for (let i = 0; i < frames; i++) {
    ...
    sequence.push(
      generateCrackClipPath(centerX, centerY, width, height, branches)
    )
  }
