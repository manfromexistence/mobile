...
import { type Animation, DEFAULT_EYE_STATE, type EyeState } from "./eyes-types";
...
function easeFunc(type: string, t: number): number {
  switch (type) {
    ...
    case "ease-out":
      return t * (2 - t);
    case "spring":
      return 1 - Math.cos(t * Math.PI * 1.2) ** 3 * Math.exp(-t * 4);
    default:
...
function interpolateEyeState(a: EyeState, b: EyeState, t: number): EyeState {
  return {
    ...
    h: lerp(a.h, b.h, t),
    r:
      typeof a.r === "number" && typeof b.r === "number"
        ? lerp(a.r as number, b.r as number, t)
        : t < 0.5
          ? a.r
          : b.r,
    x: lerp(a.x, b.x, t),
...
export function computeFrame(anim: Animation, timeMs: number): Frame {
  ...
  const left =
    prev.left || next.left ? interpolateEyeState(prevLeft, nextLeft, easedT) : baseInterp;
  const right =
    prev.right || next.right ? interpolateEyeState(prevRight, nextRight, easedT) : baseInterp;
