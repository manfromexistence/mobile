...
const getStargazerCount = unstable_cache(
  async () => {
    try {
      const response = await fetch(`https://api.github.com/repos/${SOURCE_CODE_GITHUB_REPO}`, {
        headers: {
...
export async function NavItemGitHub() {
  ...
  return <GitHubStars repo={SOURCE_CODE_GITHUB_REPO} stargazersCount={stargazersCount} />;
}
