...
export function Friday({ sidebarWidth = 56, className }: FridayProps) {
  ...
  const [phase, setPhase] = React.useState<"idle" | "entering" | "active" | "exiting">("idle");
  ...
  const smoothScrollTo = React.useCallback((target: number, duration: number): Promise<void> => {
  ...
  const runEntrySequence = React.useCallback(async () => {
    ...
    const scrollTarget = Math.min(scrollOrigin.current + scrollAmount, maxScroll);
    ...
    <>
      ...
      >
        <Button
          ...
          aria-label={isActive ? "Deactivate Friday Effect" : "Activate Friday Effect"}
        >
      ...
      <div
        className={
          showTop ? "friday-border friday-border-top visible" : "friday-border friday-border-top"
        }
        aria-hidden="true"
      />
      <div
      ...
      <div
        className={
          showLeft ? "friday-border friday-border-left visible" : "friday-border friday-border-left"
        }
      ...
      <div
        className={
          showBottom ? "friday-glow friday-glow-bottom visible" : "friday-glow friday-glow-bottom"
        }
        aria-hidden="true"
      />
      <div
        className={
          showLeft ? "friday-glow friday-glow-left visible" : "friday-glow friday-glow-left"
        }
        style={{ transitionDelay: showLeft ? "0.3s" : "0s" }}
        aria-hidden="true"
      />
      <div
        className={
          showRight ? "friday-glow friday-glow-right visible" : "friday-glow friday-glow-right"
        }
        style={{
          transitionDelay: showRight ? `${SLIDE_DURATION_MS / 2000}s` : "0s",
        }}
