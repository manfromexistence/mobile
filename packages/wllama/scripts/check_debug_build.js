...
const WASM_PATHS = ["src/wasm/wllama.wasm", "compat/src/wasm/wllama.wasm"]
...
export function checkDebugBuild() {
  for (const relPath of WASM_PATHS) {
    ...
    if (contents.includes(DEBUG_MARKER)) {
      console.error(
        `ERROR: ${relPath} contains "${DEBUG_MARKER}" - this is a debug build and cannot be merged to master or be published`
      )
      process.exit(1)
    }
  }
