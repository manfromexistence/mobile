...
import { DX_FENCE_OPEN, DX_FENCE_CLOSE } from "./dx-compact.ts";
...
function closeTagFor(fence: string): string {
  ...
  if (fence === DX_FENCE_OPEN) return DX_FENCE_CLOSE;
...
function restoreText(text: string): string {
  ...
  if (
    ...
    !text.includes(DX_FENCE_OPEN) &&
    !text.includes(TOON_FENCE_OPEN)
  )
    return text;

  let result = text;
  // Process all fence types: GCF first (new format), then DX Compact, then legacy omni-tabular, then TOON
  for (const fence of [GCF_FENCE_OPEN, DX_FENCE_OPEN, TABULAR_FENCE_OPEN, TOON_FENCE_OPEN]) {
