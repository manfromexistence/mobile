...
import { DX_FENCE_OPEN, decodeDx } from "./dx-compact.ts";
...
export const TABULAR_MARKER_RE = /```(?:gcf-generic|dx-compact|omni-tabular)\n([\s\S]*?)\n```/g;
...
export function decodeTabular(text: string): Record<string, unknown>[] {
  // DX Compact fence (fenced as ```dx-compact)
  if (text.startsWith(DX_FENCE_OPEN + "\n")) return decodeDx(text);
