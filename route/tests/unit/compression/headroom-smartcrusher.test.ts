...
describe("headroomEngine.apply — compression", () => {
  ...
  it("compressed body contains a fenced block with field declaration and row data", async () => {
    const n = 22;
    ...
    // Accept both GCF [N]{fields} marker and any fenced block with field declaration
    const hasGcfMarker = /\[22\]\{/.test(bodyStr);
    const hasFencedBlock = /```toon/.test(bodyStr);
    assert.ok(hasGcfMarker || hasFencedBlock, "compressed body must contain a fenced block");
  });
