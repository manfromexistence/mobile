...
test("recordPluginMetric stores call count", async () => {
  const { recordPluginMetric, getPluginMetrics } = await import(
    "../../src/lib/db/pluginMetrics.ts"
  );
...
test("clearPluginMetrics removes metrics", async () => {
  const { recordPluginMetric, clearPluginMetrics, getPluginMetrics } = await import(
    "../../src/lib/db/pluginMetrics.ts"
  );
