...
beforeEach(() => {
  ...
  vi.stubGlobal(
    "fetch",
    vi.fn(async () => ({ ok: true, json: async () => ({ rows: [] }) }) as any),
  );
});
afterEach(() => {
  act(() => root.unmount());
...
describe("studio page", () => {
  it("shows Play by default and switches to Compare", async () => {
    const { default: StudioPage } = await import(
      "@/app/(dashboard)/dashboard/compression/studio/page"
    );
    await act(async () => {
    ...
    await act(async () => {
      compareTab.dispatchEvent(new MouseEvent("click", { bubbles: true }));
    });
