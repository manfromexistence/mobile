...
import { DxDecodeError } from "@dx-serializer/core";
...
describe("formatError", () => {
  it("renders a decode error with line and source as a header, source line, and caret", () => {
    const error = new DxDecodeError("Tabs are not allowed in indentation in strict mode", {
      line: 2,
      source: "\tb: 1",
    });
  ...
  it("appends the cause chain under verbose mode", () => {
    ...
    const error = new DxDecodeError("Unterminated string: missing closing quote", {
      line: 2,
  ...
  it("places the caret under the first non-whitespace character of the source line", () => {
    const error = new DxDecodeError("Indentation must be exact multiple of 2, but found 3 spaces", {
      line: 2,
      source: "   b: 1",
    });
