...
export async function readInput(source: InputSource): Promise<string> {
  if (source.type === "stdin") return readFromStdin();
...
function readFromStdin(): Promise<string> {
  ...
  if (stdin.readableEnded) return Promise.resolve("");
...
export async function* readLinesFromSource(source: InputSource): AsyncIterable<string> {
  const stream =
    source.type === "stdin" ? process.stdin : createReadStream(source.path, { encoding: "utf-8" });
