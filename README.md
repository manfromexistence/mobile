...
# Build debug APK (all 4 ABIs, unsigned, ~120 MB each)
pnpm tauri android build --debug --apk

# Build release APK (optimized, ~6 MB each)
pnpm tauri android build --apk

# Build split-per-abi (separate APK per CPU architecture)
pnpm tauri android build --apk --split-per-abi

# Sign release APK with a debug keystore for sideloading
keytool -genkey -v -keystore debug.keystore -storepass android -alias androiddebugkey -keypass android -keyalg RSA -keysize 2048 -validity 10000 -dname "CN=Android Debug,O=Android,C=US"
apksigner sign --ks debug.keystore --ks-pass pass:android --ks-key-alias androiddebugkey --out app-signed.apk app-unsigned.apk

# Install to connected device via USB/WiFi
adb install -r path/to/app.apk

# Wireless pairing (one-time)
